...
#define DRM_HDMI_FORMAT_NUM          55
#define DRM_HDMI_EDID_LEN            512
#define DRM_HDMI_PIX_CLK_BASE        100

#define DRM_HDMI_VIC_PC_BASE ((HDMI_VIC_VIRTUAL_BASE) + 0x100)

pid_spin_lock_s g_pid_lock;
td_s32 g_cur_pid;
td_void hdmi_drv_mod_exit(td_void);
unsigned char g_hdmi_edid_raw_inner[DRM_HDMI_EDID_LEN];

td_void hdmi_check_pid_spin_lock_init(td_void)
{
    (td_void)pid_spin_lock_init(&g_pid_lock);
}

td_void hdmi_check_pid_spin_lock_destory(td_void)
{
    (td_void)pid_spin_lock_destory(&g_pid_lock);
}

static td_void hdmi_clear_pid(td_void)
{
    g_cur_pid = 0;
    return;
}

static td_bool hdmi_check_and_set_pid(td_void)
{
    td_bool check_pid;

    if (g_cur_pid == 0) {
        /* 如果没有pid即为第一次初始化，赋值当前的pid，返回TURE */
        g_cur_pid = current->tgid;
        check_pid = TD_TRUE;
    } else if (g_cur_pid != current->tgid) {
        /* 如果pid不一致，说明已有进程打开hdmi设备，返回FALSE */
        check_pid = TD_FALSE;
    } else {
        /* 剩下情况即为pid一致，返回TRUE */
        check_pid = TD_TRUE;
...
static td_bool hdmi_check_is_same_pid(td_void)
{
    td_ulong flags = 0;
    td_bool is_same_pid;

    pid_spin_lock_irqsave(&g_pid_lock, flags);
    is_same_pid = hdmi_check_and_set_pid();
    pid_spin_unlock_irqrestore(&g_pid_lock, flags);
...
static td_char *g_hdmi_reg[(td_s32)HDMI_ID_MAX] = {TD_NULL};
static td_char *g_hdmi_phy[(td_s32)HDMI_ID_MAX] = {TD_NULL};
...
static hdmi_device g_hdmi_ctrl[(td_s32)HDMI_ID_MAX];

/* 检查特定hdmi设备是否已经被开启 */
static td_s32 hdmi_not_open_check(td_u32 user_cnt, td_u32 kernel_cnt)
{
    /* 根据内核态及用户态的用户数判断hdmi设备是否已经被关闭 */
    if ((((user_cnt) == 0) && ((kernel_cnt) == 0))) {
        hdmi_info("hdmi is not open \n");
...
static td_void drv_hdmi_black_data_set(const hdmi_device *hdmi_dev, td_bool enable)
{
    ...
    black_info.in_bit_depth = (td_u8)hdmi_dev->attr.vo_attr.in_bit_depth;
    black_info.in_color_space = (td_u8)hdmi_dev->attr.vo_attr.in_color_space;
    black_info.in_quantization = ((td_s32)black_info.in_color_space == (td_s32)HDMI_COLORSPACE_RGB) ?
        (td_u32)hdmi_dev->attr.vo_attr.rgb_quantization : (td_u32)hdmi_dev->attr.vo_attr.ycc_quantization;
    ...
    if (app_attr->enable_hdmi == TD_FALSE) {
        ...
        *tmds_mode = HDMI_TMDS_MODE_DVI;
        ...
        if ((vo_attr->hdmi_adapt_pix_clk > HDMI_EDID_MAX_HDMI14_TMDS_RATE) &&
            (app_attr->enable_clr_space_adapt == TD_FALSE)) {
...
static td_bool hdmi_ycbcr420_fmt_check(hdmi_device *hdmi_dev)
{
    ...
    if (hdmi_dev->attr.app_attr.auth_mode == TD_TRUE) {
        hdmi_debug("auth_mode: %u \n", (td_u32)hdmi_dev->attr.app_attr.auth_mode);
        return TD_TRUE;
    }
    ...
    if ((td_u32)vic != 0) {
        for (i = 0; i < sink_cap->support_y420_vic_num && i < hdmi_array_size(sink_cap->support_y420_format); i++) {
            if ((td_u32)vic == sink_cap->support_y420_format[i]) {
                search_out = TD_TRUE;
                break;
            }
        }
        for (i = 0; (search_out == TD_FALSE) &&
            ...
            if ((td_u32)vic == sink_cap->only_support_y420_format[i]) {
                search_out = TD_TRUE;
                break;
            }
        }
    }
    hdmi_debug("is Y420 support vic=%d :%s\n", (td_s32)vic, search_out ? "YES" : "NO");
...
static td_s32 hdmi_color_space_check(hdmi_device *hdmi_dev, const hdmi_app_attr *user_app)
{
    ...
    switch (user_app->out_color_space) {
        case HDMI_COLORSPACE_RGB:
            ...
            if ((td_bool)(sink_cap->color_space.rgb444 && tx_cap.tx_rgb444) == TD_FALSE) {
        ...
        case HDMI_COLORSPACE_YCBCR420:
            ...
            break;
        default:
            hdmi_err("un-know color_space=%d!\n", (td_s32)user_app->out_color_space);
            return TD_FAILURE;
    }

    if (support_clr_space == TD_FALSE) {
        hdmi_err("fail, not support color space:%d\n", (td_s32)user_app->out_color_space);
        return TD_FAILURE;
    }
...
{
    ot_unused(hdmi_dev);
    return (td_u32)hdmi_get_current_id();
}
...
static td_void edid_invalid_or_auth_mode(hdmi_device *hdmi_dev, hdmi_app_attr *user_app)
{
    ...
    if (user_app->out_color_space != HDMI_COLORSPACE_YCBCR422) {
        ...
        if ((user_app->deep_color_mode != HDMI_DEEP_COLOR_OFF) &&
            (user_app->deep_color_mode != HDMI_DEEP_COLOR_24BIT)) {
            hdmi_debug("Y422 foce deepcolor 8bit");
            user_app->deep_color_mode = HDMI_DEEP_COLOR_24BIT;
        }
...
static td_s32 hdmi_color_and_bit_strategy(hdmi_device *hdmi_dev, hdmi_app_attr *user_app)
{
    ...
    if (edid_ret == HDMI_EDID_DATA_INVALID || (user_app->auth_mode == TD_TRUE)) {
        ...
        hdmi_warn("there is no sink capability or auth_mode = %d, adapt hdmi_adapt_pix_clk = %u\n",
                  (td_s32)hdmi_dev->attr.app_attr.auth_mode, vo_attr->hdmi_adapt_pix_clk);
        return TD_SUCCESS;
    } else {
    ...
    hdmi_if_failure_return(ret, TD_FAILURE);
    hdmi_debug("clr_space_adapt=%u, deep_clr_adapt=%u, adptclrspace=%u, adptbit=%d, adptclk=%u, max_clk=%u\n",
        (td_u32)user_app->enable_clr_space_adapt,
        (td_u32)user_app->enable_deep_clr_adapt,
        (td_u32)user_app->out_color_space,
        (td_s32)user_app->deep_color_mode,
        vo_attr->hdmi_adapt_pix_clk,
        max_tmds_clk);
...
static td_void hdmi_video_attr_construct(hdmi_attr *attr, const hdmi_hardware_status *hw_status)
{
    ...
    if (status->avi_enable == TD_TRUE) {
         ...
         */
        aspect_is256 = ((((td_s32)video_code == 0) &&
                        (status->vsif[VENDOR_OFFSET_VIC] == HDMI_VIC_4K24)) ||
                        ((video_code >= HDMI_4096X2160P25_256_135) && (video_code <= HDMI_4096X2160P60_256_135)));

        video_attr->picture_aspect = (td_s32)((aspect_is256 == TD_TRUE) ? (td_s32)HDMI_PICTURE_ASPECT_256_135 :
        ...
        hdmi_debug("video_timing: %d, video_code: %d, picture_aspect: %d\n",
            (td_s32)video_attr->video_timing, (td_s32)video_code, (td_s32)video_attr->picture_aspect);
        if ((status->vsif_enable == TD_FALSE) && ((td_u32)video_code == TD_FALSE)) {
        ...
        app_attr->out_csc_quantization = (app_attr->out_color_space == HDMI_COLORSPACE_RGB) ?
            (td_u32)video_attr->rgb_quantization : ((td_u32)video_attr->ycc_quantization + 1);
    }
...
static td_s32 hpd_event(hdmi_device *hdmi_dev)
{
    ...
    for (i_edid = 0; i_edid < HDMI_EDID_TOTAL_SIZE; i_edid++) {
        g_hdmi_edid_raw_inner[i_edid] = hdmi_dev->edid_info.edid_raw[i_edid];
    }
...
static td_void event_notify(hdmi_device *hdmi_dev, hdmi_event event)
{
    if (event <= HDMI_EVENT_HOTUNPLUG) {
        ...
        if (hdmi_dev->k_callback == TD_NULL) { /* notify to user */
            hdmi_debug("notify event(0x%x) to user\n", (td_u32)event);
            drv_hdmi_event_pool_write(hdmi_dev->hdmi_dev_id, event);
        } else { /* notify to kernel */
            hdmi_debug("notify event(0x%x) to kernel\n", (td_u32)event);
    ...
    while (1) {
        ...
        if (hdmi_dev->hal != TD_NULL) {
            if (hdmi_dev->hal->hal_hdmi_sequencer_handler_process != TD_NULL &&
                ...
                hdmi_dev->hal->hal_hdmi_sequencer_handler_process((struct hdmi_hal_ *)hdmi_dev, hdmi_dev->hdmi_dev_id);
            }
        } else {
            hdmi_debug("input param is NULL\n");
        }
...
static td_s32 hdmi_device_init(hdmi_device *hdmi_dev)
{
...
#if defined(PHY_CRAFT_S28) || defined(HDMI_PRODUCT_SD3403V100) || defined(HDMI_PRODUCT_CHIP_1)
    ...
    hdmi_debug("video_timing:%d\n", (td_s32)video_attr->video_timing);
...
static td_void destroy_task(hdmi_device *hdmi_dev)
{
    if (hdmi_dev->thread_info.thread_timer != TD_NULL) {
        hdmi_debug("stop hdmi kernel thread timer\n");
        /*
         * note: in linux,
...
static td_void hdmi_oe_transition_state_set(hdmi_device *hdmi_dev, td_bool user, td_bool hdmi_on)
{
#if !defined(HDMI_FPGA_SUPPORT) || defined(HDMI_PRODUCT_CHIP_1)
    if (hdmi_on == TD_FALSE) {
        hdmi_debug("hdmi not on, do hardware init\n");
        hdmi_dev->transition_state = HDMI_TRANSITION_NONE;
        hal_call_void(hal_hdmi_hardware_init, hdmi_dev->hal);
    } else {
        hdmi_debug("hdmi already on, do not hardware init.\n");
        if (user) {
...
static td_void drv_hdmi_hpd_status_delay_get(const hdmi_device *hdmi_dev, td_bool *hotplug)
{
    ...
    hdmi_debug("delay %u times, hot plug status is 0x%d\n", i,
        (td_s32)(*hotplug ? HDMI_EVENT_HOTPLUG : HDMI_EVENT_HOTUNPLUG));
...
static td_s32 drv_hdmi_open(hdmi_device *hdmi_dev, td_bool user)
{
    ...
    ret = drv_hdmi_event_init(hdmi_dev->hdmi_dev_id);
    if (ret != TD_SUCCESS) {
        destroy_task(hdmi_dev);
        hdmi_err("drv_hdmi_event_init fail\n");
    ...
    hdmi_debug("user_cnt: %u, kernel_cnt: %u, user_call_back_cnt: %u\n",
        hdmi_dev->user_cnt,
        hdmi_dev->kernel_cnt,
        hdmi_dev->user_callback_cnt);

    hal_call_void(hal_hdmi_phy_output_enable_get, hdmi_dev->hal, &hdmi_on);
    hdmi_oe_transition_state_set(hdmi_dev, user, hdmi_on);
    ...
    hdmi_debug("transition_state: %d\n", (td_s32)hdmi_dev->transition_state);
...
static td_s32 drv_hdmi_close(hdmi_device *hdmi_dev, td_bool user)
{
    ...
    hdmi_debug("user: %u\n", (td_u32)user);
    hdmi_debug("user_cnt: %u, kernel_cnt: %u\n", hdmi_dev->user_cnt, hdmi_dev->kernel_cnt);

    hdmi_dev->hpd_notifyed = TD_FALSE;

    if (user == TD_TRUE && hdmi_dev->user_cnt > 0) {
        hdmi_debug("delete event queue for process:%u \n", hdmi_current_id_get(hdmi_dev));
...
static td_s32 drv_hdmi_ao_attr_set(hdmi_device *hdmi_dev, const hdmi_ao_attr *ao_attr)
{
    ...
    hdmi_info("DRM not support audio\n");
    return TD_SUCCESS;
#endif
    ...
    hdmi_debug("down_sample: %u\n", (td_u32)audio_attr->down_sample);
    hdmi_debug("sample_fs: %d\n", (td_s32)audio_attr->sample_fs);
    hdmi_debug("channels: %d\n", (td_s32)audio_attr->channels);
    hdmi_debug("sound_intf: %u\n", audio_attr->sound_intf);
    hdmi_debug("sample_depth: %d\n", (td_s32)audio_attr->sample_depth);
    hdmi_debug("audio_code: %d\n", (td_s32)audio_attr->audio_code);
...
static td_s32 check_video_attr(const hdmi_vo_attr *vo_attr)
{
    if (vo_attr->video_timing >= HDMI_VIDEO_TIMING_BUTT) {
        hdmi_err("video timing(%d) is wrong\n", (td_s32)vo_attr->video_timing);
        return TD_FAILURE;
    }

    if (vo_attr->in_color_space >= HDMI_COLORSPACE_BUTT) {
        hdmi_err("video in_color_space(%d) is wrong\n", (td_s32)vo_attr->in_color_space);
        return TD_FAILURE;
    }
    ...
    if (vo_attr->colorimetry != HDMI_COLORIMETRY_ITU_601 &&
        ...
        hdmi_err("video colorimetry(%d) is wrong\n", (td_s32)vo_attr->colorimetry);
        return TD_FAILURE;
    }

    if (vo_attr->rgb_quantization > HDMI_QUANTIZATION_RANGE_FULL) {
        hdmi_err("video rgb_quantization(%d) is wrong\n", (td_s32)vo_attr->rgb_quantization);
        return TD_FAILURE;
    }
    ...
    if (vo_attr->picture_aspect != HDMI_PICTURE_ASPECT_4_3 &&
        ...
        hdmi_err("video picture_aspect(%d) is wrong\n", (td_s32)vo_attr->picture_aspect);
        return TD_FAILURE;
    }
    ...
    if (vo_attr->in_bit_depth != HDMI_VIDEO_BITDEPTH_8 &&
        ...
        hdmi_err("video in_bit_depth(%d) is wrong\n", (td_s32)vo_attr->in_bit_depth);
        return TD_FAILURE;
    }
...
static td_bool app_attr_is_changed(const hdmi_app_attr *hw_app_attr, const hdmi_app_attr *sw_app_attr)
{
    if (hw_app_attr->enable_hdmi != sw_app_attr->enable_hdmi) {
        hdmi_debug("hdmi enable change, old(%u)->new(%u)\n", (td_u32)hw_app_attr->enable_hdmi,
            (td_u32)sw_app_attr->enable_hdmi);
        return TD_TRUE;
    }

    if (hw_app_attr->out_color_space != sw_app_attr->out_color_space) {
        hdmi_debug("out color_space change,old(%d)->new(%d)\n",
            (td_s32)hw_app_attr->out_color_space, (td_s32)sw_app_attr->out_color_space);
        return TD_TRUE;
    }
    ...
    if (((sw_app_attr->deep_color_mode == HDMI_DEEP_COLOR_OFF) ||
        ...
        hdmi_debug("deepcolor mode not change: %d\n", (td_s32)hw_app_attr->deep_color_mode);
    } else if (hw_app_attr->deep_color_mode != sw_app_attr->deep_color_mode) {
        hdmi_debug("deepcolor mode change,old(%d)->new(%d)\n",
            (td_s32)hw_app_attr->deep_color_mode, (td_s32)sw_app_attr->deep_color_mode);
        return TD_TRUE;
    }

    if (hw_app_attr->xvycc_mode != sw_app_attr->xvycc_mode) {
        hdmi_debug("xvycc_mode mode change,old(%u)->new(%u)\n",
            (td_u32)hw_app_attr->xvycc_mode, (td_u32)sw_app_attr->xvycc_mode);
        return TD_TRUE;
    }

    if (hw_app_attr->enable_avi_infoframe != sw_app_attr->enable_avi_infoframe) {
        hdmi_debug("avi infoframe enable change, old(%u)->new(%u)\n",
            (td_u32)hw_app_attr->enable_avi_infoframe, (td_u32)sw_app_attr->enable_avi_infoframe);
        return TD_TRUE;
    }

    if (hw_app_attr->out_csc_quantization != sw_app_attr->out_csc_quantization) {
        hdmi_debug("out_csc_quantization change, old(%d)->new(%d)\n",
            (td_s32)hw_app_attr->out_csc_quantization, (td_s32)sw_app_attr->out_csc_quantization);
        return TD_TRUE;
    }
...
static td_bool vo_attr_is_changed(const hdmi_vo_attr *hw_vo_attr, hdmi_vo_attr *sw_vo_attr)
{
    if (hw_vo_attr->video_timing != sw_vo_attr->video_timing) {
        hdmi_debug("video timing change,old(%d)->new(%d)\n", (td_s32)hw_vo_attr->video_timing,
            (td_s32)sw_vo_attr->video_timing);
        return TD_TRUE;
    } else {
        /*
    ...
    if (hw_vo_attr->in_color_space != sw_vo_attr->in_color_space) {
        hdmi_debug("input colorspace change, old(%d)->new(%d)\n",
            (td_s32)hw_vo_attr->in_color_space, (td_s32)sw_vo_attr->in_color_space);
        return TD_TRUE;
    }
    if (hw_vo_attr->stereo_mode != sw_vo_attr->stereo_mode) {
        hdmi_debug("3d mode change, old(%d)->new(%d)\n", (td_s32)hw_vo_attr->stereo_mode,
            (td_s32)sw_vo_attr->stereo_mode);
        return TD_TRUE;
    }
    if (hw_vo_attr->pixel_repeat != sw_vo_attr->pixel_repeat) {
        hdmi_debug("pixel repeation change, old(%u)->new(%u)\n", hw_vo_attr->pixel_repeat, sw_vo_attr->pixel_repeat);
        return TD_TRUE;
    }
    if (hw_vo_attr->colorimetry != sw_vo_attr->colorimetry) {
        hdmi_debug("colorimetry change, old(%d)->new(%d)\n", (td_s32)hw_vo_attr->colorimetry,
            (td_s32)sw_vo_attr->colorimetry);
        return TD_TRUE;
    }
    if (hw_vo_attr->extended_colorimetry != sw_vo_attr->extended_colorimetry) {
        hdmi_debug("extended_colorimetry change, old(%d)->new(%d)\n",
            (td_s32)hw_vo_attr->extended_colorimetry, (td_s32)sw_vo_attr->extended_colorimetry);
        return TD_TRUE;
    }
    if (hw_vo_attr->rgb_quantization != sw_vo_attr->rgb_quantization) {
        hdmi_debug("RGB quantization change, old(%d)->new(%d)\n",
            (td_s32)hw_vo_attr->rgb_quantization, (td_s32)sw_vo_attr->rgb_quantization);
        return TD_TRUE;
    }
    if (hw_vo_attr->ycc_quantization != sw_vo_attr->ycc_quantization) {
        hdmi_debug("YCC quantization change, old(%d)->new(%d)\n",
            (td_s32)hw_vo_attr->ycc_quantization, (td_s32)sw_vo_attr->ycc_quantization);
        return TD_TRUE;
    }
    if (hw_vo_attr->picture_aspect != sw_vo_attr->picture_aspect) {
        hdmi_debug("picture aspect change, old(%d)->new(%d)\n", (td_s32)hw_vo_attr->picture_aspect,
            (td_s32)sw_vo_attr->picture_aspect);
        return TD_TRUE;
    }
...
static td_s32 check_app_attr(const hdmi_app_attr *app_attr)
{
    ...
    if (app_attr->out_color_space != HDMI_COLORSPACE_RGB &&
        ...
        hdmi_err("out_color_space=%d is wrong\n", (td_s32)app_attr->out_color_space);
        ret = TD_FAILURE;
    }
    ...
    if (app_attr->deep_color_mode != HDMI_DEEP_COLOR_24BIT &&
        ...
        hdmi_err("deep_clr_mode=%d is wrong\n", (td_s32)app_attr->deep_color_mode);
        ret = TD_FAILURE;
    }
    ...
    if (app_attr->hdcp_mode != HDMI_HDCP_MODE_1_4 &&
        ...
        hdmi_err("hdcp_mode=%d is wrong\n", (td_s32)app_attr->hdcp_mode);
        ret = TD_FAILURE;
    }
...
static td_void hdmi_vo_attr_info(const hdmi_device *hdmi_dev, const hdmi_vo_attr *video_attr)
{
    ...
    hdmi_debug("clk_fs: %u\n", video_attr->clk_fs);
    hdmi_debug("pixel_repeat: %u\n", video_attr->pixel_repeat);
    hdmi_debug("video_timing: %d\n", (td_s32)video_attr->video_timing);
    hdmi_debug("stereo_mode: %d\n", (td_s32)video_attr->stereo_mode);
    hdmi_debug("in_color_space: %d\n", (td_s32)video_attr->in_color_space);
    hdmi_debug("colorimetry: %d\n", (td_s32)video_attr->colorimetry);
    hdmi_debug("extended_colorimetry: %u\n", video_attr->extended_colorimetry);
    hdmi_debug("rgb_quantization: %d\n", (td_s32)(td_s32)video_attr->rgb_quantization);
    hdmi_debug("ycc_quantization: %d\n", (td_s32)video_attr->ycc_quantization);
    hdmi_debug("picture_aspect: %d\n", (td_s32)video_attr->picture_aspect);
    hdmi_debug("active_aspect: %d\n", (td_s32)video_attr->active_aspect);
    hdmi_debug("picture_scaling: %d\n", (td_s32)video_attr->picture_scaling);
    hdmi_debug("in_bit_depth: %d\n", (td_s32)video_attr->in_bit_depth);
    hdmi_debug("disp_fmt: %u\n", video_attr->disp_fmt);
    hdmi_debug("v_sync_pol: %u\n", (td_u32)video_attr->v_sync_pol);
    hdmi_debug("h_sync_pol: %u\n", (td_u32)video_attr->h_sync_pol);
    hdmi_debug("de_pol: %u\n", (td_u32)video_attr->de_pol);
...
static td_s32 hdmi_format_is_change(hdmi_device *hdmi_dev, const hdmi_vo_attr *video_attr)
{
    ...
    if (hdmi_color_and_bit_strategy(hdmi_dev, &tmp_app) != TD_SUCCESS) {
...
static td_s32 drv_hdmi_infoframe_get(const hdmi_device *hdmi_dev,
    hdmi_infoframe_id infoframe_id, hdmi_infoframe_unit *info_frame)
{
    errno_t ret;
...
static td_void infoframe_set_vendor(hdmi_device *hdmi_dev, hdmi_infoframe_unit *info_frame)
{
#ifdef HDMI_VENDOR_SUPPORT
    errno_t ret;
     ...
     */
    if (src_vsif_content->len > (td_u8)HDMI_VENDOR_USER_DATA_MAX_LEN) {
        return;
    }
...
static td_void infoframe_set_avi(hdmi_device *hdmi_dev, const hdmi_infoframe_unit *info_frame)
{
    errno_t ret;
    ...
    hdmi_debug("type: %d\n", (td_s32)src_avi_info->type);
    hdmi_debug("version: %d\n", src_avi_info->version);
    hdmi_debug("length: %d\n", src_avi_info->length);
    hdmi_debug("colorspace: %d\n", (td_s32)src_avi_info->colorspace);
    hdmi_debug("active_info_valid: %u\n", (td_u32)src_avi_info->active_info_valid);
    hdmi_debug("horizontal_bar_valid: %u\n", (td_u32)src_avi_info->horizontal_bar_valid);
    hdmi_debug("vertical_bar_valid: %u\n", (td_u32)src_avi_info->vertical_bar_valid);
    hdmi_debug("scan_mode: %d\n", (td_s32)src_avi_info->scan_mode);
    hdmi_debug("colorimetry: %d\n", (td_s32)src_avi_info->colorimetry);
    hdmi_debug("picture_aspect: %d\n", (td_s32)src_avi_info->picture_aspect);
    hdmi_debug("active_aspect: %d\n", (td_s32)src_avi_info->active_aspect);
    hdmi_debug("extended_colorimetry: %d\n", (td_s32)src_avi_info->extended_colorimetry);
    hdmi_debug("quantization_range: %d\n", (td_s32)src_avi_info->quantization_range);
    hdmi_debug("video_timing: %d\n", (td_s32)src_avi_info->video_timing);
    hdmi_debug("disp_fmt: %d\n", (td_s32)src_avi_info->disp_fmt);
    hdmi_debug("ycc_quantization_range: %d\n", (td_s32)src_avi_info->ycc_quantization_range);
    hdmi_debug("content_type: %d\n", (td_s32)src_avi_info->content_type);
    hdmi_debug("pixel_repeat: %d\n", (td_s32)src_avi_info->pixel_repeat);
    hdmi_debug("top_bar: %d\n", (td_s32)src_avi_info->top_bar);
    hdmi_debug("bottom_bar: %d\n", (td_s32)src_avi_info->bottom_bar);
    hdmi_debug("left_bar: %d\n", (td_s32)src_avi_info->left_bar);
    hdmi_debug("right_bar: %d\n", (td_s32)src_avi_info->right_bar);
...
static td_s32 drv_hdmi_infoframe_set(hdmi_device *hdmi_dev, hdmi_infoframe_id infoframe_id,
                                     hdmi_infoframe_unit *info_frame)
{
    errno_t ret;

    hdmi_debug(">>> in..., infoframe_id: %d\n", (td_s32)infoframe_id);
    ...
    switch (infoframe_id) {
        ...
        case HDMI_INFOFRAME_TYPE_AUDIO: {
            ...
            hdmi_debug("type: %d\n", (td_s32)dest_audio_info->type);
            hdmi_debug("version: %d\n", (td_s32)dest_audio_info->version);
            hdmi_debug("length: %d\n", (td_s32)dest_audio_info->length);
            hdmi_debug("channels: %d\n", (td_s32)dest_audio_info->channels);
            hdmi_debug("coding_type: %d\n", (td_s32)dest_audio_info->coding_type);
            hdmi_debug("sample_size: %d\n", (td_s32)dest_audio_info->sample_size);
            hdmi_debug("sample_frequency: %u\n", dest_audio_info->sample_frequency);
            hdmi_debug("coding_type_ext: %d\n", (td_s32)dest_audio_info->coding_type_ext);
            hdmi_debug("channel_allocation: %d\n", (td_s32)dest_audio_info->channel_allocation);
            hdmi_debug("lfe_playback_level: %d\n", (td_s32)dest_audio_info->lfe_playback_level);
            hdmi_debug("level_shift_value: %d\n", (td_s32)dest_audio_info->level_shift_value);
            hdmi_debug("downmix_inhibit: %u\n", (td_u32)dest_audio_info->downmix_inhibit);
            hdmi_debug("level_shift_value: %d\n", (td_s32)dest_audio_info->level_shift_value);
            break;
        }
        default:
...
static td_s32 drv_hdmi_mod_param_set(hdmi_device *hdmi_dev, const drv_hdmi_mod_param *mode_param)
{
    ...
    hdmi_debug("set trace length: %u->%u, emi: %u->%u\n",
        (td_u32)param->trace_len, (td_u32)mode_param->trace_len, (td_u32)param->emi_en, (td_u32)mode_param->emi_en);
...
static td_s32 drv_hdmi_hw_spec_set(const hdmi_device *hdmi_dev, const hdmi_hw_spec *hw_spec)
{
    ...
    hdmi_debug("tmds_clk : %u\n", tmds_clk);
    for (i = 0; i < HW_PARAM_ARRAY_COUNT; i++) {
        hdmi_debug("stage[%u]: i_de_main_clk:%d i_de_main_data:%d\
            i_main_clk:%d i_main_data:%d ft_cap_clk:%d ft_cap_data:%d\n", (td_u32)i,
            (td_s32)hw_spec->hw_param[i].i_de_main_clk, (td_s32)hw_spec->hw_param[i].i_de_main_data,
            (td_s32)hw_spec->hw_param[i].i_main_clk, (td_s32)hw_spec->hw_param[i].i_main_data,
            (td_s32)hw_spec->hw_param[i].ft_cap_clk, (td_s32)hw_spec->hw_param[i].ft_cap_data);
    }
...
static td_s32 hdmi21_caps_check(const hdmi_device *hdmi_dev, const hdmi_property *attr)
{
    ...
    if (is_4k == TD_TRUE &&
        ...
        hdmi_err("param is invalid, fmt = %u, colorspace = %u, deepcolor = %u\n",
            (td_u32)attr->disp_fmt, (td_u32)attr->out_color_space, (td_u32)attr->deep_color_mode);
        return OT_ERR_HDMI_FEATURE_NO_SUPPORT;
    }
...
static td_s32 hdmi_ioctrl_open(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_OPEN user=%u\n", (td_u32)user);
    ...
    hdmi_mutex_lock(g_hdmi_mutex);
    if (hdmi_not_open_check(hdmi_dev->user_cnt, hdmi_dev->kernel_cnt) == TD_SUCCESS) {
        hdmi_mutex_unlock(g_hdmi_mutex);
        return TD_SUCCESS;
    }
...
static td_s32 hdmi_ioctrl_close(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_CLOSE user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_start(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_START user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_stop(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_STOP user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_get_sink_capability(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_GET_SINK_CAPABILITY user=%u\n", (td_u32)user);
    ot_unused(user);
...
static td_s32 hdmi_ioctrl_set_attr(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_SET_ATTR user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_get_attr(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_GET_ATTR user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_get_vo_attr(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_GET_VO_ATTR user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_set_timing(td_void *arg, td_bool user)
{
    td_s32 ret;
    hdmi_debug("CMD_HDMI_SET_TIMING user=%u\n", (td_u32)user);

    drv_hdmi_timing_format *timing_format = TD_NULL;
    timing_format = (drv_hdmi_timing_format *)arg;
    if (timing_format == TD_NULL) {
        hdmi_err("timing_format is null.\n");
        return TD_FAILURE;
    }

    td_u32 width = timing_format->timing_width;
    td_u32 height = timing_format->timing_height;
    td_u32 fps = timing_format->fps;
    td_u32 id = timing_format->hdmi_id;
    td_u32 pattern_color_space = timing_format->pattern_color_space;

    ret = hdmi_set_timing(width, height, fps, id, pattern_color_space);
    hdmi_dfx_info("return ret==%d\n", ret);
    if (ret == (td_s32)TD_FAILURE) {
        hdmi_err("hdmi_ioctrl_set_timing failed.\n");
    }
    return ret;
}

static td_s32 hdmi_ioctrl_reset_clock(td_void *arg, td_bool user)
{
    td_s32 ret;
    hdmi_debug("CMD_HDMI_RESET_CLOCK user=%u\n", (td_u32)user);

    drv_hdmi_reset_clk *reset_clk = TD_NULL;
    reset_clk = (drv_hdmi_reset_clk *)arg;
    if (reset_clk == TD_NULL) {
        hdmi_err("reset_clk is null.\n");
        return TD_FAILURE;
    }

    drv_hdmi_reset_clk reset_clk_temp;
    reset_clk_temp.reset_mode = reset_clk->reset_mode;

    ret = hdmi_reset_clock(&reset_clk_temp);
    if (ret == (td_s32)TD_FAILURE) {
        hdmi_err("hdmi_ioctrl_reset_clock failed.\n");
    }
    return ret;
}

static td_s32 hdmi_ioctrl_set_pattern(td_void *arg, td_bool user)
{
    td_s32 ret;
    hdmi_debug("CMD_HDMI_SET_PATTERN user=%u\n", (td_u32)user);

    drv_hdmi_pattern_mode *pattern_mode = TD_NULL;
    pattern_mode = (drv_hdmi_pattern_mode *)arg;
    if (pattern_mode == TD_NULL) {
        hdmi_err("pattern_mode is null.\n");
        return TD_FAILURE;
    }

    ret = hdmi_set_pattern(pattern_mode);
    if (ret == (td_s32)TD_FAILURE) {
        hdmi_err("hdmi_ioctrl_set_pattern failed.\n");
    }
    return ret;
}

static td_s32 hdmi_ioctrl_set_hotplug(td_void *arg, td_bool user)
{
    td_s32 ret;
    hdmi_debug("CMD_HDMI_SET_HOTPLUG user=%u\n", (td_u32)user);

    drv_hdmi_hotplug_status *temp = TD_NULL;
    temp = (drv_hdmi_hotplug_status *)arg;
    if (temp == TD_NULL) {
        hdmi_err("the pointer of hotplug_status is null.\n");
        return TD_FAILURE;
    }

    drv_hdmi_hotplug_status hotplug_status;
    ret = hdmi_set_hotplug(&hotplug_status);
    if (ret == (td_s32)TD_FAILURE) {
        hdmi_err("hdmi_ioctrl_set_hotplug failed.\n");
    }
    (void)osal_printk("hotplug_status: %u\n", hotplug_status.hotplug_on);
    ((drv_hdmi_hotplug_status *)arg)->hotplug_on = hotplug_status.hotplug_on;

    return ret;
}

static td_s32 hdmi_ioctrl_set_detection(td_void *arg, td_bool user)
{
    td_s32 ret;
    hdmi_debug("CMD_HDMI_SET_DETECTION user=%u\n", (td_u32)user);
    drv_hdmi_detection *temp = TD_NULL;
    temp = (drv_hdmi_detection *)arg;
    if (temp == TD_NULL) {
        hdmi_err("the pointer of hdmi_detection is null.\n");
        return TD_FAILURE;
    }

    drv_hdmi_detection hdmi_detection;
    ret = hdmi_set_detection(&hdmi_detection);
    if (ret == (td_s32)TD_FAILURE) {
        hdmi_err("hdmi_ioctrl_set_detection failed.\n");
    }
    ((drv_hdmi_detection *)arg)->timing_height = hdmi_detection.timing_height;
    ((drv_hdmi_detection *)arg)->timing_width = hdmi_detection.timing_width;
    return ret;
}

static td_s32 hdmi_ioctrl_set_capture(td_void *arg, td_bool user)
{
    td_s32 ret;
    hdmi_debug("CMD_HDMI_SET_CAPTURE user=%u\n", (td_u32)user);

    drv_hdmi_capture *temp = TD_NULL;
    temp = (drv_hdmi_capture *)arg;
    if (temp == TD_NULL) {
        hdmi_err("the pointer of capture_index is null.\n");
        return TD_FAILURE;
    }

    drv_hdmi_capture capture_index;
    capture_index.pos_y = temp->pos_y;
    capture_index.pos_x = temp->pos_x;

    ret = hdmi_set_capture(&capture_index);
    if (ret == (td_s32)TD_FAILURE) {
        hdmi_err("hdmi_ioctrl_set_capture failed.\n");
    }
    ((drv_hdmi_capture *)arg)->color_b = capture_index.color_b;
    ((drv_hdmi_capture *)arg)->color_r = capture_index.color_r;
    ((drv_hdmi_capture *)arg)->color_g = capture_index.color_g;

    (void)osal_printk("pixel_capture result , color: [%u %u %u]\n", capture_index.color_r, capture_index.color_g,
        capture_index.color_b);
    return ret;
}

static td_s32 hdmi_ioctrl_avi_infoframe(td_void *arg, td_bool user)
{
    td_s32 ret;
    hdmi_debug("CMD_HDMI_GET_VO_ATTR user=%u\n", (td_u32)user);

    drv_hdmi_infoframe_colorspace *temp = TD_NULL;
    temp = (drv_hdmi_infoframe_colorspace *)arg;
    if (temp == TD_NULL) {
        hdmi_err("the pointer of hdmi_id is null.\n");
        return TD_FAILURE;
    }

    ret = set_avi_infoframe_hdmi_mode(temp);
    if (ret == (td_s32)TD_FAILURE) {
        hdmi_err("hdmi_ioctrl_avi_infoframe failed.\n");
    }
    return ret;
}
#endif
...
static td_s32 hdmi_ioctrl_set_vo_attr(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_SET_VO_ATTR user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_get_infoframe(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_GET_INFOFRAME user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_set_infoframe(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_SET_INFOFRAME user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_avmute(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_SET_AVMUTE user=%u\n", (td_u32)user);
    ...
    if (drv_avmute->avmute) {
        drv_hdmi_audio_path_enable(hdmi_dev, (td_bool)(drv_avmute->avmute == TD_FALSE));
    } else {
...
static td_s32 hdmi_ioctrl_update_edid(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_UPDATE_EDID user=%u\n", (td_u32)user);
    ...
    if (ret == TD_SUCCESS) {
        ...
        if (edid_len != OT_ERR_HDMI_NULL_PTR && edid_len != OT_ERR_HDMI_INVALID_PARA
            && edid_len != TD_FAILURE) {
...
static td_s32 hdmi_ioctrl_register_callback(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_REGISTER_CALLBACK user=%u\n", (td_u32)user);
    ...
    if ((hdmi_dev->user_cnt != 0) || (hdmi_dev->kernel_cnt != 0)) {
         ...
         */
        hal_call_void_without_log(hal_hdmi_hdp_intr_status_get, hdmi_dev->hal, &intr_status);
...
static td_s32 hdmi_ioctrl_ungister_callback(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_UNREGISTER_CALLBACK user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_set_mod_param(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_SET_MOD_PARAM user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_get_mod_param(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_GET_MOD_PARAM user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_get_hw_spec(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_GET_HW_SPEC user=%u\n", (td_u32)user);
...
static td_s32 hdmi_ioctrl_set_hw_spec(td_void *arg, td_bool user)
{
    ...
    hdmi_debug("CMD_HDMI_SET_HW_SPEC user=%u\n", (td_u32)user);
...
static td_s32 hdmi_file_open(void *private_data)
{
    ...
    if (hdmi_check_is_same_pid() != TD_TRUE) {
        hdmi_err("HDMI functions must work in the same pid!!!");
        hdmi_mutex_unlock(g_hdmi_mutex);
        return TD_FAILURE;
    }
    /* 防止多线程场景 */
    if (osal_atomic_read(&g_hdmi_count) == 0) {
        osal_atomic_inc_return(&g_hdmi_count);
    } else {
        hdmi_info("hdmi already opened!\n");
        hdmi_mutex_unlock(g_hdmi_mutex);
        return TD_FAILURE;
    }
    hdmi_mutex_unlock(g_hdmi_mutex);
...
static td_s32 hdmi_file_close(void *private_data)
{
    ...
    if (osal_atomic_read(&g_hdmi_count)) {
        ...
        for (; hdmi_id < (td_s32)HDMI_ID_MAX; hdmi_id++) {
            hdmi_dev = get_hdmi_device(hdmi_id);
            hdmi_if_null_return(hdmi_dev, OT_ERR_HDMI_NULL_PTR);
...
{
    if (hdmi_check_is_same_pid() != TD_TRUE) {
        hdmi_info("the ioctl cmd if not from the same pid \n");
        return OT_ERR_HDMI_FEATURE_NO_SUPPORT;
    }
#ifndef CONFIG_COMPAT
...
static td_s32 hdmi_dev_register(td_void)
{
    ...
    if (g_hdmi_device == TD_NULL) {
        hdmi_err("g_hdmi_device is null pointer!\n");
        cmpi_unregister_module(OT_ID_HDMI);
        return TD_FAILURE;
    }
...
static td_void drv_hdmi_proc_add_module(const td_char *entry_name, const hdmi_proc_item *proc_item)
{
    ...
    proc->private_osal = (td_void *)proc_item->private;
...
static td_void hdmi_proc_register(td_void)
{
    ...
    for (hdmi_id = 0; hdmi_id < (td_s32)HDMI_ID_MAX; hdmi_id++) {
        hdmi_dev = get_hdmi_device(hdmi_id);
        if (hdmi_dev != TD_NULL) {
...
static const hdmi_ioctrl_func g_hdmi_cmd_func_tab[] = {
    ...
    { CMD_HDMI_SET_TIMING,          hdmi_ioctrl_set_timing },
    { CMD_HDMI_RESET_CLOCK,         hdmi_ioctrl_reset_clock },
    { CMD_HDMI_SET_PATTERN,         hdmi_ioctrl_set_pattern },
    { CMD_HDMI_SET_HOTPLUG,         hdmi_ioctrl_set_hotplug },
    { CMD_HDMI_SET_DETECTION,       hdmi_ioctrl_set_detection },
    { CMD_HDMI_SET_CAPTURE,         hdmi_ioctrl_set_capture },
    { CMD_HDMI_AVI_INFOFRAME,       hdmi_ioctrl_avi_infoframe },
#endif
};
...
td_s32 hdmi_mode_strategy(hdmi_device *hdmi_dev)
{
    ...
    hdmi_debug("tmds mode %d->%d.\n", (td_s32)hdmi_dev->tmds_mode, (td_s32)tmds_mode);
    ...
    if (sink_cap->support_scdc == TD_TRUE || hdmi_dev->attr.app_attr.auth_mode == TD_TRUE) {
        ...
        hdmi_debug("sink not support SCDC\n");
    }
#endif
...
td_s32 drv_hdmi_start(hdmi_device *hdmi_dev)
{
    ...
    if (hdmi_dev->attr.app_attr.enable_video == TD_FALSE) {
        hdmi_debug("video was disable by user, so send blackframe.\n");
        drv_hdmi_black_data_set(hdmi_dev, TD_TRUE);
    } else {
        drv_hdmi_black_data_set(hdmi_dev, TD_FALSE);
    }
    hdmi_dev->run_state = (td_s32)hdmi_dev->run_state & (~((td_s32)HDMI_RUN_STATE_STOP));
    hdmi_dev->run_state = (td_s32)hdmi_dev->run_state | (td_s32)(HDMI_RUN_STATE_START);

    hal_hdmi_phy_product_power_set(hdmi_dev->hdmi_dev_id);
...
td_s32 drv_hdmi_vo_attr_set(hdmi_device *hdmi_dev, const hdmi_vo_attr *vo_attr)
{
    ...
    if (hdmi_format_is_change(hdmi_dev, video_attr) != TD_SUCCESS) {
        return OT_ERR_HDMI_STRATEGY_FAILED;
    }
    ...
    if (vo_attr_is_changed(&hw_attr.vo_attr, video_attr) == TD_FALSE &&
        ...
        hdmi_debug("the video and app attr is not changed\n");
        return TD_SUCCESS;
    }
...
td_s32 drv_hdmi_attr_set(hdmi_device *hdmi_dev, hdmi_attr *attr)
{
    ...
    app_attr = &hdmi_dev->attr.app_attr;
    hdmi_debug("enable_hdmi: %u\n", (td_u32)attr->app_attr.enable_hdmi);
    hdmi_debug("enable_video: %u\n", (td_u32)attr->app_attr.enable_video);
    hdmi_debug("enable_audio: %u\n", (td_u32)attr->app_attr.enable_audio);
    hdmi_debug("out_color_space: %d\n", (td_s32)attr->app_attr.out_color_space);
    hdmi_debug("deep_color_mode: %d\n", (td_s32)attr->app_attr.deep_color_mode);
    hdmi_debug("bxv_ycc_mode: %u\n", (td_u32)attr->app_attr.xvycc_mode);
    hdmi_debug("enable_avi_info_frame: %u\n", (td_u32)attr->app_attr.enable_avi_infoframe);
    hdmi_debug("enable_spd_info_frame: %u\n", (td_u32)attr->app_attr.enable_spd_infoframe);
    hdmi_debug("enable_mpeg_info_frame: %u\n", (td_u32)attr->app_attr.enable_mpeg_infoframe);
    hdmi_debug("enable_aud_info_frame: %u\n", (td_u32)attr->app_attr.enable_aud_infoframe);
    hdmi_debug("debug_flag: %u\n", (td_u32)attr->app_attr.debug_flag);
    hdmi_debug("hdcp_mode: %d\n", (td_s32)attr->app_attr.hdcp_mode);
    hdmi_debug("enable_clr_space_adapt: %u\n", (td_u32)attr->app_attr.enable_clr_space_adapt);
    hdmi_debug("enable_deep_clr_adapt: %u\n", (td_u32)attr->app_attr.enable_deep_clr_adapt);
    hdmi_debug("auth_mode: %u\n", (td_u32)attr->app_attr.auth_mode);
    hdmi_debug("out_csc_quantization: %u\n", (td_u32)attr->app_attr.out_csc_quantization);

    if (check_app_attr(&attr->app_attr) != TD_SUCCESS) {
        hdmi_err("check hdmi attr fail\n");
        return OT_ERR_HDMI_INVALID_PARA;
    }
    if (hdmi_color_and_bit_strategy(hdmi_dev, &attr->app_attr) != TD_SUCCESS) {
        hdmi_err(" hdmi_color_and_bit_strategy fail\n");
        return OT_ERR_HDMI_STRATEGY_FAILED;
    }
...
{
    if ((hdmi_id < 0) || (hdmi_id >= HDMI_ID_MAX)) {
        return TD_NULL;
    }
    g_hdmi_ctrl[hdmi_id].hdmi_dev_id = (td_u32)hdmi_id;
    return &g_hdmi_ctrl[hdmi_id];
}

static td_void hdmi_drv_sema_desotry(td_void)
{
    td_u32 hdmi_id;
    hdmi_device *hdmi_dev = TD_NULL;

    /* osal_sema_destroy内部有判断入参是否为空，所以osal_sema_init失败全部释放 */
    for (hdmi_id = 0; hdmi_id < (td_u32)HDMI_ID_MAX; hdmi_id++) {
        hdmi_dev = get_hdmi_device(hdmi_id);
        if (hdmi_dev != TD_NULL) {
...
td_s32 hdmi_drv_mod_init(td_void)
{
    td_s32 i;
    hdmi_device *hdmi_dev = TD_NULL;
    errno_t ret = TD_SUCCESS;

    /* init csc param. */
    for (i = (td_s32)HDMI_DEVICE_ID0; i < (td_s32)HDMI_ID_MAX; i++) {
        hdmi_dev = get_hdmi_device(i);
        if (hdmi_dev != TD_NULL) {
            hdmi_dev->csc_param.colorimetry = HDMI_COLORIMETRY_ITU709;
#ifdef ASCEND310B_DRM_HDMI_ENABLE
            hdmi_dev->csc_param.pixel_encoding = HDMI_EXT_COLORSPACE_RGB;
#else
            hdmi_dev->csc_param.pixel_encoding = HDMI_EXT_COLORSPACE_YCBCR444;
#endif
            hdmi_dev->csc_param.quantization = HDMI_QUANT_RANGE_LIMITED;
            ret += osal_sema_init(&hdmi_dev->mutex_proc, 1);
            ret += osal_sema_init(&hdmi_dev->mutex_thread, 1);
        }
    }

    ret += osal_sema_init(&g_hdmi_mutex, 1);
    if (ret != TD_SUCCESS) {
        osal_sema_destroy(&g_hdmi_mutex);
        hdmi_drv_sema_desotry();
        hdmi_err("sema init hdmi mutex failed!\n");
        return ret;
    }
...
#if ((defined CONFIG_OT_PROC_SHOW_SUPPORT) && (!defined ASCEND310B_DRM_HDMI_ENABLE))
        drv_hdmi_proc_un_register(hdmi_id);
#endif
        drv_hdmi_hardware_reset(hdmi_id);
    }

#ifndef ASCEND310B_DRM_HDMI_ENABLE
    cmpi_unregister_module(OT_ID_HDMI);
...
#ifndef ASCEND310B_DRM_HDMI_ENABLE
    hdmi_info("unload hdmi.ko ....OK!\n");
#endif

    return;
}

/* extern hdmi_open interface for drm */
td_s32 drm_hdmi_open(td_void *arg, td_bool user)
{
    if (arg == TD_NULL) {
        hdmi_err("receive nullptr from drm\n");
        return TD_FAILURE;
    }
    return hdmi_ioctrl_open(arg, user);
}

/* extern hdmi_set_attr interface for drm */
td_s32 drm_hdmi_set_attr(td_void *arg, td_bool user)
{
    ...
    return hdmi_ioctrl_set_attr(arg, user);
}

/* extern hdmi_set_infoframe interface for drm */
td_s32 drm_hdmi_set_infoframe(td_void *arg, td_bool user)
{
    ...
    return hdmi_ioctrl_set_infoframe(arg, user);
}

/* extern hdmi_start interface for drm */
td_s32 drm_hdmi_start(td_void *arg, td_bool user)
{
    ...
    return hdmi_ioctrl_start(arg, user);
}

/* set hdmi dphy power */
td_void drv_hdmi_phy_product_power_set_param(td_void)
{
    hal_hdmi_phy_product_power_set_param();
}

#ifdef ASCEND310B_DRM_HDMI_ENABLE
/* extern hdmi_update_edid interface for drm */
td_void drm_hdmi_update_edid(struct pppoe_tag *tag)
{
    int count;
    char *p_edid;
    p_edid = tag->tag_data;
    for (count = 0; count < DRM_HDMI_EDID_LEN; count++) {
        *(p_edid + count) = g_hdmi_edid_raw_inner[count];
    }
    return;
}
#endif

/* extern hdmi_clsoe interface for drm */
td_s32 drm_hdmi_close(td_void *arg, td_bool user)
{
    ...
    (void)hdmi_ioctrl_stop(arg, user);
    return hdmi_ioctrl_close(arg, user);
}
...
typedef struct {
    ...
    td_u32 format;
} drm_hdmi_format_param;

typedef struct {
    ot_hdmi_video_format format;
    hdmi_video_timing timing;
} drm_hdmi_format_to_timing;

static const drm_hdmi_format_param g_hdmi_format_param_table[] = {
    ...
    /* VIC  PixFreq  HACT  VACT  FldRate           EncFmt */
    { DRM_HDMI_VIC_PC_BASE +  9,  40000,  800,  600, 6000, OT_HDMI_VIDEO_FORMAT_VESA_800X600_60   },
    { DRM_HDMI_VIC_PC_BASE + 13,  65000, 1024,  768, 6000, OT_HDMI_VIDEO_FORMAT_VESA_1024X768_60  },
    { DRM_HDMI_VIC_PC_BASE + 18, 162000, 1600, 1200, 6000, OT_HDMI_VIDEO_FORMAT_VESA_1600X1200_60 },
    { DRM_HDMI_VIC_PC_BASE + 25, 108000, 1280, 1024, 6000, OT_HDMI_VIDEO_FORMAT_VESA_1280X1024_60 },
    { DRM_HDMI_VIC_PC_BASE + 28,  85500, 1360,  768, 6000, OT_HDMI_VIDEO_FORMAT_VESA_1366X768_60  },
    { DRM_HDMI_VIC_PC_BASE + 41, 154000, 1920, 1200, 6000, OT_HDMI_VIDEO_FORMAT_VESA_1920X1200_60 },
...
static drm_hdmi_format_to_timing g_hdmi_format_to_timing[] = {
...
/* Searching for the hdmi timing based on the format */
td_s32 drm_hdmi_format_to_hdmi_timing(td_u32 format)
{
    td_u32 i;

/* Traverse all time timing supported by the HDMI. */
    for (i = 0; i < hdmi_array_size(g_hdmi_format_to_timing); i++) {
        if (format == (td_u32)(g_hdmi_format_to_timing[i].format)) {
            hdmi_debug("g_hdmi_format_to_timing[i].timing =:%u\n", g_hdmi_format_to_timing[i].timing);
            return g_hdmi_format_to_timing[i].timing;
        }
    }
    /* format not supported by the HDMI. */
    hdmi_warn("Error video fmt:%u\n", format);

    return HDMI_VIDEO_TIMING_UNKNOWN;
}

td_s32 drm_hdmi_get_timing_from_mode(td_u32 vact, td_u32 hact, td_u32 crtc_refresh_rate, td_u32 *hdmi_intf_sync,
                                     td_u32 *hdmi_pix_freq)
{
    td_s32 ret = TD_FAILURE;
    td_s32 i;
    td_u32 hdmi_intf_sync_res = OT_HDMI_VIDEO_FORMAT_1080P_60;
    td_u32 hdmi_pix_freq_res = 148500; // 148500:默认像素时钟

    hdmi_debug("vact = %u, hact = %u crtc_refresh_rate = %u, hdmi_pix_freq=:%u\n", vact, hact, crtc_refresh_rate,
               *hdmi_pix_freq);

    for (i = 0; i < DRM_HDMI_FORMAT_NUM; i++) {
        if (vact == g_hdmi_format_param_table[i].ver_active && hact == g_hdmi_format_param_table[i].hor_active &&
            (*hdmi_pix_freq) == (g_hdmi_format_param_table[i].pix_freq) &&
            crtc_refresh_rate == (g_hdmi_format_param_table[i].field_rate / DRM_HDMI_PIX_CLK_BASE)) {
            hdmi_intf_sync_res = g_hdmi_format_param_table[i].format;
            hdmi_pix_freq_res = g_hdmi_format_param_table[i].pix_freq;
            hdmi_debug("get timing from mode ok!\n");
            ret = TD_SUCCESS;
            break;
        }
    }

    *hdmi_intf_sync = hdmi_intf_sync_res;
    *hdmi_pix_freq = hdmi_pix_freq_res;
    return ret;
}
...
td_s32 drm_hdmi_get_connect_status(td_u32 hdmi_id)
{
    ...
    if (ret != TD_SUCCESS) {
        hdmi_info("no HDMI status!\n");
        return TD_FAILURE;
    }
    if (current_status.status.connected == TD_FALSE) {
        hdmi_debug("status: not connect!\n");
        return TD_FAILURE;
    } else if (current_status.status.connected == TD_TRUE) {
        hdmi_debug("status: connect!\n");
        return TD_SUCCESS;
    }
    return TD_FAILURE;
}
