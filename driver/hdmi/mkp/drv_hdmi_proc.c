...
#define STR_LEN_SUPERADDITION 4U
#define PROC_LENGTH_SIZE 21U
#define PORC_ONE_LINE_STR_COUNT 4
#define PROC_TEMP_STR_LEN 25U
...
static const td_char const *g_version = "HDMI version:   " MAKE_VERSION;

static td_void hdmi_print_proc_title(const hdmi_proc_entry *file, const td_u8 *title, td_u32 len)
{
    td_u32 i;
...
static td_void proc_sw_version(const hdmi_proc_entry *file)
{
    osal_seq_printf(file, "%s\n", g_version);
    return;
}

static td_void app_attr_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    td_u32 index;
    ...
    osal_seq_printf(file, "\n");
    /* line 2 */
    index = min((td_u32)app_attr->deep_color_mode, (td_u32)HDMI_DEEP_COLOR_48BIT + 1U);
...
static td_void sw_status_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    td_u32 i, index;
    hdmi_device *hdmi_dev = TD_NULL;
    ...
    if (hdmi_dev->run_state > HDMI_RUN_STATE_NONE) {
        for (i = 0; i < hdmi_array_size(g_run_state_str); i++) {
            if ((td_u32)hdmi_dev->run_state & ((td_u32)1 << i)) {
    ...
    osal_seq_printf(file, "\n");
    /* line 2 */
    index = min((td_u32)hdmi_dev->tmds_mode, (td_u32)HDMI_TMDS_MODE_BUTT);
    osal_seq_printf(file, "%-20s: %-20s", "tmds mode", g_tmds_mode_str[index]);
    osal_seq_printf(file, "%-20s: %-20u", "kernel cnt", hdmi_dev->kernel_cnt);
    osal_seq_printf(file, "%-20s: %-20u", "user cnt", hdmi_dev->user_cnt);
    osal_seq_printf(file, "\n");
    /* line 3 */
    osal_seq_printf(file, "%-20s: %-20s", "kernel callback", (hdmi_dev->k_callback != TD_NULL) ? STR_YES : STR_NO);
    osal_seq_printf(file, "%-20s: %-20u", "user callback cnt", hdmi_dev->user_callback_cnt);
    osal_seq_printf(file, "\n");
    ...
    if (hdmi_dev->transition_state > HDMI_TRANSITION_NONE) {
        for (i = 0; i < hdmi_array_size(g_transition_sate_str); i++) {
            if ((td_u32)hdmi_dev->transition_state & ((td_u32)1 << i)) {
    ...
    index = (td_u32)hdmi_dev->mode_param.trace_len;
...
static td_void hw_scramble_status_show(const hdmi_device *hdmi_dev, const hdmi_proc_entry *file)
{
    ...
    osal_seq_printf(file, "%-20s: %-20u", "tmds_bit_clk_ratio", (td_u32)scdc_status.tmds_bit_clk_ratio);
#else
    ot_unused(hdmi_dev);
    ot_unused(file);
#endif
...
static td_void hw_detect_timming_show(const hdmi_proc_entry *file, const hdmi_video_status *video_stat)
{
    /* construct a title */
...
static td_void hw_status_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    td_u32 index;
    hdmi_device *hdmi_dev = TD_NULL;
    ...
    index = min((td_u32)hw_status->common_status.tmds_mode, (td_u32)HDMI_TMDS_MODE_BUTT);
    osal_seq_printf(file, "%-20s: %-20s", "tmds mode", g_tmds_mode_str[index]);
...
static td_void proc_if_raw_data(const hdmi_proc_entry *file, td_u32 *index, td_u32 *line,
    const td_u8 *data, td_u32 data_len)
{
    ...
    for (; (*index < len) && (*index < data_len); (*index)++) {
        ...
        osal_seq_printf(file, "%02x ", (td_u32)data[(*index)]);
    }
    osal_seq_printf(file, "\n");
...
static td_void proc_vic_timing(const hdmi_proc_entry *file, const hdmi_vo_attr *vo_attr,
    const hdmi_infoframe_status *info_status)
{
    ...
    td_char *fmt_str_temp = TD_NULL;
    hdmi_video_def *video = TD_NULL;
    hdmi_video_4k_def *video_4k = TD_NULL;

    index = min((td_u32)vo_attr->video_timing, (td_u32)HDMI_VIDEO_TIMING_BUTT);
    /* find vic according to voattr */
    vic = (td_u32)drv_hdmi_vic_search(index, vo_attr->picture_aspect, TD_FALSE);
    i = drv_hdmi_vic_to_index(vic);
    video = drv_hdmi_comm_format_param_get(vic);
    fmt_str_temp = "NONE";
    if (video != TD_NULL && i < CEA_VIDEO_CODE_MAX && video->fmt_str != TD_NULL) {
        fmt_str_temp = video->fmt_str;
    }
    osal_seq_printf(file, "%-20s: %-20s |", "video timing", fmt_str_temp);
    osal_seq_printf(file, "%-20s: %-20s \n", "avi infoframe enable", info_status->avi_enable ? STR_YES : STR_NO);
    /* line 2 */
    index = min((td_u32)vo_attr->disp_fmt, (td_u32)(hdmi_array_size(g_disp_fmt_str) - 1U));
    ...
    /* line 3 */
    osal_seq_printf(file, "%-20s: %-20u |", "pixel clk", vo_attr->clk_fs);
    osal_seq_printf(file, "%-20s: %s(HDMI vic=%2u)\n", "vsif format", (vsif_fmt_str == TD_NULL) ? "" : vsif_fmt_str,
        (format == HDMI_VIDEO_FORMAT_4K) ? (td_u32)info_status->vsif[VENDOR_OFFSET_VIC] : 0);

    return;
}

static td_void proc_video_attr(const hdmi_proc_entry *file, const hdmi_vo_attr *vo_attr,
    const hdmi_infoframe_status *info_status)
{
    td_u32 index;
    ...
    /* line 4 */
    index = min((td_u32)vo_attr->in_bit_depth, (td_u32)HDMI_VIDEO_BITDEPTH_BUTT);
    ...
    /* line 5 */
    index = min((td_u32)vo_attr->in_color_space, (td_u32)HDMI_COLORSPACE_BUTT);
    ...
    /* line 6 */
    index = min((td_u32)vo_attr->colorimetry, (td_u32)HDMI_COLORIMETRY_EXTEND);
    osal_seq_printf(file, "%-20s: %-20s |", "colorimetry", g_colorimetry_str[index]);
    osal_seq_printf(file, "%-20s: %-20s \n", "colorimetry", g_colorimetry_str[byte2.bits.colorimetry]);
    /* line 7 */
    index = min((td_u32)vo_attr->picture_aspect, (td_u32)HDMI_PICTURE_ASPECT_BUTT);
    ...
    /* line 8 */
    index = min((td_u32)vo_attr->active_aspect, (td_u32)HDMI_ACTIVE_ASPECT_BUTT);
    ...
    /* line 9 */
    osal_seq_printf(file, "%-20s: %-20u |", "pixel repeat", vo_attr->pixel_repeat);
    osal_seq_printf(file, "%-20s: %-20s \n", "pixel repeat", g_pixel_repetition_str[byte5.bits.pixel_repetition]);
    /* line 10 */
    index = min((td_u32)vo_attr->ycc_quantization, (td_u32)HDMI_YCC_QUANTIZATION_RANGE_BUTT);
    ...
    /* line 11 */
    index = min((td_u32)vo_attr->rgb_quantization, (td_u32)HDMI_QUANTIZATION_RANGE_BUTT);
    ...
    /* line 12 */
    index = min((td_u32)vo_attr->extended_colorimetry, (td_u32)HDMI_EXTENDED_COLORIMETRY_BUTT);
    ...
    /* line 13 */
    index = min((td_u32)vo_attr->stereo_mode, (td_u32)HDMI_3D_BUTT);
    ...
    /* line 14 */
    osal_seq_printf(file, "%-20s: %-20u |", "vsync pol", (td_u32)vo_attr->v_sync_pol);
    osal_seq_printf(file, "%-20s: %-20u \n", "hsync pol", (td_u32)vo_attr->h_sync_pol);

    return;
}

static td_void proc_video_path(const hdmi_proc_entry *file, const hdmi_hardware_status *hw_status)
{
    td_u32 i, index, line;
    ...
    /* line 17 */
    index = min((td_u32)hw_status->video_status.out_bit_depth, (td_u32)HDMI_VIDEO_BITDEPTH_BUTT);
    ...
    /* line 18 */
    index = min((td_u32)hw_status->video_status.out_color_space, (td_u32)HDMI_COLORSPACE_BUTT);
    ...
    /* line 25 */
    index = min((td_u32)hw_status->video_status.dither, (td_u32)HDMI_VIDEO_DITHER_DISALBE);
...
static td_void video_info_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    td_u32 len;
    hdmi_device  *hdmi_dev = TD_NULL;
    hdmi_vo_attr *vo_attr  = TD_NULL;
    static hdmi_hardware_status hw_status = {0};
...
static td_void porc_auido_attr(const hdmi_proc_entry *file, const hdmi_ao_attr *ao_attr,
    const hdmi_hardware_status *hw_status)
{
    ...
    /* line 1 */
    index = min((td_u32)ao_attr->sound_intf, (td_u32)HDMI_AUDIO_INTF_BUTT);
    osal_seq_printf(file, "%-20s: %-20s |", "sound intf", g_sound_intf_str[index]);
    osal_seq_printf(file, "%-20s: %-20s \n", "audio info enable",
        hw_status->info_frame_status.audio_enable ? STR_YES : STR_NO);
    /* line 2 */
    index = min((td_u32)ao_attr->audio_code, (td_u32)HDMI_AUDIO_CODING_TYPE_BUTT);
    ...
    /* line 3 */
    index = min((td_u32)ao_attr->channels, (td_u32)HDMI_AUDIO_FORMAT_BUTT);
    ...
    /* line 4 */
    osal_seq_printf(file, "%-20s: %-20u |", "sample freq", (td_u32)ao_attr->sample_fs);
    osal_seq_printf(file, "%-20s: %-20s \n", "sample freq",
        g_audio_samping_freq_str[audio_byte2.bits.audio_samping_freq]);
    /* line 5 */
    osal_seq_printf(file, "%-20s: %-20u |", "sample depth", (td_u32)ao_attr->sample_depth);
    osal_seq_printf(file, "%-20s: %-20u \n", "sample depth", (td_u32)hw_status->audio_status.sample_depth);
    /* line 6 */
...
static td_void proc_audio_path(const hdmi_proc_entry *file, const hdmi_hardware_status *hw_status)
{
    td_u32 i, len, index;
    ...
    /* line 10 */
    index = min((td_u32)hw_status->audio_status.sound_intf, (td_u32)HDMI_AUDIO_INTF_BUTT);
    osal_seq_printf(file, "%-20s: %-20s |", "sound intf", g_sound_intf_str[index]);
    osal_seq_printf(file, "%-20s: 0x%02x (%u)\n", "channel/speakeralloc",
        (td_u32)hw_status->info_frame_status.audio[AUDIO_OFFSET_CA],
        (td_u32)hw_status->info_frame_status.audio[AUDIO_OFFSET_CA]);
    /* line 11 */
    index = min((td_u32)hw_status->audio_status.layout, (td_u32)HDMI_AUDIO_FORMAT_BUTT);
    osal_seq_printf(file, "%-20s: %-20s |", "channel cnt", g_audio_channel_str[index]);
    osal_seq_printf(file, "%-20s: \n", "audio info raw data");
    /* line 12 */
    osal_seq_printf(file, "%-20s: %-20u |", "sample freq", (td_u32)hw_status->audio_status.sample_fs);
    ...
    for (i = 0; (i < len) && (i < HDMI_INFO_FRAME_MAX_SIZE); i++) {
        ...
        osal_seq_printf(file, "%02x ", (td_u32)hw_status->info_frame_status.audio[i]);
    }
    osal_seq_printf(file, "\n");
    /* line 13 */
    osal_seq_printf(file, "%-20s: %-20u |", "sample depth", (td_u32)hw_status->audio_status.sample_depth);
    len = hw_status->info_frame_status.audio[AUDIO_OFFSET_LENGHT] + STR_LEN_SUPERADDITION;
...
static td_void audio_info_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    td_u32 len;
    hdmi_device *hdmi_dev = TD_NULL;
...
static td_void event_info_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    td_u32 i, j;
    td_s32 ret;
    td_u32 total_pool = 0;
    ...
    for (i = 0; i < total_pool; i++) {
        ...
        /* line4 */
        osal_seq_printf(file, "memory[wkflg=%1u |rdable=%2u| rdptr=%-2u| wrptr=%-2u]:",
            (td_u32)ctrl.wakeup_flag, ctrl.readable_cnt, ctrl.read_ptr, ctrl.write_ptr);
        for (j = 0; j < hdmi_array_size(ctrl.event_pool); j++) {
            if (ctrl.event_pool[j] < HDMI_EVENT_BUTT) {
                osal_seq_printf(file, " 0x%02x", (td_u32)ctrl.event_pool[j]);
            }
        }
        osal_seq_printf(file, "\n");
    }
...
static td_void edid_raw_data_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    td_s32 ret;
    td_u32 raw_len, i;
    ...
    if (raw_len != 0xffffffff) {
        for (i = 0; i < raw_len && i < HDMI_EDID_TOTAL_SIZE; i++) {
            ...
            osal_seq_printf(file, "0x%02x,", (td_u32)edid_raw[i]);
...
static td_void edid_status_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    td_s32 ret;
    ...
    osal_seq_printf(file, "%-20s: %-20u \n", "parse error type", (td_u32)status.parse_err);
    /* line 3 */
    ...
    /* line 4 */
    osal_seq_printf(file, "%-20s: %-20u", "raw get err cnt", status.raw_get_err_cnt);
    osal_seq_printf(file, "%-20s: %-20u", "raw length", status.raw_len);
...
static td_s32 proc_get_cap_info(hdmi_device_id hdmi_id, hdmi_sink_capability **cap)
{
    ...
    edid_info = &hdmi_dev->edid_info;
    ret = (td_s32)drv_hdmi_edid_capability_get(edid_info, cap);
...
static td_void sink_basic_cap_proc_show(const hdmi_proc_entry *file, const hdmi_sink_capability *cap)
{
    td_u32 i;
    errno_t ret;
    ...
    /* line 3 */
    osal_seq_printf(file, "%-20s: %-20u", "max tmds clock(MHz)", cap->max_tmds_clock);
    ...
    /* line 5 */
    osal_seq_printf(file, "%-20s: %-20u", "max disp width", (td_u32)cap->basic_disp_para.max_image_width);
    osal_seq_printf(file, "%-20s: %-20u\n", "max disp height", (td_u32)cap->basic_disp_para.max_image_height);
    /* line 6 */
    ...
    osal_seq_printf(file, "%-20s: %02x.%02x.%02x.%02x\n", "cec addr",
        (td_u32)cap->cec_addr.phy_addr_a, (td_u32)cap->cec_addr.phy_addr_b,
        (td_u32)cap->cec_addr.phy_addr_c, (td_u32)cap->cec_addr.phy_addr_d);
    /* line 9 */
    osal_seq_printf(file, "%-20s: %-20u", "ext block cnt", (td_u32)cap->ext_block_num);
    /* speaker */
...
static td_void proc_sink_video_deepcolor(const hdmi_proc_entry *file, const hdmi_sink_capability *cap)
{
    /* line 3 deep_color */
...
static td_void proc_sink_video_colorimetry(const hdmi_proc_entry *file, const hdmi_edid_colorimetry *colorimetry)
{
...
static td_void sink_video_cap_proc_show(const hdmi_proc_entry *file, const hdmi_sink_capability *cap)
{
    td_u32 i;
    ...
    /* line 1 */
    osal_seq_printf(file, "%-20s: %s(vic %2u)\n", "native format",
...
static td_bool proc_sink_std_format(const hdmi_proc_entry *file, const hdmi_edid_std_timing *std_timing)
{
    td_s32 n;
    td_s32 ret;
    ...
    if (fmt_str != TD_NULL) {
        ...
        n = snprintf_s(tmp_array, PROC_TEMP_STR_LEN, (PROC_TEMP_STR_LEN - 1U), "%ux%u",
        ...
        if ((std_timing->refresh_rate) != 0) {
            ret = snprintf_s((tmp_array + n), (PROC_LENGTH_SIZE - (td_u32)n),
                (PROC_LENGTH_SIZE - (td_u32)n - 1U), "_%u", std_timing->refresh_rate);
            if (ret < 0) {
                hdmi_err("snprintf_s err\n");
                return TD_FALSE;
            }
...
static td_void sink_format_cap_proc_show(const hdmi_proc_entry *file, const hdmi_sink_capability *cap)
{
    td_u32 i;
    td_u32 j = 0;
    td_char *tmp_fmt = TD_NULL;
...
static td_void sink_audio_cap_proc_show(const hdmi_proc_entry *file, const hdmi_sink_capability *cap)
{
    td_u32 i, j;
    ...
    for (i = 0; (i < cap->audio_info_num) && (i < HDMI_EDID_MAX_AUDIO_CAP_COUNT); i++) {
        ...
        osal_seq_printf(file, "%-20s: %-20u\n", "max channel num", (td_u32)audio_info->aud_channel);
        /* line 3 */
        if (audio_info->max_bit_rate) {
            osal_seq_printf(file, "%-20s: %-20u", "max bit rate(KHz)", audio_info->max_bit_rate);
        } else {
        ...
        for (j = 0; (j < audio_info->support_bit_depth_num) && (j < HDMI_EDID_MAX_BIT_DEPTH_NUM); j++) {
            osal_seq_printf(file, " %u", (td_u32)audio_info->support_bit_depth[j]);
        }
        osal_seq_printf(file, "\n");
        /* line 4 */
        osal_seq_printf(file, "%-20s:", "sample rate(Hz)");
        for (j = 0; (j < audio_info->support_sample_rate_num) && (j < HDMI_EDID_MAX_SAMPE_RATE_NUM); j++) {
            osal_seq_printf(file, " %u", (td_u32)audio_info->support_sample_rate[j]);
        }
...
static td_void sink_detail_timing_proc_show(const hdmi_proc_entry *file, const hdmi_sink_capability *cap)
{
    td_u32 i;
...
td_void spec_info_printf(const hdmi_proc_entry *file, const hdmi_hw_spec *user,
                         const hdmi_hw_spec *def, const hdmi_hw_param *cur)
{
    ...
    osal_seq_printf(file, "%-14s|%-6u|%-6u|%-6u|%-7u|%-6u|%-6u|%-6u|%-6u|%-13u\n", "ft_cap_clk",
        ...
        user->hw_param[2U].ft_cap_clk, user->hw_param[HW_PARAM_600M_BAND].ft_cap_clk,
        def->hw_param[HW_PARAM_100M_BAND].ft_cap_clk, def->hw_param[HW_PARAM_165M_BAND].ft_cap_clk,
        def->hw_param[2U].ft_cap_clk, def->hw_param[HW_PARAM_600M_BAND].ft_cap_clk,
        cur->ft_cap_clk);
...
static td_void phy_spec_info_proc_show(const hdmi_proc_entry *file, hdmi_device_id hdmi_id)
{
    hdmi_device *hdmi_dev = TD_NULL;
...
td_s32 drv_hdmi_proc_show(hdmi_proc_entry *file)
{
    ...
    hdmi_dev = (hdmi_device *)file->private_osal;
