...
#define HDMI_4K_30_HZ_TMDS_CLK  297000U
...
static hdmi_phy_info g_hdmi_phy_info[(td_s32)HDMI_ID_MAX];
...
static const struct tmds_spec_params g_tmds_spec_4k_30hz[] = {
    { 165000, 340000,
      {{ TD_FALSE, TD_TRUE, TD_TRUE },
       { 0x00, 0x02, 0x1B, 0x02 },
       { 0x7f, 0x7e, 0x3e, 0xf7, 0x06, 0x0A, 0x6C, 0x80, 0x00, 0x60 }},
      {{ TD_FALSE, TD_TRUE, TD_TRUE },
       { 0x00, 0x02, 0x17, 0x00 },
       { 0x7f, 0x7e, 0x3e, 0xf7, 0x06, 0x0A, 0x6C, 0x80, 0x00, 0x60 }}
    }
...
static td_void phy_hw_write_stb1_byte(td_u32 id, td_u32 cs, aphy_offset_addr aphy_offset, td_u32 wdata)
{
    ...
    /* ADDR set */
    hdmi_reg_stb_addr_set(id, (td_u8)aphy_offset);
    osal_udelay(1);
    /* WR set */
...
static td_void phy_hw_write_stb(td_u32 id, write_param param)
{
    ...
    if ((param.msb == 0x7) && (param.lsb == 0x0)) {
        ...
        mask_value  = (0xffU >> (0x7 - param.msb)) & (0xffU << param.lsb);
...
static const struct tmds_spec_params *get_tmds_spec_params(const hdmi_phy_tmds_cfg *tmds_cfg)
{
    ...
    if ((tmds_cfg->tmds_clk == HDMI_4K_30_HZ_TMDS_CLK) && (tmds_cfg->pixel_clk == HDMI_4K_30_HZ_TMDS_CLK)) {
        tmp = &g_tmds_spec_4k_30hz[0];
        return &tmp[0];
    }
...
void hdmi_write_mask(td_u32 id, td_u32 offset, td_u32 val, td_u32 mask)
{
    td_u32 tmp;
    td_char *reg = TD_NULL;
...
static void phy_hw_tmds_dphy_spec_set_4k_30hz(td_u32 id)
{
    /* dphy power set:data lane0 ,post2 = 02,post1 = 1b,pre = 02 */
    hdmi_write_mask(id, 0x510, 0x40021B02, 0xffffffff);
    /* dphy power set:data lane1 ,post2 = 02,post1 = 1b,pre = 02 */
    hdmi_write_mask(id, 0x514, 0x40021B02, 0xffffffff);
    /* dphy power set:data lane2 ,post2 = 02,post1 = 1b,pre = 02 */
    hdmi_write_mask(id, 0x518, 0x40021B02, 0xffffffff);
    /* dphy power set:clk lane ,post2 = 00,post1 = 17,pre = 00 */
    hdmi_write_mask(id, 0x51c, 0x40001700, 0xffffffff);
}
...
static td_s32 phy_user_spec_param_set(td_u32 id, td_s32 tmds_clk, hdmi_trace_len len, struct tmds_spec_params *spec)
{
    ...
    for (i = 0; tmp != TD_NULL && (i < hdmi_array_size(g_tmds_spec)); i++, tmp++) {
        ...
        if ((td_u32)tmds_clk >= tmp->min_tmds_clk && (td_u32)tmds_clk <= tmp->max_tmds_clk) {
...
static td_s32 phy_tmds_spec_set(td_u32 id, const hdmi_phy_tmds_cfg *tmds_cfg)
{
    ...
    (td_void)phy_user_spec_param_set(id, (td_s32)tmds_cfg->tmds_clk, tmds_cfg->trace_len, &tmds_spec);

    phy_hw_tmds_aphy_spec_set(id, &tmds_spec);

    if ((tmds_cfg->tmds_clk == HDMI_4K_30_HZ_TMDS_CLK) && (tmds_cfg->pixel_clk == HDMI_4K_30_HZ_TMDS_CLK)) {
        phy_hw_tmds_dphy_spec_set_4k_30hz(id);
        return TD_SUCCESS;
    }
...
static td_void phy_hw_ref_clk_div_set(td_u32 id, td_u32 pixel_clk)
{
    ...
    } else if (pixel_clk >= g_phy_hw_def_clk_div[2].clk_range.clk_min && /* 2, array index */
               pixel_clk < g_phy_hw_def_clk_div[2].clk_range.clk_max) { /* 2, array index */
        ref_clk_div = g_phy_hw_def_clk_div[2].seek_value; /* 2, array index */
    } else if (pixel_clk >= g_phy_hw_def_clk_div[3].clk_range.clk_min && /* 3, array index */
               pixel_clk < g_phy_hw_def_clk_div[3].clk_range.clk_max) { /* 3, array index */
        ref_clk_div = g_phy_hw_def_clk_div[3].seek_value; /* 3, array index */
    } else if (pixel_clk >= g_phy_hw_def_clk_div[4].clk_range.clk_min && /* 4, array index */
               pixel_clk < g_phy_hw_def_clk_div[4].clk_range.clk_max) { /* 4, array index */
        ref_clk_div = g_phy_hw_def_clk_div[4].seek_value; /* 4, array index */
    }
    ref_clk_div = ref_clk_div | 0x10; // bit 4 is 1
...
static td_void phy_hw_init_tmds(td_u32 id, const hdmi_phy_tmds_cfg *hdmi_spec_cfg)
{
    ...
    phy_hw_write_stb1_byte(id, APHY_CS_8, APHY_OFFSET_0, 0x9F); // FF
    phy_hw_write_stb1_byte(id, APHY_CS_8, APHY_OFFSET_1, 0x10); // 11

    /* aphy refclk select */
    phy_hw_ref_clk_div_set(id, hdmi_spec_cfg->pixel_clk);
    phy_hw_tmds_clk_div_set(id, hdmi_spec_cfg->tmds_clk);

    phy_hw_write_stb1_byte(id, APHY_CS_8, APHY_OFFSET_4, 0x10); // 0
    phy_hw_write_stb1_byte(id, APHY_CS_8, APHY_OFFSET_9, 0x00);
    phy_hw_write_stb1_byte(id, APHY_CS_8, APHY_OFFSET_A, 0x00);
    phy_hw_write_stb1_byte(id, APHY_CS_8, APHY_OFFSET_B, 0x0);  // E0
    phy_hw_write_stb1_byte(id, APHY_CS_8, APHY_OFFSET_C, 0x14);
    phy_hw_write_stb1_byte(id, APHY_CS_8, APHY_OFFSET_D, 0xb4); // b4
...
{
    hdmi_debug("mode_cfg = %u\n", (td_u32)hdmi_spec_cfg->mode_cfg);
...
static td_s32 phy_hw_input_clock_check(td_u32 id, td_u32 *pixel_clk)
{
    ...
    hdmi_debug("input clock = : %u  khz\n", input_clk);
    if ((reg_ret_value & 0xF) == 0xF) {
        hdmi_debug(">> input clock quality : stable\n");
        *pixel_clk = input_clk;
    } else {
        hdmi_debug(">> warning! input clock is unstable!\n");
        *pixel_clk = input_clk;
        return TD_FAILURE;
    }
...
td_s32 hal_hdmi_phy_power_get(td_u32 id, td_bool *enable)
{
    ...
    *enable = TD_TRUE;
    ...
    if (hdmi_reg_resetn_get(id) == (td_s32)TD_FALSE) {
        *enable = TD_FALSE;
    }
...
td_s32 hal_hdmi_phy_power_set(td_u32 id, td_bool enable)
{
    ...
    hdmi_reg_resetn_set(id, (td_u8)enable);
...
td_s32 hal_hdmi_phy_oe_get(td_u32 id, td_bool *enable)
{
    ...
    if (hdmi_reg_resetn_get(id) == (td_s32)TD_FALSE) {
        hdmi_debug("phy is reset now, OE disable.\n");
        *enable = TD_FALSE;
        return TD_SUCCESS;
    }
...
td_s32 hal_hdmi_phy_tmds_set(td_u32 id, const hdmi_phy_tmds_cfg *tmds_cfg)
{
    ...
    if ((tmds_cfg->mode_cfg == HDMI_PHY_MODE_CFG_TMDS) || (tmds_cfg->mode_cfg == HDMI_PHY_MODE_CFG_FRL)) {
        hdmi_debug("TMDS mode: %u, pixel_clk: %u, tmds_clk: %u, deep_color: %u, trace_len: %u\n",
            (td_u32)tmds_cfg->mode_cfg, tmds_cfg->pixel_clk, tmds_cfg->tmds_clk, (td_u32)tmds_cfg->deep_color,
            (td_u32)tmds_cfg->trace_len);
        ...
        if (phy_hw_input_clock_check(id, &pixel_clk) != TD_SUCCESS) {
            hdmi_debug("input clock unstable\n");
        }
...
static td_s32 hdmi_phy_spec_set(td_u32 id, const hdmi_phy_hw_spec_cfg *cfg)
{
    ...
    ret = phy_user_spec_param_set(id, (td_s32)tmds_cfg.tmds_clk, tmds_cfg.trace_len, &tmds_spec);
...
td_s32 hal_hdmi_phy_params_get(td_u32 id, hdmi_phy_hw_param *hw_param)
{
    ...
    if (g_hdmi_phy_info[id].spec_user == TD_NULL) {
        return TD_FAILURE;
    }

    if (g_hdmi_phy_info[id].hw_spec == TD_NULL) {
        return TD_FAILURE;
    }

    if ((hdmi_dev->mode_param.trace_len >= HDMI_TRACE_COUNT) || (hdmi_dev->mode_param.trace_len < 0)) {
        return TD_FAILURE;
    }
