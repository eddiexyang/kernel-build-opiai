...
td_u32 g_hdmi_1_4_power_config_val[HDMI_PHY_POWER_LENGTH] = {0};
td_u32 g_hdmi_2_0_power_config_val[HDMI_PHY_POWER_LENGTH] = {0};
td_u32 g_hdmi_1_4_power_config_flag = TD_FAILURE;
td_u32 g_hdmi_2_0_power_config_flag = TD_FAILURE;

#define TX_MAX_TMDS_CLK 600
#define CTRL_RESET_WAIT 20
#define TH_DK_A 51100U
#define TH_DK_B 51101U
#define FH_A    50150U
#define FH_B    50151U

void hal_hdmi_close(struct hdmi_hal_ *hal);
static hdmi_hal_cfg g_hal_cfg[(td_s32)HDMI_ID_MAX];
...
static td_void hal_hdmi_ctrl_reset(const struct hdmi_hal_ *hal)
{
    ...
    if (output == TD_TRUE) {
        hdmi_debug("oe enable, do not reset!\n");
        return;
    }

    (td_void)hal_hdmi_ctrl_data_reset(hal->hal_ctx.hdmi_id, TD_FALSE, 0);

    for (i = 0; (tmds_stable == TD_FALSE) && (i < CTRL_RESET_WAIT); i++) {
        osal_msleep(1);
        hal_hdmi_ctrl_tmds_stable_get(hal->hal_ctx.hdmi_id, &tmds_stable);
    }

    hdmi_debug("wait %ums, tmds_stable=%u\n", i, (td_u32)tmds_stable);
...
/* read phy power from dts according to borad */
td_void hal_hdmi_phy_product_power_set_param(void)
{
    td_s32 ret = 0;
    const struct device_node *np = NULL;
    /* Get hdmi dts node */
    np = of_find_compatible_node(NULL, NULL, "vendor,hdmi");
    if (np == NULL) {
        hdmi_info("Default board, not config hdmi power\n");
        return;
    }

    /* Get phy power of hdmi2.0 */
    ret = of_property_read_u32_array(np, "TMDS_DRV_CFG_CH_2_0", g_hdmi_2_0_power_config_val, HDMI_PHY_POWER_LENGTH);
    if (ret != 0) {
        hdmi_warn("Default power set for hdmi2.0\n");
    } else {
        g_hdmi_2_0_power_config_flag = TD_SUCCESS;
    }

    /* Get phy power of hdmi1.4 */
    ret = of_property_read_u32_array(np, "TMDS_DRV_CFG_CH_1_4", g_hdmi_1_4_power_config_val, HDMI_PHY_POWER_LENGTH);
    if (ret != 0) {
        hdmi_warn("Default power set for hdmi1.4\n");
    } else {
        g_hdmi_1_4_power_config_flag = TD_SUCCESS;
    }
    /* Release resources of dts */
    of_node_put(np);
    return;
}

/* feedback status of dts and copy phy power of hdmi1.4 */
static td_s32 hal_hdmi_1_4_phy_product_power_param_get(hdmi_power_config_info *hdmi_power_config)
{
    td_s32 param_count;
    if (g_hdmi_1_4_power_config_flag == TD_SUCCESS) {
        for (param_count = 0; param_count < HDMI_PHY_POWER_LENGTH; param_count++) {
            hdmi_power_config->hdmi_power_config_val[param_count] = g_hdmi_1_4_power_config_val[param_count];
        }
        return TD_SUCCESS;
    }

    /* dts of the board has no hdmi phy power message */
    return TD_FAILURE;
}

/* feedback status of dts and copy phy power of hdmi2.0 */
static td_s32 hal_hdmi_2_0_phy_product_power_param_get(hdmi_power_config_info *hdmi_power_config)
{
    td_s32 param_count;
    if (g_hdmi_2_0_power_config_flag == TD_SUCCESS) {
        for (param_count = 0; param_count < HDMI_PHY_POWER_LENGTH; param_count++) {
            hdmi_power_config->hdmi_power_config_val[param_count] = g_hdmi_2_0_power_config_val[param_count];
        }
        return TD_SUCCESS;
    }
...
td_void hal_hdmi_phy_product_power_set(td_u32 hdmi_dev_id)
{
    hdmi_power_config_info hdmi_power_config_info;
    td_u32 pix_clk;
    hdmi_device *hdmi_dev = TD_NULL;
    td_s32 ret;

    hdmi_dev = get_hdmi_device(hdmi_dev_id);
    if (hdmi_dev == TD_NULL) {
        hdmi_err("hdmi_dev is null pointer!\n");
        return;
    }
    pix_clk = hdmi_dev->attr.vo_attr.hdmi_adapt_pix_clk;

    /* Get hdmi phy power */
    if (pix_clk > HDMI_EDID_MAX_HDMI14_TMDS_RATE) {
        ret = hal_hdmi_2_0_phy_product_power_param_get(&hdmi_power_config_info);
    } else {
        ret = hal_hdmi_1_4_phy_product_power_param_get(&hdmi_power_config_info);
    }
    if (ret != TD_SUCCESS) {
        hdmi_debug("hdmi_phy_power is not set!\n");
        return;
    }

    if (hdmi_dev->hdmi_dev_id == 0) {
        hdmi_write_mask(0, 0x510, hdmi_power_config_info.hdmi_power_config_val[HDMI0_TMDS_DRV_CFG_CH_ID1], 0xffffffff);
        hdmi_write_mask(0, 0x514, hdmi_power_config_info.hdmi_power_config_val[HDMI0_TMDS_DRV_CFG_CH_ID2], 0xffffffff);
        hdmi_write_mask(0, 0x518, hdmi_power_config_info.hdmi_power_config_val[HDMI0_TMDS_DRV_CFG_CH_ID3], 0xffffffff);
    } else {
        hdmi_write_mask(1, 0x510, hdmi_power_config_info.hdmi_power_config_val[HDMI1_TMDS_DRV_CFG_CH_ID1], 0xffffffff);
        hdmi_write_mask(1, 0x514, hdmi_power_config_info.hdmi_power_config_val[HDMI1_TMDS_DRV_CFG_CH_ID2], 0xffffffff);
        hdmi_write_mask(1, 0x518, hdmi_power_config_info.hdmi_power_config_val[HDMI1_TMDS_DRV_CFG_CH_ID3], 0xffffffff);
    }
...
static td_void hal_hdmi_sequencer_handler_process(const struct hdmi_hal_ *hal, const td_u32 hdmi_dev_id)
{
    ot_unused(hal);
    hal_hdmi_mach_invoke(hdmi_dev_id);
    return;
}
...
static td_void hal_hdmi_hw_video_status_get(td_u32 hdmi_id, hdmi_hardware_status *hw_status)
{
    ...
    hw_status->video_status.out_bit_depth   =
        (video_stat.out_deep_color == (td_s32)HDMI_DEEP_COLOR_OFF) ? HDMI_VIDEO_BITDEPTH_OFF :
        video_stat.out_deep_color;
...
static td_s32 hal_hdmi_edid_raw_data_read(const struct hdmi_hal_ *hal, td_u32 size, td_u8 out_buffer[])
{
    ...
    ret = hal_hdmi_ddc_edid_raw_get(hal->hal_ctx.hdmi_id, (td_s32)size, out_buffer);
...
td_s32 hal_hdmi_open(const hdmi_hal_init *hal_init, hdmi_hal **hal_handle)
{
    ...
    if (hal_init != TD_NULL) {
        ...
        hdmi_debug("hal_init null! open in boot!\n");
    }
    ...
    ret = hal_hdmi_phy_init(hal_init);
    if (ret != TD_SUCCESS) {
        hal_hdmi_close(hal);
        hdmi_err("hal_hdmi_phy_init fail!\n");
        return TD_FAILURE;
    }
    ret = hal_hdmi_mach_init();
    if (ret != TD_SUCCESS) {
        hal_hdmi_close(hal);
        hdmi_err("hal_hdmi_mach_init fail!\n");
        return TD_FAILURE;
    }
    ret = hal_hdmi_ctrl_init(hal->hal_ctx.hdmi_id, hal_init);
    if (ret != TD_SUCCESS) {
        hal_hdmi_close(hal);
        hdmi_err("hal_hdmi_ctrl_init fail!\n");
        return TD_FAILURE;
    }
    ret = hal_hdmi_ddc_init(hal->hal_ctx.hdmi_id);
    if (ret != TD_SUCCESS) {
        hal_hdmi_close(hal);
        hdmi_err("hal_hdmi_ddc_init fail!\n");
        return TD_FAILURE;
    }
#ifdef HDMI_SCDC_SUPPORT
    ret = hal_hdmi_scdc_init(hal->hal_ctx.hdmi_id);
    if (ret != TD_SUCCESS) {
        hal_hdmi_close(hal);
        hdmi_err("hal_hdmi_scdc_init fail!\n");
        return TD_FAILURE;
    }
#endif
    hal_pfn_init(hal);
