...
static scdc_infomation g_scdc_info[(td_s32)HDMI_ID_MAX] = {{{0}, {0}}};
...
static td_s32 scdc_scramble_set(hdmi_device_id hdmi_id, const scdc_scramble *scramble)
{
    ...
    /* source scramble */
    hdmi_reg_enc_scr_on_set((td_u32)hdmi_id, (td_u8)scramble->src_scramble);
    /* sink scramble */
...
static td_s32 scdc_scramble_get(hdmi_device_id hdmi_id, scdc_scramble *scramble)
{
    ...
    /* source scrambled */
    scramble->src_scramble = hdmi_reg_enc_scr_on_get((td_u32)hdmi_id) ? TD_TRUE : TD_FALSE;
...
td_s32 hal_hdmi_scdc_init(hdmi_device_id hdmi_id)
{
    ...
    if (scdc_info->status.init == TD_FALSE) {
...
td_s32 hal_hdmi_scdc_attr_set(hdmi_device_id hdmi_id, scdc_attr *attr)
{
    ...
    do {
        /* oe disable */
        scdc_oe_set((td_u32)hdmi_id, TD_FALSE, SCDC_OE_DISABLE_WAIT_TIME, scdc_info);
        ...
        osal_msleep(timeout);
#endif
        scdc_oe_set((td_u32)hdmi_id, TD_TRUE, timeout, scdc_info);
        /* poll status */
    ...
    hdmi_debug("\ndelay_interval=%u, timeout=%u, src_scarmble=%u, sink_scramble=%u, tmds_clk_ratio40x=%u\n", timeout,
        (scdc_time_cur - scdc_timestart), (td_u32)scramble.src_scramble, (td_u32)scramble.sink_scramble,
        scramble.tmds_clk_ratio40x);

    if (ret != TD_SUCCESS) {
        hdmi_warn("scdc scramble %s not pass!\n", scdc_info->attr.sink_scramble ? "ON" : "OFF");
    } else {
        hdmi_debug("scdc scramble %s success!\n", scdc_info->attr.sink_scramble ? "ON" : "OFF");
    }
