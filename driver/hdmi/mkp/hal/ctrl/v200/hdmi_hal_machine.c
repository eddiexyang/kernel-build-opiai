...
td_s32 hal_hdmi_mach_init(td_void)
{
    ...
    if (mach_info->init == TD_FALSE) {
        ret = osal_sema_init(&mach_info->mach_mutex, 1);
        if (ret != TD_SUCCESS) {
            hdmi_err("sema init mach_mutex failed!\n");
            return ret;
        }
...
td_s32 hal_hdmi_mach_deinit(td_void)
{
    ...
    if (mach_info->total != 0) {
        hdmi_debug("mach total: %u, do not deinit.\n", mach_info->total);
        return TD_SUCCESS;
    }
...
td_s32 hal_hdmi_mach_invoke(td_u32 hdmi_dev_id)
{
    td_u32 i;
    ...
    if (mach_info->total == 0) {
        ...
        for (i = 0; i < MACHINE_MAX_ID_NUM; ++i) {
            ...
            if (tmp_elem->mach_ctrl.data != TD_NULL &&
                ((hdmi_ctrl_info *)(tmp_elem->mach_ctrl.data))->hdmi_id == hdmi_dev_id) {
    ...
    if (mach_info->total >= MACHINE_MAX_ID_NUM) {
        ...
        for (i = 0; i < MACHINE_MAX_ID_NUM; ++i) {
            ...
            if (tmp_elem->mach_run.valid_id == TD_FALSE) {
                ...
                name_len = (td_s32)osal_strlen(tmp_elem->mach_ctrl.name);
        ...
        if (i < MACHINE_MAX_ID_NUM) {
            hdmi_debug("register new machine id=%u success!\n", *mach_id);
            ret = TD_SUCCESS;
        } else {
            hdmi_debug("register new machine id=%u fail!\n", *mach_id);
            ret = TD_FAILURE;
        }
    }
