...
#define DEBUG_NUMBER_STR_MAX_LEN 5
...
static const compatibility_debug_info g_debug_compat_table[(td_s32)COMPAT_COMPAT_INFO_MAX] = {
...
static td_void debug_echo_helper_vargs(td_char *buf, td_u32 size, const td_char *fmt, osal_va_list args)
{
    ...
    if (ret < 0) {
        hdmi_err("vsnprintf_s err, ret=%d\n", ret);
        return;
    }
...
static td_bool is_hex_start(const td_char *p, const td_char **end)
{
    ...
    const td_s32 hex_space_mark = 2;
    if (*p == '0' && (p[1] == 'x' || p[1] == 'X')) {
        if (end != TD_NULL) {
            *end = p + 2; /* P + 2, mean 16 hex value addr */
        }
        return TD_TRUE;
    } else if (is_hex_char(p[0]) && (is_hex_space(p[1]) || (is_hex_char(p[1]) && is_hex_space(p[hex_space_mark])))) {
        if (end != TD_NULL) {
            *end = p;
        }
        return TD_TRUE;
...
static td_u32 debug_str_to_u32(const td_char *str, td_char **end, hdmi_debug_base base)
{
    ...
    if (strlen(str) > DEBUG_NUMBER_STR_MAX_LEN) {
        hdmi_warn("number is too large! length of string=%u\n", (td_u32)strlen(str));
        return (td_u32)TD_FAILURE;
    }
    ...
    while (*str) {
        ...
        /* value size of 10 - 15 */
        value = (td_u32)((((*str) >= '0') && ((*str) <= '9')) ? ((*str) - '0') :
                ((to_lower((td_u8)(*str)) - 'a') + 10)); /* 10, value offset */
        if (value >= (td_u32)base) {
            break;
        }
        result = result * (td_u32)base + value;
        str++;
    }
    if (end != TD_NULL) {
        *end = (td_char *)str;
...
{
    return osal_klib_fopen(filename, flags, (unsigned short)mode);
}
...
static td_s32 debug_thread(const hdmi_debug_cmd_arg *cmd_arg)
{
    ...
    const char *thread_stand = "0";
    const char *thread_end = "1";
    ...
    if (is_str_equal(cmd_arg->argv[1], thread_stand)) {
        ret = hdmi_thread_state_set(hdmi_dev, HDMI_THREAD_STATE_STOP);
        hdmi_dbg_print("\n thread set OFF %s! \n\n", (ret != TD_SUCCESS) ? STR_FAIL : STR_SUCC);
    } else if (is_str_equal(cmd_arg->argv[1], thread_end)) {
...
static td_s32 debug_oe(const hdmi_debug_cmd_arg *cmd_arg)
{
    ...
    black_info.in_quantization = (black_info.in_color_space == (td_u8)HDMI_COLORSPACE_RGB) ?
        (td_u8)hdmi_dev->attr.vo_attr.rgb_quantization : (td_u8)hdmi_dev->attr.vo_attr.ycc_quantization;
...
static td_s32 debug_event(const hdmi_debug_cmd_arg *cmd_arg)
{
    ...
    if ((event >= HDMI_EVENT_HOTPLUG) &&
        ...
        if ((td_u32)((td_u32)event - (td_u32)HDMI_EVENT_HOTPLUG) < hdmi_array_size(g_event_str_table)) {
            hdmi_dbg_print("hardware event %s simulate %s.\n",
                g_event_str_table[(td_u32)((td_u32)event - (td_u32)HDMI_EVENT_HOTPLUG)],
                (ret != TD_SUCCESS) ? STR_FAIL : STR_SUCC);
        }
    } else {
        goto usage_error;
...
usage_error:
    ...
    for (event = HDMI_EVENT_HOTPLUG;
         event < HDMI_EVENT_BUTT && ((td_u32)(event - HDMI_EVENT_HOTPLUG) < hdmi_array_size(g_event_str_table));
         (td_u32)event++) {
        hdmi_dbg_print("%-10s %d\t-- (0x%02x)%s\n", "", (td_s32)event, (td_u32)event,
            g_event_str_table[(td_u32)event - (td_u32)HDMI_EVENT_HOTPLUG]);
    }
...
static td_s32 debug_edid_from_file(hdmi_device *hdmi_dev, const hdmi_debug_cmd_arg *cmd_arg)
{
    ...
    f = debug_file_open(cmd_arg->argv[2], OSAL_O_RDONLY, 0);  /* 2, array index */
    if (f == TD_NULL) {
    ...
    if (len > 0) {
        ...
        if (len >= 0) {
            ...
            hdmi_dev->edid_info.status.raw_len = (td_u32)len;
            hdmi_dev->edid_info.status.raw_valid = TD_TRUE;
            hdmi_dev->edid_info.status.cap_valid = TD_TRUE;
            ret = memcpy_s(hdmi_dev->edid_info.edid_raw, (size_t)HDMI_EDID_TOTAL_SIZE, dbg_edid->data, (size_t)len);
            if (ret != EOK) {
...
{
    hal_call_void(hal_hdmi_scdc_status_get, hdmi_dev->hal, scdc_status);
    hdmi_dbg_print("scdc get source_scramble_on[%d] \n"
                   "           sink_scramble_on[%d] \n"
                   "         tmds_bit_clk_ratio[%u] \n\n",
                   (td_s32)scdc_status->source_scramble_on,
                   (td_s32)scdc_status->sink_scramble_on,
...
static td_s32 debug_out_color_space(const hdmi_debug_cmd_arg *cmd_arg)
{
    ...
    ret = drv_hdmi_attr_set(hdmi_dev, &attr);
    drv_hdmi_start(hdmi_dev);
    hdmi_dbg_print("\n output colorspace mode %u->%s %s! \n\n", old_mode,
                   (td_s32)attr.app_attr.out_color_space, (ret == TD_SUCCESS) ? STR_SUCC : STR_FAIL);
...
static td_s32 debug_deep_color(const hdmi_debug_cmd_arg *cmd_arg)
{
    ...
    ret = drv_hdmi_attr_set(hdmi_dev, &attr);
    drv_hdmi_start(hdmi_dev);
    hdmi_dbg_print("\n deppcolor colorspace mode %u->%d %s! \n\n", old_mode,
                   (td_s32)attr.app_attr.deep_color_mode, (ret == TD_SUCCESS) ? STR_SUCC : STR_FAIL);
...
static td_s32 debug_cmd(const hdmi_debug_cmd_arg *cmd_arg)
{
    ...
    if (is_str_equal(cmd_arg->argv[1], "0")) {
        ...
        str = cmd_arg->argv[2]; /* 2, array index */
        debug_null_check_return(str);
        data = debug_str_to_u32(str, &str, HDMI_DEBUG_BASE_DEC);
        data = (data > HDMI_TRACE_LEN_1) ? HDMI_TRACE_LEN_1 : data;
        hdmi_dbg_print("\n PCB trace len old(%d)->new(%u)! \n\n", (td_s32)hdmi_dev->mode_param.trace_len, data);
...
static td_void set_compat_debug_info(compatibility_info *compat_info, compat_info_type type, td_u32 value)
{
    switch (type) {
        case COMPAT_AVI_USE_GEN5:
            hdmi_dbg_print("set AVI by gen5 old(%d)->new(%u).\n", (td_s32)compat_info->avi_use_gen5, value);
...
static td_s32 debug_cmd_handle(const hdmi_debug_cmd_arg *cmd_arg)
{
    ...
    if (cmd_arg->argc > 0) {
        ...
        for (i = 0; i < hdmi_array_size(g_cmd_table); i++) {
            ...
            if (((cmd_num != i) || (*str != '\0')) &&
                ((td_s32)is_str_equal(g_cmd_table[i].name, cmd_arg->argv[0]) == (td_s32)TD_FALSE) &&
                ((td_s32)is_str_equal(g_cmd_table[i].short_name, cmd_arg->argv[0]) == (td_s32)TD_FALSE)) {
                continue;
            }
...
td_s32 drv_hdmi_debug_edid_raw_read(hdmi_edid_updata_mode mode, td_u8 *data, td_u32 len)
{
    ...
    if ((mode != HDMI_EDID_UPDATE_DEBUG) || (dbg_edid->data_valid == TD_FALSE) ||
        ...
        hdmi_err("debug edid read fail,data_valid=%d !\n", (td_s32)dbg_edid->data_valid);
        return TD_FAILURE;
    }
    ret = memcpy_s(data, len, dbg_edid->data, (size_t)dbg_edid->len);
