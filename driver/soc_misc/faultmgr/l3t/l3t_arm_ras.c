...
#define L3T_ARM_RAS_ECC_COVERGE_NODE(bit_id, msg, _sensor_type, ras_err) { \
    .subsys_id = DMS_DEV_TYPE_L3T,             \
    .module_id = 0,                            \
...
STATIC struct ras_fault_converge_item g_l3t_arm_ras_converge_tab[] = {
...
const struct ras_fault_converge_item *l3t_arm_ras_converge(const struct ras_error *error_info)
{
    u32 num = sizeof(g_l3t_arm_ras_converge_tab) / sizeof(struct ras_fault_converge_item);
    return soc_misc_parse_table_handle(error_info, g_l3t_arm_ras_converge_tab, num);
}

const struct ras_fault_converge_item *l3t_arm_ras_ecc_converge(u32 section_type, u64 ras_code)
{
    static struct ras_fault_converge_item l3t_arm_ras_ecc_converge_tab[] = {
        L3T_ARM_RAS_ECC_COVERGE_NODE(L3T_SINGLE_ECC, "single-bit ECC",
            DMS_SEN_TYPE_RAS_SENSOR, RAS_ERROR_TYPE_SBECCOverThold),
        L3T_ARM_RAS_ECC_COVERGE_NODE(L3T_DOUBLE_ECC, "double-bit ECC", DMS_SEN_TYPE_RAS_SENSOR, RAS_ERROR_TYPE_MBECC),
    };

    u32 items_num = (u32)ARRAY_SIZE(l3t_arm_ras_ecc_converge_tab);
    return soc_safety_converge(l3t_arm_ras_ecc_converge_tab, items_num, ras_code);
};
...
#define L3T_IRQ_HW_INFO(reg_base, id, _mask) {  \
    .emu_id = (CPU_CLUSTER0_EMU_ID + id),       \
    .src_id = L3T_SAFETY_ERR_SRC_ID,            \
    .bit_id = L3T_SAFETY_ERR_BIT_ID,            \
    .base_paddr = reg_base,                     \
    .base_size = SZ_64K,                        \
    .base_vaddr = 0,                            \
    .status = { L3T_SAFETY_ERR_SRC, _mask },    \
    .clear = { L3T_SAFETY_ERR_CLR, _mask },     \
    .mask = { L3T_SAFETY_ERR_INT_MASK, _mask }, \
    .check_mask[0] = {DRVFAULT_FILED_VALID, 0x0848, 0x0}, \
}

STATIC struct safety_irq_hw_info g_l3t_hw_info[L3T_NODE_NUM] = {
    L3T_IRQ_HW_INFO(CPU_CLUSTER0_L3T_REG_BASE, SOC_CPU_CLUSTER_0, 0),
    L3T_IRQ_HW_INFO(CPU_CLUSTER1_L3T_REG_BASE, SOC_CPU_CLUSTER_1, 0x800),
    L3T_IRQ_HW_INFO(CPU_CLUSTER2_L3T_REG_BASE, SOC_CPU_CLUSTER_2, 0x800),
    L3T_IRQ_HW_INFO(CPU_CLUSTER3_L3T_REG_BASE, SOC_CPU_CLUSTER_3, 0x800),

};

#define L3T_SUBSYS_INFO_NODE(cluster_id) {             \
    .dev_id = 0,                                       \
    .irq_type = SAFETY_IRQ_TYPE_ARM_RAS,               \
    .irq_name = "int_eri_l3t" NUM_TO_STR(cluster_id),  \
    .safety_irq_func = l3t_arm_ras_irq_handler,        \
    .irq_hwinfo_num = 1,                               \
    .irq_hwinfo_list = &g_l3t_hw_info[cluster_id] }

STATIC struct safety_irq_info g_l3t_safety_info[L3T_NODE_NUM] = {
    L3T_SUBSYS_INFO_NODE(SOC_CPU_CLUSTER_0),
    L3T_SUBSYS_INFO_NODE(SOC_CPU_CLUSTER_1),
    L3T_SUBSYS_INFO_NODE(SOC_CPU_CLUSTER_2),
    L3T_SUBSYS_INFO_NODE(SOC_CPU_CLUSTER_3),
};
#endif

#ifndef AOS_LLVM_BUILD
STATIC int l3t_get_ierr(const struct notify_data *pdata, u32 *ierr)
{
    u32 err_status = 0;
    int ret;
...
#ifdef CFG_FEATURE_SAFETY_MANAGER
STATIC bool l3t_is_safety_fault(u32 ierr)
{
...
void l3t_fault_handler_arm(const struct notify_data *pdata)
{
    ...
    if (l3t_get_ierr(pdata, &ierr) != 0) {
        soc_misc_drv_err("get l3t ierr failed\n");
        return;
    }
#ifdef CFG_FEATURE_SAFETY_MANAGER
    if (l3t_is_safety_fault(ierr)) {
        if ((drvfault_ras_irq_notify(&g_l3t_safety_info[node_id])) != 0) {
    ...
    event = l3t_arm_ras_converge(&error_info);
    if (event == NULL) {
        soc_misc_drv_err("l3t (node id=%u) get converge item failed.\n", node_id);
        return;
    }
    ret = dfm_add_event(&g_l3t_dfm[pdata->chip_id], node_id, event);
    if (ret != 0) {
        if (ret != -EEXIST) {
    ...
    if ((!(arm_err_info->error_info & RAS_UNMASK_CORRECTED)) && (vendor_info != NULL)) {
        l3t_relieve_suppresion(pdata->chip_id, vendor_info->oem_sub_module);
    }
}

void l3t_fault_handler_hisi_common(const struct notify_data *pdata)
{
    u32 ierr = 0;
    int ret;
    u32 node_id;
...
#ifdef CFG_FEATURE_SAFETY_MANAGER
    if (l3t_is_safety_fault(ierr)) {
        if ((drvfault_ras_irq_notify(&g_l3t_safety_info[hisi_error->device_or_core_id])) != 0) {
            soc_misc_drv_err("handle safety fault error\n");
        } else {
            soc_misc_drv_info("handle safety event success\n");
        }
    ...
    if (error_info.ras_err_severity != RAS_CORRECTED_ERROR) {
        l3t_relieve_suppresion(pdata->chip_id, hisi_error->device_or_core_id);
    }
}

void l3t_arm_ras_fpdc_handler(const struct notify_data *pdata)
{
    ...
    if (soc_misc_sec_type_guid_cmp(pdata->section_type, &hisi_common_sec_type)) {
        l3t_fault_handler_hisi_common(pdata);
    } else {
        l3t_fault_handler_arm(pdata);
    }
}
#else
void l3t_arm_ras_fpdc_handler(const struct notify_data *pdata)
{
    soc_misc_drv_warn("l3t arm ras fault handle is not supported\n");
}
#endif

#ifdef CFG_FEATURE_SAFETY_MANAGER
int l3t_arm_ras_irq_init(u32 dev_id)
{
    ...
    for (node_id = 0; node_id < L3T_NODE_NUM; node_id++) {
        ret = drvfault_register_safety_irq_info(&g_l3t_safety_info[node_id]);
        if (ret != 0) {
            soc_misc_drv_err("l3t arm ras register fault notify failed."
...
int l3t_arm_ras_irq_uninit(u32 dev_id)
{
    u32 soc_type = soc_misc_get_chip_type();
    u32 node_id;
    int ret;
    ...
    for (node_id = 0; node_id < L3T_NODE_NUM; node_id++) {
        ret = drvfault_unregister_safety_irq_info(dev_id, g_l3t_safety_info[node_id].irq_name);
        if (ret != 0) {
            soc_misc_drv_err("l3t arm ras unregister fault notifier failed."
                "(dev_id=%u, node_id=%u, ret=%d)\n", dev_id, node_id, ret);
        }
    }
...
int l3t_arm_ras_fpdc_init(u32 dev_id)
{
    int ret;

    ret = fpdc_register_fault_notifier(DMS_DEV_TYPE_L3T, l3t_arm_ras_fpdc_handler);
    if (ret != 0) {
        soc_misc_drv_err("l3t arm ras register fpdc fault notifier failed.(ret=%d, node_type=%u)\n",
            ret, DMS_DEV_TYPE_L3T);
    }
    return 0;
}

int l3t_arm_ras_fpdc_uninit(u32 dev_id)
{
    int ret;

    ret = fpdc_unregister_fault_notifier(DMS_DEV_TYPE_L3T);
    if (ret != 0) {
        soc_misc_drv_err("l3t arm ras register fault notify failed.(ret=%d, node_type=%u)\n",
            ret, DMS_DEV_TYPE_L3T);
    }
...
int l3t_arm_ras_init(void)
{
    u32 dev_id;

    for (dev_id = 0; dev_id < SOC_MISC_DEVICE_NUM_MAX; dev_id++) {
        l3t_arm_ras_fpdc_init(dev_id);
#ifdef CFG_FEATURE_SAFETY_MANAGER
        l3t_arm_ras_irq_init(dev_id);
#endif
        l3t_arm_ras_unmask_init(dev_id);
    }

    return 0;
}

int l3t_arm_ras_uninit(void)
{
    u32 dev_id;

    for (dev_id = 0; dev_id < SOC_MISC_DEVICE_NUM_MAX; dev_id++) {
        l3t_arm_ras_fpdc_uninit(dev_id);
#ifdef CFG_FEATURE_SAFETY_MANAGER
        l3t_arm_ras_irq_uninit(dev_id);
#endif
        l3t_arm_ras_unmask_uninit(dev_id);
    }

    return 0;
}

int l3t_arm_ras_irq_handler(struct safety_fault_info *safety_fault,
    unsigned int *event_num, struct safety_event **event_list)
{
    ...
    *event_list = (struct safety_event *)kmalloc(
    ...
    for (i = 0; i < safety_fault->fault_reg_num; ++i) {
        ...
        module_info.dev_node->get_converage_node = l3t_arm_ras_ecc_converge;
