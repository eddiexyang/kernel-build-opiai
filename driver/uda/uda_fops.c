...
static int ioctl_uda_devid_trans(unsigned int cmd, unsigned long arg)
{
    ...
    if (ret != 0) {
        uda_err("Copy from user failed. (cmd=%d; raw_devid=%u)\n", _IOC_NR(cmd), trans.raw_devid);
        return ret;
    }
...
int __init uda_init_module(void)
{
    int ret;
...
void __exit uda_exit_module(void)
{
    uda_proc_fs_uninit();
#ifdef CFG_FEATURE_DEVID_TRANS
...
module_init(uda_init_module);
module_exit(uda_exit_module);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Huawei Tech. Co., Ltd.");
MODULE_DESCRIPTION("UDA DRIVER");
