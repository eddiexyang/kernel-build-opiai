...
static td_s32 tde_osi_get_bpp_by_fmt(drv_tde_color_fmt fmt);
static td_bool tde_osi_check_format_support_draw(drv_tde_color_fmt format)
{
    ...
    if (format >= DRV_TDE_COLOR_FMT_YCBCR422) {
        tde_error("This color format %u not support!\n", (td_u32)format);
        return TD_FALSE;
    }
#endif
    return TD_TRUE;
...
static td_s32 tde_check_subbyte_startx(td_s32 startx, td_u32 w, drv_tde_color_fmt format)
{
    ...
    if (bpp < 8) { /* 8 bits */
        ...
        if ((td_u32)(startx * bpp % 8) || (td_u32)((td_s32)w * bpp % 8)) { /* 8 bits */
...
#define ycc2rgb(y, cb, cr, r, g, b)                                                                     \
    do {                                                                                                \
        r = (td_u8)(((298U * ((y) - 16U) + 409U * ((cr) - 128U)) >> 7U) + 1U) >> 1U;                               \
        g = (td_u8)(((298U * ((y) - 16U) - 100U * ((cb) - 128U) - 208U * ((cr) - 128U)) >> 7U) + 1U) >> 1U;     \
        b = (td_u8)(((298U * ((y) - 16U) + 517U * ((cb) - 128U)) >> 7U) + 1U) >> 1U;                               \
    } while (0)

#define rgb2ycc(r, g, b, y, cb, cr)                                                     \
    do {                                                                                \
        y  = (td_u8)(((((263U * (r) + 516U * (g) + 100U * (b)) >> 9U) + 1U) >> 1U) + 16U);            \
        cb = (td_u8)(((((450U * (b) - 152U * (r) - 298U * (g)) >> 9U) + 1U) >> 1U) + 128U);        \
        cr = (td_u8)(((((450U * (r) - 377U * (g) - 73U * (b)) >> 9U) + 1U) >> 1U) + 128U);        \
    } while (0)
...
tde_color_fmt g_tde_common_drv_color_fmt[(td_s32)DRV_TDE_COLOR_FMT_MAX + 1] = {
...
static tde_argb_order_mode g_tde_argb_order[(td_s32)DRV_TDE_COLOR_FMT_MAX + 1] = {
...
static td_s32 tde_osi_1_source_fill(td_s32 handle, drv_tde_surface *dst_surface,
                                    const drv_tde_rect *dst_rect, drv_tde_fill_color *fill_color,
...
static tde2_fmt_bitoffset g_fmt_bit_and_offset_array[(td_s32)DRV_TDE_COLOR_FMT_AYCBCR8888 + 1] = {
...
static td_s32 tde_osi_color_convert(const drv_tde_fill_color *fill_color, const drv_tde_surface *sur, td_u32 *out_color)
{
    ...
    ret = color_convert_check(fill_color, sur, &color_trans);
    if (ret != TD_SUCCESS) {
        return ret;
    }
                                        ...
                                        td_u32 out_width, td_u32 out_height)
{
    ...
    if (((out_width * TDE_MAX_MINIFICATION_H) < in_width) || ((out_height * TDE_MAX_MINIFICATION_V) < in_height)) {
...
static td_s32 tde_osi_check_rotate_para(const drv_tde_single_src *single_src, drv_tde_rotate_angle rotate_angle)
{
    ...
    if (is_invalid == TD_TRUE) {
        tde_error("bitmap address is not aligned nor stride is not aligned!\n");
                                   ...
                                   tde_clut_usage *pen_clut_usage, td_bool clut_reload, tde_hw_node *hw_node)
{
    ...
    if (((clut_sur->is_ycbcr_clut== TD_FALSE) && (fmt_cate == TDE_COLORFMT_CATEGORY_YCBCR)) ||
    ...
    *pen_clut_usage = tde_osi_get_clut_usage(clut_sur->color_format, out_sur->color_format);
    ...
    if (osal_div_u64_rem(clut_phyaddr, 4)) { /* 4 alg data */
        tde_error("clut_phys_addr is not aligned!\n");
        return DRV_ERR_TDE_NOT_ALIGNED;
    }
...
static td_s32 tde_osi_check_dst_fmt(drv_tde_color_fmt fmt)
{
    if (((fmt >= DRV_TDE_COLOR_FMT_JPG_YCBCR400MBP) && (fmt <= DRV_TDE_COLOR_FMT_JPG_YCBCR422MBVP)) ||
        ...
        tde_error("This fmt %u doesn't support!\n", (td_u32)fmt);
        return DRV_ERR_TDE_INVALID_PARA;
    }
    return TD_SUCCESS;
}
                                              ...
                                              const drv_tde_opt *opt)
{
    ...
    if ((td_u32)opt->alpha_blending_cmd & DRV_TDE_ALPHA_BLENDING_ROP) {
        ...
        if ((tde_osi_is_single_src_to_rop(opt->rop_alpha) == TD_FALSE) ||
            (tde_osi_is_single_src_to_rop(opt->rop_color) == TD_FALSE)) {
    ...
    tde_colorfmt_category dst_category, const drv_tde_opt *opt, td_bool temp_fmt)
{
    ...
    is_invalid = ((src1_category == TDE_COLORFMT_CATEGORY_ARGB) || (src1_category == TDE_COLORFMT_CATEGORY_YCBCR)) &&
        (src2_category == TDE_COLORFMT_CATEGORY_AN) && (temp_fmt == HI_FALSE);
    if (is_invalid == TD_TRUE) {
                                ...
                                const tde_scandirection_mode *dst_direction)
{
    ...
    if (bpp < 8) { /* 8 is bpp */
        ...
        if ((td_u32)(single_src->dst_rect->pos_x * bpp % 8) ||
        (td_u32)((td_s32)single_src->dst_rect->width * bpp % 8)) { /* 8 is alg */
            tde_error("Subbyte's start or width for writing is not aligned!\n");
            return DRV_ERR_TDE_INVALID_PARA;
        }
    }
...
static td_void tde_osi_get_src_direction(drv_tde_mirror_mode mirror, tde_scandirection_mode *src_direction)
{
    switch (mirror) {
        case DRV_TDE_MIRROR_HORIZONTAL:
            src_direction->hor_scan = (src_direction->hor_scan == TDE_SCAN_LEFT_RIGHT ?
            TDE_SCAN_RIGHT_LEFT : TDE_SCAN_LEFT_RIGHT);
            break;
        case DRV_TDE_MIRROR_VERTICAL:
            src_direction->ver_scan = (src_direction->ver_scan == TDE_SCAN_UP_DOWN ?
            TDE_SCAN_DOWN_UP : TDE_SCAN_UP_DOWN);
            break;
...
static td_s32 tde_osi_check_single_src(const drv_tde_single_src *single_src)
{
    drv_tde_surface *src_surface = single_src->src_surface;
    drv_tde_rect *src_rect = single_src->src_rect;

    drv_tde_surface *dst_surface = single_src->dst_surface;
    drv_tde_rect *dst_rect = single_src->dst_rect;

    if ((TDE_MAX_SURFACE_WIDTH < src_surface->width) || (TDE_MAX_SURFACE_HEIGHT < src_surface->height) ||
        (TDE_MAX_SURFACE_WIDTH < dst_surface->width) || (TDE_MAX_SURFACE_HEIGHT < dst_surface->height)) {
        tde_error("surface is over maximum! "
                  "src_surface->width:%u,src_surface->height:%u,dst_surface->width:%u,dst_surface->height:%u\n",
            src_surface->width,
            src_surface->height,
            dst_surface->width,
            dst_surface->height);
    ...
    td_bool src_real =
        ((src_surface->stride > TDE_MAX_SURFACE_PITCH) || (src_surface->stride == 0) || (src_rect->height == 0) ||
            (src_rect->width == 0) || (src_rect->pos_x < 0) || ((td_u32)src_rect->pos_x >= src_surface->width) ||
            (src_rect->pos_y < 0) || ((td_u32)src_rect->pos_y >= src_surface->height) || (src_surface->phys_addr == 0));
    td_bool dst_real =
        ((dst_surface->stride > TDE_MAX_SURFACE_PITCH) || (dst_surface->stride == 0) || (dst_rect->height == 0) ||
            (dst_rect->width == 0) || (dst_rect->pos_x < 0) || ((td_u32)dst_rect->pos_x >= dst_surface->width) ||
            (dst_rect->pos_y < 0) || ((td_u32)dst_rect->pos_y >= dst_surface->height) || (dst_surface->phys_addr == 0));
    if (src_real || dst_real) {
        tde_error("src_surface_w:%u, src_surface_h:%u, src_stride:%u, src_x:%d, src_y:%d, src_w:%u, src_h:%u\n",
            src_surface->width,
            src_surface->height,
            src_surface->stride,
            src_rect->pos_x,
            ...
            src_rect->width,
            src_rect->height);
        tde_error("dst_surface_w:%u, dst_surface_h:%u, dst_stride:%u, dst_x:%d, dst_y:%d, dst_w:%u, dst_h:%u\n",
            dst_surface->width,
            dst_surface->height,
            dst_surface->stride,
            dst_rect->pos_x,
    ...
    tde_scandirection_mode *src_direction, tde_scandirection_mode *dst_direction)
{
    ...
    if (mirror != DRV_TDE_MIRROR_NONE) {
        ...
        if ((dst_bpp < 0) || (srcd_bpp < 0)) {
            tde_error("bits per pixel less than 0!src fmt:%u,dst fmt:%u\n",
                (td_u32)single_src->src_surface->color_format, (td_u32)single_src->dst_surface->color_format);
            return -1;
        }

        ret = tde_osi_check_single_src(single_src);
        if (ret != TD_SUCCESS) {
            return ret;
        }

        src_addr = single_src->src_surface->phys_addr + ((td_phys_addr_t)single_src->src_rect->pos_y *
            single_src->src_surface->stride) +
            (td_phys_addr_t)((single_src->src_rect->pos_x * srcd_bpp) / 8); /* 8 bits */
        dst_addr = single_src->dst_surface->phys_addr + ((td_phys_addr_t)single_src->dst_rect->pos_y *
            single_src->dst_surface->stride)  +
            (td_phys_addr_t)((single_src->dst_rect->pos_x * dst_bpp) / 8); /* 8 bits */
                                     ...
                                     drv_tde_rect *inter_rect)
{
    ...
    right1 = pos_x + (td_s32)dst_rect->width - 1;
    right2 = clip_rect->pos_x + (td_s32)clip_rect->width - 1;
    right = (right1 > right2) ? right2 : right1;

    bottom1 = pos_y + (td_s32)dst_rect->height - 1;
    bottom2 = clip_rect->pos_y + (td_s32)clip_rect->height - 1;
    bottom = (bottom1 > bottom2) ? bottom2 : bottom1;

    if ((left > right) || (top > bottom)) {
        tde_error("There is no inter zone!");
        return DRV_ERR_TDE_INVALID_PARA;
    }
...
{
    td_s32 right1 = rect1->pos_x + (td_s32)rect1->width - 1;
    td_s32 right2 = rect2->pos_x + (td_s32)rect2->width - 1;

    td_s32 bottom1 = rect1->pos_y + (td_s32)rect1->height - 1;
    td_s32 bottom2 = rect2->pos_y + (td_s32)rect2->height - 1;
...
static td_u32 tde_true_value_to_complement(td_s32 data_in, td_u32 bit)
{
    td_u32 data_out;
    td_u32 data_in_tmp;
    /* Bit operation is allowed with integer in Image operation */
    if (data_in >> (bit - 1)) {
...
static td_void tde_osi_calc_slice_node(tde_slice_data *slice_data, tde_hw_node *child_node, td_u32 i)
{
    ...
    set_slice_size = ((slice_data->hpzme_en == 0) && ((slice_data->slice_width % 2) == 1) &&
        ...
        (slice_data->fmt == 52)) && /* 52 alg */
        (slice_data->xed_pos_cord_in_tap_hpzme_hso == (slice_data->width - 1)) &&
...
static td_void tde_osi_get_hupdate_info(const tde_update_config *reg, tde_update_info *info, td_bool scaler)
{
    ...
    if ((update_hstart >= 0) && (update_hstart < 3)) { /* 3 alg data */
        ...
        /* 12 4096 alg data */
        zme_houtstart = (td_s32)((zme_hinstart % 4096) == 0 ? ((td_u32)zme_hinstart >> 12) :
        (((td_u32)zme_hinstart >> 12) + 1U)); /* 12 alg data */
        zme_hinstart = (td_s32)((td_u32)(zme_houtstart * ratio) >> 12); /* 12 alg data */
    ...
    if ((update_hstop > (reg->ori_in_width - 3)) && (update_hstop < reg->ori_in_width)) { /* 3 alg data */
        ...
        /* 12 4096 alg data */
        zme_houtstop = (td_s32)((zme_hinstop % 4096) == 0 ? (((td_u32)zme_hinstop >> 12) - 1U) :
        ((td_u32)zme_hinstop >> 12)); /* 12 alg data */
        zme_hinstop = (td_s32)((td_u32)(zme_houtstop * ratio) >> 12);  /* 12 alg data */
...
static td_void tde_osi_get_vupdate_info(const tde_update_config *reg, tde_update_info *info, int scaler)
{
    ...
    if ((update_vstart >= 0) && (update_vstart < 2)) { /* 2 Vstart */
        ...
        /* 4096 12 alg data */
        zme_voutstart = (td_s32)((zme_vinstart % 4096) == 0 ? ((td_u32)zme_vinstart >> 12) :
        (((td_u32)zme_vinstart >> 12) + 1U)); /* 12 alg data */
        zme_vinstart = (td_s32)((td_u32)(zme_voutstart * ratio) >> 12); /* 12 alg data */
        if (zme_vinstart - 1 < 0) {
    ...
    if ((update_vstop > (reg->ori_in_height - 2)) && (update_vstop < reg->ori_in_height)) {
        ...
        /* 4096 12 alg data */
        zme_voutstop = (td_s32)((zme_vinstop % 4096) == 0 ? (((td_u32)zme_vinstop >> 12) - 1U) :
        ((td_u32)zme_vinstop >> 12)); /* 12 alg data */
        zme_vinstop = (td_s32)((td_u32)(zme_voutstop * ratio) >> 12); /* 12 alg data */
...
{
    reg->ori_in_height = (td_s32)double_src->fg_surface->height;
    reg->ori_in_width = (td_s32)double_src->fg_surface->width;
    reg->zme_out_height = (td_s32)double_src->dst_surface->height;
    reg->zme_out_width = (td_s32)double_src->dst_surface->width;
    reg->update_instart_w = double_src->fg_rect->pos_x;
    reg->update_instart_h = double_src->fg_rect->pos_y;
    reg->update_in_width = (td_s32)double_src->fg_rect->width;
    reg->update_in_height = (td_s32)double_src->fg_rect->height;
}
    ...
    drv_tde_deflicker_mode deflicker_mode, drv_tde_deflicker_mode filter_mode)
{
    ...
    tde_info("is_v_scale: 0x%x\n", (td_u32)scale);
    tde_osi_get_vupdate_info(&reg, &info, (td_s32)scale);

    node->src2_imgsize.bits.src2_width = (td_u32)(info.zme_in_width - 1);
    node->src2_zmeireso.bits.iw = (td_u32)(info.zme_in_width - 1);

    node->src2_imgsize.bits.src2_height = (td_u32)(info.zme_in_height - 1);
    node->src2_zmeireso.bits.ih = (td_u32)(info.zme_in_height - 1);
    ...
    node->src2_voffset.bits.vluma_offset = (td_u32)info.zme_vphase;
    ...
    drv_tde_deflicker_mode deflicker_mode, drv_tde_deflicker_mode filter_mode)
{
    ...
    if (((double_src->fg_surface->width != double_src->fg_rect->width) ||
        ...
        if (double_src->dst_surface->width <= 1) {
            ...
            node->src2_hsp.bits.hratio = (td_u32)osal_div_u64(((td_u64)(double_src->fg_surface->width) <<
        ...
        if (double_src->dst_surface->height <= 1) {
            ...
            node->src2_vsr.bits.vratio =
            (td_u32)osal_div_u64((td_u64)(double_src->fg_surface->height << TDE_FLOAT_BITLEN),
            (double_src->dst_surface->height));
        }
        src2_addr = double_src->fg_surface->phys_addr;
        dst_addr = double_src->dst_surface->phys_addr;
        src2_addr += (td_phys_addr_t)(rect_opt.in_rect->pos_y * node->src2_ch0_stride.bits.src2_ch0_stride  +
            ((rect_opt.in_rect->pos_x * tde_osi_get_bpp_by_fmt(rect_opt.in_fmt)) / 8)); /* 8 bits */
        dst_addr += (td_phys_addr_t)(rect_opt.out_rect->pos_y * node->des_ch0_stride.bits.des_ch0_stride +
            ((rect_opt.out_rect->pos_x * tde_osi_get_bpp_by_fmt(out_fmt)) / 8)); /* 8 bits */

        node->src2_ch0_addr_low.bits.src2_ch0_addr_low = (td_u32)get_low_addr(src2_addr);
        node->src2_ch0_addr_high.bits.src2_ch0_addr_high = (td_u32)get_high_addr(src2_addr);

        node->des_ch0_addr_low.bits.des_ch0_addr_low = (td_u32)get_low_addr(dst_addr);
        node->des_ch0_addr_high.bits.des_ch0_addr_high = (td_u32)get_high_addr(dst_addr);
                                     ...
                                     drv_tde_fill_color *fill_color, drv_tde_rect *rect)
{
    ...
    if (bpp < 8) { /* 8 is bpp */
        /* when writing, 8 subbyte format align ask start point byte align */
        if ((td_u32)(rect->pos_x * bpp % 8) || (td_u32)((td_s32)rect->width * bpp % 8)) {
            tde_error("Subbyte's start or width for writing is not aligned!\n");
            return DRV_ERR_TDE_INVALID_PARA;
        }
    }
 ...
 */
static td_s32 tde_osi_1_source_fill(td_s32 handle, drv_tde_surface *dst_surface, const drv_tde_rect *dst_rect,
                                        ...
                                        td_u32 num)
{
    ...
    if (dst_surface == TD_NULL) {
        tde_error("dst_surface is NULL!\n");
        return DRV_ERR_TDE_NULL_PTR;
    }

    if (line == TD_NULL) {
        tde_error("line is NULL!\n");
        return DRV_ERR_TDE_NULL_PTR;
    }

    if (tde_hal_node_init_nd(&hw_node) != TD_SUCCESS) {
        tde_error("tde_hal_node_init_nd failed!\n");
        return DRV_ERR_TDE_NO_MEM;
    }
    if (num > TDE_MAX_LINE_NUM) {
        tde_hal_free_node_buf(hw_node);
        hw_node = TD_NULL;
        tde_error("num is out of bounds!\n");
        return DRV_ERR_TDE_INVALID_PARA;
    }
    for (i = 0; i < num; i++) {
...
#if (TDE_CAPABILITY & COLORIZE)
    ret = tde_osi_set_colorize(hw_node, opt->alpha_blending_cmd, (td_s32)opt->color_resize);
    if (ret != TD_SUCCESS) {
        return ret;
    }
#endif
...
static td_s32 tde_osi_check_surface(const drv_tde_surface *surface, drv_tde_rect *rect)
{
    ...
    if (invalid_operation_area) {
        tde_error("operation area is over maximum!Width:%u,Height:%u\n", rect->width, rect->height);
        return DRV_ERR_TDE_INVALID_PARA;
    }
...
static td_s32 tde_osi_pre_check_surface(const drv_tde_surface *surface, const drv_tde_rect *rect)
{
    ...
    if (real) {
        tde_error("surface_w:%u, surface_h:%u, stride:%u, x:%d, y:%d, w:%u, h:%u\n", surface->width,
            surface->height, surface->stride, rect->pos_x,
...
static td_s32 tde_osi_pre_check_mmz(const drv_tde_surface *surface, td_u32 size)
{
    td_s32 ret = cmpi_check_mmz_phy_addr(surface->phys_addr, size);
    if (ret != TD_SUCCESS) {
    ...
    if (surface->cbcr_phys_addr != 0) {
        ...
        ret = cmpi_check_mmz_phy_addr(surface->cbcr_phys_addr, size);
        if (ret != TD_SUCCESS) {
...
{
    if ((rect->pos_x + (td_s32)rect->width) > (td_s32)surface->width) {
        rect->width = (td_u32)((td_s32)surface->width - rect->pos_x);
    }

    if ((rect->pos_y + (td_s32)rect->height) > (td_s32)surface->height) {
        rect->height = (td_u32)(((td_s32)surface->height > rect->pos_y) ? (td_s32)surface->height - rect->pos_y : 0);
    }
}
...
static td_s32 tde_osi_pre_check_surface_ex(const drv_tde_surface *surface, drv_tde_rect *rect)
{
    ...
    if (bpp == TD_FAILURE) {
        tde_error("surface->color_format is invalid\n");
        return DRV_ERR_TDE_INVALID_PARA;
    }
    /* CMPI_CheckMmzphy_addr Call a semaphore，Cannot be used in the interrupt */
    if (!osal_in_interrupt()) {
        if (bpp >= 8) { /* 8 alg data */
            size = surface->stride * (surface->height - 1U) + (td_u32)((td_s32)surface->width * bpp / 8);
        } else {
...
static td_s32 tde_osi_raster_fmt_check_align(const drv_tde_surface *surface)
{
    ...
    if ((bpp >= 8) && (bpp != 24)) { /* 8 24 Bpp */
        byte_per_pixel = (td_u32)(bpp / 8); /* 8 bits */
...
static td_s32 tde_osi_check_mb_surface_ex(const drv_tde_mb_surface *mb_surface, drv_tde_rect *rect)
{
    ...
    if (real) {
        tde_error("syw:%u, syh:%u, systride:%u, scbcrstride:%u, x:%d, y:%d, w:%u, h:%u\n",
            mb_surface->y_width, mb_surface->y_height, mb_surface->y_stride,
            mb_surface->cbcr_stride, rect->pos_x, rect->pos_y, rect->width,
            rect->height);
    ...
    if ((td_u32)(rect->pos_x + (td_s32)rect->width) > mb_surface->y_width) {
        rect->width = (td_u32)((td_s32)mb_surface->y_width - rect->pos_x);
    }

    if ((td_u32)(rect->pos_y + (td_s32)rect->height) > mb_surface->y_height) {
        rect->height = (td_u32)((td_s32)mb_surface->y_height - rect->pos_y);
    }
...
static tde_operation_category tde_osi_single_src_operation(const drv_tde_double_src *double_src, const drv_tde_opt *opt)
{
    ...
    if ((opt == TD_NULL) || (opt->resize == TD_FALSE)) {
                                       ...
                                       tde_surface_msg *drv_sur)
{
    if (sur->color_format > DRV_TDE_COLOR_FMT_MAX) {
        tde_error("sur->color_format is invalid %s(line=%d)\n", __FUNCTION__, __LINE__);
        return;
    }
                                      ...
                                      const tde_hw_node *hw_node)
{
    ...
    if (opt->clip_mode != DRV_TDE_CLIP_MODE_NONE) {
        if ((opt->clip_rect.height == 0) || (opt->clip_rect.width == 0) || (opt->clip_rect.pos_x < 0) ||
            (opt->clip_rect.pos_x > (td_s32)TDE_MAX_RECT_WIDTH) ||
            (opt->clip_rect.pos_y < 0) || (opt->clip_rect.pos_y > (td_s32)TDE_MAX_RECT_HEIGHT) ||
            (opt->clip_rect.width > TDE_MAX_RECT_WIDTH) ||
            (opt->clip_rect.height > TDE_MAX_RECT_HEIGHT)) {
...
{
    clip->clip_start_x = (td_u16)(opt->clip_rect.pos_x - double_src->dst_rect->pos_x);
    clip->clip_start_y = (td_u16)(opt->clip_rect.pos_y - double_src->dst_rect->pos_y);
    clip->clip_end_x = (td_u16)(clip->clip_start_x + inter_rect->width - 1U);
    clip->clip_end_y = (td_u16)(clip->clip_start_y + inter_rect->height - 1U);
    return;
}
/*
...
static td_s32 tde_osi_set_clip_para(drv_tde_double_src *double_src, const drv_tde_opt *opt, tde_hw_node *hw_node)
{
    ...
    if ((opt->clip_mode == DRV_TDE_CLIP_MODE_INSIDE) && (opt->resize == TD_FALSE)) {
        ...
        ret = tde_osi_get_inter_rect(double_src->dst_rect, &opt->clip_rect, &inter_rect);
        if (ret != TD_SUCCESS) {
            return ret;
        }
        ...
        tde_osi_get_clip_pos(&clip, opt, double_src, &inter_rect);
        ret = tde_hal_node_set_clipping(hw_node, &clip);
        if (ret != TD_SUCCESS) {
            return ret;
        }
...
static td_void tde_osi_none_cmd(tde_hw_node *hw_node, tde_sw_node *cmd, const tde_sw_job *job)
{
    ...
    cmd->node_buf.node_sz = (td_u32)sizeof(tde_hw_node);
    cmd->node_buf.update = (0xffffffff) | ((td_u64)0x000003ff << 32); /* 32 alg data */
    cmd->node_buf.phy_addr = wget_user_addr(cmd->node_buf.buf) + TDE_NODE_HEAD_BYTE;
    /*
                                      ...
                                      td_u32 work_buf_num, tde_node_subm_type subm_type)
{
    ...
    cmd->index = (td_s32)job->cmd_num;
    cmd->submit_type = subm_type;
    cmd->phy_buf_num = work_buf_num;
    *(((td_u64 *)cmd->node_buf.buf) + 1) = (td_u64)handle;
...
td_s32 tde_osi_end_job(drv_tde_end_job_cmd *const end_job, drv_tde_func_callback func_compl_cb, td_void *func_para)
{
...
static td_s32 tde_osi_rotate_check_surface(const tde_surface_msg *surface)
{
    ...
    bpp = (td_u32)tde_osi_get_bpp_by_fmt(surface->color_format);
    if (bpp == 0xffffffff) {
        tde_error("surface->color_format is invalid\n");
        return TD_FAILURE;
    }
...
td_s32 tde_osi_blit(td_s32 handle, drv_tde_double_src *double_src, const drv_tde_opt *opt)
{
    ...
    switch (opt_category) {
        case TDE_OPERATION_SINGLE_SRC1:
            ...
            ret = tde_osi_single_src_1_blit(handle, &single_src, TD_FALSE, TD_FALSE);
            if (ret < 0) {
                return ret;
            }
            break;
...
static td_s32 tde_osi_draw_line_check_format(const drv_tde_surface *dst_surface)
{
    ...
    tde_error("unsupported color format %u\n", (td_u32)dst_surface->color_format);
    return DRV_ERR_TDE_INVALID_PARA;
}
...
static td_s32 tde_osi_draw_line_check(const drv_tde_line *line)
{
    ...
    if ((line->thick == 0) || (line->thick > DRD_MAX_LINE_WIDTH)) {
        tde_error("invalid line width:%u.\n", line->thick);
        return DRV_ERR_TDE_INVALID_PARA;
    }
                                        ...
                                        const drv_tde_opt *opt, td_bool mmz_for_src, td_bool mmz_for_dst)
{
    ...
    if (drv_surface == TD_NULL) {
        tde_error ("malloc pstDrvSurface failed, size=%lu!\n", (unsigned long)(sizeof(tde_surface_msg)));
        tde_hal_free_node_buf(hw_node);
        hw_node = TD_NULL;
        return DRV_ERR_TDE_NO_MEM;
    }
    ...
    td_u32 *byte_per_pixel, td_u32 *cb_cr_byte_per_pixel)
{
    ...
    *cb_cr_byte_per_pixel = ((mb_src->mb_surface->mb_color_format == DRV_TDE_MB_COLOR_FMT_JPG_YCBCR444MBP) ||
    ...
    if (bpp == -1) {
        tde_error("mb_src->dst_surface->color_format is invalid\n");
        return DRV_ERR_TDE_INVALID_PARA;
    }
...
static td_s32 tde_osi_check_surface_ex(const drv_tde_surface *surface, drv_tde_rect *rect)
{
    ...
    if (invalid_operation_area) {
        tde_error("invalid operation SurfaceEX area!width=%u,height=%u\n", rect->width, rect->height);
        return DRV_ERR_TDE_INVALID_PARA;
    }
...
static td_s32 tde_osi_check_mb_blit_para(const drv_tde_mb_src *mb_src, const drv_tde_mb_opt *mb_opt)
{
    ...
    if (!osal_in_interrupt()) {
        ...
        ret = cmpi_check_mmz_phy_addr(mb_src->mb_surface->y_addr, size);
        if (ret != TD_SUCCESS) {
            tde_error("Check MB surface phy_addr addr failed!\n");
            return DRV_ERR_TDE_INVALID_PARA;
        }

        if (mb_src->mb_surface->cbcr_phys_addr != 0) {
            size = mb_src->mb_surface->cbcr_stride * mb_src->mb_surface->y_height;
            if ((mb_src->mb_surface->mb_color_format == DRV_TDE_MB_COLOR_FMT_JPG_YCBCR422MBVP)  ||
                (mb_src->mb_surface->mb_color_format == DRV_TDE_MB_COLOR_FMT_MP1_YCBCR420MBP) ||
                (mb_src->mb_surface->mb_color_format == DRV_TDE_MB_COLOR_FMT_MP2_YCBCR420MBP)   ||
                (mb_src->mb_surface->mb_color_format == DRV_TDE_MB_COLOR_FMT_MP2_YCBCR420MBI) ||
                (mb_src->mb_surface->mb_color_format == DRV_TDE_MB_COLOR_FMT_JPG_YCBCR420MBP)) {
                size = mb_src->mb_surface->cbcr_stride * (mb_src->mb_surface->y_height / 2); /* 2 alg data */
            }
            ret = cmpi_check_mmz_phy_addr(mb_src->mb_surface->cbcr_phys_addr, size);
            if (ret != TD_SUCCESS) {
                tde_error("Check MB surface cbcr_phy_addr addr failed!\n");
                return DRV_ERR_TDE_INVALID_PARA;
            }
...
{
    if (tde_osi_whether_contain_alpha(triple_src->fg_surface->color_format) == TD_FALSE) {
    ...
    if (triple_src->dst_surface->color_format >= DRV_TDE_COLOR_FMT_CLUT1 &&
        ...
        tde_error("dst color format %u not support\n", (td_u32)triple_src->dst_surface->color_format);
        return DRV_ERR_TDE_INVALID_PARA;
    }
    return TD_SUCCESS;
}
                                                    ...
                                                    drv_tde_alpha_blending blend_mode)
{
    ...
    if (blend_mode < 0 || blend_mode >= (td_u32)DRV_TDE_ALPHA_BLENDING_MAX) {
                                                   ...
                                                   drv_tde_color_fmt dst_fmt, const drv_tde_pattern_fill_opt *opt)
{
    ...
    if ((bg_category == TDE_COLORFMT_CATEGORY_ARGB) &&
        (fg_category == TDE_COLORFMT_CATEGORY_AN) && (temp_fmt == TD_FALSE)) {
        tde_error("Target must have alpha component!\n");
        return DRV_ERR_TDE_INVALID_PARA;
...
static td_s32 tde_osi_check_pattern_dst_color_format(drv_tde_color_fmt color_format)
{
    ...
    if ((color_format >= DRV_TDE_COLOR_FMT_A1) && (color_format != DRV_TDE_COLOR_FMT_YCBCR888) &&
        ...
        tde_error("pattern fill:dst color format doesn't support format:%u!\n", (td_u32)color_format);
        return TD_FAILURE;
    }
...
static td_s32 tde_osi_check_pattern_src_color_format(drv_tde_color_fmt color_format)
{
    ...
    if ((color_format >= DRV_TDE_COLOR_FMT_A1) && (color_format != DRV_TDE_COLOR_FMT_YCBCR888) &&
        (color_format != DRV_TDE_COLOR_FMT_AYCBCR8888)) {
        tde_error("pattern fill:bg or fg color format doesn't support format:%u!\n", (td_u32)color_format);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
                                                                           ...
                                                                           const drv_tde_pattern_fill_opt *opt)
{
    ...
    if ((double_src->bg_rect->height != double_src->dst_rect->height) ||
        ...
        tde_error("background x:%d, y:%d, w:%u, h:%u; dst x:%d, y:%d, w:%u, h:%u\n", double_src->bg_rect->pos_x,
                                ...
                                tde_alu_mode *alu_mode, td_bool check_blend)
{
    ...
    *alu_mode = TDE_ALU_BLEND;
    ...
    ret = tde_hal_node_set_blend(hw_node, &blend_opt);
    if (ret != TD_SUCCESS) {
        return ret;
    }
#if (TDE_CAPABILITY & ROP)
    ...
    td_s32 color_resize)
{
    ...
    if (set_colorize == TD_FALSE) {
        return TD_SUCCESS;
    }

    if (tde_hal_node_set_colorize(hw_node, (td_u32)color_resize) != TD_SUCCESS) {
        return DRV_ERR_TDE_UNSUPPORTED_OPERATION;
    }
...
static td_s32 tde_osi_set_rop(tde_hw_node *hw_node, const tde_rop_opt *rop_opt, tde_alu_mode *alu_mode)
{
    ...
    td_bool only_support_single_sr2_rop = ((tde_osi_is_single_src_to_rop(rop_opt->rop_code_alpha) == TD_FALSE) ||
                                           (tde_osi_is_single_src_to_rop(rop_opt->rop_code_color) == TD_FALSE));
...
static td_s32 tde_osi_pattern_fill_param_check(drv_tde_pattern_fill_opt *fill_opt)
{
    if (fill_opt == NULL || fill_opt->alpha_blending_cmd != DRV_TDE_ALPHA_BLENDING_BLEND) {
        tde_error("tde pattern_fill param check failed in pram maybe null and cur mode only support alpha blending");
        return DRV_ERR_TDE_INVALID_PARA;
    }
    fill_opt->colorkey_mode = DRV_TDE_COLOR_KEY_MODE_NONE;
    fill_opt->clip_mode = DRV_TDE_CLIP_MODE_NONE;
    fill_opt->clut_reload = TD_FALSE;
    (void)memset_s(&fill_opt->csc_opt, sizeof(drv_tde_csc_opt), 0x0, sizeof(drv_tde_csc_opt));
    return TD_SUCCESS;
}
...
td_s32 tde_osi_pattern_fill(td_s32 handle, const drv_tde_double_src *double_src, const drv_tde_pattern_fill_opt *opt)
{
    ...
    drv_tde_pattern_fill_opt cur_opt = {0};
    td_s32 ret = 0;
    opt_category_en = tde_osi_get_pattern_opt_category(double_src, opt);
    if (opt == NULL || opt_category_en == TDE_PATTERN_OPERATION_SINGLE_SRC) {
        tde_error("opt ptr is %d. opt category is %d single operation is not supported\n",
                  opt == NULL,
                  opt_category_en);
        return DRV_ERR_TDE_INVALID_PARA;
    }
    switch (opt_category_en) {
        ...
        case TDE_PATTERN_OPERATION_DOUBLE_SRC:
            cur_opt = *opt;
            ret = tde_osi_pattern_fill_param_check(&cur_opt);
            if (ret != TD_SUCCESS) {
                return ret;
            }
            return tde_osi_double_src_pattern_fill(handle, double_src, &cur_opt);
        default:
            return DRV_ERR_TDE_INVALID_PARA;
    }
}
                          ...
                          drv_tde_rect *rect_in_src, drv_tde_rect *rect_in_dst)
{
    ...
    tde_osi_get_hupdate_info(&reg, &info, (td_s32)TD_TRUE);
    tde_osi_get_vupdate_info(&reg, &info, (td_s32)TD_TRUE);
