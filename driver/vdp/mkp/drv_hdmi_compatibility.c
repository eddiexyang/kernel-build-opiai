...
static compatibility_info g_compat_info[(td_s32)HDMI_ID_MAX];
    ...
    const hdmi_edid_manufacture_info *sink_manu_info)
{
    ...
    match = (tmp_manu_info->mfrs_name[0] == sink_manu_info->mfrs_name[0]) && /* 0, array index */
            (tmp_manu_info->mfrs_name[1] == sink_manu_info->mfrs_name[1]) && /* 1, array index */
            (tmp_manu_info->mfrs_name[2] == sink_manu_info->mfrs_name[2]) && /* 2, array index */
            (tmp_manu_info->mfrs_name[3] == sink_manu_info->mfrs_name[3]) && /* 3, array index */
...
static td_bool compat_avi_use_gen5(const hdmi_sink_capability *sink_caps)
{
    ...
    for (i = 0, tmp_manuinfo = &g_avi_use_gen5[0];
        ...
        if (compat_edid_match_chk(tmp_manuinfo, &sink_caps->mfrs_info) ||
            ...
            (sink_caps->mfrs_info.mfrs_name[2] == 'P') && /* 2, array index */
...
{
    if ((td_s32)hdmi_id < (td_s32)HDMI_ID_MAX) {
...
td_s32 drv_hdmi_compat_avmute_get(hdmi_device_id hdmi_id, td_bool avmute, hdmi_avmute_cfg *avmute_cfg)
{
    ...
    avmute_cfg->mute_clr    = (td_bool)((!avmute) && sink_avmute->avmute_cfg.mute_clr);
    avmute_cfg->mute_rpt_en = (td_bool)(!avmute);
    ...
    if (compat_info->setmute_send == TD_TRUE) {
        ...
        hdmi_debug("debug config, not send avmute.\n");
        return TD_SUCCESS;
    }
...
td_s32 drv_hdmi_compat_stop_delay(hdmi_device_id hdmi_id)
{
    ...
    if (drv_hdmi_edid_capability_get(&hdmi_dev->edid_info, &sink_cap) == HDMI_EDID_DATA_INVALID) {
        hdmi_warn("there is no sink capability\n");
        return TD_FAILURE;
    } else {
        sink_manu_info = &sink_cap->mfrs_info;
    ...
    if (drv_hdmi_edid_capability_get(&hdmi_dev->edid_info, &sink_caps) == HDMI_EDID_DATA_INVALID) {
        ...
        if (compat_avi_use_gen5(sink_caps)) {
            if (compat_info->avi_use_gen5_debug == TD_FALSE) {
                hdmi_debug("use gen5 send avi.\n");
                compat_info->avi_use_gen5 = TD_TRUE;
            }
            if (!!(compat_info->frl_compress_mode & HDMI_FRL_COMPRESS_DEBUG_MASK) == (td_s32)TD_FALSE) {
                hdmi_debug("frl not compress.\n");
