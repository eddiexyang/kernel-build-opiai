...
static const td_char *g_establish_timing_str[] = {
...
static td_s32 edid_block_check_sum(const td_u8 *data, hdmi_edid_status *status)
{
    ...
    /* EDID header */
    edid_info("    %-20s : %02X\n", "check sum", (td_u32)data[127]); /* 127, EDID header msg */

    if ((check_sum & 0xff) != 0x00) {
        if (data[32] != 0x00) { /* 32, array index */
...
static td_s32 edid_vendor_info(hdmi_edid_info *info)
{
    ...
    data = (td_u16)((fst_blk->mfg_id[0] << 8) | (fst_blk->mfg_id[1]));

    /* 2, word width */
    for (i = 0; i <= 2; i++) {
        /* 2, word width */
        vendor->mfrs_name[2 - i] = (td_u32)((data &
                                            ((td_u16)EDID_VEND_NAME_CHAR_MASK <<
                                             (5 * i))) >> (5 * i)); /* 5, bit a character */
        ...
        if ((vendor->mfrs_name[2 - i] > EDID_VEND_CHAR_LOW_INVALID) &&
            ...
            status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_VENDOR_INVALID;
        }
    }
    /* 8, offset. express product code 15-8 bits */
    vendor->product_code = (td_u32)(fst_blk->prod_code[1] << 8) | fst_blk->prod_code[0];
    /* 24, 16, is serial number 3 and 4 byte */
    vendor->serial_number = (td_u32)((fst_blk->serial[3] << 24) | (fst_blk->serial[2] << 16) |
...
static td_s32 edid_version(hdmi_edid_info *info)
{
    ...
    if ((cap->version != 1) || (cap->revision != 3)) {
        edid_err("EDID first blk is not version1.3:%u.%u\n!", (td_u32)cap->version, (td_u32)cap->revision);
        status->parse_err = EDID_PARSE_ERR_FST_BLK_VER;
    }
    edid_info("    %-20s : %02u\n", "version no.", (td_u32)cap->version);
    edid_info("    %-20s : %02u\n", "revision no.", (td_u32)cap->revision);
...
static td_s32 edid_std_timing(hdmi_edid_info *info)
{
    ...
    for (i = 0; i < HDMI_EDID_MAX_STDTIMNG_COUNT; i++, data += 2) { /* std_timing data length is 2 */
        if ((data[0] != EDID_STDTMING_UNUSED_FLAG) || (data[1] != EDID_STDTMING_UNUSED_FLAG)) {
            cap->std_timing[i].hor_active = ((td_u32)data[0] + (td_u32)EDID_HORACTIVE_BASE) *
                                             (td_u32)EDID_HORACTIVE_FACTOR;
...
static td_s32 edid_detail_timing(hdmi_sink_capability *cap, hdmi_edid_status *status, const td_u8 *data, td_u8 len)
{
    ...
    if (cap->perfer_timing_num >= HDMI_EDID_MAX_DETAIL_TIMING_COUNT ||
        len < EDID_DTD_SIZE) {
        status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_DTD_OVER;
    ...
    if (osal_memcmp(data, monitor_flag, sizeof(monitor_flag)) == 0) {
        /* case 1 */
        if (data[3] == EDID_TYPE_MONITOR_NAME) { /* 3, display product name */
...
static td_s32 edid_ext_num(hdmi_edid_info *info)
{
    ...
    edid_info("    %-20s : %02u\n", "extensions", (td_u32)cap->ext_block_num);

    if (cap->ext_block_num == 0) {
        status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_EXT_BLK_ZERO;
        edid_warn("ext-BLK zero: 0x%02x \n", (td_u32)cap->ext_block_num);
    } else if (cap->ext_block_num > (HDMI_EDID_MAX_BLOCK_NUM - 1)) {
        status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_EXT_BLK_OVER;
...
static td_void edid_audio_rate(td_u8 byte, td_u8 frm_code, hdmi_edid_audio_info *audio_info)
{
    ...
    if (byte & BIT0_MASK) {
        ...
        edid_info("    %-20s : %d hz\n", "sample frequencies", (td_s32)audio_info->support_sample_rate[count]);
        count++;
    }
    if (byte & BIT1_MASK) {
...
static td_void edid_audio_depth_and_max_rate(td_u8 byte, td_u8 frm_code, hdmi_edid_audio_info *audio_info)
{
    ...
    if (frm_code == 1) {
        if (byte & BIT0_MASK) {
            ...
            edid_info("    %-20s : %d bit\n", "bit depth", (td_s32)audio_info->support_bit_depth[count]);
            count++;
        }
        if (byte & BIT1_MASK) {
...
static td_s32 edid_audio_db(hdmi_sink_capability *cap, hdmi_edid_status *status, const td_u8 *data, td_u8 len)
{
    ...
    if ((len / EDID_AUDIO_DESCRIPTOR_SIZE) > 10) { /* 10, audio capability count */
        ...
        status->parse_warn |= (1 << (td_u32)EDID_PARSE_WARN_AUDIO_CNT_OVER);
        return TD_FAILURE;
    }

    for (i = 0; i < (len / EDID_AUDIO_DESCRIPTOR_SIZE); i++) {
        cur_num = (td_u8)cap->audio_info_num;
        if (cur_num >= HDMI_EDID_MAX_AUDIO_CAP_COUNT) {
            status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_AUDIO_CNT_OVER;
            edid_warn(" audio capability count over max-num:%u \n", (td_u32)HDMI_EDID_MAX_AUDIO_CAP_COUNT);
            break;
        }
        /* byte-0 */
        edid_info("    %-20s : 0x%02X, 0x%02X, 0x%02X\n", "short audio descriptor",
            (td_u32)data[i * EDID_AUDIO_DESCRIPTOR_SIZE], (td_u32)data[i * EDID_AUDIO_DESCRIPTOR_SIZE + 1],
            (td_u32)data[i * EDID_AUDIO_DESCRIPTOR_SIZE + 2]); /* 2, offset */
        ...
        edid_info("    %-20s : %02u\n", "max. number of channels", (td_u32)audio_info[cur_num].aud_channel);
        edid_info("    %-20s : %s\n", "audio format code", audio_code_str);
#endif
        if (frm_code > (td_s32)HDMI_AUDIO_CODING_TYPE_STREAM && frm_code < (td_s32)HDMI_AUDIO_CODING_TYPE_BUTT) {
            cap->audio_info_num++;
        } else {
            status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_AUDIO_FMT_INVALID;
...
static td_s32 edid_video_db(hdmi_sink_capability *cap, hdmi_edid_status *status, const td_u8 *data, td_u8 len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (cap->support_vic_num >= HDMI_EDID_MAX_VIC_COUNT) {
            status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_VIC_CNT_OVER;
            edid_warn("VDB vic count over:%u !\n", (td_u32)HDMI_EDID_MAX_VIC_COUNT);
            break;
        }

        /* avoid invalid vic */
        if (vic_all == EDID_VIC_INVALID_ZERO) {
            status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_VIC_INVALID;
...
static td_void edid_hfvs21_db(hdmi_sink_capability *cap, const td_u8 *data)
{
    ...
    cap->dsc_info.dsc_1p2        = (data[3] & BIT7_MASK) ? TD_TRUE : TD_FALSE; /* 3, array index */
    cap->dsc_info.dsc_native_420 = (data[3] & BIT6_MASK) ? TD_TRUE : TD_FALSE; /* 3, array index */
    cap->dsc_info.dsc_all_bpp    = (data[3] & BIT3_MASK) ? TD_TRUE : TD_FALSE; /* 3, array index */
    cap->dsc_info.dsc_16bpc      = (data[3] & BIT2_MASK) ? TD_TRUE : TD_FALSE; /* 3, array index */
    cap->dsc_info.dsc_12bpc      = (data[3] & BIT1_MASK) ? TD_TRUE : TD_FALSE; /* 3, array index */
    cap->dsc_info.dsc_10bpc      = (data[3] & BIT0_MASK) ? TD_TRUE : TD_FALSE; /* 3, array index */
    cap->dsc_info.dsc_max_slices = (data[4] & EDID_LOWER_NIBBLE_MASK); /* 4, array index */
    cap->dsc_info.dsc_max_frl_rate = (data[4] & EDID_UPPER_NIBBLE_MASK); /* 4, array index */
    cap->dsc_info.dsc_total_chunk_k_bytes = (data[5] & 0x3F); /* 0x3F, dsc total chunk k byte mask; 5, array index */
...
static td_s32 edid_hfvs_db(hdmi_sink_capability *cap, hdmi_edid_status *status, const td_u8 *data, td_u8 len)
{
    if (len < 7 || len > 31) { /* 7 31, hfvs db length extent 7~31 */
        status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_BLOCKLEN_INVALID;
        edid_warn("HFVSDB len:%u\n", (td_u32)len);
        return TD_SUCCESS;
    }

    /* byte-3 ,version */
    if (data[3] != EDID_HFVSDB_VERSION) { /* 3, array index */
        status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_HFVSDB_INVALID;
        edid_warn("    HFVSDB version:%u\n", (td_u32)data[3]); /* 3, array index */
    }
    edid_info("    HDMI forum vendor specific data block        %-36s : %02u\n", "version",
        (td_u32)data[3]); /* 3, array index */
    /* byte-4 */
    cap->max_tmds_clock  = data[4]; /* 4, array index */
    ...
    /* byte-5 */
    cap->support3d_osd_disparity       = edid_flags_mask_check(data[5], BIT0_MASK); /* 5, array index */
    cap->support3d_dual_view           = edid_flags_mask_check(data[5], BIT1_MASK); /* 5, array index */
    cap->support3d_independent_view    = edid_flags_mask_check(data[5], BIT2_MASK); /* 5, array index */
    cap->support_lte340_mcsc_scrameble = edid_flags_mask_check(data[5], BIT3_MASK); /* 5, array index */
    cap->ccbpci                        = edid_flags_mask_check(data[5], BIT4_MASK); /* 5, array index */
    cap->support_rr_capable            = edid_flags_mask_check(data[5], BIT6_MASK); /* 5, array index */
    cap->support_scdc                  = edid_flags_mask_check(data[5], BIT7_MASK); /* 5, array index */
    ...
    /* byte-6 */
    cap->deep_color_y420.deep_color30_bit = edid_flags_mask_check(data[6], BIT0_MASK); /* 6, array index */
    cap->deep_color_y420.deep_color36_bit = edid_flags_mask_check(data[6], BIT1_MASK); /* 6, array index */
    cap->deep_color_y420.deep_color48_bit = edid_flags_mask_check(data[6], BIT2_MASK); /* 6, array index */
    ...
    cap->max_frl_rate = (data[6] & EDID_UPPER_NIBBLE_MASK) >> 4; /* 4, bit[4:1] 6, array index */
...
static td_void edid_latency_parse(hdmi_sink_capability *cap, const td_u8 *data, td_u8 *offset)
{
    if (cap->latency_fields_present) {
        ...
        edid_info("    %-20s : 0x%02X\n", "video latency", (td_u32)cap->video_latency);
        edid_info("    %-20s : 0x%02X\n", "audio latency", (td_u32)cap->audio_latency);
    }
    if (cap->i_latency_fields_present) {
        cap->interlaced_video_latency = data[(*offset)++];
        cap->interlaced_audio_latency = data[(*offset)++];
        edid_info("    %-20s : 0x%02X\n", "interlaced video latency", (td_u32)cap->interlaced_video_latency);
        edid_info("    %-20s : 0x%02X\n", "interlaced audio latency", (td_u32)cap->interlaced_audio_latency);
    }
...
static td_void edid_vsdb_prase_step9(hdmi_sink_capability *cap, const td_u8 *data, hdmi_edid_status *status, td_u8 len)
{
    ...
    if (cap->hdmi_video_present) {
        ...
        edid_info("    %-20s : %02u\n", "3_d_multi_present", (td_u32)_3d_multi_present);
        edid_info("    %-20s : %02u\n", "image_size",
                  (td_u32)((byte & (BIT4_MASK | BIT3_MASK)) >> 3)); /* 3'b, BIT[1] */
    }
    ...
    edid_info("    %-20s : %02u\n", "HDMI_VIC_LEN", (td_u32)hdmi_vic_len);
    edid_info("    %-20s : %02u\n", "HDMI_3D_LEN", (td_u32)hdmi_3d_len);
...
static td_s32 edid_vsdb(hdmi_sink_capability *cap, hdmi_edid_status *status, const td_u8 *data, td_u8 len)
{
    ...
    if (len >= 7) { /* 7, byte-6, max tmds clock */
        cap->max_tmds_clock  = data[6]; /* 6, array index */
        cap->max_tmds_clock *= EDID_TMDS_FACTOR;
        cap->support_hdmi_20 = (cap->max_tmds_clock >= EDID_MAX_HDMI14_TMDS_RATE) ? TD_TRUE : TD_FALSE;
    ...
    if (len >= 8) { /* 8, byte-7, sink present */
        cap->hdmi_video_present       = (data[7] & BIT5_MASK) ? TD_TRUE : TD_FALSE; /* 7, array index */
        cap->i_latency_fields_present = (data[7] & BIT6_MASK) ? TD_TRUE : TD_FALSE; /* 7, array index */
        cap->latency_fields_present   = (data[7] & BIT7_MASK) ? TD_TRUE : TD_FALSE; /* 7, array index */
    }
...
static td_s32 edid_speaker_db(hdmi_sink_capability *cap, hdmi_edid_status *status, const td_u8 *data, td_u8 len)
{
    ...
    if (len < 2) { /* 2, speaker_db min length */
        status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_BLOCKLEN_INVALID;
        status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_SPKDB_INVALID;
...
static td_void edid_y420_video_db(hdmi_sink_capability *cap, hdmi_edid_status *status, const td_u8 *data, td_u8 len)
{
    ...
    for (i = 0; i < len - 1; i++) {
        ...
        if (cap->only_support_y420_vic_num >= HDMI_EDID_MAX_VIC_COUNT) {
            status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_Y420VDB_VIC_OVER;
    ...
    hdmi_edid_status *status, const td_u8 *data, td_u8 blk_len)
{
    ...
    is_vsdb = ((blk_len >= EDID_VENDOR_BLOCK_LEN) && (data[0] == EDID_IEEE_VSDB_1ST) &&
               (data[1] == EDID_IEEE_VSDB_2ND) && (data[2] == EDID_IEEE_VSDB_3RD)); /* 2, array index */

    is_hfvsdb = ((blk_len >= EDID_VENDOR_BLOCK_LEN) && (data[0] == EDID_IEEE_HFVSDB_1ST) &&
                 (data[1] == EDID_IEEE_HFVSDB_2ND) && (data[2] == EDID_IEEE_HFVSDB_3RD));  /* 2, array index */
    ...
    if (is_vsdb == TD_TRUE) {
        ...
        if (blk_len >= EDID_VENDOR_BLOCK_LEN) {
            status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_VSDB_INVALID;
            edid_warn("(HF)VSDB IEEE:0x%02x, 0x%02x, 0x%02x\n", (td_u32)data[0],
                (td_u32)data[1], (td_u32)data[2]); /* 2, array index */
        } else {
            status->parse_warn |= 1 << (td_u32)EDID_PARSE_WARN_BLOCKLEN_INVALID;
...
{
    edid_info("    %-20s : %02X\n", "CEA EDID timing extension tag", (td_u32)data[0]);
    edid_info("    %-20s : %02X\n", "CEA 861B version", (td_u32)data[1]);
    edid_info("    %-20s : %02u\n", "number of native DTD's", (td_u32)(data[3] & 0x0F)); /* 3, array index */
    edid_info("    %-20s : %s\n", "basic audio support", (data[3] & 0x40) ? "yes" : "no"); /* 3, array index */
    edid_info("    %-20s : RGB444 %s %s\n", "pixel format",
        (data[3] & 0x20) ? "ycbcr444" : "", (data[3] & 0x10) ? "ycbcr422" : ""); /* 3, array index */
...
static td_s32 edid_extention_block_parse(hdmi_edid_info *edid_info, td_u8 blk_num)
{
    ...
    data = edid_info->edid_raw + (blk_num * (td_u8)HDMI_EDID_BLOCK_SIZE);
    ...
    clr_space->ycbcr422 = ((data[3] & BIT4_MASK) ? TD_TRUE : TD_FALSE); /* 3, array index */
    clr_space->ycbcr444 = ((data[3] & BIT5_MASK) ? TD_TRUE : TD_FALSE); /* 3, array index */
