...
static ddc_info g_ddc_info[(td_s32)HDMI_ID_MAX];
...
static td_s32 ddc_access_enable_wait(hdmi_device_id id, td_u32 timeout)
{
    ...
    hdmi_reg_cpu_ddc_req_set((td_u32)id, TD_TRUE);

    while (!hdmi_reg_cpu_ddc_req_ack_get((td_u32)id)) {
        osal_msleep(1);
        tmp_time++;
        if (tmp_time > timeout) {
            ret = TD_FAILURE;
...
static td_s32 ddc_access_disable_wait(hdmi_device_id id, td_u32 timeout)
{
    ...
    hdmi_reg_cpu_ddc_req_set((td_u32)id, TD_FALSE);
...
static td_s32 ddc_scl_wait(hdmi_device_id id, td_u32 timeout)
{
    ...
    while (!hdmi_reg_ddc_scl_st_get((td_u32)id)) {
        osal_msleep(1);
        tmp_time += 1;
        if (tmp_time > timeout) {
            ret = TD_FAILURE;
...
static td_s32 ddc_sda_wait(hdmi_device_id id, td_u32 timeout)
{
    ...
    if (!hdmi_reg_ddc_sda_st_get((td_u32)id)) {
        hdmi_reg_dcc_man_en_set((td_u32)id, TD_TRUE);
        while ((!hdmi_reg_ddc_sda_st_get((td_u32)id)) && tmp_timeout++ < timeout) {
            /* pull scl high */
            hdmi_reg_ddc_scl_oen_set((td_u32)id, TD_TRUE);
            osal_udelay(DDC_DEFAULT_DELAY);
            /* pull scl low */
            hdmi_reg_ddc_scl_oen_set((td_u32)id, TD_FALSE);
            osal_udelay(DDC_DEFAULT_DELAY);
        }

        /* STOP contition */
        if (tmp_timeout < timeout && (hdmi_reg_ddc_sda_st_get((td_u32)id))) {
            /* pull sda low */
            hdmi_reg_ddc_sda_oen_set((td_u32)id, TD_FALSE);
            osal_udelay(DDC_DEFAULT_DELAY);
            /* pull scl high */
            hdmi_reg_ddc_scl_oen_set((td_u32)id, TD_TRUE);
            osal_udelay(DDC_DEFAULT_DELAY);
            /* pull sda high */
            hdmi_reg_ddc_sda_oen_set((td_u32)id, TD_TRUE);
            osal_udelay(DDC_DEFAULT_DELAY);
            hdmi_debug("deadlock clear success\n");
            ret = TD_SUCCESS;
        } else {
            hdmi_warn("deadlock clear fail\n");
            ret = TD_FAILURE;
        }
        hdmi_reg_dcc_man_en_set((td_u32)id, TD_FALSE);
    }
...
static td_s32 ddc_in_prog_wait(hdmi_device_id id, td_u32 timeout)
{
    ...
    while (hdmi_reg_pwd_i2c_in_prog_get((td_u32)id)) {
        osal_msleep(1);
        tmp_time += 1;
        if (tmp_time > timeout) {
            ret = TD_FAILURE;
...
static td_s32 ddc_cmd_issue(hdmi_device_id id, const ddc_cfg *cfg)
{
    ...
    if (cfg->master_mode == DDC_MASTER_MODE_PWD) {
        hdmi_reg_pwd_mst_cmd_set((td_u32)id, DDC_CMD_FIFO_CLR);
        hdmi_reg_pwd_slave_addr_set((td_u32)id, slave_addr);
        hdmi_reg_pwd_slave_seg_set((td_u32)id, segment);
        hdmi_reg_pwd_slave_offset_set((td_u32)id, offset);
        hdmi_reg_pwd_data_out_cnt_set((td_u32)id, data_size);
        osal_udelay(DDC_DEFAULT_DELAY);
        hdmi_reg_pwd_mst_cmd_set((td_u32)id, (td_u8)cfg->issue_mode);
    } else {
        hdmi_err("unknown master_mode=%u\n", (td_u32)cfg->master_mode);
    }
...
static td_s32 ddc_read(hdmi_device_id id, const ddc_cfg *cfg)
{
    ...
    for (data_size = 0; data_size < len; data_size++, data++) {
        ...
        for (i = 0;
             ((i < retry) && (hdmi_reg_pwd_fifo_empty_get((td_u32)id) ||
             (hdmi_reg_pwd_fifo_data_out_get((td_u32)id) == 0))) &&
             ((time_curr - time_start) <= DDC_DEFAULT_RETRY_TIMEOUT_ISSUE);
             i++) {
            /* wait ddc status update after DDC cmd set. */
            osal_msleep(1);
            if (hdmi_reg_ddc_i2c_no_ack_get((td_u32)id) || hdmi_reg_ddc_i2c_bus_low_get((td_u32)id)) {
                hdmi_reg_pwd_mst_cmd_set((td_u32)id, DDC_CMD_MASTER_ABORT);
        ...
        if ((i >= retry) || ((time_curr - time_start) > DDC_DEFAULT_RETRY_TIMEOUT_ISSUE)) {
            if (type != DDC_FUNC_TYPE_SCDC) {
                ...
                hdmi_debug("read fifo retry=%u ms, size=%u, timeout:%u!\n", retry, len, (time_curr - time_start));
            }
            return TD_FAILURE;
        }
        if (data != TD_NULL) {
            *data = hdmi_reg_rdata_pwd_fifo_data_out_get((td_u32)id);
            /*
...
static td_s32 ddc_write(hdmi_device_id hdmi, const ddc_cfg *cfg)
{
    ...
    for (data_size = 0; data_size < len; data_size++, data++) {
        ...
        for (i = 0;
             (((i < retry) && (hdmi_reg_pwd_fifo_data_out_get((td_u32)hdmi) >= DDC_MAX_FIFO_SIZE)) &&
              ((time_curr - time_start) <= DDC_DEFAULT_RETRY_TIMEOUT_ISSUE));
             i++) {
            /* wait ddc status update after DDC cmd set. */
            osal_msleep(1);
            if (hdmi_reg_ddc_i2c_no_ack_get((td_u32)hdmi) || hdmi_reg_ddc_i2c_bus_low_get((td_u32)hdmi)) {
                hdmi_reg_pwd_mst_cmd_set((td_u32)hdmi, DDC_CMD_MASTER_ABORT);
                hdmi_warn("DDC status error!\n");
                return TD_FAILURE;
            }
            time_curr = hdmi_osal_get_time_in_ms();
        ...
        if (i >= retry || ((time_curr - time_start) > DDC_DEFAULT_RETRY_TIMEOUT_ISSUE)) {
            if (type != DDC_FUNC_TYPE_SCDC) {
                ...
                hdmi_debug("write fifo retry=%u ms, size=%u, timeout:%u!\n", retry, len, (time_curr - time_start));
            }
            return TD_FAILURE;
        }

        if (data != TD_NULL) {
            hdmi_reg_pwd_fifo_data_in_set((td_u32)hdmi, *data);
            osal_udelay(1);
        } else {
            hdmi_err("edid &data[%u]=null\n", data_size);
            return TD_FAILURE;
...
static td_s32 edid_get_extern_blk(hdmi_device_id hdmi, const td_u8 ext_blk_num, const td_s32 raw_len, ddc_cfg *cfg)
{
    ...
    if (ret > 0) {
        hdmi_debug("EDID read EXT-block 2~%u success!\n", ext_blk_num + 1);
    } else {
...
td_s32 hal_hdmi_ddc_init(hdmi_device_id hdmi)
{
    ...
    if (tmp->run.init == TD_FALSE) {
        ret = osal_sema_init(&tmp->ddc_wr_mutex, 1);
        if (ret != TD_SUCCESS) {
            hdmi_err("sema init ddc_wr_mutex failed!\n");
            return ret;
        }
...
td_s32 hal_hdmi_ddc_issue(hdmi_device_id hdmi, const ddc_cfg *cfg)
{
    ...
    hdmi_if_null_warn_return(cfg, (td_s32)OT_ERR_NULL_PTR);
    hdmi_if_null_warn_return(tmp, (td_s32)OT_ERR_NULL_PTR);
    hdmi_if_false_warn_return(tmp->run.init, OT_ERR_HDMI_NOT_INIT);
    ...
    if (tmp_elem->un_err.word) {
        ...
        if ((td_bool)hpd.hpd_on == TD_FALSE) {
...
td_s32 hal_hdmi_ddc_edid_raw_get(hdmi_device_id hdmi, td_s32 size, td_u8 *data)
{
    ...
    hdmi_if_null_warn_return(data, (td_s32)OT_ERR_NULL_PTR);
    ...
    hdmi_debug("EDID block 0 read success!\n");
    ...
    ret += hal_hdmi_ddc_issue(hdmi, &cfg);
    if (ret > 0) {
        hdmi_debug("EDID EXT-block 1 read success!\n");
        ext_block_num--;
        if (ext_block_num) {
            /* block 2&3 */
