...
#define TDE_RESET_WAIT_COUNT 100
#define TDE_DERESET_WAIT_COUNT 100
...
typedef struct {
    td_phys_addr_t hf_coef_uaddr;
    td_phys_addr_t vf_coef_uaddr;
    td_void* hf_coef_kaddr;
    td_void* vf_coef_kaddr;

#if (TDE_CAPABILITY & DEFLICKER)
    td_phys_addr_t deflicker_vf_coef_uaddr;
    td_void* deflicker_vf_coef_kaddr;
#endif
} tde_para_table;
...
td_u32 *g_base_vir_addr = TD_NULL;
static td_bool g_use_dtsi = TD_TRUE;

td_u32 *g_tde_wrap_vir_addr = TD_NULL;
td_u32 *g_media_subctl_vir_addr = TD_NULL;

/* Pointer of TDE clock register after mapping */
static td_u32 *g_tde_clock_vir = TD_NULL;
...
static tde_alu_mode g_cbmctrl_alu_mode[(td_s32)TDE_BUTT + 1] = {
 ...
 * Return:        success/fail
 */
td_s32 tde_hal_init(td_u64 base_addr)
{
    /* map address for the register, CNcomment: Register mapping */
    g_use_dtsi = TD_TRUE;
    if (g_base_vir_addr == TD_NULL) {
        g_base_vir_addr = (td_u32 *)tde_reg_map(base_addr, TDE_REG_SIZE);
        if (g_base_vir_addr == TD_NULL) {
            tde_error("g_base_vir_addr init failed!\n");
            return TD_FAILURE;
        }
        g_use_dtsi = TD_FALSE;
    }
    ...
    g_tde_wrap_vir_addr = (td_u32 *)(void *)((char *)g_base_vir_addr + TDE_WRAP_OFFSET);
    g_media_subctl_vir_addr = (td_u32 *)tde_reg_map(MEDIA_SUBCTL_BASEADDR, MEDIA_REG_SIZE);
    if (g_media_subctl_vir_addr == TD_NULL) {
        tde_error("g_media_subctl_vir_addr init failed!\n");
        tde_ptable_release();
        wmemterm();
        tde_reg_unmap(g_base_vir_addr);
        g_base_vir_addr = TD_NULL;
...
td_s32 tde_ptable_open(td_void)
{
    g_para_table.hf_coef_uaddr = wget_user_addr(g_para_table.hf_coef_kaddr);
    g_para_table.vf_coef_uaddr = wget_user_addr(g_para_table.vf_coef_kaddr);
    if ((g_para_table.hf_coef_uaddr == 0) || (g_para_table.vf_coef_uaddr == 0)) {
        tde_error("g_para_table.hf_coef_uaddr is %#llx, g_para_table.vf_coef_uaddr is %#llx",
                  g_para_table.hf_coef_uaddr,
                  g_para_table.vf_coef_uaddr);
        return TD_FAILURE;
    }
#if (TDE_CAPABILITY & DEFLICKER)
    g_para_table.deflicker_vf_coef_uaddr = wget_user_addr(g_para_table.deflicker_vf_coef_kaddr);
    if (g_para_table.deflicker_vf_coef_uaddr == 0) {
        tde_error("g_para_table.deflicker_vf_coef_uaddr is %#llx", g_para_table.deflicker_vf_coef_uaddr);
        return TD_FAILURE;
    }
#endif
    return TD_SUCCESS;
...
td_void tde_ptable_release(td_void)
{
    if (g_para_table.hf_coef_kaddr != TD_NULL) {
        tde_free(g_para_table.hf_coef_kaddr);
        g_para_table.hf_coef_kaddr = TD_NULL;
        g_para_table.hf_coef_uaddr = 0;
    }
    if (g_para_table.vf_coef_kaddr != TD_NULL) {
        tde_free(g_para_table.vf_coef_kaddr);
        g_para_table.vf_coef_kaddr = TD_NULL;
        g_para_table.vf_coef_uaddr = 0;
    }

#if (TDE_CAPABILITY & DEFLICKER)
    if (g_para_table.deflicker_vf_coef_kaddr != TD_NULL) {
        tde_free(g_para_table.deflicker_vf_coef_kaddr);
        g_para_table.deflicker_vf_coef_kaddr = TD_NULL;
        g_para_table.deflicker_vf_coef_uaddr = 0;
    }
#endif
}

td_void tde_ptable_close(td_void)
{
    g_para_table.hf_coef_uaddr = 0;
    g_para_table.vf_coef_uaddr = 0;
#if (TDE_CAPABILITY & DEFLICKER)
...
td_void tde_hal_release(td_void)
{
    ...
    g_tde_wrap_vir_addr = TD_NULL;
    if (g_media_subctl_vir_addr != TD_NULL) {
        tde_reg_unmap(g_media_subctl_vir_addr);
    }
    g_media_subctl_vir_addr = TD_NULL;
...
td_bool tde_hal_ctl_is_idle_safely(td_void)
{
    ...
    for (i = 0; i < TDE_MAX_READ_STATUS_TIME; i++) {
        if (tde_hal_ctl_is_idle() == TD_FALSE) {
            return TD_FALSE;
        }
    }
    return TD_TRUE;
...
td_void tde_hal_ctl_reset(td_void)
{
    ...
    tde_write_reg(g_base_vir_addr, TDE_MISCELLANEOUS, 0x0900647f);
    return;
}

td_bool tde_wrap_soft_reset(td_void)
{
    td_u32 reset_status;
    td_u32 complete_flag;
    td_s32 i;
    tde_write_reg(g_tde_wrap_vir_addr, TDE_SRST_REQ, 0x1);
    reset_status = tde_read_reg(g_tde_wrap_vir_addr, TDE_SRST_REQ);
    if (reset_status != 0x1) {
        return TD_FALSE;
    }

    /* tde wrap wait reset complete */
    for (i = 0; i < TDE_RESET_WAIT_COUNT; i++) {
        complete_flag = tde_read_reg(g_tde_wrap_vir_addr, TDE_SOFTRST_STATE);
        if (complete_flag == 0x1) {
            return TD_TRUE;
        }
    }
    return TD_FALSE;
...
td_bool tde_wrap_soft_dereset(td_void)
{
    td_u32 reset_status;
    td_u32 complete_flag;
    td_s32 i;
    /* tde wrap cancel reset */
    tde_write_reg(g_tde_wrap_vir_addr, TDE_SRST_REQ, 0x0);
    reset_status = tde_read_reg(g_tde_wrap_vir_addr, TDE_SRST_REQ);
    if (reset_status != 0x0) {
        return TD_FALSE;
    }

    /* tde wrap wait cancel reset complete */
    for (i = 0; i < TDE_DERESET_WAIT_COUNT; i++) {
        complete_flag = tde_read_reg(g_tde_wrap_vir_addr, TDE_SOFTRST_STATE);
        if (complete_flag == 0x0) {
            return TD_TRUE;
        }
    }
    tde_error("tde wrap cancel reset does not complete\n");
    return TD_FALSE;
}

td_bool tde_hal_poweron_action(td_void)
{
    td_bool complete_flag;

    /* media subctl enable clock */
    complete_flag = tde_hal_media_subctl_enable();
    if (complete_flag != TD_TRUE) {
        return TD_FALSE;
    }

    /* media subctl cancel reset */
    complete_flag = tde_hal_media_subctl_cancel_reset();
    if (complete_flag != TD_TRUE) {
        return TD_FALSE;
    }

    /* tde wrap enable clock */
    complete_flag = tde_hal_wrap_enable_clock();
    if (complete_flag != TD_TRUE) {
        return TD_FALSE;
    }

    /* tde wrap reset */
    complete_flag = tde_wrap_soft_reset();
    if (complete_flag != TD_TRUE) {
        return TD_FALSE;
    }

    /* tde wrap cancel reset */
    complete_flag = tde_wrap_soft_dereset();
    if (complete_flag != TD_TRUE) {
        return TD_FALSE;
    }
    return TD_TRUE;
...
td_bool tde_hal_poweroff_action(td_void)
{
    td_bool complete_flag = TD_TRUE;
    ...
    /* media subctl reset */
    complete_flag = tde_hal_media_subctl_reset();
    if (complete_flag != TD_TRUE) {
        return TD_FALSE;
    }

    /* media subctl disable clock */
    complete_flag = tde_hal_media_subctl_disable();
    if (complete_flag != TD_TRUE) {
        return TD_FALSE;
    }
    return TD_TRUE;
...
td_bool tde_hal_media_subctl_reset(td_void)
{
    td_u32 reset_status;

    tde_write_reg(g_media_subctl_vir_addr, SC_TDE_RESET_REQ, 0x1);
    reset_status = tde_read_reg(g_media_subctl_vir_addr, SC_TDE_RESET_ST);
    if (reset_status != 0x1U) {
        return TD_FALSE;
    }
    return TD_TRUE;
}

td_bool tde_hal_media_subctl_disable(td_void)
{
    td_u32 clock_status;

    tde_write_reg(g_media_subctl_vir_addr, SC_TDE_ICG_DIS, 0x1);
    clock_status = tde_read_reg(g_media_subctl_vir_addr, SC_TDE_ICG_ST);
    if (clock_status != 0x0U) {
        return TD_FALSE;
    }
    return TD_TRUE;
}

td_bool tde_hal_media_subctl_enable(td_void)
{
    td_u32 clock_status;

    tde_write_reg(g_media_subctl_vir_addr, SC_TDE_ICG_EN, 0x1);
    clock_status = tde_read_reg(g_media_subctl_vir_addr, SC_TDE_ICG_ST);
    if (clock_status != 0x1U) {
        return TD_FALSE;
    }
    return TD_TRUE;
}

td_bool tde_hal_media_subctl_cancel_reset(td_void)
{
    td_u32 reset_status;

    tde_write_reg(g_media_subctl_vir_addr, SC_TDE_RESET_DREQ, 0x1);
    reset_status = tde_read_reg(g_media_subctl_vir_addr, SC_TDE_RESET_ST);
    if (reset_status != 0x0U) {
        return TD_FALSE;
    }
    return TD_TRUE;
}

td_bool tde_hal_wrap_enable_clock(td_void)
{
    td_u32 clock_status;

    tde_write_reg(g_tde_wrap_vir_addr, TDE_CKEN, 0x1);
    clock_status = tde_read_reg(g_tde_wrap_vir_addr, TDE_CKEN);
    if (clock_status != 0x1U) {
        return TD_FALSE;
    }
    return TD_TRUE;
...
td_void tde_hal_set_clock(td_bool enable)
{
    ...
    if (g_tde_wrap_vir_addr == TD_NULL) {
        return;
    }

    if (enable) {
        tde_hal_poweron_action();
    } else {
        tde_hal_poweroff_action();
    }
}
...
td_s32 tde_hal_node_init_nd(tde_hw_node **hw_node)
{
    ...
    *hw_node = (tde_hw_node *)(void *)((td_u8 *)buf + TDE_NODE_HEAD_BYTE);
...
td_void tde_hal_next_node_addr(td_void *buf, td_phys_addr_t phy_addr)
{
    ...
    tde_hw_node *hw_node = (tde_hw_node *)buf;
    hw_node->tde_pnext_low.bits.p_next_low = (td_u32)get_low_addr(phy_addr);
    hw_node->tde_pnext_hi.bits.p_next_hi = (td_u32)get_high_addr(phy_addr);
    return;
}
...
td_void tde_hal_node_set_src1(tde_hw_node *hw_node, const tde_surface_msg *drv_surface)
{
    ...
    if (bpp == 0xffffffff) {
        tde_error("drv_surface->color_format is invalid!\n");
        return;
    }
    ...
    hw_node->src1_ch0_addr_low.bits.src1_ch0_addr_low = (td_u32)get_low_addr(phy_addr);
    hw_node->src1_ch0_addr_high.bits.src1_ch0_addr_high = (td_u32)get_high_addr(phy_addr);

    if (drv_surface->color_format <= TDE_DRV_COLOR_FMT_RGB565) {
        hw_node->src1_ctrl.bits.src1_argb_order = (td_u32)drv_surface->rgb_order;
    }
    /*
    ...
    td_phys_addr_t *cb_crphy_addr)
{
    ...
    if (drv_surface->color_format >= TDE_DRV_COLOR_FMT_YCBCR400MBP) {
        *phy_addr = drv_surface->phys_addr + (td_u64)(drv_surface->ypos) * (td_u64)(drv_surface->pitch) +
        ...
        hw_node->src2_ch0_addr_low.bits.src2_ch0_addr_low = (td_u32)get_low_addr(*phy_addr);
        hw_node->src2_ch0_addr_high.bits.src2_ch0_addr_high = (td_u32)get_high_addr(*phy_addr);
        ...
        hw_node->src2_ch1_addr_low.bits.src2_ch1_addr_low = (td_u32)get_low_addr(*cb_crphy_addr);
        hw_node->src2_ch1_addr_high.bits.src2_ch1_addr_high = (td_u32)get_high_addr(*cb_crphy_addr);
        ...
        *phy_addr = drv_surface->phys_addr + (td_u64)(drv_surface->ypos) *
        ...
        if (drv_surface->color_format <= TDE_DRV_COLOR_FMT_RGB565) {
            hw_node->src2_ctrl.bits.src2_argb_order = (td_u32)drv_surface->rgb_order;
        }
    }
}
    ...
    tde_drv_outalpha_from alpha_from)
{
    ...
    hw_node->des_ctrl.bits.des_fmt = (td_u32)drv_surface->color_format;
...
td_void tde_hal_node_set_tqt(tde_hw_node *hw_node, tde_surface_msg *drv_surface, tde_drv_outalpha_from alpha_from)
{
    ...
    if ((drv_surface->color_format <= TDE_DRV_COLOR_FMT_RGB565) ||
        ...
        hw_node->des_ctrl.bits.des_argb_order = (td_u32)drv_surface->rgb_order;
        if (drv_surface->color_format == TDE_DRV_COLOR_FMT_AYCBCR8888) {
            drv_surface->color_format = TDE_DRV_COLOR_FMT_ARGB8888;
            hw_node->des_ctrl.bits.des_argb_order = 0x17;
        }
    ...
    hw_node->des_ch0_addr_low.bits.des_ch0_addr_low = (td_u32)get_low_addr(phy_addr);
    hw_node->des_ch0_addr_high.bits.des_ch0_addr_high = (td_u32)get_high_addr(phy_addr);

    cb_crphy_addr = drv_surface->cbcr_phys_addr + (td_u64)(drv_surface->ypos) * (td_u64)(drv_surface->pitch) +
                    (((td_u64)(drv_surface->xpos) * (td_u64)(bpp)) >> 3); /* 3 bpp narrow 8 */
    hw_node->des_ch1_addr_low.bits.des_ch1_addr_low = (td_u32)get_low_addr(cb_crphy_addr);
    hw_node->des_ch1_addr_high.bits.des_ch1_addr_hi = (td_u32)get_high_addr(cb_crphy_addr);
...
td_void tde_hal_node_set_corner_rect(tde_hw_node *hw_node, const tde_corner_rect_info *corner_hal_info)
{
    ...
    if (corner_hal_info == TD_NULL) {
        tde_error("corner_hal_info is null\n");
        return;
    }
...
{
    ot_unused(length);
    if ((hw_node->des_ctrl.bits.des_fmt == (td_s32)TDE_DRV_COLOR_FMT_PKGVYUY) ||
        (hw_node->des_ctrl.bits.des_fmt == (td_s32)TDE_DRV_COLOR_FMT_YCBCR888) ||
        (hw_node->des_ctrl.bits.des_fmt == (td_s32)TDE_DRV_COLOR_FMT_AYCBCR8888) ||
        (hw_node->des_ctrl.bits.des_fmt == (td_s32)TDE_DRV_COLOR_FMT_YCBCR422)) {
    ...
    hw_node->tde_drd_line0_st.bits.line0_st_x = (td_u32)line_hal_info[0].start_x;
    hw_node->tde_drd_line0_st.bits.line0_st_y = (td_u32)line_hal_info[0].start_y;
    hw_node->tde_drd_line0_ed.bits.line0_ed_x = (td_u32)line_hal_info[0].end_x;
    hw_node->tde_drd_line0_ed.bits.line0_ed_y = (td_u32)line_hal_info[0].end_y;

    hw_node->tde_drd_line1_st.bits.line1_st_x = (td_u32)line_hal_info[1].start_x;
    hw_node->tde_drd_line1_st.bits.line1_st_y = (td_u32)line_hal_info[1].start_y;
    hw_node->tde_drd_line1_ed.bits.line1_ed_x = (td_u32)line_hal_info[1].end_x;
    hw_node->tde_drd_line1_ed.bits.line1_ed_y = (td_u32)line_hal_info[1].end_y;

    hw_node->tde_drd_line2_st.bits.line2_st_x = (td_u32)line_hal_info[2].start_x; /* 2 line2 */
    hw_node->tde_drd_line2_st.bits.line2_st_y = (td_u32)line_hal_info[2].start_y; /* 2 line2 */
    hw_node->tde_drd_line2_ed.bits.line2_ed_x = (td_u32)line_hal_info[2].end_x; /* 2 line2 */
    hw_node->tde_drd_line2_ed.bits.line2_ed_y = (td_u32)line_hal_info[2].end_y; /* 2 line2 */

    hw_node->tde_drd_line3_st.bits.line3_st_x = (td_u32)line_hal_info[3].start_x; /* 3 line3 */
    hw_node->tde_drd_line3_st.bits.line3_st_y = (td_u32)line_hal_info[3].start_y; /* 3 line3 */
    hw_node->tde_drd_line3_ed.bits.line3_ed_x = (td_u32)line_hal_info[3].end_x; /* 3 line3 */
    hw_node->tde_drd_line3_ed.bits.line3_ed_y = (td_u32)line_hal_info[3].end_y; /* 3 line3 */
    return;
}
#endif
...
static td_u32 tde_node_clip(td_u32 a)
{
    return (a >= 0U) ? a : 0U;
}
#endif
...
static td_void tde_hal_node_set_cmp_cfg(tde_hw_node *hw_node, td_u32 max_mb_qp, td_u32 bit_depth_rgb)
{
    ...
    if (max_mb_qp > (bit_depth_rgb / 2U)) { /* 2 alg data */
        qp_rge_reg0 = (tde_node_clip(max_mb_qp - 4U) << 28) | (0U << 24) | /* 4 28 24 alg data */
                      (tde_node_clip(max_mb_qp - 5U) << 20) | (0U << 16) | /* 5 20 16 alg data */
                      (tde_node_clip(max_mb_qp - 6U) << 12) | (0U << 8) | /* 6 12 8 alg data */
                      (tde_node_clip(max_mb_qp - 7U) << 4) | 0U; /* 7 4 alg data */
        qp_rge_reg1 = (tde_node_clip(max_mb_qp - 3U) << 28) | (0U << 24) | /* 3 28 24 alg data */
                      (tde_node_clip(max_mb_qp - 4U) << 20) | (0U << 16) | /* 4 20 16 alg data */
                      (tde_node_clip(max_mb_qp - 4U) << 12) | (0U << 8) | /* 4 12 8 alg data */
                      (tde_node_clip(max_mb_qp - 4U) << 4) | 0U; /* 4 alg data */
        /* 28 4 24 20 16 alg data */
        qp_rge_reg2 = (max_mb_qp << 28U) | (4U << 24) | (tde_node_clip(max_mb_qp - 1U) << 20) | (4U << 16) |
            /* 2 12 8 4 alg data */
            (tde_node_clip(max_mb_qp - 2U) << 12) | (2U << 8) | (tde_node_clip(max_mb_qp - 2U) << 4) | 0U;
    } else {
        qp_rge_reg0 = (tde_node_clip(max_mb_qp - 3U) << 28) | (0U << 24) | /* 3 28 24 alg data */
                      (tde_node_clip(max_mb_qp - 3U) << 20) | (0U << 16) | /* 3 20 16 alg data */
                      (tde_node_clip(max_mb_qp - 3U) << 12) | (0U << 8) | /* 3 12 8 alg data */
                      (tde_node_clip(max_mb_qp - 3U) << 4) | 0U; /* 3 4 alg data */
        qp_rge_reg1 = (tde_node_clip(max_mb_qp - 2U) << 28) | (1U << 24) | /* 2 28 24 alg data */
                      (tde_node_clip(max_mb_qp - 2U) << 20) | (1U << 16) | /* 2 20 16 alg data */
                      (tde_node_clip(max_mb_qp - 2U) << 12) | (1U << 8) | /* 2 12 8 alg data */
                      (tde_node_clip(max_mb_qp - 3U) << 4) | 1U; /* 3 4 alg data */
        qp_rge_reg2 = (max_mb_qp << 28) | (2U << 24) | /* 28 2 24 alg data */
                        (tde_node_clip(max_mb_qp - 1U) << 20) | /* 20 alg data */
                        (2U << 16) | (tde_node_clip(max_mb_qp - 1U) << 12) | /* 2 16 12 alg data */
                        (2U << 8) | /* 2 8 alg data */
                        (tde_node_clip(max_mb_qp - 1U) << 4) | 2U; /* 4 2 alg data */
    }
    ...
    hw_node->tde_line_osd_cmp_rc_cfg13.bits.bits_offset_reg2 =
        ((-4 & 0xff) << 24) | ((-3 & 0xff) << 16) | /* -4 0xff 24 -3 16 alg data */
...
static td_void tde_hal_node_set_cmp(tde_hw_node *hw_node, td_u32 width)
{
    ...
    if (osd_mode == 2) { /* 2 alg data */
        /* 32 3 alg data */
        max_mb_qp = ((bit_depth_rgb) - ((((budget_mb_bits - (bit_depth_a * 32)) - max_head_bits_raw) - 1)
            / (32 * 3))) + 1; /* 32 3 alg data */
    } else {
        /* 5 2 alg data */
...
td_void tde_hal_node_set_compress(tde_hw_node *hw_node)
{
    ...
    hw_node->tde_line_osd_cmp_glb_info.bits.osd_mode =
        (hw_node->des_ctrl.bits.des_fmt == (td_s32)TDE_DRV_COLOR_FMT_RGB888) ?
        0 : ((hw_node->des_ctrl.bits.des_fmt == (td_s32)TDE_DRV_COLOR_FMT_ARGB8888) ?
        1 : ((hw_node->des_ctrl.bits.des_fmt == (td_s32)TDE_DRV_COLOR_FMT_ARGB1555) ?
...
td_void tde_hal_node_set_rotate(tde_hw_node *hw_node, drv_tde_rotate_angle rotate_angle)
{
    ...
    if ((td_s32)TDE_DRV_COLOR_FMT_YCBCR422 == hw_node->src2_ctrl.bits.src2_fmt) {
        hw_node->src2_rtt_ctrl.bits.s2_rtt_fmt = 0;
    }
    if ((hw_node->src2_ctrl.bits.src2_fmt == (td_s32)TDE_DRV_COLOR_FMT_RGB888) ||
        ((td_s32)TDE_DRV_COLOR_FMT_YCBCR888 == hw_node->src2_ctrl.bits.src2_fmt)) {
        hw_node->src2_rtt_ctrl.bits.s2_rtt_fmt = 1;
    }
    if ((hw_node->src2_ctrl.bits.src2_fmt == (td_s32)TDE_DRV_COLOR_FMT_ARGB4444) ||
        (hw_node->src2_ctrl.bits.src2_fmt == (td_s32)TDE_DRV_COLOR_FMT_ARGB1555)) {
        hw_node->src2_rtt_ctrl.bits.s2_rtt_fmt = 2; /* 2 ARGB4444 ARGB1555 */
    }
    if (hw_node->src2_ctrl.bits.src2_fmt == (td_s32)TDE_DRV_COLOR_FMT_ARGB8888) {
                                ...
                                const tde_color_fill *color_fill, td_u32 capability)
{
    ...
    if (mode == TDE_QUIKE_COPY) {
        if (tde_hal_is_operation_support(alu, capability, QUICKCOPY) == TD_FALSE) {
    ...
    if (mode == TDE_NORM_FILL_2OPT) {
        ...
        hw_node->cbmctrl.bits.alu_mode = (td_u32)g_cbmctrl_alu_mode[alu];
    }
    return TD_SUCCESS;
}
...
static td_s32 tde_hal_node_opt(tde_hw_node *hw_node, tde_base_opt_mode mode, tde_alu_mode alu, td_u32 capability)
{
    ...
    if (mode == TDE_SINGLE_SRC_PATTERN_FILL_OPT) {
        if (tde_hal_is_operation_support(alu, capability, PATTERFILL) == TD_FALSE) {
            tde_error("It does not support PatternFill\n");
            return DRV_ERR_TDE_UNSUPPORTED_OPERATION;
        }
    ...
    const tde_color_fill *color_fill, td_u32 capability)
{
    ...
    if (tde_hal_is_operation_support(alu, capability, QUICKFILL) == TD_FALSE) {
        tde_error("It does not support QuickFill\n");
        return DRV_ERR_TDE_UNSUPPORTED_OPERATION;
    }
...
static td_s32 tde_hal_node_operation_support(tde_alu_mode alu, td_u32 capability)
{
    ...
    if (tde_hal_is_operation_support(alu, capability, MASKROP) == TD_FALSE) {
        tde_error("It does not support MaskRop\n");
        return DRV_ERR_TDE_UNSUPPORTED_OPERATION;
    }
#endif
#if (TDE_CAPABILITY & MASKBLEND)
    if (tde_hal_is_operation_support(alu, capability, MASKBLEND) == TD_FALSE) {
        tde_error("It does not support MaskBlend\n");
        return DRV_ERR_TDE_UNSUPPORTED_OPERATION;
    }
#endif
    return TD_SUCCESS;
...
static td_bool tde_hal_is_operation_support(tde_alu_mode alu_mode, td_u32 capability, td_u32 oper_mode)
{
    if (!(MASKBLEND & oper_mode) && !(MASKROP & oper_mode)) {
        return (capability & oper_mode) ? TD_TRUE : TD_FALSE;
    }

#if (TDE_CAPABILITY & MASKBLEND)
    if (MASKBLEND & oper_mode) {
        return !((alu_mode == TDE_ALU_MASK_BLEND) && (!(capability & oper_mode))) ? TD_TRUE : TD_FALSE;
    }
#endif

#if (TDE_CAPABILITY & ROP)
    if (MASKROP & oper_mode) {
        return !(((alu_mode == TDE_ALU_MASK_ROP1) || (alu_mode == TDE_ALU_MASK_ROP2)) && (!(capability & oper_mode)))
               ? TD_TRUE : TD_FALSE;
    }
#endif
...
td_s32 tde_hal_node_set_rop(tde_hw_node *hw_node, drv_tde_rop_mode rgb_rop, drv_tde_rop_mode alpha_rop)
{
    ...
    if ((rgb_rop >= DRV_TDE_ROP_MAX) || (alpha_rop >= DRV_TDE_ROP_MAX)) {
        tde_error("rgb_rop and alpha_rop invalid %s(line=%d)", __FUNCTION__, __LINE__);
        return DRV_ERR_TDE_INVALID_PARA;
    }
...
static td_void tde_hal_node_blend_mode(tde_hw_node *hw_node, const drv_tde_blend_opt *blend_opt)
{
    if (blend_opt->blend_cmd == DRV_TDE_BLEND_CMD_NONE) {
        ...
        drv_tde_node_blend_mode(hw_node, (td_u32)blend_opt->src1_blend_mode, (td_u32)blend_opt->src2_blend_mode);
    }
}
...
td_s32 tde_hal_node_set_blend(tde_hw_node *hw_node, const drv_tde_blend_opt *blend_opt)
{
    ...
    hw_node->src1_cbmpara.bits.s1_premulten = (td_u32)blend_opt->src1_alpha_premulti;
    hw_node->src2_cbmpara.bits.s2_premulten = (td_u32)blend_opt->src2_alpha_premulti;
    hw_node->src2_cbmpara.bits.s2_palphaen = (td_u32)blend_opt->pixel_alpha_en;
    hw_node->src2_cbmpara.bits.s2_galphaen = (td_u32)blend_opt->global_alpha_en;
...
td_s32 tde_hal_node_set_clut_opt(tde_hw_node *hw_node, const tde_clut_cmd *clut_cmd, td_bool reload)
{
    ...
    if (clut_cmd->phy_clut_addr) {
        hw_node->tde_clut_addr_low = (td_u32)get_low_addr(clut_cmd->phy_clut_addr);
        hw_node->tde_clut_addr_high = (td_u32)get_high_addr(clut_cmd->phy_clut_addr);
    }
...
static td_s32 tde_hal_calc_src2_filter_opt_check(const tde_hw_node *node, const tde_rect_opt *rect_opt)
{
    ot_unused(node);
    if (rect_opt == TD_NULL) {
        tde_error("rect_opt ptr is null!\n");
        return DRV_ERR_TDE_NULL_PTR;
    }

    if (rect_opt->in_rect == TD_NULL || rect_opt->out_rect == TD_NULL) {
        tde_error("rect_opt->in_rect ptr is %d rect_opt->out_rect is %d\n",
                  rect_opt->in_rect == TD_NULL,
                  rect_opt->out_rect == TD_NULL);
        return DRV_ERR_TDE_NULL_PTR;
    }

    if (rect_opt->in_fmt > DRV_TDE_COLOR_FMT_MAX) {
        return DRV_ERR_TDE_INVALID_PARA;
    }
    return TD_SUCCESS;
}
...
static td_s32 tde_hal_set_zme_in_limit(tde_hw_node *node, const tde_rect_opt *rect_opt)
{
    ...
    if ((rect_opt->out_rect->width * 16) <= rect_opt->in_rect->width) { /* 16 2^4 alg data */
        ...
        node->src2_hpzme_size.bits.src2_hpzme_width =
            rect_opt->in_rect->width / (node->src2_hpzme.bits.src2_hpzme_mode + 1U) +
            ((rect_opt->in_rect->width % (node->src2_hpzme.bits.src2_hpzme_mode + 1U)) ? 1 : 0);
        node->src2_hsp.bits.hratio = (rect_opt->out_rect->width <= 1) ? 0 :
            ((td_u32)osal_div_u64(((td_u64)(node->src2_hpzme_size.bits.src2_hpzme_width) << TDE_HAL_HSTEP_FLOATLEN),
    ...
    if ((rect_opt->out_rect->height * 16) <= rect_opt->in_rect->height) { /* 16 2^4 alg data */
        ...
        node->src2_zmeireso.bits.ih = rect_opt->in_rect->height / (td_u32)ver_scale_coeff - 1U;
        node->src2_imgsize.bits.src2_height = rect_opt->in_rect->height / (td_u32)ver_scale_coeff - 1U;
        node->src2_ch0_stride.bits.src2_ch0_stride = node->src2_ch0_stride.bits.src2_ch0_stride *
            (td_u32)ver_scale_coeff;
        if (rect_opt->out_rect->height == 0) {
            return DRV_ERR_TDE_INVALID_PARA;
        }
        node->src2_vsr.bits.vratio = (td_u32)osal_div_u64(((td_u64)(node->src2_imgsize.bits.src2_height) <<
        ...
        node->src2_vsr.bits.vratio = (rect_opt->out_rect->height <= 1) ? 0 :
            ((td_u32)osal_div_u64(((td_u64)(rect_opt->in_rect->height) << TDE_FLOAT_BITLEN),
    ...
    td_bool defilicker)
{
    ...
    if (node->src2_hsp.bits.hratio != NO_HSCALE_STEP) {
        ...
        if (node->src2_vsp.bits.zme_in_fmt == (td_u32)TDE_ZME_FMT_ARGB) {
        ...
        ch_coef_addr = g_para_table.hf_coef_uaddr + tde_hal_get_resize_para_htable(node->src2_hsp.bits.hratio) *
                       TDE_PARA_HTABLE_SIZE;

        node->tde_src2_zme_chaddr_low = (td_u32)get_low_addr(ch_coef_addr);
        node->tde_src2_zme_chaddr_high = (td_u32)get_high_addr(ch_coef_addr);
    ...
    if ((node->src2_vsr.bits.vratio != NO_VSCALE_STEP) && (defilicker == TD_FALSE)) {
        ...
        cv_coef_addr = g_para_table.vf_coef_uaddr + tde_hal_get_resize_para_vtable(node->src2_vsr.bits.vratio) *
                       TDE_PARA_VTABLE_SIZE;
        node->tde_src2_zme_cvaddr_low = (td_u32)get_low_addr(cv_coef_addr);
        node->tde_src2_zme_cvaddr_high = (td_u32)get_high_addr(cv_coef_addr);
...
static td_void tde_hal_set_defilicker(tde_hw_node *node)
{
    ...
    cv_deflicker_coef_addr = g_para_table.deflicker_vf_coef_uaddr +
                             tde_hal_get_resize_para_vtable(node->src2_vsr.bits.vratio) * TDE_PARA_VTABLE_SIZE;

    node->tde_src2_zme_cvaddr_low = (td_u32)get_low_addr(cv_deflicker_coef_addr);
    node->tde_src2_zme_cvaddr_high = (td_u32)get_high_addr(cv_deflicker_coef_addr);
                                    ...
                                    drv_tde_deflicker_mode filter_mode)
{
    ...
    if (in_drv_fmt == TDE_DRV_COLOR_FMT_MAX) {
        tde_error("format is invalid!\n");
        return DRV_ERR_TDE_INVALID_PARA;
    }
    node->src2_vsp.bits.zme_in_fmt = tde_hal_get_zme_in_fmt(in_drv_fmt);

    node->src2_vsp.bits.zme_out_fmt = (node->src2_vsp.bits.zme_in_fmt == (td_u32)TDE_ZME_FMT_ARGB) ? TDE_ZME_FMT_ARGB :
                                       TDE_ZME_FMT_YUV444;
    ...
    node->src2_hsp.bits.hratio = (rect_opt->out_rect->width <= 1) ? 0 :
        ((td_u32)osal_div_u64(((td_u64)(rect_opt->in_rect->width) << TDE_HAL_HSTEP_FLOATLEN),
        (rect_opt->out_rect->width)));
    ...
    node->src2_zmeireso.bits.iw = node->src2_hpzme_size.bits.src2_hpzme_width - 1U;

    node->src2_hpzme_size.bits.src2_hpzme_width = node->src2_hpzme_size.bits.src2_hpzme_width - 1U;
...
static td_void *tde_hal_make_hf_coef(td_void)
{
    td_u32 i;
    td_void *hf_coef = TD_NULL;

    hf_coef = tde_malloc(TDE_PARA_HTABLE_SIZE * TDE_PARA_HTABLE_NUM);
    if (hf_coef == TD_NULL) {
    ...
    for (i = 0; i < TDE_PARA_HTABLE_NUM; i++) {
         /* 4 size value */
        if (memcpy_s(hf_coef + i * (td_u32)(TDE_PARA_HTABLE_SIZE / 4), TDE_PARA_HTABLE_SIZE, g_tde_6x32_coef +
    ...
    g_para_table.hf_coef_kaddr = hf_coef;
    g_para_table.hf_coef_uaddr = 0;
    return hf_coef;
}

static td_void *tde_hal_make_vf_coef(td_void)
{
    td_u32 i;
    td_void *vf_coef = TD_NULL;

    vf_coef = tde_malloc(TDE_PARA_VTABLE_SIZE * TDE_PARA_VTABLE_NUM);
    if (vf_coef == TD_NULL) {
    ...
    for (i = 0; i < TDE_PARA_VTABLE_NUM; i++) {
        if (memcpy_s(vf_coef + i * (td_u32)(TDE_PARA_VTABLE_SIZE / 4), /* 4 size value */
            TDE_PARA_VTABLE_SIZE, g_org_vf_coef +
    ...
    g_para_table.vf_coef_kaddr = vf_coef;
    g_para_table.vf_coef_uaddr = 0;
    return vf_coef;
}

#if (TDE_CAPABILITY & DEFLICKER)
static td_void *tde_hal_make_deflicker_vf_coef(td_void)
{
    td_u32 i;
    td_void *deflicker_vf_coef = TD_NULL;

    deflicker_vf_coef = tde_malloc(TDE_PARA_VTABLE_SIZE * TDE_PARA_VTABLE_NUM);
    ...
    for (i = 0; i < TDE_PARA_VTABLE_NUM; i++) {
        /* 4 alg data */
        if (memcpy_s(deflicker_vf_coef + i * (td_u32)(TDE_PARA_VTABLE_SIZE / 4),
            TDE_PARA_VTABLE_SIZE, g_deflicker_vf_coef +
            i * (TDE_PARA_VTABLE_ORG_SIZE), (TDE_PARA_VTABLE_ORG_SIZE) * 4) != EOK) { /* 4 alg data */
            tde_error("secure function failure\n");
            tde_free(deflicker_vf_coef);
            deflicker_vf_coef = TD_NULL;
            return TD_NULL;
        }
    }

    g_para_table.deflicker_vf_coef_kaddr = deflicker_vf_coef;
    g_para_table.deflicker_vf_coef_uaddr = 0;
 ...
 */
td_phys_addr_t tde_hal_cur_node()
{
...
td_void tde_smmu_init(td_u32 sid, td_u32 ssid)
{
    td_u32 awuser_0 = 0;
    awuser_0 += ssid << 16U; /* 16 high bits store ssid */
    awuser_0 += sid;
    tde_write_reg(g_tde_wrap_vir_addr, SC_AWUSER_0_0, awuser_0); // write
    tde_write_reg(g_tde_wrap_vir_addr, SC_ARUSER_0_0, awuser_0); // read

    // enable smmu, maybe have been enable by svm
    // config smmu high addr 34-52位
    tde_write_reg(g_tde_wrap_vir_addr, SC_AWADDR_EXT_0, SC_AXADDR_EXT_VAULE);
    tde_write_reg(g_tde_wrap_vir_addr, SC_ARADDR_EXT_0, SC_AXADDR_EXT_VAULE);
}
...
static td_void tde_hal_node_print_31d(const osal_proc_entry_t *p, const td_u32 *cur_node)
{
    td_s32 i;
    /* 22 SRC1_CTRL 0x0 */
    for (i = 0; i < 22; i++) {
...
const osal_proc_entry_t *tde_hal_node_print_info(const osal_proc_entry_t *p, const td_u32 *cur_node)
{
