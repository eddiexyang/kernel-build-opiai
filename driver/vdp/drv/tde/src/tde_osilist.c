...
td_s32 tde_osi_list_init(td_void)
{
    ...
    ret = osal_spin_lock_init(&g_tde_osi_job_list->lock);
    if (ret != 0) {
        tde_free(g_tde_osi_job_list);
        destroy_handle();
        osal_wait_destroy(&g_tde_block_job_wq);
        tde_error("Initialize g_tde_osi_job_list->lock failed!\n");
        return TD_FAILURE;
    }
    ret = osal_spin_lock_init(&g_tde_buf_lock);
    if (ret != 0) {
        osal_spin_lock_destroy(&g_tde_osi_job_list->lock);
        tde_free(g_tde_osi_job_list);
        destroy_handle();
        osal_wait_destroy(&g_tde_block_job_wq);
        tde_error("Initialize g_tde_buf_lock failed!\n");
        return TD_FAILURE;
    }
...
static td_void tde_osi_list_flush_node(const tde_sw_node *hw_node)
{
    ...
    if (p_node_buf_info->vir_buf != TD_NULL) {
        cur_node_phy = wget_user_addr(p_node_buf_info->vir_buf);
...
static td_void tde_osi_list_destroy_job(tde_sw_job *job)
{
    ...
    if (tde_query_handle(job->handle, &handle_mgr) == HI_FALSE) {
        return;
    }
...
td_s32 tde_osi_list_cancel_job(td_s32 handle)
{
    ...
    if (tde_query_handle(handle, &handle_mgr) == HI_FALSE) {
...
static td_void tde_set_job_wake_notify(tde_sw_job *job, tde_notify_mode *noti_type, td_u32 *time_out, td_bool *asynflag)
{
    if (*noti_type != TDE_JOB_WAKE_NOTIFY) {
        if ((!osal_in_interrupt()) && (wgetfreenum() < 5)) { /* 5 num */
            ...
            *asynflag = TD_FALSE; /* By sign non-blocking way to block the way */
        }
    }
...
td_s32 tde_osi_list_submit_job(const drv_tde_end_job_cmd *end_job, drv_tde_func_callback func_compl_cb,
                               td_void *func_para, tde_notify_mode noti_type, td_bool is_sync)
{
    tde_sw_job *job = TD_NULL;
    td_s32 wait_ret, ret;
    td_s32 handle;
    ...
    time_out = time_out > TDE_MAX_WAIT_TIMEOUT ? TDE_MAX_WAIT_TIMEOUT : time_out; // 2s
 ...
 * Modification : Created function
 */
td_void tde_osi_list_node_comp()
{
    tde_handle_mgr *handle_mgr = TD_NULL;
    ...
    finish_handle = (td_u32 *)wget_kernel_addr(running_sw_node_addr - TDE_NODE_HEAD_BYTE + 8); /* 8 alg data */
    if (finish_handle == TD_NULL) {
        tde_spin_unlock(&g_tde_osi_job_list->lock, lockflags);
        return;
    }

    finished_handle = (td_s32)*finish_handle;
    if (tde_query_handle(finished_handle, &handle_mgr) == HI_FALSE) {
        tde_spin_unlock(&g_tde_osi_job_list->lock, lockflags);
        return;
    }
    if (work == HI_FALSE) {
...
{
    if (buf_num != 0) {
        tde_osi_list_do_free_phy_buff(buf_num);
    }
    return;
}

td_void tde_list_free_pending_job(const td_void *private_data)
{
    ...
    while (handle != tde_handle_list) {
        ...
        if ((job->has_submitted == HI_FALSE) && (private_data == job->private_data)) {
