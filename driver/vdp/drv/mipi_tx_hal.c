...
/* no ddr reg */
static void hal_tx_write_reg(unsigned *address, unsigned value)
{
    if (address == TD_NULL) {
        return;
    }
...
static unsigned int hal_tx_read_reg(const unsigned int *address)
{
    if (address == TD_NULL) {
        return 0U;
    }
 ...
 * addr:寄存器地址
 * val: 写寄存器值
 * bw: 写多少位寄存器，如32bit全改则为32
 * bs: 从哪位开始写
 */
static void hal_mipi_set_reg(unsigned int *addr, unsigned int val, uint8_t bw, uint8_t bs)
{
    unsigned long temporary = (1UL << bw) - 1UL;
    unsigned int mask = (unsigned int)temporary;
    unsigned int tmp = 0;

    tmp = hal_tx_read_reg(addr);
    tmp &= ~(mask << bs);

    hal_tx_write_reg(addr, tmp | ((val & mask) << bs));
}
...
#ifdef MIPI_TX_DEBUG
    (void)osal_printk("\n==========phy pll info=======\n");
    (void)osal_printk("pll_set0(0x14): 0x%x\n", pll_set0);
    (void)osal_printk("pll_set1(0x15): 0x%x\n", pll_set1);
    (void)osal_printk("pll_set2(0x16): 0x%x\n", pll_set2);
    (void)osal_printk("pll_set3(0x17): 0x%x\n", pll_set3);
    (void)osal_printk("pll_set4(0x1e): 0x%x\n", pll_set4);
    (void)osal_printk("=========================\n");
#endif
}
...
static unsigned char mipi_tx_drv_get_phy_pll_set_0x63(unsigned int phy_data_rate, unsigned char pll_set_0x65)
{
    ...
    /*
     * fbdiv = phy data rate/ref_clk(48MHz)*(2^prediv)*(2^postdiv)
     * prediv与postdiv当前均默认为1
     */
    set_0x63 = mipi_tx_ceil(phy_data_rate, MIPI_TX_REF_CLK) * 0x2 * 0x2;
...
static void mipi_tx_drv_set_phy_pll_setx(unsigned int phy_data_rate)
{
    ...
    unsigned char pll_set_0x65;

    pll_set_0x65 = 0x11; /* pre_div对应0x65[5:4],post_div对应0x65[2:0]; 均配置1默认2分频 */
    pll_set_0x64 = 0x0;  /* 默认配置0 */
    pll_set_0x63 = mipi_tx_drv_get_phy_pll_set_0x63(phy_data_rate, pll_set_0x65); /* fbdiv */
...
#ifdef MIPI_TX_DEBUG
    (void)osal_printk("\n==========phy pll info=======\n");
    (void)osal_printk("pll_set_0x63(0x14): 0x%x\n", pll_set_0x63);
    (void)osal_printk("pll_set_0x64(0x15): 0x%x\n", pll_set_0x64);
    (void)osal_printk("pll_set_0x65(0x16): 0x%x\n", pll_set_0x65);
    (void)osal_printk("=========================\n");
#endif
}
...
static void mipi_tx_drv_set_phy_timing_parameters(const mipi_tx_phy_timing_parameters *tp)
{
    ...
    (void)osal_printk("data_tpre_delay(0x30/40/50/60): 0x%x\n", tp->data_tpre_delay);
    (void)osal_printk("clk_post_delay(0x21): 0x%x\n", tp->clk_post_delay);
    (void)osal_printk("clk_tlpx(0x22): 0x%x\n", tp->clk_tlpx);
    (void)osal_printk("clk_tclk_prepare(0x23): 0x%x\n", tp->clk_tclk_prepare);
    (void)osal_printk("clk_tclk_zero(0x24): 0x%x\n", tp->clk_tclk_zero);
    (void)osal_printk("clk_tclk_trail(0x25): 0x%x\n", tp->clk_tclk_trail);
    (void)osal_printk("data_tlpx(0x32/42/52/62): 0x%x\n", tp->data_tlpx);
    (void)osal_printk("data_ths_prepare(0x33/43/53/63): 0x%x\n", tp->data_ths_prepare);
    (void)osal_printk("data_ths_zero(0x34/44/54/64): 0x%x\n", tp->data_ths_zero);
    (void)osal_printk("data_ths_trail(0x35/45/55/65): 0x%x\n", tp->data_ths_trail);
    (void)osal_printk("=========================\n");
#endif
}
...
static void mipi_tx_drv_set_phy_hs_lp_switch_time(const mipi_tx_phy_timing_parameters *tp)
{
    ...
    g_mipi_tx_regs_va->datalane_time.u32 = 0x30010; // 0x6001D;
    /* clk lp2hs,hs2lp time */
    g_mipi_tx_regs_va->clklane_time.u32 = 0x230011; // 0x26001E;

#ifdef MIPI_TX_DEBUG
    (void)osal_printk("datalane_time(0x9C): 0x%x\n", g_mipi_tx_regs_va->datalane_time.u32);
    (void)osal_printk("clklane_time(0x98): 0x%x\n", g_mipi_tx_regs_va->clklane_time.u32);
#endif
}
...
void mipi_tx_drv_set_phy_cfg(void)
{
    ...
    (void)osal_printk("actual_data_rate: 0x%x\n", actual_data_rate);
...
static void mipi_tx_hal_set_cmd_video_mode(cmd_video_mode cmd_video)
{
    ...
    mode_cfg.bits.cmd_video_mode = (unsigned int)cmd_video;
...
static void set_timing_config(const combo_dev_cfg_t *dev_cfg)
{
    ...
    if (dev_cfg->pixel_clk == 0) {
        (void)osal_printk("dev_cfg->pixel_clk is 0, illegal.\n");
        return;
    }
...
#ifdef MIPI_TX_DEBUG
    (void)osal_printk("hsa_pixels(0x48): 0x%x\n", dev_cfg->sync_info.hsa_pixels);
    (void)osal_printk("hbp_pixels(0x4c): 0x%x\n", dev_cfg->sync_info.hbp_pixels);
    (void)osal_printk("hact_pixels(0x50): 0x%x\n", dev_cfg->sync_info.hact_pixels);
    (void)osal_printk("hfp_pixels(0x50): 0x%x\n", dev_cfg->sync_info.hfp_pixels);
    (void)osal_printk("vact_lines(0x54): 0x%x\n", dev_cfg->sync_info.vact_lines);
    (void)osal_printk("vbp_lines(0x58): 0x%x\n", dev_cfg->sync_info.vbp_lines);
    (void)osal_printk("vfp_lines(0x5c): 0x%x\n", dev_cfg->sync_info.vfp_lines);
    (void)osal_printk("vsa_lines(0x60): 0x%x\n", dev_cfg->sync_info.vsa_lines);
#endif
}
...
static void mipi_tx_set_lane_id(int lane_idx, short lane_id)
{
    ...
    switch (lane_id) {
        case 0:
            lane_id_reg.bits.lane0_id = (unsigned int)lane_idx;
            break;

        case 1:
            lane_id_reg.bits.lane1_id = (unsigned int)lane_idx;
            break;

        case 2: /* 2: lane2 */
            lane_id_reg.bits.lane2_id = (unsigned int)lane_idx;
            break;

        case 3: /* 3: lane3 */
            lane_id_reg.bits.lane3_id = (unsigned int)lane_idx;
            break;
...
static void set_lane_config(const short lane_id[], int lane_id_len)
{
    ...
    g_mipi_tx_regs_va->lane_num.u32 = (unsigned int)mipi_tx_get_lane_num(lane_id, lane_id_len) - 1U;
...
void mipi_tx_hal_set_clklane_continue_en(unsigned char clk_en)
{
    volatile reg_clklane_ctrl clk_lane_ctrl;
...
#ifdef OT_FPGA
    data_rate = input_data_rate;
#else
    data_rate = input_data_rate * 110 / 100; /* 110, 100: data_rate * 1.1 for datatype, ECC, Checksum, lp~hs turn */
#endif
    actual_data_rate = input_data_rate; /* For B220, will fix in the future: 0x2D0U for rpi, 0x3C0U for mate9 */
...
void mipi_tx_drv_set_controller_cfg(const combo_dev_cfg_t *dev_cfg)
{
    ...
    if (dev_cfg->out_mode == OUT_MODE_CSI) {
        ...
        g_mipi_tx_regs_va->pck_en.u32 = 0x1E; /* dsi  0x1C in FPGA */
    }
    /* gen_vcid_rx */
...
static void mipi_tx_drv_set_payload_data(const unsigned char *cmd, unsigned short cmd_size)
{
    ...
    for (i = 0; i < (cmd_size / MIPI_TX_BYTES_PER_WORD); i++) {
        ...
        (void)mipi_tx_wait_write_fifo_not_full();
...
#ifdef MIPI_TX_DEBUG
    (void)osal_printk("\n=====set cmd=======\n");
    (void)osal_printk("gen_pld_data(0x70): 0x%x\n", gen_pld_data.u32);
#endif
...
{
    unsigned char trans_mode[(int)MIPI_TX_WORK_MODE_BUTT] = {LOW_POWER_MODE, HIGH_SPEED_MODE};
...
#ifdef MIPI_TX_DEBUG
    (void)osal_printk("\n=====set cmd=======\n");
    (void)osal_printk("gen_hdr(0x6C): 0x%x\n", gen_hdr.u32);
#endif
}
...
int mipi_tx_drv_set_cmd_info(const cmd_info_t *cmd_info)
{
    ...
    (void)mipi_tx_wait_cmd_fifo_empty();
    (void)mipi_tx_wait_write_fifo_empty();

#ifdef MIPI_TX_DEBUG
    (void)osal_printk("cmd_info->data_type: 0x%x\n", cmd_info->data_type);
    (void)osal_printk("cmd_info->cmd_size: 0x%x\n", cmd_info->cmd_size);
#endif
...
{
    volatile reg_phy_ctrl phy_ctrl;
    volatile reg_ctrl_reset ctrl_reset;
...
int mipi_tx_drv_get_cmd_info(const get_cmd_info_t *get_cmd_info)
{
    ...
    (void)osal_copy_to_user(get_cmd_info->get_data, data_buf, get_cmd_info->get_data_size);
...
void mipi_tx_drv_enable_input(const out_mode_t output_mode)
{
    ...
    mipi_base = osal_ioremap(MIPI_TX_REGS_ADDR, (unsigned int)MIPI_TX_REGS_SIZE);
    if (mipi_base == NULL) {
        mipi_tx_err("remap mipi_base reg addr fail\n");
        return;
    }
    /* enable input */
    ...
    int count = 30;
    do {
        pll_status &= hal_tx_read_reg(mipi_base + 0xb0);
        count--;
        osal_udelay(1);
    } while (pll_status != 1 && count > 0);
    if (pll_status != 1) {
        mipi_tx_err("pll lock fail\n");
    }
    mipi_tx_drv_phy_unreset();
    g_mipi_tx_regs_va->ctrl_reset.u32 = 0x1;

    osal_iounmap(mipi_base);
...
void __iomem *g_media_subctrl_base = HI_NULL;

static void hal_vo_map_base_reg(void)
{
    g_media_subctrl_base = osal_ioremap(MEDIA_SUBCTRL_REG, MEDIA_SUBCTRL_SIZE);
}

static void hal_vo_unmap_base_reg(void)
{
    if (g_media_subctrl_base != HI_NULL) {
        osal_iounmap(g_media_subctrl_base);
        g_media_subctrl_base = HI_NULL;
    }
}
...
static void mipi_tx_drv_hw_init(int smooth)
{
    hal_vo_map_base_reg();
    /* reset */
    hal_tx_write_reg(g_media_subctrl_base + SC_MIPI_RESET_REQ, 0x1U);
    if (hal_tx_read_reg(g_media_subctrl_base + SC_MIPI_RESET_ST) != 0x1U) {
        mipi_tx_err("mipitx reset failed!");
        hal_vo_unmap_base_reg();
        return;
    }

    /* select pll */
    hal_tx_write_reg(g_media_subctrl_base + SC_MIPI_CLK_SEL, 0x0U);

    /* enable clk */
    hal_tx_write_reg(g_media_subctrl_base + SC_MIPI_ICG_EN, 0x7U);
    if (hal_tx_read_reg(g_media_subctrl_base + SC_MIPI_ICG_ST) != 0x7U) {
        mipi_tx_err("enable mipitx clk failed!");
        hal_vo_unmap_base_reg();
        return;
    }

    /* de-reset */
    hal_tx_write_reg(g_media_subctrl_base + SC_MIPI_RESET_DREQ, 0x1U);
    if (hal_tx_read_reg(g_media_subctrl_base + SC_MIPI_RESET_ST) != 0x0U) {
        mipi_tx_err("mipitx de-reset failed!");
        hal_vo_unmap_base_reg();
        return;
    }
    hal_vo_unmap_base_reg();
...
static void mipi_tx_drv_hw_deinit(void)
{
    hal_vo_map_base_reg();

    /* disable clk */
    hal_tx_write_reg(g_media_subctrl_base + SC_MIPI_ICG_DIS, 0x7U);
    if (hal_tx_read_reg(g_media_subctrl_base + SC_MIPI_ICG_ST) != 0x0U) {
        mipi_tx_err("disable mipitx clk failed!");
        hal_vo_unmap_base_reg();
        return;
    }
...
#ifdef MIPITX_LG_DEBUG
    mipi_tx_info("mipi_tx_drv_init set panel!\n");
    unsigned long gpio4_crg_addr;
    unsigned long gpio5_crg_addr;
    gpio4_crg_addr = (unsigned long)(uintptr_t)osal_ioremap(0x00A0160000, (unsigned long)0x10000);
    if (gpio4_crg_addr == TD_NULL) {
        mipi_tx_err("gpio4_crg_addr ioremap fail!\n");
        goto fail1;
    }

    gpio5_crg_addr = (unsigned long)(uintptr_t)osal_ioremap(0x0300160000, (unsigned long)0x10000);
    if (gpio5_crg_addr == TD_NULL) {
        mipi_tx_err("gpio5_crg_addr ioremap fail!\n");
        osal_iounmap((void *)(uintptr_t)gpio4_crg_addr);
        goto fail1;
    }

    hal_tx_write_reg(gpio4_crg_addr, 0x480000); /* gpio_lcd_p5v5_enable */
    hal_tx_write_reg(gpio4_crg_addr + 0x4, 0x80000);
    mdelay(MIPI_TX_CRG_DELAY_LONG); /* delay 5 ms */
    hal_tx_write_reg(gpio4_crg_addr, 0x580000); /* gpio_lcd_n5v5_enable */
    hal_tx_write_reg(gpio4_crg_addr + 0x4, 0x180000);
    mdelay(MIPI_TX_CRG_DELAY_LONG); /* delay 5 ms */
    hal_tx_write_reg(gpio5_crg_addr, 0x200); /* gpio_lcd_reset ->1 */
    hal_tx_write_reg(gpio5_crg_addr + 0x4, 0x200);
    mdelay(MIPI_TX_CRG_DELAY_SHORT); /* delay 3 ms */
    hal_tx_write_reg(gpio4_crg_addr, 0x780000); /* gpio_lcd_bl_enable */
    hal_tx_write_reg(gpio4_crg_addr + 0x4, 0x380000);

    osal_iounmap((void *)(uintptr_t)gpio4_crg_addr);
    osal_iounmap((void *)(uintptr_t)gpio5_crg_addr);
    mipi_tx_info("mipi_tx_drv_init set panel done!\n");
#endif
