...
// 要映射的起始的IO地址
#define VOU_REGS_ADDR   0x0401000000
...
td_void fb_hal_vou_init(td_void)
{
    ...
    if (g_gfbg_reg == TD_NULL) {
        gfbg_error("ioremap_nocache failed\n");
    }
}
...
static td_ulong fb_vou_get_abs_addr(hal_disp_layer layer, td_ulong reg)
{
    ...
    switch (layer) {
        case HAL_DISP_LAYER_GFX0:                                                 // 4
        case HAL_DISP_LAYER_GFX1:                                                 // 5
        case HAL_DISP_LAYER_GFX2:                                                 // 6
        case HAL_DISP_LAYER_GFX3:                                                 // 7
        case HAL_DISP_LAYER_GFX4:                                                 // 8
            reg_abs_addr = (reg) + (layer - HAL_DISP_LAYER_GFX0) * GFX_REGS_LEN;  // 根据layer返回g0~v4寄存器的地址
            break;
        /* one wbc dev */
...
static td_ulong fb_vou_get_gfx_abs_addr(hal_disp_layer layer, td_ulong reg)
{
    ...
    switch (layer) {
        ...
        default:
            gfbg_error("Error layer id found in FUNC:%s,LINE:%d\n", __FUNCTION__, __LINE__);
            return 0;
    }
...
/*
 * fb_hal_disp_set_int_mask - set the interrupt register
 * @int_msk: interrupt mask reg value
 */
td_void fb_hal_disp_set_int_mask(td_u32 int_msk)
{
    ...
    /* read interrupt mask */
    vointmsk1.u32 = fb_hal_read_reg((td_u32 *)&(g_gfbg_reg->vointmsk1.u32));
    vointmsk1.u32 |= int_msk;
    fb_hal_write_reg((td_u32 *)&(g_gfbg_reg->vointmsk1.u32), vointmsk1.u32);

    return;
}

/*
 * fb_hal_disp_clear_int_mask - clear the interrupt register
 * @int_msk: interrupt mask reg value
 */
td_void fb_hal_disp_clear_int_mask(td_u32 int_msk)
{
    volatile u_vointmsk1 vointmsk1;
...
/*
 * fb_hal_disp_set_dhd0_ctrl - set the DHD0 ctrl register
 * @int_ctrl: DHD0 ctrl register value
 */
td_void fb_hal_disp_set_dhd0_ctrl(td_u32 int_ctrl)
{
    volatile u_dhd0_ctrl dhd0_ctrl;
    ...
    /* read dhd0 ctrl status */
    dhd0_ctrl.u32 = fb_hal_read_reg((td_u32 *)&(g_gfbg_reg->dhd0_ctrl.u32));
    dhd0_ctrl.u32 |= int_ctrl;
    fb_hal_write_reg((td_u32 *)&(g_gfbg_reg->dhd0_ctrl.u32), dhd0_ctrl.u32);
    return;
}

/*
 * fb_hal_disp_set_dhd1_ctrl - set the DHD1 ctrl register
 * @int_ctrl: DHD1 ctrl register value
 */
td_void fb_hal_disp_set_dhd1_ctrl(td_u32 int_ctrl)
{
    volatile u_dhd1_ctrl dhd1_ctrl;

    if (g_gfbg_reg == TD_NULL) {
        gfbg_error("NULL pointer %s: L%d\n", __FUNCTION__, __LINE__);
        return;
    }
    /* read dhd1 ctrl status */
    dhd1_ctrl.u32 = fb_hal_read_reg((td_u32 *)&(g_gfbg_reg->dhd1_ctrl.u32));
    dhd1_ctrl.u32 |= int_ctrl;
    fb_hal_write_reg((td_u32 *)&(g_gfbg_reg->dhd1_ctrl.u32), dhd1_ctrl.u32);
    return;
}
...
td_bool fb_hal_video_set_layer_disp_rect(hal_disp_layer layer, const ot_fb_rect *rect)
{
    ...
    switch (layer) {
        ...
        case HAL_DISP_LAYER_GFX4:
            ...
            break;
        default:
            gfbg_error("Error layer id found in %s: L%d\n", __FUNCTION__, __LINE__);
            return TD_FALSE;
    }
...
td_bool fb_hal_video_set_layer_video_rect(hal_disp_layer layer, const ot_fb_rect *rect)
{
    ...
    switch (layer) {
        ...
        case HAL_DISP_LAYER_GFX4:
            ...
            break;
        default:
            gfbg_error("Error layer id %d# found in %s: L%d\n", layer, __FUNCTION__, __LINE__);
            return TD_FALSE;
    }
...
static td_void fb_hal_layer_csc_set_coef(hal_disp_layer layer, const vdp_csc_coef *coef)
{
    ...
    if ((layer >= HAL_DISP_LAYER_GFX0) && (layer <= HAL_DISP_LAYER_GFX4)) {
        ...
        gfbg_error("Error layer id found in %s, %d\n", __FUNCTION__, __LINE__);
    }
    return;
}
...
td_bool fb_hal_layer_set_csc_coef(hal_disp_layer layer, const csc_coef *coef)
{
    if ((layer < HAL_DISP_LAYER_VHD0) || (layer > HAL_DISP_LAYER_GFX4)) {
        gfbg_error("Error, Wrong layer ID!%d\n", __LINE__);
        return TD_FALSE;
    }

    if (coef == TD_NULL) {
        gfbg_error("Error, null pointer\n");
        return TD_FALSE;
    }
...
td_bool fb_hal_layer_set_src_resolution(hal_disp_layer layer, const ot_fb_rect *rect)
{
    ...
    if (layer == HAL_DISP_LAYER_GFX0 ||
        ...
        gfbg_error("Error:layer id not found in %s: L%d\n", __FUNCTION__, __LINE__);
        return TD_FALSE;
    }
...
td_bool fb_hal_graphic_set_gfx_key_en(hal_disp_layer layer, td_u32 key_enable)
{
    ...
    if (layer == HAL_DISP_LAYER_GFX0 ||
        ...
        gfbg_error("Error layer id %d not support colorkey in %s: L%d\n", (td_s32)layer, __FUNCTION__, __LINE__);
        return TD_FALSE;
    }
    return TD_TRUE;
}
...
td_bool fb_hal_graphic_set_gfx_key_mode(hal_disp_layer layer, td_u32 key_out)
{
    ...
    if (layer == HAL_DISP_LAYER_GFX0 ||
        ...
        gfbg_error("Error layer id %d not support colorkey mode in %s: L%d\n", (td_s32)layer, __FUNCTION__, __LINE__);
        return TD_FALSE;
    }
    return TD_TRUE;
}
...
td_bool fb_hal_graphic_set_color_key_mask(hal_disp_layer layer, hal_gfx_mask msk)
{
    ...
    if (layer == HAL_DISP_LAYER_GFX0 ||
        ...
        gfbg_error("Error layer id %d not support colorkey mask in %s: L%d\n", (td_s32)layer, __FUNCTION__, __LINE__);
        return TD_FALSE;
    }
...
td_bool fb_hal_graphic_set_gfx_dcmp_enable(hal_disp_layer layer, td_u32 enable)
{
    ...
    if (layer == HAL_DISP_LAYER_GFX0 ||
        ...
        gfbg_error("Error layer id %d not support dcmp in %s: L%d\n", (td_s32)layer, __FUNCTION__, __LINE__);
        return TD_FALSE;
    }
...
td_void fb_hal_v3r2_gfx_set_source_mode(td_u32 layer, td_u32 source_mode)
{
    ...
    if (layer >= GFX_MAX) {
        OT_TRACE(HI_DBG_ERR, OT_ID_FB, "Error, %s(),%d Select Wrong Layer ID\n", __FUNCTION__, __LINE__);
    }
...
td_void hal_clut_set_up(td_u32 layer_id, td_phys_addr_t clut_phy_addr)
{
    ...
    if (layer_id != SMART_RECT_LAYER_G3) {
        gfbg_error("layer%u doesn't support clut\n", layer_id);
        return;
    }
...
td_void hal_smart_rect_set_up(td_u32 layer_id, td_phys_addr_t phys_addr)
{
    ...
    // PARA_HADDR_VHD_CHN05:PARA高清通道05申请高地址寄存器 para_haddr_vhd_chn05:WBC_DHD层缩放申请1高地址寄存器
    ...
    // PARA_ADDR_VHD_CHN05:PARA高清通道05申请地址寄存器 para_haddr_vhd_chn05:WBC_DHD层缩放申请1地址寄存器
...
td_void hal_smart_rect_set_up(td_u32 layer_id, td_phys_addr_t phys_addr)
{
    ...
    if (layer_id == SMART_RECT_LAYER_G3) {
        // PARA_HADDR_VHD_CHN03:PARA高清通道03申请高地址寄存器 para_haddr_vhd_chn03:G3层OSB申请高地址寄存器
        ...
        // PARA_ADDR_VHD_CHN03:PARA高清通道03申请地址寄存器 para_addr_vhd_chn03:G3层OSB申请地址寄存器
        ...
        // PARA_HADDR_VHD_CHN02:PARA高清通道02申请高地址寄存器 para_haddr_vhd_chn02:G4层OSB申请高地址寄存器
        ...
        // PARA_ADDR_VHD_CHN02:PARA高清通道02申请地址寄存器
...
td_void __iomem *g_vdp_smmu_base = TD_NULL;
td_s32 hal_map_smmu_reg(td_void)
{
    g_vdp_smmu_base = osal_ioremap(VDP_WRAP_REG, VDP_WRAP_SIZE);
    if (g_vdp_smmu_base == TD_NULL) {
        gfbg_error("mapping subctrl_base addr error!\n");
        return TD_FAILURE;
    }
...
td_void hal_set_addr_ext(td_void)
{
    const td_u32 addr_ext = 0xF000U;

    /* 配置高位 */
    fb_hal_write_reg(g_vdp_smmu_base + SC_AWADDR_EXT_1, addr_ext);
    fb_hal_write_reg(g_vdp_smmu_base + SC_ARADDR_EXT_1, addr_ext);
}

td_void hal_set_sid_ssid(td_u32 sid, td_u32 ssid)
{
    td_u32 axi_user_0 = 0U;
    axi_user_0 += (ssid << 16U);
    axi_user_0 += sid;

    /* 配置AWUSER1/ARUSER1 */
    fb_hal_write_reg(g_vdp_smmu_base + SC_AWUSER_0_1, axi_user_0);
    fb_hal_write_reg(g_vdp_smmu_base + SC_ARUSER_0_1, axi_user_0);
}

td_void hal_set_smmu_enable(td_void)
{
    td_u32 awuser_val;
    td_u32 aruser_val;

    /* 读取当前写AXI SMMU配置 */
    awuser_val = fb_hal_read_reg(g_vdp_smmu_base + SC_AWUSER_1_1);
    /* 配置写AXI SMMU使能 */
    awuser_val = awuser_val & 0x3C0000U;
    awuser_val = awuser_val | (VO_SMMU_ENABLE & 0xFFC3FFFFU);
    fb_hal_write_reg(g_vdp_smmu_base + SC_AWUSER_1_1, awuser_val);

    /* 读取当前读AXI SMMU配置 */
    aruser_val = fb_hal_read_reg(g_vdp_smmu_base + SC_ARUSER_1_1);
    /* 配置读AXI SMMU使能 */
    aruser_val = aruser_val & 0x3C0000U;
    aruser_val = aruser_val | (VO_SMMU_ENABLE & 0xFFC3FFFFU);
    fb_hal_write_reg(g_vdp_smmu_base + SC_ARUSER_1_1, aruser_val);
}

td_void hal_unmap_smmu_reg(td_void)
{
    if (g_vdp_smmu_base != TD_NULL) {
        osal_iounmap(g_vdp_smmu_base);
        g_vdp_smmu_base = TD_NULL;
    }
}

td_void fb_hal_set_axi_type(td_void)
{
    if (g_gfbg_reg == TD_NULL) {
        gfbg_error("g_gfbg_reg is NULL!\n");
        return;
    }
    fb_hal_write_reg((td_u32*)&(g_gfbg_reg->mac_rchn_sel0), GRAPHIC_USE_AXI1);  // 图形层使用axi1通道
    return;
}

td_s32 fb_hal_set_smmu_enable(td_u32 sid, td_u32 ssid)
{
    if (hal_map_smmu_reg() != TD_SUCCESS) {
        gfbg_error("fb smmu ioremap failed!\n");
        return TD_FAILURE;
    }
