...
#define GFBG_INT_MASK_CLEAR 0X0

gfbg_coef_addr g_gfbg_coef_buf_addr;
gfbg_mmz_buffer g_clut_table_addr;
static struct media_mem_file* g_clut_mem_file;
...
td_bool graphic_drv_set_layer_data_fmt(hal_disp_layer layer, hal_disp_pixel_format data_fmt)
{
    td_u32 layer_index = 0;
    if (fb_graphic_drv_get_layer_index(layer, &layer_index) != TD_SUCCESS) {
        gfbg_error("gfx_layer(%d) is invalid!\n", layer);
        return TD_FALSE;
    }
...
{
    hal_clut_set_up(layer_id, clut_phy_addr);
    return;
}
...
static td_s32 graphic_drv_allocate_mem(td_u32 size, gfbg_mmz_buffer *vdp_mmz_buffer, const char* name)
{
    ...
    if (call_sys_get_mmz_name(&chn, &mmz_name)) {
        gfbg_error("vo_drv_allocate_mem get mmz name fail!\n");
        return TD_FAILURE;
    }

    ret = media_kernel_mem_malloc(size, DVPP_MEM_PLACEMENT_MBUF | DVPP_PAGE_FLAG_NORMAL,
                                 &vdp_mmz_buffer->start_vir_addr);
    if (ret != TD_SUCCESS) {
        gfbg_error("malloc vdp_mmz_buffer buffer failed\n");
        return TD_FAILURE;
    }

    ret = media_kernel_mem_map(g_clut_mem_file, size, vdp_mmz_buffer->start_vir_addr,
        (td_u64 *)&(vdp_mmz_buffer->start_phy_addr));
    if (ret != TD_SUCCESS) {
        gfbg_error("map vdp_mmz_buffer buffer failed\n");
        media_kernel_mem_free(vdp_mmz_buffer->start_vir_addr);
...
static td_void graphic_drv_delete_mem(gfbg_mmz_buffer *vdp_mmz_buffer)
{
    ...
    ret = media_kernel_mem_unmap(g_clut_mem_file, vdp_mmz_buffer->size, vdp_mmz_buffer->start_phy_addr);
    if (ret != TD_SUCCESS) {
        gfbg_error("unmap clut buffer failed!\n");
    }
    media_kernel_mem_free(vdp_mmz_buffer->start_vir_addr);
    vdp_mmz_buffer->start_phy_addr = 0;
    vdp_mmz_buffer->start_vir_addr = TD_NULL;
...
static td_s32 graphic_clut_table_init(td_void)
{
    td_s32 ret;
    char name[16] = "gfbg_clut_table"; /* 16 name length */
    ret = graphic_drv_allocate_mem(CLUT_TABLE_SIZE, &g_clut_table_addr, name);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    return TD_SUCCESS;
...
static td_s32 graphic_drv_get_layer_bind_dev(ot_gfx_layer gfx_layer, ot_vo_dev *vo_dev)
{
    ...
    if ((gfx_layer != GRAPHICS_LAYER_G3) && (gfx_layer != GRAPHICS_LAYER_G2) && (gfx_layer != GRAPHICS_LAYER_G4)) {
        gfbg_error("gfx_layer %d is illeagal\n", (td_s32)gfx_layer);
        return TD_FAILURE;
    }
...
td_s32 fb_int_msk_init(td_u32 layer_id)
{
    ot_vo_dev vo_dev;

    switch (layer_id) {
        case GRAPHICS_LAYER_G0:
            /* enable the dhd0 interrupt */
            graphic_drv_int_set_mask(VO_INT_MSK_DHD0);
            break;
        case GRAPHICS_LAYER_G1:
            /* enable the dhd1 interrupt */
            graphic_drv_int_set_mask(VO_INT_MSK_DHD1);
            break;
        case GRAPHICS_LAYER_G2:
        case GRAPHICS_LAYER_G3:
        case GRAPHICS_LAYER_G4:
            /* enable dhd0 or dhd1 interrupt depends on the dev */
            fb_hal_link_get_hc_link(layer_id + HAL_DISP_LAYER_GFX0, &vo_dev);
            ...
            break;
        default:
            gfbg_error("disp layer_id(%u) is invalid!\n", layer_id);
            return TD_FAILURE;
    }
...
{
    graphic_drv_int_clear_mask(GFBG_INT_MASK_CLEAR);
    return TD_SUCCESS;
}

static td_s32 graphic_drv_mem_file_init(td_void)
{
    hi_s32 ret;
    g_clut_mem_file = (struct media_mem_file *)osal_kmalloc(sizeof(struct media_mem_file), osal_gfp_kernel);
    if (g_clut_mem_file == TD_NULL) {
        gfbg_error("g_clut_mem_file is null\n");
        return TD_FAILURE;
    }
    /* initialize the value of mem file */
    (td_void)memset_s(g_clut_mem_file, sizeof(struct media_mem_file), 0, sizeof(struct media_mem_file));

    /* init the mem file */
    ret = media_mem_file_init(g_clut_mem_file, current->tgid, 0, (hi_s32)HI_ID_FB);
    if (ret != TD_SUCCESS) {
        gfbg_error("media_mem_file_init failed\n");
        osal_kfree(g_clut_mem_file);
        g_clut_mem_file = TD_NULL;
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}

static td_void graphic_drv_mem_file_exit(td_void)
{
    media_mem_file_uninit(g_clut_mem_file);
    osal_kfree(g_clut_mem_file);
    g_clut_mem_file = TD_NULL;
}
...
td_void graphic_zme_coef_exit(td_void)
{
    graphic_drv_delete_mem(&g_gfbg_coef_buf_addr.buf_base_addr);
}

td_void graphic_clut_table_exit(td_void)
{
    graphic_drv_delete_mem(&g_clut_table_addr);
}

td_s32 graphic_drv_clut_init(td_void)
{
    td_s32 ret;
    // init clut mem file
    ret = graphic_drv_mem_file_init();
    if (ret != TD_SUCCESS) {
        return TD_FAILURE;
    }
    ...
    if (ret != TD_SUCCESS) {
        graphic_drv_mem_file_exit();
        return TD_FAILURE;
    }

    ret = graphic_clut_table_init();
    if (ret != TD_SUCCESS) {
        graphic_zme_coef_exit();
        graphic_drv_mem_file_exit();
        return TD_FAILURE;
    }
...
td_void graphic_drv_clut_exit(td_void)
{
    /* mem delete */
    graphic_clut_table_exit();
    graphic_zme_coef_exit();
    /* mem file free */
    graphic_drv_mem_file_exit();
}
...
td_s32 graphic_drv_get_layer_enable(hal_disp_layer gfx_layer, td_bool *enable)
{
    if (fb_hal_get_layer_enable(gfx_layer, enable) == TD_FALSE) {
        gfbg_error("graphics layer %d enable failed!\n", gfx_layer);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
...
gfbg_mmz_buffer *graphic_drv_get_clut_table(td_void)
{
    return &g_clut_table_addr;
}
...
td_s32 fb_graphic_drv_set_csc_coef(hal_disp_layer gfx_layer, const vo_csc *gfx_csc, const csc_coef_param *csc_param)
{
    ...
    if (ret != TD_SUCCESS) {
        gfbg_error("gfx_layer(%u) get CSC mode failed!\n", (td_u32)gfx_layer);
        return ret;
    }
    ...
    if (ret != TD_SUCCESS) {
        gfbg_error("gfx_layer(%u) calculate CSC materix failed!\n", (td_u32)gfx_layer);
        return ret;
    }
...
#ifdef CONFIG_OT_VO_FB_SEPARATE
td_void graphic_drv_int_set_mask(td_u32 int_msk)
{
    /* set the interrupt mask reg */
    fb_hal_disp_set_int_mask(int_msk);
}

td_void graphic_drv_int_clear_mask(td_u32 int_msk)
{
    /* clear the interrupt mask reg */
    fb_hal_disp_clear_int_mask(int_msk);
}


td_void graphic_drv_int_set_dhd0_ctrl(td_u32 int_ctrl)
{
    /* set the dhd0 ctrl reg */
    fb_hal_disp_set_dhd0_ctrl(int_ctrl);
}

td_void graphic_drv_int_set_dhd1_ctrl(td_u32 int_ctrl)
{
    /* set the DHD1 ctrl reg */
    fb_hal_disp_set_dhd1_ctrl(int_ctrl);
}
...
td_s32 graphic_drv_get_interrupt_dev(td_u32 int_status, ot_vo_dev *vo_dev)
{
    ...
    if (int_status & GFBG_INTMSK_DHD0_VTTHD2) {
        gfbg_debug("Graphic: DHD0 INTERRUPT\n");
        ...
        *vo_dev = VO_DEV_DHD0;
        ...
        gfbg_debug("Graphic: DHD1 INTERRUPT\n");
        ...
        *vo_dev = VO_DEV_DHD1;
        ...
        gfbg_debug("Graphic: DSD INTERRUPT\n");
...
td_void graphic_drv_set_axi_type(td_void)
{
    fb_hal_set_axi_type();
}

td_s32 graphic_set_smmu_enable(td_u32 sid, td_u32 ssid)
{
    return fb_hal_set_smmu_enable(sid, ssid);
}
