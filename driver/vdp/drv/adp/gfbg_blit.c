...
td_s32 gfbg_drv_fill(const ot_fb_buf *dst_img, td_u32 fill_data)
{
    ...
    ot_tde_export_func *tde_export_func = func_entry(ot_tde_export_func, HI_ID_TDE);
    ...
    if (ret != TD_SUCCESS) {
        gfbg_error("FILL job submitted to TDE failed!!! Ret = 0x%x\n", ret);
        return ret;
    }
                                       ...
                                       td_bool is_refresh_screen)
{
    if (is_refresh_screen == TD_TRUE) {
        ...
        dst_rect->pos_x   =  (td_s32)(src_rect->pos_x * (td_u64)dst_surface->width / src_surface->width);
        dst_rect->pos_y   =  (td_s32)(src_rect->pos_y * (td_u64)dst_surface->height / src_surface->height);
        dst_rect->width  =  (td_u32)(src_rect->width * (td_u64)dst_surface->width / src_surface->width);
        dst_rect->height =  (td_u32)(src_rect->height * (td_u64)dst_surface->height / src_surface->height);
    }
    return;
}
