...
#if defined(HDMI_PRODUCT_SD3403V100)
...
static hdmi_video_timing hdmi_format_to_hdmi_timing(td_u32 format)
{
    ...
    for (i = 0; i < hdmi_array_size(g_format_to_timing); i++) {
        if (format == (td_u32)(g_format_to_timing[i].format)) {
            return g_format_to_timing[i].timing;
        }
    }
    hdmi_debug_trace("Error video fmt:%u\n", format);
...
static ot_hdmi_video_format hdmi_timing_to_format(td_u32 timing)
{
    ...
    for (i = 0; i < hdmi_array_size(g_format_to_timing); i++) {
        if (timing == (td_u32)g_format_to_timing[i].timing) {
            return g_format_to_timing[i].format;
        }
    }
    hdmi_debug_trace("Error video timing:%u\n", timing);
...
static td_s32 hdmi_avi_param_check(const ot_hdmi_avi_infoframe *user_avi_infoframe)
{
    ...
    if (user_avi_infoframe->active_info_present != TD_FALSE && user_avi_infoframe->active_info_present != TD_TRUE) {
        hdmi_err_trace("active_infor_present error:%d\n", (td_s32)user_avi_infoframe->active_info_present);
        return OT_ERR_HDMI_INVALID_PARA;
    }
    hdmi_check_max_return_invalid(user_avi_infoframe->bar_info, OT_HDMI_BAR_INFO_VH);
    hdmi_check_max_return_invalid((td_u32)user_avi_infoframe->scan_info, ((td_s32)OT_HDMI_SCAN_INFO_BUTT - 1));
    ...
    if (hdmi_avi_active_aspect_check(user_avi_infoframe->active_aspect_ratio) != TD_SUCCESS) {
        hdmi_err_trace("active_aspect_ratio error:%d\n", (td_s32)user_avi_infoframe->active_aspect_ratio);
        return OT_ERR_HDMI_INVALID_PARA;
    }
    ...
    if (user_avi_infoframe->is_it_content != TD_FALSE && user_avi_infoframe->is_it_content != TD_TRUE) {
        hdmi_err_trace("is_it_content error:%d\n", (td_s32)user_avi_infoframe->is_it_content);
        return OT_ERR_HDMI_INVALID_PARA;
    }
...
static td_s32 hdmi_audio_infoframe_param_check(const ot_hdmi_audio_infoframe *user_audio_infoframe)
{
    ...
    if (user_audio_infoframe->chn_cnt > OT_HDMI_AUDIO_CHN_CNT_8) {
        hdmi_err_trace("chn_cnt error:%d\n", (td_s32)user_audio_infoframe->chn_cnt);
        ret = OT_ERR_HDMI_INVALID_PARA;
        goto err;
    }
    if ((td_s32)user_audio_infoframe->coding_type > ((td_s32)OT_HDMI_AUDIO_CODING_BUTT - 1)) {
        hdmi_err_trace("coding_type error:%d\n", (td_s32)user_audio_infoframe->coding_type);
        ret = OT_ERR_HDMI_INVALID_PARA;
        goto err;
    }
    if (user_audio_infoframe->sample_size > OT_HDMI_AUDIO_SAMPLE_SIZE_24) {
        hdmi_err_trace("sample_size error:%d\n", (td_s32)user_audio_infoframe->sample_size);
        ret = OT_ERR_HDMI_INVALID_PARA;
        goto err;
    }
    if (user_audio_infoframe->sampling_freq > OT_HDMI_AUDIO_SAMPLE_FREQ_192000) {
        hdmi_err_trace("sampling_frequency error:%d\n", (td_s32)user_audio_infoframe->sampling_freq);
        ret = OT_ERR_HDMI_INVALID_PARA;
        goto err;
    }
    if (user_audio_infoframe->chn_alloc > 0x1f) { /* 0x1f, channel_alloc max effective size for CEA861-D */
        hdmi_err_trace("channel_alloc error:%u\n", user_audio_infoframe->chn_alloc);
        ret = OT_ERR_HDMI_INVALID_PARA;
        goto err;
    }
    if (user_audio_infoframe->level_shift >= OT_HDMI_LEVEL_SHIFT_VAL_BUTT) {
        hdmi_err_trace("level_shift error:%d\n", (td_s32)user_audio_infoframe->level_shift);
        ret = OT_ERR_HDMI_INVALID_PARA;
        goto err;
    }
    if (user_audio_infoframe->lfe_playback_level >= OT_HDMI_LFE_PLAYBACK_BUTT) {
        hdmi_err_trace("lfe_playback_level error:%d\n", (td_s32)user_audio_infoframe->lfe_playback_level);
        ret = OT_ERR_HDMI_INVALID_PARA;
        goto err;
    }
    if (user_audio_infoframe->down_mix_inhibit != TD_FALSE && user_audio_infoframe->down_mix_inhibit != TD_TRUE) {
        hdmi_err_trace("down_mix_inhibit error:%d\n", (td_s32)user_audio_infoframe->down_mix_inhibit);
        ret = OT_ERR_HDMI_INVALID_PARA;
        goto err;
    }
    ...
    const drv_hdmi_infoframe *src_infoframe)
{
    ...
    g_avi_video_format = (td_u32)hdmi_timing_to_format((td_u32)drv_avi_infoframe->video_timing);
...
static td_s32 hdmi_infoframe_drv_to_user(ot_hdmi_infoframe *dest_infoframe, const drv_hdmi_infoframe *src_infoframe)
{
    ...
    switch (src_infoframe->infoframe_id) {
        ...
        case HDMI_INFOFRAME_TYPE_AUDIO: {
            ...
            user_audio_infoframe->sample_size        = drv_audio_infoframe->sample_size;
            user_audio_infoframe->sampling_freq      = drv_audio_infoframe->sample_frequency;
            ...
            user_audio_infoframe->lfe_playback_level = drv_audio_infoframe->lfe_playback_level;
        ...
        case HDMI_INFOFRAME_TYPE_VENDOR:{
            ...
            if (des_vendor->data_len > OT_HDMI_VENDOR_USER_DATA_MAX_LEN) {
                return OT_ERR_HDMI_INVALID_PARA;
            }
...
static td_s32 hdmi_infoframe_user2drv_avi(drv_hdmi_infoframe *dest_infoframe, const ot_hdmi_infoframe *src_infoframe)
{
    ...
    g_avi_video_format = (td_u32)user_avi_infoframe->timing_mode;
    ...
    drv_avi_infoframe->colorimetry       = user_avi_infoframe->colorimetry;
    ...
    drv_avi_infoframe->nups                 = user_avi_infoframe->pic_scaling;
    drv_avi_infoframe->quantization_range   = user_avi_infoframe->rgb_quant;
    drv_avi_infoframe->itc                  = user_avi_infoframe->is_it_content;
    drv_avi_infoframe->pixel_repeat         = (td_u8)user_avi_infoframe->pixel_repetition;
    drv_avi_infoframe->pixel_repeat        += 1;
    drv_avi_infoframe->content_type         = user_avi_infoframe->content_type;
...
static td_s32 hdmi_infoframe_user2drv_audio(drv_hdmi_infoframe *dest_infoframe, const ot_hdmi_infoframe *src_infoframe)
{
    ...
    drv_audio_infoframe->channels           = (td_u8)user_audio_infoframe->chn_cnt;
    ...
    drv_audio_infoframe->level_shift_value  = (td_u8)user_audio_infoframe->level_shift;
    ...
    switch (src_infoframe->infoframe_type) {
        ...
        case OT_INFOFRAME_TYPE_VENDORSPEC: {
            ...
            if (user_infoframe->data_len > OT_HDMI_VENDOR_USER_DATA_MAX_LEN) {
                hdmi_err_trace("data is too long! data_len:%u\n", user_infoframe->data_len);
                return OT_ERR_HDMI_INVALID_PARA;
            }
...
#endif
        default:
            hdmi_warn_trace("invalid infoframe type:%d\n", (td_s32)src_infoframe->infoframe_type);
            return OT_ERR_HDMI_INVALID_PARA;
    }
...
{
    user_cap->manufacture_name[0]        = drv_cap->mfrs_info.mfrs_name[0]; /* 0, array index */
    user_cap->manufacture_name[1]        = drv_cap->mfrs_info.mfrs_name[1]; /* 1, array index */
    user_cap->manufacture_name[2]        = drv_cap->mfrs_info.mfrs_name[2]; /* 2, array index */
    user_cap->manufacture_name[3]        = drv_cap->mfrs_info.mfrs_name[3]; /* 3, array index */
...
static td_s32 hdmi_y420_format_drv2user(ot_hdmi_sink_capability *user_cap, const hdmi_sink_capability *drv_cap)
{
    ...
    (td_void)memset_s(user_cap->support_y420_format, sizeof(user_cap->support_y420_format),
        (td_s32)TD_FALSE, sizeof(user_cap->support_y420_format));
    ...
    (td_void)memset_s(user_cap->only_support_y420_format, sizeof(user_cap->only_support_y420_format),
        (td_s32)TD_FALSE, sizeof(user_cap->only_support_y420_format));
...
static td_s32 hdmi_capability_drv2user(ot_hdmi_sink_capability *user_cap, const hdmi_sink_capability *drv_cap)
{
    ...
    (td_void)memset_s(user_cap->support_video_format, sizeof(user_cap->support_video_format),
        (td_s32)TD_FALSE, sizeof(user_cap->support_video_format));
...
static td_s32 hdmi_audio_sample_rate_check(const ot_hdmi_attr *attr)
{
    ...
    if (support == TD_TRUE) {
        hdmi_warn_trace("sample_rate(%d) is invalid\n", (td_s32)attr->sample_rate);
        return OT_ERR_HDMI_INVALID_PARA;
    }
    if ((attr->sample_rate != OT_HDMI_SAMPLE_RATE_32K) &&
        (attr->sample_rate != OT_HDMI_SAMPLE_RATE_44K) &&
        (attr->sample_rate != OT_HDMI_SAMPLE_RATE_48K)) {
        hdmi_warn_trace("not support sample_rate(%d)\n", (td_s32)attr->sample_rate);
        return OT_ERR_HDMI_FEATURE_NO_SUPPORT;
    }
...
static td_s32 hdmi_audio_attr_check(const ot_hdmi_attr *attr)
{
    ...
    if (support == TD_TRUE) {
        hdmi_warn_trace("bit_depth(%d) is invalid\n", (td_s32)attr->bit_depth);
        return OT_ERR_HDMI_INVALID_PARA;
    }
...
static td_s32 hdmi_attr_check_valid(const ot_hdmi_attr *attr)
{
    ...
    hdmi_check_boolval_return((td_u32)attr->hdmi_en);
    hdmi_check_boolval_return(attr->audio_en);
    hdmi_check_boolval_return((td_u32)attr->auth_mode_en);
    hdmi_check_boolval_return((td_u32)attr->deep_color_adapt_en);

    if (attr->video_format >= OT_HDMI_VIDEO_FORMAT_BUTT) {
        hdmi_warn_trace("envideofmt(%d) is invalid\n", (td_s32)attr->video_format);
        return OT_ERR_HDMI_INVALID_PARA;
    }
    support = (attr->deep_color_mode != OT_HDMI_DEEP_COLOR_24BIT &&
               ...
               attr->deep_color_mode != OT_HDMI_DEEP_COLOR_36BIT);
    if (support == TD_TRUE) {
        hdmi_warn_trace("deep_color_mode(%d) is invalid\n", (td_s32)attr->deep_color_mode);
        return OT_ERR_HDMI_INVALID_PARA;
    }
    /* AO */
...
static td_s32 hdmi_hw_spec_check_valid_inner(const ot_hdmi_hw_spec *hw_spec, td_u8 i)
{
#if defined(HDMI_PRODUCT_SD3403V100)
...
static td_s32 hdmi_hw_spec_check_valid(const ot_hdmi_hw_spec *hw_spec)
{
    ...
    for (i = 0; i < OT_HDMI_HW_PARAM_NUM; i++) {
        td_s32 result = hdmi_hw_spec_check_valid_inner(hw_spec, i);
        if (result == OT_ERR_HDMI_INVALID_PARA) {
            return result;
        }
    }
...
td_s32 ot_mpi_hdmi_open(ot_hdmi_id hdmi)
{
    ...
    open_para.default_mode = HDMI_FORCE_HDMI;
...
td_s32 ot_mpi_hdmi_set_attr(ot_hdmi_id hdmi, const ot_hdmi_attr *attr)
{
    ...
    property.prop.video_timing          = hdmi_format_to_hdmi_timing((td_u32)attr->video_format);
    ...
    property.prop.pix_clk = (attr->video_format == OT_HDMI_VIDEO_FORMAT_VESA_CUSTOMER_DEFINE) ?
        attr->pix_clk : hdmi_pixel_freq_search((td_u32)attr->video_format);
...
td_s32 ot_mpi_hdmi_set_timing(ot_hdmi_id hdmi, ot_hdmi_timing_format timing_format)
{
    if (timing_format.fps < 0 || timing_format.timing_height < 0 || timing_format.fps < 0 || hdmi < 0 ||
    timing_format.pattern_color_space < 0 || timing_format.pattern_color_space > HDMI_AVI_INFOFRAME_YUV420) {
    hdmi_err_trace("error:index of timing_format is out of range\n");
    return TD_FAILURE;
    }
    hdmi_check_id_return_invalid(hdmi);
    drv_hdmi_timing_format temp = { 0 };
    temp.timing_height = (td_u32)timing_format.timing_height;
    temp.timing_width = (td_u32)timing_format.timing_width;
    temp.fps = (td_u32)timing_format.fps;
    temp.hdmi_id = (td_u32)hdmi;
    temp.pattern_color_space = (td_u32)timing_format.pattern_color_space;
    return mpi_hdmi_set_timing(hdmi, temp);
}

td_s32 ot_mpi_hdmi_reset_clock(ot_hdmi_id hdmi, ot_hdmi_reset_clk reset_clk)
{
    if (reset_clk.reset_mode < 0) {
        hdmi_err_trace("error:index of reset_clk is out of range\n");
    }
    hdmi_check_id_return_invalid(hdmi);
    drv_hdmi_reset_clk temp = { 0 };
    temp.reset_mode = (td_u32)reset_clk.reset_mode;

    return mpi_hdmi_reset_clock(hdmi, temp);
}

td_s32 ot_mpi_hdmi_set_pattern(ot_hdmi_id hdmi, ot_hdmi_pattern_mode pattern_mode)
{
    if (pattern_mode.pattern_mode < 0 || pattern_mode.pattern_color_b < 0 || pattern_mode.pattern_color_g < 0 ||
        pattern_mode.pattern_color_r < 0) {
        hdmi_err_trace("error:index of pattern_mode is out of range\n");
    }
    hdmi_check_id_return_invalid(hdmi);
    drv_hdmi_pattern_mode temp = { 0 };
    temp.pattern_color_b = (td_u32)pattern_mode.pattern_color_b;
    temp.pattern_color_g = (td_u32)pattern_mode.pattern_color_g;
    temp.pattern_color_r = (td_u32)pattern_mode.pattern_color_r;
    temp.pattern_mode = (td_u32)pattern_mode.pattern_mode;

    return mpi_hdmi_set_pattern(hdmi, temp);
}

td_s32 ot_mpi_hdmi_set_hotplug(ot_hdmi_id hdmi, ot_hdmi_hotplug_status *hotplug_status)
{
    ...
    hdmi_check_id_return_invalid(hdmi);
    drv_hdmi_hotplug_status temp;
    td_s32 ret = mpi_hdmi_set_hotplug(hdmi, &temp);
    hotplug_status->hotplug_on = (td_s32)temp.hotplug_on;
    hdmi_debug_trace("ot_mpi_hdmi_set_hotplug== %d\n", hotplug_status->hotplug_on); // 查询hotplug

    return ret;
}

td_s32 ot_mpi_hdmi_set_detection(ot_hdmi_id hdmi, ot_hdmi_detection *hdmi_detection)
{
    ...
    hdmi_check_id_return_invalid(hdmi);
    drv_hdmi_detection temp;
    td_s32 ret = mpi_hdmi_set_detection(hdmi, &temp);

    hdmi_detection->timing_height = temp.timing_height;
    hdmi_detection->timing_width = temp.timing_width;

    return ret;
}

td_s32 ot_mpi_hdmi_set_capture(ot_hdmi_id hdmi, ot_hdmi_capture *hdmi_capture)
{
    if (hdmi_capture == TD_NULL) {
        hdmi_err_trace("the pointer of hdmi_capture is null.\n");
        return TD_FAILURE;
    }
    if (hdmi_capture->pos_x < 0 || hdmi_capture->pos_y < 0) {
        hdmi_err_trace("error:index of hdmi_capture is out of range\n");
    }
    hdmi_check_id_return_invalid(hdmi);
    drv_hdmi_capture temp;
    temp.pos_x = (td_u32)hdmi_capture->pos_x;
    temp.pos_y = (td_u32)hdmi_capture->pos_y;
    td_s32 ret = mpi_hdmi_set_capture(hdmi, &temp);

    hdmi_capture->color_b = temp.color_b;
    hdmi_capture->color_g = temp.color_g;
    hdmi_capture->color_r = temp.color_r;
    hdmi_debug_trace("temp.pos_x== %u,temp.pos_y= %u\n", temp.pos_x, temp.pos_y);
...
td_s32 ot_mpi_hdmi_avi_infoframe(ot_hdmi_id hdmi)
{
    hdmi_check_id_return_invalid(hdmi);
    drv_hdmi_infoframe_colorspace hdmi_infoframe_colorspace;
    hdmi_infoframe_colorspace.colorspace = 0;
    hdmi_infoframe_colorspace.id = (td_u32)hdmi;
    return mpi_hdmi_avi_infoframe(hdmi_infoframe_colorspace);
}

td_s32 ot_mpi_hdmi_avi_infoframe_colorspace(ot_hdmi_colorspace hdmi_colorspace)
{
    ot_hdmi_id hdmi = hdmi_colorspace.id;
    hdmi_check_id_return_invalid(hdmi);
    if (hdmi_colorspace.colorspace < 0) {
        hdmi_err_trace("the colorspace is out of range.\n");
        return TD_FAILURE;
    }
    drv_hdmi_infoframe_colorspace hdmi_infoframe_colorspace;
    hdmi_infoframe_colorspace.colorspace = hdmi_colorspace.colorspace;
    hdmi_infoframe_colorspace.id = hdmi_colorspace.id;
    return mpi_hdmi_avi_infoframe(hdmi_infoframe_colorspace);
}
#endif
...
{
    hdmi_check_id_return_invalid(hdmi);
    hdmi_check_boolval_return((td_u32)avmute_en);
