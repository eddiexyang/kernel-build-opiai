...
typedef struct {
    hi_u32 events;  /* event bit field */
    hi_void *data;  /* user data set by dvpp_epoll_ctl */
} hi_dvpp_epoll_event;

typedef struct {
    hi_mod_id mode;
    hi_s32 chn; // 内部通道id
    hi_csc_matrix_mode csc_matrix;
    hi_csc_coefficient csc_coefficient;
    hi_user_csc_coefficient user_csc_coefficient; // 只是保存用户态double参数，在内核不做运算
} hi_csc_param;

// add for svm smmu by s00605447
typedef struct {
    hi_u32 sid;
    hi_u32 ssid;
} hi_smmu_config;
