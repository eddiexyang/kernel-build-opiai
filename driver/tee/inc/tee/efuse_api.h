 ...
 * Description: efuse api
 * Author: liangguanchao
 * Create: 2019-04-18
 */
#ifndef EFUSE_API_H
#define EFUSE_API_H
...
#define EFUSE_WRITEED_DATA      (0x1)
#define EFUSE_NOT_WRITEED_DATA  (0x0)

/* 两块不同的efuse */
#define EFUSE_DIE_ID     (0x0)
#define EFUSE_S_DIE_ID   (0x1)
 ...
 * @retval #其它值, 失败
 */
int tf_write_efuse(uint8_t obj, uint32_t dest_bit, uint32_t dest_size, uint32_t *src);
 ...
 * @retval #其它值, 失败
 */
int tf_efuse_data_check(uint8_t obj, uint32_t dest_bit, uint32_t dest_size, uint32_t *input, uint32_t *out_flag);
...
*/
int tf_efuse_flash_power(uint8_t onoff);
