 ...
 * Create: 2020-05-15
 */
#ifndef __OPIAI_USER_CONFIG_MINIV2_H
#define __OPIAI_USER_CONFIG_MINIV2_H
 ...
 * 用户配置区静态选项
 */
#if (defined(CFG_SOC_MDC_V51_LITE))
#define CURRENT_USER_CONFIG_VERSION     1                       /* 当前软件能支持的用户配置的版本号 */
#define UC_FLASH_MAIN_ADDR              0x3800000               /* 配置所在的flash主区偏移地址 */
#define UC_FLASH_BK_ADDR                0x3880000                /* 配置所在的flash备区偏移地址 */
#else
#define CURRENT_USER_CONFIG_VERSION     1                       /* 当前软件能支持的用户配置的版本号 */
#define UC_FLASH_MAIN_ADDR              0xa40000                /* 配置所在的flash主区偏移地址 */
#define UC_FLASH_BK_ADDR                0xc40000                /* 配置所在的flash备区偏移地址 */
#endif

#if (defined(CFG_SOC_PLATFORM_HELPER))
#define UC_FLASH_PARTITION_NUM          60                      /* 用户配置分区数量 */
#define UC_FLASH_PARTITION_MAIN_NUM     30                      /* 用户配置区主区数量 */
#define UC_FLASH_PARTITION_BK_NUM       30                      /* 用户配置区备区数量 */
...
#define UC_FLASH_PARTITION_SIZE         (64 * 1024)             /* 用户配置分区大小(字节数) */
#else
#define UC_FLASH_PARTITION_NUM          64                      /* 用户配置分区数量 */
#define UC_FLASH_PARTITION_MAIN_NUM     32                      /* 用户配置区主区数量 */
#define UC_FLASH_PARTITION_BK_NUM       32                      /* 用户配置区备区数量 */
#define UC_FLASH_PARTITION_SIZE         (64 * 1024)             /* 用户配置分区大小(字节数) */
#endif
...
#define UC_ITEM_INDEX_USER_SHADOW       4
#define UC_ITEM_INDEX_SSH_STATUS        5

#ifdef CFG_SOC_PLATFORM_MDC_V51
#define UC_ITEM_MAX_NUM                 1                       /* 配置项数量 */
#else
...
#define UC_P2P_MEM_CFG_SIZE             (4 + 1)                 /* P2P MEM大小配置 */
...
#define UC_PSS_CFG_SIZE                 2                       /* PSS/PKCS 签名大小配置，已删除 */

/* 配置内容中各项偏移，注意：此偏移确定后不能再做修改，删除配置项时，此处不能删除偏移值 */
#define UC_CPU_CFG_OFFSET               0                       /* CPU数量配置偏移 */
#define UC_P2P_MEM_CFG_OFFSET           (UC_CPU_CFG_OFFSET + UC_CPU_CFG_SIZE) /* P2P MEM配置偏移 */
#define UC_DDR_ECC_CFG_OFFSET           (UC_P2P_MEM_CFG_OFFSET + UC_P2P_MEM_CFG_SIZE) /* DDR ECC 配置偏移 */
...
#define UC_PSS_CFG_OFFSET               (UC_SSH_STATUS_OFFSET + UC_SSH_STATUS_SIZE) /* PSS/PKCS签名配置项偏移，已删除 */
...
#else
    {CPU_NUM_CONFIG_NAME, -1, UC_CPU_CFG_OFFSET, UC_CPU_CFG_SIZE, UC_AUTHORITY_USER_WR, "0x010007", NULL},
    {"p2p_mem_cfg", -1, UC_P2P_MEM_CFG_OFFSET, UC_P2P_MEM_CFG_SIZE, UC_AUTHORITY_USER_WR, "0x0002", NULL},
    {"ddr_ecc_enable", -1, UC_DDR_ECC_CFG_OFFSET, UC_DDR_ECC_SIZE, UC_AUTHORITY_USER_WR, "0x01",
        ddr_ecc_enable_check_para},
