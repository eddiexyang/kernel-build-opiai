...
STATIC DRV_SERDES_FREQUENCY_LEVEL HilinkRateToSerdesFrequency(HilinkRate targetRate)
{
    switch (targetRate) {
        case HILINK_XGE_1_25G:
            return SERDES_FREQ_ETH_1_25G;
        case HILINK_XGE_3_125G:
            return SERDES_FREQ_ETH_3_125G;
        default:
            HILINK_ERR("targetRate %d out of range.\n", targetRate);
            return SERDES_FREQ_NULL;
    }
}
...
{
    u32 ret;
    DRV_SERDES_FREQUENCY_LEVEL serdesRate;
    CHECK_RETURN(macro, MACRO_MAX, RET_FAIL);
    CHECK_RETURN(dsIndex, DS_MAX, RET_FAIL);
    CHECK_RETURN(targetRate, HILINK_XGE_BUTT, RET_FAIL);
    serdesRate = HilinkRateToSerdesFrequency(targetRate);
    if (serdesRate == SERDES_FREQ_NULL) {
        return RET_FAIL;
    }
    SwitchControlMode(macro, dsIndex, CTRL_MODE_REG);
    EthScenarioDataRateConfig(macro, CS0, serdesRate);
    ret = DsDataRateSwitch(macro, dsIndex, targetRate, dsWidth);
    if (ret == RET_FAIL) {
        HILINK_ERR("HILINK_DsDataRateSwitch failed.\n");
        goto exit;
    }
    ret = eth_rx_para_config(macro, dsIndex, serdesRate);
    if (ret != RET_OK) {
...
u32 HILINK_LinkDataRateSwitch(u32 macro, u32 sliceMask, HilinkRate targetRate, u32 dsWidth)
{
    ...
    for (i = 0; i < DS_MAX; ++i) {
        if (((sliceMask >> i) & 0x1) == 0x1) {
            ret = eth_rx_para_config(macro, i, serdesRate);
            if (ret != RET_OK) {
                HILINK_ERR("Eth rx para configuration failed.\n");
            }
        }
