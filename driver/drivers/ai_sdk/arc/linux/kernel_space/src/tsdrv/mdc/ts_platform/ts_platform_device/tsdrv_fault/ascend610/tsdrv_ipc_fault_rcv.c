...
#ifndef TSDRV_UT
STATIC const char *get_fault_source(enum mdc_dev_node_id node_type_id)
{
    const char *fault_source[MDC_DEV_NODE_ID_MAX] = {
        "from hwts", "from tscpu", "from aic", "from sdma", "from aiv", "from ts", "from hwtscpu", "from tsfw"};
    return fault_source[node_type_id];
}

STATIC int tsdrv_get_ipc_event(dfm_event *event, struct ras_message *info, enum mdc_dev_node_id node_type_id)
{
#ifndef TSDRV_UT
    int ret;
...
STATIC void tsdrv_ipc_fault_sensor_scan_immediately(u32 devid, struct dfm_struct *dfm, struct ras_message *info)
{
    u32 i;
...
STATIC int tsdrv_ipc_fault_msg_para_check(u32 devid, struct notifier_block *nb, struct ipcdrv_message *ipc_msg)
{
    struct dfm_struct *dfm = NULL;
    enum mdc_dev_node_id node_type_id;
    struct ras_message *info = (struct ras_message *)(ipc_msg->ipcdrv_payload);
