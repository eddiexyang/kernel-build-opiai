...
STATIC void devdrv_get_aicore_info(struct devdrv_platform_data *pdata)
{
    ...
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_WARN("get aicore bitmap from tee warn, (dev=%u).\n", pdata->dev_id);
#endif
    }
    ...
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_WARN("get aicore freq from tee warn, (dev=%u).\n", pdata->dev_id);
#endif
    }
    TSDRV_PRINT_INFO("dev(%u) aicore num(%u) aicore freq(%u) bitmap(0x%llx).\n",
...
STATIC void devdrv_get_vector_core_info(struct devdrv_platform_data *pdata)
{
    ...
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_WARN("get vectorcore bitmap from tee warn, (dev=%u).\n", pdata->dev_id);
#endif
    }
    ...
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_WARN("get vectorcore freq from tee warn, (dev=%u).\n", pdata->dev_id);
#endif
    }
    TSDRV_PRINT_INFO("dev(%u) vectorcore num(%u) vectorcore freq(%u) bitmap(0x%llx).\n",
...
STATIC int devdrv_get_slot_type(u32 *slot_type)
{
    int ret;

    ret = devdrv_get_gpio_value(GPIO_266, slot_type);
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_ERR("get slot_type failed, ret = %d\n", ret);
        return ret;
#endif
    }
...
STATIC int devdrv_get_slot_id_in_slot_type_non_4p(u32 *slot_id)
{
#ifndef TSDRV_UT
    int ret;

    ret = devdrv_get_gpio_value(GPIO_262, slot_id);
    if (ret != 0) {
        TSDRV_PRINT_ERR("get slot_id in 2p_scene failed, ret = %d\n", ret);
        return ret;
    }
#endif
    return 0;
...
STATIC int devdrv_get_slot_id_in_slot_type_4p(u32 *slot_id)
{
    int32_t ret;
    u32 gpio262_value = 0;
    u32 gpio258_value = 0;

    ret = devdrv_get_gpio_value(GPIO_258, &gpio258_value);
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_ERR("get gpio value failed, ret = %d\n", ret);
        return ret;
#endif
    }

    gpio258_value = gpio258_value << SLOT_ID_OFFSET;

    ret = devdrv_get_gpio_value(GPIO_262, &gpio262_value);
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_ERR("get gpio value failed, ret = %d\n", ret);
        return ret;
    ...
    gpio258_value |= gpio262_value;

    *slot_id = gpio258_value;
...
STATIC int devdrv_get_slot_id_by_gpio(u32 *slot_id)
{
    ...
    ret = devdrv_get_slot_type(&slot_type);
    if (ret != 0) {
        TSDRV_PRINT_ERR("get slot_type failed, ret = %d\n", ret);
        return ret;
    }

    if (slot_type == SLOT_TYPE_4P) {
        ret = devdrv_get_slot_id_in_slot_type_4p(slot_id);
        if (ret != 0) {
#ifndef TSDRV_UT
            TSDRV_PRINT_ERR("get slot_id in 4p_scene failed, (ret=%d)\n", ret);
            return ret;
#endif
        }
    } else if (slot_type == SLOT_TYPE_NON_4P) {
#ifndef TSDRV_UT
        ret = devdrv_get_slot_id_in_slot_type_non_4p(slot_id);
        if (ret != 0) {
            TSDRV_PRINT_ERR("get slot_id in non_4p_scene failed, (ret=%d)\n", ret);
            return ret;
        }
#endif
    } else {
#ifndef TSDRV_UT
        TSDRV_PRINT_ERR("slot_type is invalid, slot_type = %u\n", slot_type);
        return -EINVAL;
#endif
    }
...
EXPORT_SYMBOL(get_devdrv_affinity_cpuid);
...
int tsdrv_get_aicpu_occupy_bitmap(u32 dev_id, u32 plat_type, u32 *bitmap)
{
    ...
    struct cpu_domain_info domain_info = {0};
    u64 total_core_num;
    u32 i;
    int err;
