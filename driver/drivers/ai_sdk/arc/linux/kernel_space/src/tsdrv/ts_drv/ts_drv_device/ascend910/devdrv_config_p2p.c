...
#define CUR_MSECS_TO_JIFFIES 1000
    ...
    static u32 configed_devid[TSDRV_MAX_DAVINCI_NUM][TSDRV_MAX_DAVINCI_NUM] = {{DEVID_NOT_CONFIG}};
    ...
    for (devid = 0; devid < num_dev; devid++) {
        for (local_id = 0; local_id < num_dev; local_id++) {
            if (configed_devid[devid][local_id] == DEVID_CONFIGED) {
                continue;
            }
            flag = devdrv_manager_get_devid_flag(local_id);
            if (flag == 0) {
                continue;
            }
            host_id = devdrv_manager_get_devid(local_id);
            if (host_id >= TSDRV_HOST_MAX_DAVINCI_NUM) {
                continue;
            }
            ...
            ret = devdrv_send_hwts_addr_to_ts(devid, 0, local_id, host_id);
            if (ret != 0) {
                TSDRV_PRINT_ERR("devdrv_send_hwts_addr_to_ts failed, ret(%d).\n", ret);
            }
            TSDRV_PRINT_INFO("p2p config: host_id(%u) local_id(%u) devid(%u).\n", host_id, local_id, devid);
            configed_devid[devid][local_id] = DEVID_CONFIGED;
            configed_dev_num++;
        }
    }

    if (configed_dev_num < num_dev * num_dev) {
...
void tsdrv_init_taskid_share_memory(u32 devid, u32 tsnum, u32 chipid, u32 dieid)
{
    ...
    shm_addr = TASKID_SHARE_MEM_START_ADDR + chipid * CHIP_BASEADDR_PA_OFFSET + dieid * DIE_BASEADDR_PA_OFFSET;

    TSDRV_PRINT_INFO("devid(%u), chipid(%u), dieid(%u), shm_addr(0x%pK).\n", devid, chipid, dieid, (void *)shm_addr);
