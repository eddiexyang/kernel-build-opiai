 ...
 */
STATIC soc_verify_func *devdrv_lookup_soc_verify_symbol(void)
{
    soc_verify_func *func = NULL;

    if (func == NULL) {
        func = (soc_verify_func *)(uintptr_t)__kallsyms_lookup_name("soc_verify");
        if (IS_ERR_OR_NULL(func)) {
#ifndef TSDRV_UT
            TSDRV_PRINT_ERR("fail to find symbol soc_verify\n");
            return NULL;
#endif
        }
    }
...
STATIC int devdrv_tsfw_soc_verify_thread(void *data)
{
    ...
    tsfw_soc_verify = devdrv_lookup_soc_verify_symbol();
...
STATIC int tsdrv_file_soc_verify(u32 dev_id, int file_type, void *file_src_addr, u32 file_size)
{
#define SLEEP_MS 10
