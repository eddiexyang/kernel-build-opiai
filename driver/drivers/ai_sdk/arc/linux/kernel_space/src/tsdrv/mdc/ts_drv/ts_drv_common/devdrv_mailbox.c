...
STATIC int devdrv_mailbox_init(u32 devid, struct devdrv_mailbox *mailbox)
{
    ...
    if (mailbox->send_sram != NULL) {
        ret = memset_s(mailbox->send_sram, DEVDRV_MAILBOX_PAYLOAD_LENGTH, 0, DEVDRV_MAILBOX_PAYLOAD_LENGTH);
        if (ret != 0) {
#ifndef TSDRV_UT
            TSDRV_PRINT_WARN("memset mailbox send addr, (ret=%d).\n", ret);
#endif
        }
    }
...
STATIC void devdrv_mailbox_data_send(struct devdrv_mailbox *mailbox, struct devdrv_mailbox_message *message)
{
    ...
    if (env_type == TSDRV_ENV_OFFLINE) {
        ...
        if (header->valid != 0U) {
            TSDRV_PRINT_WARN("valid is not as espected, (devid=%u, tsid=%u, valid=0x%x, result=%u, cmd_type=%u).\n",
                ts_dev->devid, tsid, (u32)header->valid, header->result, (u32)header->cmd_type);
        }
        ...
        TSDRV_PRINT_WARN("invalid type, (env_type=%u).\n", (u32)env_type);
    }
#ifndef TSDRV_UT
    ...
    for (i = 0; i < DEVDRV_MAILBOX_DOWN_TRY_TIMES; i++) {
        ...
        if (ret != 0) {
            ...
            TSDRV_PRINT_WARN("mailbox down timeout, i(%d), start[%lu s %lu us], end[%lu s %lu us], irq[%ld s %ld us], "
                "ret(%d), valid(%u), result(%u), cmd_type(0x%x)\n",
