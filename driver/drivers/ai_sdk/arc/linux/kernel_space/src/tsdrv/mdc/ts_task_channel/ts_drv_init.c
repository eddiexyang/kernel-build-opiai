...
STATIC int ts_fop_send_msg_to_ts_async(unsigned long arg)
{
...
STATIC int ts_fop_aicpu_record_msg(unsigned long arg)
{
    struct ts_ioctl_para para_info;
...
STATIC int ts_fop_reset_stream(unsigned long arg)
{
#if !defined(CFG_SOC_PLATFORM_CLOUD_V2) && !defined(CFG_SOC_PLATFORM_MINIV3)
    ts_drv_err("Not support.");
    return -ESRCH;
#else
    struct ts_ioctl_para para_info;

    if (copy_from_user_safe(&para_info, (void *)(uintptr_t)arg, sizeof(struct ts_ioctl_para)) != 0) {
        ts_drv_err("Failed to copy_from_user arg.\n");
        return TS_COPY_USER_ERR;
    }

    return trs_reset_stream(para_info.dev_id, para_info.ts_id, current->tgid, para_info.u.stream_id);
#endif
}
...
STATIC long ts_fop_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
    ...
    return ts_ioctl_handler[_IOC_NR(cmd)](arg);
}
...
STATIC int __init ts_drv_platform_init(void)
{
    int ret;
#ifdef CFG_SOC_PLATFORM_MDC_V51
...
STATIC void __exit ts_drv_platform_exit(void)
{
