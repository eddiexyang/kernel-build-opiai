...
#ifdef CFG_SOC_MDC_V51_LITE
#define TS0CPU_BASE_ADDR 0xb9060000
#define TS0HWTSCPU_BASE_ADDR 0xb9070000
#else
...
#define BASE_SPEC_ATS_ADDR_LEN1_VALUE 0x120FFFFF
#define PREMIUM_SPEC_ATS_ADDR_LEN1_VALUE 0x12AFFFFF
#define ULTIMATE_SPEC_ATS_ADDR_LEN1_VALUE 0x13AFFFFF
#define ATS_ADDR_LEN1_VALUE 0x1B400000

STATIC u32 tsdrv_get_ats_addr_len1_value(u32 devid)
{
    ...
    if (dev_info == NULL) {
        ...
        return ATS_ADDR_LEN1_VALUE;
    }
    switch (dev_info->pdata->ts_spec) {
        case HWTS_SPEC_BASE:
            return BASE_SPEC_ATS_ADDR_LEN1_VALUE;
        case HWTS_SPEC_PREMIUM:
            return PREMIUM_SPEC_ATS_ADDR_LEN1_VALUE;
        case HWTS_SPEC_ULTIMATE:
            return ULTIMATE_SPEC_ATS_ADDR_LEN1_VALUE;
        default:
            break;
    }
    TSDRV_PRINT_INFO("Get default ats addr len1 values.\n");
    return ATS_ADDR_LEN1_VALUE;
}
...
void tsdrv_soc_startup(u32 tsid, u32 devid, u64 boot_addr)
{
    ...
    TSDRV_PRINT_DEBUG("begin to startup tsdrv firmware.(tsid=%u, startup_base=%pK ts_ats_addr=%pK)\n",
    ...
    if (tsid == 1) { /* tsv for mdc, ats0 can't used under un-security mode, use ats1 instead */
        ...
        reg_val = tsdrv_get_ats_addr_len1_value(devid);
        writel(reg_val, (void *)(tscpu_addr + SC_ATS_ADDR_LEN1));
...
int tsdrv_soc_reset_tscpu(void __iomem *sysctrl, uint32_t tsid)
{
    ...
    while (1) {
        ...
        usleep_range(DEVDRV_POLL_TS_RETRY_INTERVAL, DEVDRV_POLL_TS_RETRY_INTERVAL_MAX);
        times++;
