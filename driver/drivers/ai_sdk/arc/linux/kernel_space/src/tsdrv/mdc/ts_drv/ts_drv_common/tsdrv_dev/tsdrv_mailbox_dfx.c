...
ssize_t mailbox_proc_dfx_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    dfx_id = data->dfx_id;
    tsid = (dfx_id >> (TSDRV_DFX_DEV_OFFSET + TSDRV_DFX_FID_OFFSET)) & TSDRV_DFX_TS_MASK;
    fid = (dfx_id >> TSDRV_DFX_DEV_OFFSET) & TSDRV_DFX_FID_MASK;
    devid = dfx_id & TSDRV_DFX_DEV_MASK;
