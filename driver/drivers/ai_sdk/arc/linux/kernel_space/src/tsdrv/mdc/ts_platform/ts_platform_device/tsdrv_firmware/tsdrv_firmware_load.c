...
#ifdef CFG_SOC_PLATFORM_CLOUD_V2
    config = (struct aicpu_system_config *)ioremap(SYSTEM_CONFIG_BASE +
        dev_info->chip_id * CHIP_BASEADDR_PA_OFFSET +
        dev_info->die_id * DIE_BASEADDR_PA_OFFSET, SYSTEM_CONFIG_SIZE);
#else
...
#ifndef AOS_LLVM_BUILD
#ifndef TSDRV_UT
    valid_nid_type = devdrv_config_ts_valid_nid_type(&nid_num);
    if (nid_num > TS_MEM_VALID_RANGE_MAX_NUM || valid_nid_type == NULL) {
        devdrv_drv_err("valid_nid_type is invalid, nid_num: %u, max nid_num: %u.\n", nid_num,
            TS_MEM_VALID_RANGE_MAX_NUM);
        return;
    }

    config->valid_range_num = nid_num;
    for (i = 0; i < nid_num; i++) {
        int nid = devdrv_manager_devid_to_nid(dev_info->dev_id, valid_nid_type[i]);
        if (nid == NUMA_NO_NODE) {
            devdrv_drv_err("Failed to get nid. (devid=%u; nid=%d)\n", dev_info->dev_id, nid);
            return;
        }
        config->addr_ranges[i].start = __pfn_to_phys(node_start_pfn(nid));
        config->addr_ranges[i].end = __pfn_to_phys(node_end_pfn(nid));
    }
    devdrv_drv_info("devid %u, nid_num %u.\n", dev_info->dev_id, nid_num);
#endif
#else
    devdrv_drv_info("ts_mem_restrict_valid == valid, devid %u\n", dev_info->dev_id);
#endif
}
...
#define RISCV_HEAD_SIZE 0x20
    ...
    ret = memset_s(riscv_vir_addr, RISCV_HEAD_SIZE, 0, RISCV_HEAD_SIZE);
    if (ret != 0) {
#ifndef TSDRV_UT
        devdrv_drv_err("memset_s riscv addr fail, (ret = %d).", ret);
...
int devdrv_load_ffts_fw(struct devdrv_info *dev_info, u32 tsid)
{
    ...
    ffts_reserved_addr = FFTS_RESERVED_ADDR + chipid * CHIP_BASEADDR_PA_OFFSET + dieid * DIE_BASEADDR_PA_OFFSET;
    devdrv_drv_info("devid(%u), chipid(%u), die(%u)\n", dev_info->dev_id, chipid, dieid);
    ...
    ret = devdrv_ffts_dereset(dev_info, tsid, ffts_reserved_addr);
...
#endif
        devdrv_drv_info("TS_SYSCTRL reg_ral[%u] = %u.\n", i, reg_val);
    }
}
...
int devdrv_load_cpu_fw(struct devdrv_info *dev_info, u32 tsid)
{
    ...
    if (ret != 0) {
#ifndef TSDRV_UT
        devdrv_drv_warn("Can not get aicpu dts config. (ret=%d)", ret);
#endif
    }
...
#endif
        devdrv_drv_warn("ai cpu exist broken core, (dev_id=%u, ai_cpu_num=%u, err_bitmap=0x%x).\n",
...
#endif
        devdrv_drv_warn("ai core exist broken core, (dev_id=%u, ai_core_num=%u, err_bimap=0x%x).\n",
...
void devdrv_get_active_core(struct devdrv_info *dev_info)
{
    ...
    if (ret != 0) {
#ifndef TSDRV_UT
        devdrv_drv_warn("down timeout, do not get TS core info ipc in 5s.\n");
#endif
        return;
    }
}
