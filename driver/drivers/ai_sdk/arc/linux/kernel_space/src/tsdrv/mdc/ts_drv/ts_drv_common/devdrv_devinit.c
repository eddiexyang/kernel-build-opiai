...
/* index 0: cq 0-1023(bit 0: 0-31, bit 1: 32-63 ...), 1: cq 1024-2047 */
...
#ifdef CFG_SOC_PLATFORM_MDC_V51
    [_IOC_NR(TSDRV_BIND_STL)] = tsdrv_ioctl_bind_stl_operator,
#endif
};
...
int devdrv_open(struct inode *inode, struct file *filep)
{
    ...
    if (err != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_WARN("set runtime run, devid=%u\n", devid);
        tsdrv_dev_ref_dec(devid, TSDRV_PM_FID);
        return err;
#endif
    }
    ...
    struct devdrv_ts_cq_info *cq_info, struct devdrv_cq_sub_info *cq_sub_info)
{
    ...
    while (devdrv_report_get_phase(report) == cq_info->phase) {
        ...
        if ((sqid >= DEVDRV_MAX_SQ_NUM) || (sq_head >= DEVDRV_MAX_SQ_DEPTH)) {
            TSDRV_SPINLOCK_ERR("wrong sqid or sq_head from cq, sq_id(%u), sq_head(%u).\n", sqid, sq_head);
            break;
        }
        ...
        if (!tsdrv_is_in_pm(devid)) {
            ...
            if (sq_id_info == NULL) {
                TSDRV_SPINLOCK_ERR("invalid para, devid(%u), fid(%u), tsid(%u), sqid(%d)\n",
...
long devdrv_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
    ...
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_ERR("invalid param, cmd_type=%u cmd_nr=%u, arg=%pK, ctx=%pK\n",
...
{
#ifndef TSDRV_UT
    TSDRV_PRINT_DEBUG("devdrv vm fault. flags:0x%x, start:%pK, end:%pK\n",
...
{
    TSDRV_PRINT_DEBUG("devdrv vm fault. start:%pK, end:%pK\n",
...
STATIC const struct notifier_operations notifier_ops = {
...
#ifndef TSDRV_UT
static bool tsdrv_is_cq_valid(u32 devid, u32 tsid, u32 group, u32 cqid)
{
    void __iomem *io_base = tsdrv_get_stars_sqcq_intr_hwinfo_addr(devid, tsid);
    u32 cqid_in_group = cqid % STARS_INTR_CQ_NUM_PER_GROUP;
    u32 reg_index = cqid_in_group / STARS_INTR_REG_BIT_NUM;
    u32 offset = cqid_in_group % STARS_INTR_REG_BIT_NUM;
    u32 vf_offset = 0;
    u32 l2_val;

    if (devdrv_get_pfvf_type_by_devid(devid) == DEVDRV_SRIOV_TYPE_VF) {
        vf_offset = STARS_CQ_INT_VF_OFFSET;
    }
    l2_val = readl(io_base + STARS_L2_NORMAL_CQE_WRITTEN_NS_STATUS2(group, reg_index) - vf_offset);

    writel(((0x1 << offset) & l2_val), io_base + STARS_L2_NORMAL_CQE_WRITTEN_NS_CLR2(group, reg_index) - vf_offset);
    return (((0x1 << offset) & l2_val) != 0);
}
#endif
...
#if defined(CFG_SOC_PLATFORM_CLOUD_V2) || defined(CFG_SOC_PLATFORM_MINIV3)
    if (!tsdrv_is_cq_valid(devid, tsid, cq_index / STARS_INTR_CQ_NUM_PER_GROUP, cq_index)) {
        return true;
    }
#endif
...
STATIC void tsdrv_rr_cq_proc(struct devdrv_int_context *int_context)
{
    ...
    for (cq_index = int_context->first_cq_index; cq_index <= int_context->last_cq_index; cq_index++) {
        ...
        TSDRV_PRINT_DEBUG("receive report irq, (cq_index=%d)", cq_index);
        ...
        report = tsdrv_normal_cq_get_report(env_type, cq_sub_info, cq_info, cq_info->head);
        fid = tsdrv_get_fid_by_ctx(ctx);
        if (tsdrv_stream_is_bind_logic_cq(devid, fid, tsid, report, cq_info->phase)) {
...
STATIC int32_t tsdrv_set_irq_cpumask(u32 devid, u32 irq, const char *irq_node_name, const char *irq_cpu_name)
{
...
{
    return bind_irq_to_core(irq, irq_node_name, irq_cpu_name);
}
#endif
#endif
...
int devdrv_drv_register(struct devdrv_info *dev_info)
{
    ...
    err = tsdrv_alloc_resv_sqcq(devid, TSDRV_PM_FID, 0);
    if (err != 0) {
        goto err_resv_sqcq_init;
    }
    ...
    return 0;
err_ts_sqcq_init:
    tsdrv_free_resv_sqcq(devid, TSDRV_PM_FID, 0);
err_resv_sqcq_init:
    if (tsdrv_get_pfvf_type_by_devid(devid) == DEVDRV_SRIOV_TYPE_PF) {
