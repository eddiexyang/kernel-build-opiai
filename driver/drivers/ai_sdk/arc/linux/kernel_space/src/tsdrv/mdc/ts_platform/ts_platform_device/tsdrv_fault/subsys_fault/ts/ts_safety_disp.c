...
#define TS_DISP_SAFETY_CONVERGE_NODE(status_reg, bit_id, msg, _sensor_type, ras_err) \
        DISP_SAFETY_CONVERGE_NODE(HISI_SUBSYS_TSSUB,            \
...
static struct ras_fault_converge_item g_disp_converge_table[] = {
    /* safety err bit of DISP_INTSTAT2 */
    TS_DISP_SAFETY_CONVERGE_NODE(0x0820,
    ...
    TS_DISP_SAFETY_CONVERGE_NODE(0x0820,
        PORT2_TIMEOUT, "port 2 timeout", DMS_SEN_TYPE_RAS_SENSOR, 0xA),
    TS_DISP_SAFETY_CONVERGE_NODE(0x0820,
        PARITY_REG_CHECK, "parity err", DMS_SEN_TYPE_RAS_SENSOR, 5),
};

int ts_disp_init_safety_cfg(struct subsys_safety_cfg *safety_cfg)
{
    static reg_offset_t g_ts_disp_status_regs[] = {
        0x0820
    };

    safety_cfg->dump_reg_num = ARRAY_SIZE(g_disp_dump_regs);
    safety_cfg->status_reg_num = ARRAY_SIZE(g_ts_disp_status_regs);
    safety_cfg->dump_regs = g_disp_dump_regs;
    safety_cfg->status_regs = g_ts_disp_status_regs;
    ...
    safety_cfg->safety_int_entry.bit_mask[1] = 0;
    ...
    safety_cfg->safety_err_entry.bit_mask[0] = 0x3FEFFFFF;
...
#else
void ts_disp_init_safety_stub(int devid)
{
    return;
}
#endif
