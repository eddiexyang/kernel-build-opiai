...
#define DEVDRV_CQSQ_RESERVED_MEM_ADDR (0x2C381000U) /* reserve mem: start addr + offset */
#define DEVDRV_CQSQ_RESERVED_MEM_END (0x2C981000U - 0x20000U) /* 0x20000 reserved for callback cq buffer */
#define DEVDRV_CQSQ_MAX_SIZE (128U * 1024U)

#define TSDRV_GET_RESERVED_MEM_MAX_TIME (30U)
STATIC int devdrv_get_cqsq_reserved_mem(u32 devid, u32 size, u64 *rsv_addr)
{
    static atomic64_t malloc_addr = ATOMIC_INIT(DEVDRV_CQSQ_RESERVED_MEM_ADDR);
    u64 rsv_malloc_addr;
    u64 addr_old;
    u32 i;
    ...
    for (i = 0; i < TSDRV_GET_RESERVED_MEM_MAX_TIME; i++) {
        addr_old = (u64)atomic64_read(&malloc_addr);
        rsv_malloc_addr = addr_old + size;
        if (rsv_malloc_addr >= DEVDRV_CQSQ_RESERVED_MEM_END) {
            break;
        }

        if ((u64)atomic64_cmpxchg(&malloc_addr, addr_old, addr_old + size) == addr_old) {
            *rsv_addr = addr_old + ((u64)devid * CHIP_BASEADDR_PA_OFFSET);
            return 0;
        }
    }
    TSDRV_PRINT_ERR("Can not get cqsq reserved memory, (looptime=%u).\n", i);
    return -ENOMEM;
}
#endif

static inline void devdrv_functional_free_sqcq_memory(u8 *addr)
{
    if (addr != NULL) {
...
STATIC void devdrv_heart_beat_cq_callback(struct devdrv_functional_cq_info *cq_info)
{
    ...
    while (cq_slot[0] == cq_info->phase) {
        ...
        if (sq_index >= DEVDRV_MAX_FUNCTIONAL_SQ_NUM) {
            TSDRV_SPINLOCK_ERR("invalid index:%u.\n", sq_index);
            return;
        }
        sq_info = &cqsq->sq_info[sq_index];
...
#ifndef AOS_LLVM_BUILD
        cq_info[i].wq = alloc_workqueue("devdrv-cqsq-work", WQ_UNBOUND, 1);
#else
        cq_info[i].wq = create_singlethread_workqueue("devdrv-cqsq-work");
#endif
#else
...
#ifdef CFG_SOC_PLATFORM_MDC_V51
        ret = tsdrv_set_workqueue_affinity(cq_info[i].wq, 0);
        if (ret != 0) {
#ifndef TSDRV_UT
            TSDRV_PRINT_WARN("Can not bind devdrv-functional-cq irq core, (dev_id=%u, ret=%d).\n", devid, ret);
#endif
        }
#endif
...
STATIC int devdrv_get_functional_sq_memory(u32 devid, struct devdrv_functional_sq_info *sq_info, u32 size)
{
    ...
    env_type = tsdrv_get_env_type();
    if (env_type == TSDRV_ENV_OFFLINE) {
#if defined(CFG_SOC_PLATFORM_MINIV3) || !defined(CFG_SOC_PLATFORM_MINIV2)
        sq_info->addr = kzalloc_node(size, GFP_KERNEL | __GFP_THISNODE,
...
void devdrv_destroy_functional_sq(u32 devid, u32 tsid, u32 sq_index)
{
    ...
    devdrv_functional_free_sqcq_memory(sq_addr);
}
...
STATIC int devdrv_get_functional_cq_memory(u32 devid, struct devdrv_functional_cq_info *cq_info, u32 size)
{
    ...
    if (env_type == TSDRV_ENV_OFFLINE) {
#if defined(CFG_SOC_PLATFORM_MINIV3) || !defined(CFG_SOC_PLATFORM_MINIV2)
        cq_info->addr = kzalloc_node(size, GFP_KERNEL | __GFP_THISNODE,
            devdrv_manager_devid_to_nid(devid, (u32)DEVDRV_TS_NODE_DDR_MEM));
        if (cq_info->addr == NULL) {
...
void devdrv_destroy_functional_cq(u32 devid, u32 tsid, u32 cq_index)
{
    ...
    if (cq_info[cq_index].addr != 0) {
        ...
        if (env_type == TSDRV_ENV_OFFLINE) {
            devdrv_functional_free_sqcq_memory(cq_addr);
        } else if (env_type == TSDRV_ENV_ONLINE) {
...
int devdrv_mailbox_send_cqsq(u32 devid, u32 tsid, struct devdrv_mailbox_cqsq *cqsq)
{
    ...
    if (ts_resource == NULL) {
#ifndef TSDRV_UT
        TSDRV_PRINT_ERR("device not exist, (dev_id=%u, tsid=%u).\n", devid, tsid);
        return -ENODEV;
#endif
    }
