    ...
    phys_addr_t *paddr, size_t *size, u32 addr_index)
{
    ...
    for_each_child_of_node(node, son) {
        ...
        if (of_tsid == tsid) {
            if (of_property_read_u64(son, dts_ts_addr_name[addr_index], (u64 *)paddr) != 0) {
    ...
    phys_addr_t *paddr, size_t *size, u32 addr_index)
{
    ...
    ret = device_property_read_u64(&pdev->dev, dts_ts_addr_name[addr_index], (u64 *)paddr);
    if (ret != 0) {
...
u32 tsdrv_get_ts_num(u32 devid)
{
    ...
    if (acpi_disabled) {
        ...
        if (device_property_read_u32(&pdev->dev, "tsid", &tsid) == 0) {
            ...
            TSDRV_PRINT_WARN("read tsid warn.\n");
            ts_num = 1;
        }
    }
...
int tsdrv_get_partial_good(u32 devid, u32 *enable)
{
    ...
    if (ret != 0) {
#ifndef TSDRV_UT
        TSDRV_PRINT_ERR("Get aicpu partial good enable failed. (ret=%d)\n", ret);
#endif
        return -EINVAL;
    }
