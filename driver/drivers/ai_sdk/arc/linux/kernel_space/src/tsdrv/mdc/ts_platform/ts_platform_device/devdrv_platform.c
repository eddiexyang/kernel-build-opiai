...
STATIC u32 tscpu_start_irq[MAX_CHIP_NUM] = {0, };
STATIC u32 tscpu_irq_num[MAX_CHIP_NUM] = {0, };
...
#define STARTS_FFTS_BASE_ADDR 0x03C00000
#define FFTS_CC2_CTRL         0x200000
#define FFTS_CC2_CTRL_SIZE    0x20

int devdrv_get_stars_c2c_ctrl_reg(u32 dev_id, u64 *addr, u64 *size)
{
    int ret;

    ret = devdrv_get_stars_reg_base(dev_id, addr);
    if (ret != 0) {
        TSDRV_PRINT_ERR("get reg base failed, dev_id=%u\n", dev_id);
        return ret;
    }

    *addr += (STARTS_FFTS_BASE_ADDR + FFTS_CC2_CTRL);
    *size = FFTS_CC2_CTRL_SIZE;

    return 0;
}
EXPORT_SYMBOL(devdrv_get_stars_c2c_ctrl_reg);
#endif
#endif /* TSDRV_UT */
...
STATIC int devdrv_get_hardware_version(struct devdrv_info *dev_info, unsigned int *hardware_version)
{
    ...
    *hardware_version = SOC_HARDWARE_VERSION_ASCEND610LITE;
#else
...
STATIC struct devdrv_info *devdrv_devinfo_create(struct platform_device *pdev, struct devdrv_platform_data *pdata)
{
    ...
    if (pdata->platform_info.occupy_bitmap == 0U) {
        dev_info->ai_cpu_core_num = 0U;
        dev_info->ai_cpu_core_id = 0U;
    } else {
        dev_info->ai_cpu_core_num = __fls(pdata->platform_info.occupy_bitmap) -
                                    __ffs(pdata->platform_info.occupy_bitmap) + 1U;
...
STATIC int devdrv_device_register_no_tsnode(struct platform_device *pdev)
{
    ...
    uda_dev_para_pack(&para, dev_info->dev_id, UDA_INVALID_UDEVID, HISI_MINI_V2, dev_info->dev);
...
#if (!defined AOS_LLVM_BUILD) && (!defined TSDRV_UT)
uda_add_dev_failed:
#ifndef AOS_LLVM_BUILD
...
#ifdef CFG_SOC_PLATFORM_MDC_V51
    /* In multi-domain scenarios, the Trace Server can be deployed on the
     * AOSCORE or TAIShan side.  The deployment mode is determined by
     * whether the TS node is configured in the DTS.
     *
     * If the TS node does not exist in the DTS in the current environment,
     * only device information is registered for device management.
     */
    if (devdrv_get_ts_node_num() == 0) {
        TSDRV_PRINT_INFO("The TS node does not exist in the DTS, do not need to load the TS core.\n");
...
#else
    err = trs_device_init(dev_info->dev_id);
#endif
    if (err != 0) {
...
STATIC int tscpu_device_probe(struct platform_device *pdev)
{
    ...
    if (!acpi_disabled) {
        ...
        for (i = 0; i < irq_num; i++) {
            ...
            (void)devdrv_set_irq_affinity(tscpu_irq[i], get_cpu_mask(IRQ_ROUTE_TS_CPU_ID));
#endif
        }
#ifndef CFG_SOC_PLATFORM_CLOUD_V2
...
#endif
STATIC int ffts_cpu_device_probe(struct platform_device *pdev)
{
#ifndef TSDRV_UT
