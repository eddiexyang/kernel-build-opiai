...
#define GET_SYNC_FLAG(x) (((x) >> 15) & 0x1)
...
STATIC u32 tsdrv_get_logic_cq_id_max_num(void)
{
...
{
#ifndef CFG_FEATURE_STARS
    uint32_t cq_size = sizeof(struct tag_ts_logic_cq_report_msg);
#else
    uint32_t cq_size = sizeof(struct tag_stars_logic_cq_report_msg);
#endif
    if (arg->cq_size != cq_size) {
        TSDRV_PRINT_ERR("invalid cq size, cqeSize=%u cqeSizeRange:[%u %u]\n", arg->cq_size,
            cq_size, cq_size);
        return -ENODEV;
    }
...
STATIC struct logic_cq_sub *logic_cqid_get(struct logic_cq_info *logic_cq)
{
    ...
    u32 useflag;
    u32 cqid;
    u32 logic_cq_used_num;

    spin_lock_bh(&logic_cq->lock);
    logic_cq_used_num = TSDRV_MAX_LOGIC_CQ_NUM - logic_cq->logic_cq_num;
    if (logic_cq_used_num >= tsdrv_get_logic_cq_id_max_num()) {
        spin_unlock_bh(&logic_cq->lock);
        TSDRV_PRINT_ERR("Logic_cq exceeds the specification, (logic_cq_used_num=%u, max_num=%u).\n",
            logic_cq_used_num, tsdrv_get_logic_cq_id_max_num());
        return NULL;
    }
...
#endif
    err = logic_cqid_init(logic_cq, logic_ctx, sub_cq);
#ifndef TSDRV_UT
    if (err != 0) {
...
STATIC int logic_cq_free(struct tsdrv_ctx *ctx, u32 tsid, struct logic_cq_free_para *free)
{
...
#endif
    sub_cq = logic_cqid_exit(logic_cq, logic_ctx, free->cq_id);
#ifndef TSDRV_UT
    if (sub_cq == NULL) {
        TSDRV_PRINT_ERR("cqid=%u isn't allocated by tgid=%d, devid=%u fid=%u tsid=%u\n", free->cq_id, logic_ctx->tgid,
            devid, fid, tsid);
        return -EINVAL;
    }
#endif
    logic_cq_mem_exit(sub_cq);
    logic_cqid_put(logic_cq, sub_cq);
    logic_cq_wakeup_ctx(logic_ctx, free->cq_id);
...
STATIC int logic_sqcq_phy_cq_free(struct tsdrv_ctx *ctx, u32 tsid, struct logic_sqcq_ts_ctx *logic_ctx)
{
#ifdef CFG_FEATURE_STARS
    return 0;
#else
...
STATIC void logic_sqcq_mbox_free_msg_pack(struct logic_cqsq_mbox_msg *mbox_msg,
    struct logic_sqcq_ts_ctx *logic_ctx, struct logic_cq_free_para *free_logic_cq, u32 fid)
{
#ifdef CFG_FEATURE_STARS
    return;
#endif
    ...
    mbox_msg->free.logic_cqid = free_logic_cq->cq_id;
    ...
    u32 cqe_start, u32 cqe_end, u32 *num)
{
...
#else
    struct tag_stars_logic_cq_report_msg *report = NULL;
#endif
    u8 match_flag;
    u32 cqe_cnt, report_cnt, i, pos, search_start, search_num, stream_id;
    ...
    */
    for (i = 0; i < cqe_cnt; i++) {
        pos = cqe_start + i;
#ifndef CFG_FEATURE_STARS
        report = (struct tag_ts_logic_cq_report_msg *)(sub_cq->vaddr + ((unsigned long)pos * sub_cq->size));
        match_flag = report->match_flag;
        stream_id = report->stream_id;
#else
        report = (struct tag_stars_logic_cq_report_msg *)(sub_cq->vaddr + ((unsigned long)pos * sub_cq->size));
        match_flag = GET_SYNC_FLAG(report->stream_id);
        ...
        TSDRV_PRINT_DEBUG("Cmp."
            ...
            sub_cq->cqid, pos, (u32)match_flag, (u32)report->stream_id, (u32)report->task_id,
            wait->stream_id, wait->task_id);

        if (match_flag == 1) {
            /* matched, not copy the following cqes. */
            if ((stream_id == wait->stream_id) && (report->task_id == wait->task_id)) {
                if (search_num == 0) {
                    search_start = pos;
                }
                search_num++;
...
{
#ifdef CFG_FEATURE_STARS
    struct tag_stars_logic_cq_report_msg *report_s = NULL, *report_d = NULL;
#else
...
#ifdef CFG_FEATURE_STARS
    report_s = (struct tag_stars_logic_cq_report_msg *)(sub_cq->vaddr + ((unsigned long)src_last * sub_cq->size));
    report_d = (struct tag_stars_logic_cq_report_msg *)(sub_cq->vaddr + ((unsigned long)dst_last * sub_cq->size));
#else
    ...
    struct logic_cq_wait_cq_para *wait)
{
    ...
    if (unlikely(sub_cq->vpid != (u32)logic_ctx->tgid)) {
        spin_unlock_bh(&logic_cq->lock);
...
retry:
        ...
        TSDRV_PRINT_DEBUG("wait event time out, err=%d devid=%u fid=%u tsid=%u cqid=%u\n", err, devid,
            fid, tsid, wait->cqid);
#else
        TSDRV_PRINT_WARN("wait event time out, err=%d devid=%u fid=%u tsid=%u cqid=%u\n", err, devid,
            fid, tsid, wait->cqid);
#endif
            return err;
        }
...
int logic_ioctl_cq_alloc(struct tsdrv_ctx *ctx, struct devdrv_ioctl_arg *arg)
{
    ...
    struct logic_sqcq_ts_ctx *logic_ctx = NULL;
    struct logic_cq_free_para free_logic_cq;
    u32 devid = tsdrv_get_devid_by_ctx(ctx);
    struct logic_cq_info *logic_cq = NULL;
    u32 fid = tsdrv_get_fid_by_ctx(ctx);
    ...
    TSDRV_PRINT_DEBUG("Alloc logic cq. (logic_cqid=%u; devid=%u; tsid=%u; tgid=(%d, %d)\n", alloc_logic_cq.cq_id,
        devid, tsid, logic_ctx->tgid, ctx->tgid);
    return 0;
#ifndef TSDRV_UT
err_mbox_send:
    ...
    free_logic_cq.cq_id = alloc_logic_cq.cq_id;
    err = logic_cq_free(ctx, tsid, &free_logic_cq);
    if (err != 0) {
        TSDRV_PRINT_INFO("logic_cq_free unable to do, err=%d\n", err);
    }
err_logic_cq_alloc:
    (void)logic_sqcq_phy_cq_free(ctx, tsid, logic_ctx);
    mutex_unlock(&logic_ctx->lock);
    return -ENODEV;
#endif
}

STATIC void logic_cq_abandon(struct tsdrv_ctx *ctx, u32 tsid, struct logic_sqcq_ts_ctx *logic_ctx, u32 logic_cqid)
{
#ifndef TSDRV_UT
    struct logic_cq_info *logic_cq = NULL;
    struct logic_cq_sub *sub_cq = NULL;
    ...
    err = logic_cq_phy_cq_clean(devid, fid, tsid, phy_cqid, logic_cqid);
    if (err != 0) {
    ...
    u32 logic_cqid)
{
    ...
    free_logic_cq.cq_id = logic_cqid;
    logic_sqcq_mbox_free_msg_pack(&mbox_msg, logic_ctx, &free_logic_cq, fid);
    ...
    err = logic_sqcq_mbox_send(devid, tsid, (unsigned char *)&mbox_msg);
    if (err != 0) {
        logic_cq_abandon(ctx, tsid, logic_ctx, logic_cqid);
        return -EFAULT;
    }
    ...
    err = logic_sqcq_phy_cq_free(ctx, tsid, logic_ctx);
    if (err != 0) {
        return err;
    }
...
STATIC int logic_cq_free_param_chk(u32 devid, u32 tsid, u32 fid, struct logic_sqcq_free_para *para,
    struct logic_sqcq_ts_ctx *logic_ctx)
{
    struct logic_cq_info *logic_cq = NULL;
    struct logic_cq_sub *sub_cq = NULL;
    ...
    struct list_head *n = NULL;
    u32 useflag;
    if (para->cq_id >= TSDRV_MAX_LOGIC_CQ_NUM) {
        TSDRV_PRINT_ERR("invalid logic_cq=%u\n", para->cq_id);
        return -EFAULT;
    }
    logic_cq = logic_cq_info_get(devid, fid, tsid);
    if (logic_cq == NULL) {
        ...
        if (sub_cq->cqid == para->cq_id) {
            ...
            if (sub_cq->useflag != LOGIC_CQ_IN_USE) {
                ...
                TSDRV_PRINT_ERR("invalid cqid(%u) useflag(%u), devid=%u fid=%u tsid=%u\n",
                    para->cq_id, useflag, devid, fid, tsid);
                return -ENODEV;
#endif
            }
            sub_cq->useflag = LOGIC_CQ_IS_FREE;
    ...
    TSDRV_PRINT_ERR("logic_cq=%u doesn't exist in this process\n", para->cq_id);
    return -ENODEV;
}
...
int logic_ioctl_cq_free(struct tsdrv_ctx *ctx, struct devdrv_ioctl_arg *arg)
{
    ...
    mutex_lock(&logic_ctx->lock);
    err = logic_cq_free_param_chk(devid, tsid, fid, para, logic_ctx);
    if (err != 0) {
        mutex_unlock(&logic_ctx->lock);
...
int tsdrv_fill_logic_cqe(u32 logic_cqid, struct logic_cq_info *logic_cq, stars_logic_cq_report_msg_t *logic_cqe)
{
    ...
    err = memcpy_s(sub_cq->vaddr + offset, sub_cq->size, logic_cqe, sizeof(struct tag_stars_logic_cq_report_msg));
    if (err != EOK) {
        TSDRV_PRINT_ERR("cq dispach memcpy fail.(logic_cqid=%u, cqe_size=%u)\n", logic_cqid, sub_cq->size);
        return err;
    }
...
STATIC void logic_sqcq_recycle(struct tsdrv_ctx *ctx, u32 tsid)
{
    ...
    if (!list_empty_careful(&logic_ctx->logic_cq_list)) {
        list_for_each_safe(pos, n, &logic_ctx->logic_cq_list) {
            ...
            if (err != 0) {
                TSDRV_PRINT_ERR("recycle logic_cqid=%u fail, devid=%u fid=%u tsid=%u\n", sub_cq->cqid, devid,
                    fid, tsid);
                continue;
            }
