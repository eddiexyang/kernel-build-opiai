...
#ifdef AOS_LLVM_BUILD
#define PG_DATA_CORE_MAP_STUB 0x3FF
#define PG_DATA_VECTOR_CORE_MAP_STUB 0xFF
#define PG_DATA_FREQ_STUB 960
#define PG_DATA_VECTOR_FREQ_STUB 1000
...
STATIC int tsdrv_sec_read_pg_info_thread(void *arg)
{
    ...
    int ret = sec_read_pg_info(thread_data->dev_id, thread_data->query_cmd, &thread_data->query_cmd_result);
    if (ret != 0) {
...
int tsdrv_sec_read_pg_info(u32 dev_id, pg_cmd_data cmd, u64 *cmd_ret_val)
{
    ...
    info->task = kthread_run(tsdrv_sec_read_pg_info_thread, (void *)&info->thread_data,
