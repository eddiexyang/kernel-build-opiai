...
#define MEM_NODE3 3U
    *node = MEM_NODE2;
    ...
    return (uintptr_t)kmalloc_node(size, GFP_ATOMIC | __GFP_THISNODE, *node);
#else

    *node = (u32)devdrv_manager_devid_to_nid(devid, (u32)DEVDRV_TS_NODE_DDR_MEM);
...
failed:
    kfree((void *)(uintptr_t)*vaddr);
    *vaddr = 0;
    return;
}

static void tsdrv_ts_free_mem_proc(struct tsdrv_ts_mem_mgmt *mem_mgmt, u64 vaddr)
{
    struct tsdrv_ts_mem_node *mem_node = NULL;
...
static void tsdrv_ts_alloc_mem_request(struct notifier_block *nb, unsigned long len, void *data)
{
    ...
    if (vaddr == 0) {
        mem_mgmt->alloced_mem_size -= req_msg->size;
#ifndef TSDRV_UT
        TSDRV_PRINT_WARN("Alloc mem warn. (devid=%u; tsid=%u; req_size=%u)\n",
...
mem_outof_range:
    ...
    if (ret && vaddr) {
        tsdrv_ts_free_mem_proc(mem_mgmt, vaddr);
        mem_mgmt->alloced_mem_size -= req_msg->size;
    }
    ...
    for (devid = 0; devid < DEVDRV_MAX_DAVINCI_NUM; devid++) {
        for (tsid = 0; tsid < DEVDRV_MAX_TS_NUM; tsid++) {
            ...
            list_for_each_safe(pos, n, &mem_mgmt->alloced_mem_node_head) {
                mem_node = list_entry(pos, struct tsdrv_ts_mem_node, list);
                list_del(&mem_node->list);
                kfree((void *)(uintptr_t)mem_node->vaddr);
                kfree((void *)mem_node);
            }
        }
    }
