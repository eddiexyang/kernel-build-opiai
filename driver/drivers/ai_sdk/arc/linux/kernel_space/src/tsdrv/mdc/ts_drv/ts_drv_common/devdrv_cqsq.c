...
struct devdrv_ts_cq_info *devdrv_get_new_cq(u32 devid, u32 tsid, struct tsdrv_ctx *ctx)
{
    ...
    if (!list_empty_careful(&ts_ctx->id_ctx[TSDRV_CQ_ID].id_list)) {
        ...
        TSDRV_PRINT_ERR("[dev_id=%u]:context already has a cq\n", devid);
        return NULL;
    }
    spin_unlock(&ts_res->id_res[TSDRV_CQ_ID].spinlock);
...
#ifndef CFG_SOC_PLATFORM_MDC_V51
STATIC void hvtsdrv_clear_vcq_info(struct tsdrv_ts_resource *ts_res, u32 vcq_id)
{
...
STATIC void hvtsdrv_clear_vsq_info(struct tsdrv_ts_resource *ts_res, u32 vsq_id)
{
...
void devdrv_free_cq(struct tsdrv_ts_resource *ts_res, struct tsdrv_ctx *ctx, struct devdrv_ts_cq_info *cq_info)
{
    ...
    if (cq_id_info->phy_id != (int)cq_info->index) {
        spin_unlock(&ts_res->id_res[TSDRV_CQ_ID].spinlock);
        return;
    }
...
int tsdrv_get_phy_cq_mem_phy_addr(struct tsdrv_ctx *ctx, u32 tsId, u32 cqId, struct tsdrv_phy_addr_get *info)
{
    ...
    info->paddr = paddr;
    info->paddr_len = info->len;
    TSDRV_PRINT_DEBUG("cq_id=%u, paddr=%pK, len=%u\n",
...
#ifndef AOS_LLVM_BUILD
static int tsdrv_map_sq_db(u32 devid, u32 tsid, struct normal_alloc_sqcq_para *sqcq_alloc,
    struct devdrv_ts_sq_info *sq_info)
{
    ...
    if ((tsdrv_is_in_container(devid) == true) || (TSDRV_USERMAP_DB_SIZE != PAGE_SIZE)) {
...
#endif
void tsdrv_unmap_sq_db(u32 devid, u32 tsid, struct devdrv_ts_sq_info *sq_info)
{
#ifndef AOS_LLVM_BUILD
    struct vm_area_struct *vma = NULL;
    ...
    struct devdrv_normal_cqsq_mailbox *cqsq_mailbox)
{
    ...
    if (!tsdrv_is_in_vm(devid)) {
#ifndef AOS_LLVM_BUILD
        ret = tsdrv_map_sq_db(devid, ts_res->tsid, sqcq_alloc, sq_info);
        if (ret != 0) {
#ifndef TSDRV_UT
            goto err_remap_sqdb;
#endif
...
#ifndef TSDRV_UT
err_remap_sq:
    tsdrv_unmap_sq_db(devid, ts_res->tsid, sq_info);
#ifndef AOS_LLVM_BUILD
err_remap_sqdb:
#endif
    if (sq_sub->addr_side == TSDRV_MEM_ON_HOST_SIDE) {
    ...
    struct normal_alloc_sqcq_para *sqcq_alloc)
{
    ...
    if ((tsdrv_ctx_is_nothread_type(ctx) == true) && (tsdrv_get_fid_by_ctx(ctx) == TSDRV_PM_FID)) {
        thre_val = 1;
    }
    ...
    u32 local_available_num)
{
    ...
    if (tsdrv_get_fid_by_ctx(ctx) != TSDRV_PM_FID) {
        ...
        remain_sq_num = DEVDRV_MAX_SQ_NUM - (local_cur_num - local_available_num);
...
STATIC void tsdrv_record_stream_sqcq_no_sync(u32 dev_id, struct tsdrv_ts_resource *ts_resource,
    u32 stream_id, u32 sq_id)
{
    ts_resource->stream_info[stream_id].sq = sq_id;
    ts_resource->stream_info[stream_id].valid = 1;
    ts_resource->stream_info[stream_id].remote_tgid = current->tgid;
}

STATIC void tsdrv_record_stream_sqcq(u32 dev_id, struct tsdrv_ts_resource *ts_resource,
    u32 stream_id, u32 sq_id, u32 cq_id)
{
    tsdrv_record_stream_sqcq_no_sync(dev_id, ts_resource, stream_id, sq_id);
    (void)tsdrv_msg_sync_stream_sqcq(dev_id, ts_resource->tsid, stream_id, 1, sq_id, cq_id);
}

STATIC void tsdrv_clear_stream_sqcq(u32 dev_id, struct tsdrv_ts_resource *ts_resource, u32 sq_id, u32 cq_id)
{
    u32 stream_id;

    for (stream_id = 0; stream_id < tsdrv_get_stream_id_max_num(dev_id, ts_resource->tsid); stream_id++) {
        if ((ts_resource->stream_info[stream_id].valid == 1) && (ts_resource->stream_info[stream_id].sq == sq_id)) {
            ts_resource->stream_info[stream_id].valid = 0;

            (void)tsdrv_msg_sync_stream_sqcq(dev_id, ts_resource->tsid, stream_id, 0, sq_id, cq_id);
            break;
        }
    }
}
...
STATIC int tsdrv_sqcq_alloc_para_check(struct normal_alloc_sqcq_para *sqcq_para)
{
    if (sqcq_para->sqeDepth < DEVDRV_MIN_SQ_DEPTH || sqcq_para->sqeDepth > DEVDRV_MAX_SQ_DEPTH ||
        ...
        sqcq_para->cqeSize != DEVDRV_CQ_SLOT_SIZE) {
...
#if defined(CFG_SOC_PLATFORM_CLOUD_V2) || defined(CFG_SOC_PLATFORM_MINIV3)
STATIC int tsdrv_query_sqcq_stream_id(struct tsdrv_ts_resource *ts_resource, u32 sq_id, u32 *stream_id)
{
    u32 i;

    for (i = 0; i < DEVDRV_MAX_STREAM_ID; i++) {
        if ((ts_resource->stream_info[i].valid == 1) && (ts_resource->stream_info[i].sq == sq_id)) {
            *stream_id = i;
            return 0;
        }
    }
...
STATIC int tsdrv_phy_sqcq_unmap(struct tsdrv_ctx *ctx, u32 tsid, u32 sq_id, u32 cq_id)
{
    ...
    TSDRV_PRINT_DEBUG("Ummap normal sq cq success. (sqid=%u; va=%pK; len=0x%lx; cqid=%u;"
        " va=%pK; len=0x%lx; tgid=%d)\n",
...
STATIC int tsdrv_phy_sqcq_mmap(struct tsdrv_ctx *ctx, u32 tsid, struct normal_alloc_sqcq_para *arg)
{
    ...
    TSDRV_PRINT_DEBUG("Mmap normal sq cq success. (sqid=%u; va=%pK; len=0x%lx; cqid=%u;"
        " va=%pK; len=0x%lx; tgid=%d)\n",
...
STATIC int tsdrv_alloc_phy_sqcq(struct tsdrv_ctx *ctx, u32 tsid, struct normal_alloc_sqcq_para *sqcq_alloc)
{
    ...
    sq_alloc_para.type = NORMAL_SQCQ_TYPE;
    ...
    cq_alloc_para.type = NORMAL_SQCQ_TYPE;
...
int tsdrv_alloc_phy_cqsq_mbox_send(struct tsdrv_ctx *ctx, u32 tsid, struct normal_alloc_sqcq_para *sqcq_alloc)
{
    ...
    TSDRV_PRINT_DEBUG("start send Msg to Ts. (tgid=%d; devid=%u, fid=%u; tsid=%u;"
        " phy_sqid=%u; sq_pa=%pK; sqesize=%u; sqdepth=%u;"
        " phy_cqid=%u; cq_pa=%pK; cqesize=%u; cqdepth=%u; cq_irq=%u; sq_cq_side=%u)\n", ctx->tgid, devid, fid, tsid,
        (u32)cqsq_mailbox.sq_index, (void *)(uintptr_t)cqsq_mailbox.sq_addr, (u32)cqsq_mailbox.sqesize,
        (u32)cqsq_mailbox.sqdepth, (u32)cqsq_mailbox.cq0_index, (void *)(uintptr_t)cqsq_mailbox.cq0_addr,
        (u32)cqsq_mailbox.cqesize, (u32)cqsq_mailbox.cqdepth,
        (u32)cqsq_mailbox.cq_irq, (u32)cqsq_mailbox.sq_cq_side);

    ret = devdrv_mailbox_cqsq_inform(&pm_ts_resource->mailbox, ctx, DEVDRV_MAILBOX_CREATE_CQSQ_CALC, &cqsq_mailbox);
    if (ret != 0) {
    ...
    struct normal_alloc_sqcq_para *sqcq_alloc)
{
    ...
    tsdrv_record_stream_sqcq(devid, tsdrv_get_ts_resoruce(devid, TSDRV_PM_FID, tsid),
        sqcq_alloc->info[0], sqcq_alloc->sqId, sqcq_alloc->cqId);

    TSDRV_PRINT_EVENT("alloc one pair cqsq success. (device_id=%u; tsid=%u; sq_id=%u; cq_id=%u; stream_id=%u)\n",
    ...
    struct normal_free_sqcq_para *sqcq_free)
{
    ...
    ret = tsdrv_sq_exist_check(ts_resource, ctx, sqcq_free->sqId);
    if (ret != 0) {
...
int tsdrv_free_phy_cqsq_mbox_send(struct tsdrv_ctx *ctx, u32 tsid, struct normal_free_sqcq_para *sqcq_free)
{
    ...
    ret = tsdrv_query_sqcq_stream_id(ts_resource, sqcq_free->sqId, &stream_id);
    if (ret != 0) {
        TSDRV_PRINT_ERR("sq %u cq %u query stream id failed ret %d\n", sqcq_free->sqId, sqcq_free->cqId, ret);
        return ret;
    }
    cqsq_mailbox.info[0] = stream_id;
    TSDRV_PRINT_DEBUG("sq %u cq %u stream id %u\n", sqcq_free->sqId, sqcq_free->cqId, stream_id);

    pm_ts_resource = tsdrv_get_ts_resoruce(devid, TSDRV_PM_FID, tsid);
    ret = devdrv_mailbox_cqsq_inform(&pm_ts_resource->mailbox, ctx, DEVDRV_MAILBOX_RELEASE_CQSQ_CALC, &cqsq_mailbox);
    if (ret != 0) {
    ...
    struct normal_free_sqcq_para *sqcq_free)
{
    ...
    tsdrv_clear_stream_sqcq(devid, pm_ts_resource, sqcq_free->sqId, sqcq_free->cqId);
...
int devdrv_ioctl_sqcq_alloc(struct tsdrv_ctx *ctx, struct devdrv_ioctl_arg *arg)
{
    ...
    if (type != CHIP_TYPE_MDC_BS9SX1A) {
        id_inst.devid = devid;
        id_inst.tsid = ts_resource->tsid;
        id_inst.fid = tsdrv_get_fid_by_ctx(ctx);
        ret = tsdrv_id_is_belong_to_proc(&id_inst, ctx->tgid, sqcq_alloc->info[0], TSDRV_STREAM_ID);
        if (ret != 0) {
            TSDRV_PRINT_ERR("StreamId is not belong to this proc. (devid=%u; fid=%u; tsid=%u; streamId=%u; ret=%d)\n",
                id_inst.devid, id_inst.fid, id_inst.tsid, sqcq_alloc->info[0], ret);
            return ret;
        }
    }
    TSDRV_PRINT_DEBUG("(devid=%u; flag=%u)\n", devid, sqcq_alloc->flag);

    sqcq_alloc->sqeSize = DEVDRV_SQ_SLOT_SIZE;
#if defined(CFG_SOC_PLATFORM_CLOUD_V2) || defined(CFG_SOC_PLATFORM_MINIV3)
    return tsdrv_alloc_one_pair_phy_sqcq(ctx, arg->tsid, sqcq_alloc);
#endif
...
#if defined(CFG_SOC_PLATFORM_CLOUD_V2) || defined(CFG_SOC_PLATFORM_MINIV3)
    return tsdrv_free_one_pair_phy_sqcq(ctx, arg->tsid, sqcq_free);
#endif
...
int tsdrv_phy_sq_head_update(u32 devid, u32 fid, u32 tsid, u32 sq_id, u32 sq_head)
{
    ...
    if (tsdrv_is_in_pm(devid) == false) {
        ...
        if (sq_id_info == NULL) {
            TSDRV_SPINLOCK_ERR("Invalid para. (devid=%u; fid=%u; tsid=%u; sqid=%u)\n",
                devid, fid, ts_res->tsid, sq_info->index);
            return -ENODEV;
        }
...
STATIC int tsrdv_get_phy_sqid(u32 devid, u32 fid, u32 tsid, struct devdrv_sq_sub_info **sq_sub)
{
    struct tsdrv_ts_resource *ts_res = tsdrv_get_ts_resoruce(devid, fid, tsid);
...
STATIC int tsdrv_dev_alloc_reserve_sqcqmem(struct tsdrv_cqsq_mem_alloc_free_para *para)
{
...
STATIC void tsdrv_dev_free_reserve_sqcqmem(struct tsdrv_cqsq_mem_alloc_free_para *para)
{
#ifdef CFG_SOC_PLATFORM_MINIV2
...
STATIC int tsrdv_phy_sq_dev_mem_init(struct tsdrv_cqsq_mem_alloc_free_para *para)
{
    int err;
...
STATIC void tsrdv_phy_sq_dev_mem_exit(struct tsdrv_cqsq_mem_alloc_free_para *para)
{
...
int tsdrv_phy_cqid_exist_chk(struct tsdrv_ctx *ctx, u32 tsid, u32 cqid)
{
#ifndef TSDRV_UT
    struct tsdrv_id_info *cq_id_info = NULL;
    struct tsdrv_ts_resource *ts_res = NULL;
...
int tsdrv_phy_sqid_exist_chk(struct tsdrv_ctx *ctx, u32 tsid, u32 sqid, enum phy_sqcq_type type)
{
    ...
    if (!list_empty_careful(&ts_ctx->id_ctx[TSDRV_SQ_ID].id_list)) {
        list_for_each_safe(pos, n, &ts_ctx->id_ctx[TSDRV_SQ_ID].id_list) {
            ...
            if (id_info->id == (int)sqid) {
                ret = tsdrv_sq_type_check(devid, TSDRV_PM_FID, tsid, sqid, type);
                if (ret != 0) {
                    spin_unlock(&ts_res->id_res[TSDRV_SQ_ID].spinlock);
...
STATIC int tsrdv_get_phy_cqid(u32 devid, u32 fid, u32 tsid, struct devdrv_cq_sub_info **cq_sub)
{
    struct tsdrv_ts_resource *ts_res = tsdrv_get_ts_resoruce(devid, fid, tsid);
    struct tsdrv_id_info *cq_id_info = NULL;
    struct devdrv_ts_cq_info *cq_info = NULL;
    ...
    struct devdrv_cq_sub_info *cq_sub, struct phy_cq_alloc_para *para)
{
    ...
    spin_unlock_irq(&cq_sub->spinlock);
    spin_unlock(&ts_res->id_res[TSDRV_CQ_ID].spinlock);
    TSDRV_PRINT_DEBUG("(type=%d; cqid=%u; cq_info->phase=%u)\n", (int)cq_info->type, cq_info->index, cq_info->phase);
...
#ifndef AOS_LLVM_BUILD
        sq_sub->vaddr = (uintptr_t)kzalloc_node(sq_sub->size * sq_sub->depth, GFP_KERNEL | __GFP_THISNODE,
            devdrv_manager_devid_to_nid(devid, (u32)DEVDRV_TS_NODE_DDR_MEM));
#else
        sq_sub->vaddr = (phys_addr_t)kmalloc(sq_sub->size * sq_sub->depth, GFP_USER);
#endif
        if (sq_sub->vaddr == 0) {
            goto err_alloc;
        }
...
void tsdrv_kernel_free_sq(u32 devid, u32 vfid, u32 tsid, u32 sqid)
{
    ...
    if (tsdrv_get_env_type() == TSDRV_ENV_ONLINE) {
        ...
        if (sq_sub->vaddr != 0) {
            kfree((void *)(uintptr_t)sq_sub->vaddr);
            sq_sub->vaddr = 0;
        }
    }
...
int tsdrv_kernel_alloc_cq(u32 devid, u32 vfid, u32 tsid, u32 cqe_size, u32 depth, enum phy_sqcq_type type)
{
    ...
    vaddr = (uintptr_t)tsdrv_alloc_pages_exact_nid(devdrv_manager_devid_to_nid(devid, (u32)DEVDRV_TS_NODE_DDR_MEM),
        cq_sub->size, GFP_KERNEL | __GFP_THISNODE);
#else
    vaddr = (phys_addr_t)kmalloc(cq_sub->size, GFP_USER);
#endif
    if (vaddr == 0) {
        TSDRV_PRINT_ERR("devid %u vfid %u tsid %u sq %u alloc sq mem failed\n", devid, vfid, tsid,
            cq_sub->index);
...
STATIC void tsdrv_pack_reserved_sqcq_info(u32 sqid, u32 cqid, u32 pool_id, u32 pri,
...
/* MINIV3 use refactor code */
#define RES_SQCQ_NUM_VF 4 // 1 ts_sqcq + 5 esched_sqcq
#define RES_SQCQ_NUM_PF 2 // 1 ts_sqcq + 7 esced_sqcq (host:2 device:5)

int tsdrv_alloc_resv_sqcq(u32 devid, u32 vfid, u32 tsid)
{
#if !defined(CFG_SOC_PLATFORM_CLOUD_V2) && !defined(CFG_SOC_PLATFORM_MINIV3)
    return 0;
#else
    int sqid, cqid, i, j, resv_sqcq_num;
    struct tsdrv_ts_resource *vf_ts_res = NULL;
    struct tsdrv_ts_resource *ts_res = NULL;
    ...
    if (tsdrv_get_pfvf_type_by_devid(devid) == DEVDRV_SRIOV_TYPE_PF) {
        resv_sqcq_num = RES_SQCQ_NUM_PF;
    } else {
        resv_sqcq_num = RES_SQCQ_NUM_VF;
    }

    for (i = 0; i < resv_sqcq_num; i++) {
        sqid = tsdrv_kernel_alloc_sq(devid, vfid, tsid, DEVDRV_SQ_SLOT_SIZE, 0, TS_SQCQ_TYPE);
        if (sqid < 0) {
#ifndef TSDRV_UT
            TSDRV_PRINT_ERR("devid %u vfid %u tsid %u alloc sq failed\n", devid, vfid, tsid);
            goto KERNEL_ALLOC_SQCQ_ERR;
#endif
        }
    ...
    TSDRV_PRINT_DEBUG("Alloc unused sqcq. (num=%d)\n", resv_sqcq_num);
    return 0;

#ifndef TSDRV_UT
KERNEL_ALLOC_SQCQ_ERR:
    ts_res = tsdrv_get_ts_resoruce(devid, vfid, tsid);
    for (j = 0; j < i; j++) {
        sqid = ts_res->reserved_sqcq[ts_res->sqcq_reserved_num].sq_info.index;
        cqid = ts_res->reserved_sqcq[ts_res->sqcq_reserved_num].cq_info.index;
        tsdrv_kernel_free_sq(devid, vfid, tsid, sqid);
        tsdrv_kernel_free_cq(devid, vfid, tsid, cqid);
        ts_res->sqcq_reserved_num--;
    }
    return -EFAULT;
#endif
...
void tsdrv_free_resv_sqcq(u32 devid, u32 vfid, u32 tsid)
{
#ifndef TSDRV_UT
#if !defined(CFG_SOC_PLATFORM_CLOUD_V2) && !defined(CFG_SOC_PLATFORM_MINIV3)
        return;
    ...
    for (i = 0; i < resv_sqcq_num; i++) {
        sqid = ts_res->reserved_sqcq[i].sq_info.index;
        cqid = ts_res->reserved_sqcq[i].cq_info.index;
        tsdrv_kernel_free_sq(devid, vfid, tsid, sqid);
        tsdrv_kernel_free_cq(devid, vfid, tsid, cqid);
        ts_res->sqcq_reserved_num--;
    }

    TSDRV_PRINT_INFO("Free unused sqcq. (num=%d)\n", resv_sqcq_num);
#endif
#endif
}
