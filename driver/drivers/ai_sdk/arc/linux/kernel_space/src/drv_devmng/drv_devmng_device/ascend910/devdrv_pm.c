...
STATIC void devdrv_fresh_error_code(u32 devid)
{
    ...
    if (dev_info == NULL) {
        devdrv_drv_err("device[%u] dev_info is NULL!.\n", devid);
        return;
    }

    if (dev_info->heart_beat_wq != NULL) {
#ifndef CFG_HOST_ENV
        queue_work_affinity(dev_info->heart_beat_wq, &dev_info->work);
#else
...
STATIC enum hrtimer_restart devdrv_refresh_error_code_event(struct hrtimer *t)
{
    ...
    dev_num = devdrv_manager_container_get_devnum_bare(DEVDRV_PF_TYPE);
#else
    dev_num = uda_get_dev_num();
#endif

    for (i = 0; i < dev_num; i++) {
        devdrv_fresh_error_code(i);
    }
...
int devdrv_manager_msg_resume(void)
{
    ...
    list_for_each_safe(pos, n, &d_info->msg_pm_list_header) {
        ...
        if (msg_pm->resume != NULL) {
            ...
            if (ret) {
                spin_unlock_irqrestore(&d_info->msg_pm_list_lock, flags);
                devdrv_drv_err("one resume func failed ret = %d.\n", ret);
                return ret;
            }
        } else {
            spin_unlock_irqrestore(&d_info->msg_pm_list_lock, flags);
...
STATIC void devdrv_inform_device_manager(struct devdrv_info *info, uint32_t tsid, enum devdrv_ts_status status)
{
    struct devdrv_manager_info *d_info = NULL;
    struct list_head *pos = NULL, *n = NULL;
    struct devdrv_pm *pm = NULL;
    ...
    list_for_each_safe(pos, n, &d_info->pm_list_header)
    {
        ...
        if ((pm->suspend != NULL) && (pm->run_stage == DEVDRV_PLATFORM)) {
...
STATIC int devdrv_manager_suspend_prev(struct devdrv_info *info)
{
    ...
    if (info->pdata == NULL) {
        devdrv_drv_err("The pdata is NULL.\n");
        return -EINVAL;
    }

    devdrv_drv_info("ts_num = %u\n", info->pdata->ts_num);
    ...
    for (tsid = 0; tsid < info->pdata->ts_num; tsid++) {
        ...
        if (info->drv_ops->tsdrv_heart_beat_set_work_state != NULL) {
            info->drv_ops->tsdrv_heart_beat_set_work_state(info->dev_id, tsid, 0);
        }
...
int devdrv_manager_suspend_proc(int devid, int run_stage)
{
    ...
    list_for_each_safe(pos, n, &d_info->pm_list_header) {
        ...
        pm = list_entry(pos, struct devdrv_pm, list);
        if (pm->suspend == NULL) {
            devdrv_drv_err("pm->suspend NULL.\n");
            continue;
        }
        if (pm->run_stage == run_stage) {
            devdrv_drv_info("pm->suspend exec.\n");
            ...
            devdrv_drv_info("pm->run_stage=%d want_run_stage=%d\n", pm->run_stage, run_stage);
        }
    }
    spin_unlock_irqrestore(&d_info->pm_list_lock, flags);
...
typedef void (*soc_misc_suspend_notify)(void);
int devdrv_manager_suspend(struct devdrv_info *info)
{
    ...
    struct list_head *stop = NULL;
    struct devdrv_pm *pm = NULL;
    soc_misc_suspend_notify soc_misc_func = NULL;
    unsigned long flags;
    u32 tsid = 0;
    int ret;
    ...
    soc_misc_func = (soc_misc_suspend_notify)(uintptr_t)__kallsyms_lookup_name("soc_misc_suspend_notify");
    if (soc_misc_func != NULL) {
        soc_misc_func();
    } else {
        devdrv_drv_err("soc_misc_func is NULL.\n");
    }

    /* 5. all module's suspend callback func */
    ret = devdrv_manager_suspend_proc(info->dev_id, DEVDRV_PLATFORM);
    if (ret) {
        goto error;
    }
...
error:
    ...
    for (tsid = 0; tsid < info->pdata->ts_num; tsid++) {
        if (info->drv_ops->tsdrv_heart_beat_set_work_state != NULL) {
            info->drv_ops->tsdrv_heart_beat_set_work_state(info->dev_id, tsid, 1);
        }
...
{
    devdrv_drv_debug("call devdrv_manager_shutdown.\n");
    if (info == NULL) {
    ...
    /* 1. stop heart beat */
    if (devdrv_manager_suspend_prev(info)) {
        devdrv_drv_err("stop all heat beat in shutdown fail.\n");
    }
...
int devdrv_inform_lpm3_upper_ddr_freq(u32 devid)
{
    ...
    if (devid >= MAX_CHIP_NUM) {
        devdrv_drv_err("device id %u is invalid.\n", devid);
        return ret;
    }
    ...
    ret = icm_msg_send_async(ICM_FD_BUILD(0, HISI_RPROC_TX_LPM3), (rproc_msg_t *)&ipc_msg, DRV_IPC_RPROC_LEN);
    if (ret) {
...
void devdrv_inform_lpm3_change_aicore_freq(int gpio_value, int flag)
{
    ...
    devdrv_drv_debug("inform lpm3 change aicore freq, gpio_value=%d, flag=%d\n", gpio_value, flag);
    ...
    devdrv_drv_debug("inform lpm3 set freq success, status = %d, gpio_value= %d, flag= %d\n",
...
void devdrv_ts_exception_task(unsigned long data)
{
    ...
    devdrv_drv_debug("begin to inform ts[%u] status: %u.\n", info->dev_id, status);
    devdrv_inform_device_manager(info, 0, status);
...
#define DEVDRV_DISP_TS_DAW_INDEX 7
    ...
    /* print err info */
    devdrv_drv_debug("receive a nfe irq raised by hardware.\n");
...
STATIC void devdrv_heart_beat_ai_down(struct devdrv_info *info, u32 tsid, const void *data)
{
...
void tsdrv_heart_beat_ai_down(u32 devid, u32 tsid, const void *data)
{
    ...
    devdrv_heart_beat_ai_down(info, tsid, data);
}
