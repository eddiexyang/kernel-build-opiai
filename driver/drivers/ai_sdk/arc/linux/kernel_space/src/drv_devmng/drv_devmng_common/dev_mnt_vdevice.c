...
#define VDAVINCI_SUPPORT_DIVIDE_AICORE_NUM 25
...
STATIC long dev_mnt_vdev_ioctl_fake(struct file *filep, unsigned int cmd,
    unsigned long arg)
{
    if (dev_mnt_vdev_fops_real.unlocked_ioctl != NULL) {
        return dev_mnt_vdev_fops_real.unlocked_ioctl(filep, cmd, arg);
    }

    return 0;
}

STATIC int dev_mnt_vdev_open_fake(struct inode *inode, struct file *filep)
{
    if (dev_mnt_vdev_fops_real.open != NULL) {
        return dev_mnt_vdev_fops_real.open(inode, filep);
    }

    return -EINVAL;
}

STATIC int dev_mnt_vdev_release_fake(struct inode *inode, struct file *filep)
{
    if (dev_mnt_vdev_fops_real.release != NULL) {
        return dev_mnt_vdev_fops_real.release(inode, filep);
    }

    return 0;
}

STATIC int dev_mnt_vdev_mmap_fake(struct file *filep,
    struct vm_area_struct *vma)
{
    if (dev_mnt_vdev_fops_real.mmap != NULL) {
        return dev_mnt_vdev_fops_real.mmap(filep, vma);
    }

    return 0;
}

STATIC const struct file_operations dev_mnt_vdev_fops_fake = {
    .owner = THIS_MODULE,
    .open = dev_mnt_vdev_open_fake,
    .release = dev_mnt_vdev_release_fake,
    .unlocked_ioctl = dev_mnt_vdev_ioctl_fake,
    .mmap = dev_mnt_vdev_mmap_fake,
};
#endif
...
#ifndef CFG_FEATURE_VF_USE_DEVID
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
STATIC int dev_mnt_vdevice_create_one_device(u32 vdev_id)
{
    u32 offset_vdevid = VDAVINCI_VDEV_OFFSET_PLUS(vdev_id);
    struct device *i_device = NULL;
    dev_t devno;
    int ret;

    devno = MKDEV(g_mnt_vdev_cb->vdev_major, offset_vdevid);

    g_mnt_vdev_cb->vdev[vdev_id].cdev.owner = THIS_MODULE;
    cdev_init(&g_mnt_vdev_cb->vdev[vdev_id].cdev, &dev_mnt_vdev_fops_fake);
    ret = cdev_add(&g_mnt_vdev_cb->vdev[vdev_id].cdev, devno, MAX_VDEV_NUM);
    if (ret) {
        devdrv_drv_err("cdev_add failed, ret(%d). dev_id(%u)\n", ret, offset_vdevid);
        return ret;
    }

    i_device = device_create(g_mnt_vdev_cb->vdev_class, NULL, devno, NULL, "vdavinci%d", offset_vdevid);
    if (IS_ERR(i_device)) {
        cdev_del(&g_mnt_vdev_cb->vdev[vdev_id].cdev);
        devdrv_drv_err("device_create failed. dev_id(%u)\n", offset_vdevid);
        return -ENODEV;
    }
    g_mnt_vdev_cb->vdev[vdev_id].dev = i_device;
    return 0;
}
#endif
#endif
...
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
STATIC void dev_mnt_vdevice_release_one_device(u32 vdev_id)
{
    u32 offset_vdevid = VDAVINCI_VDEV_OFFSET_PLUS(vdev_id);
    dev_t devno;

    devno = MKDEV(g_mnt_vdev_cb->vdev_major, offset_vdevid);
    if (g_mnt_vdev_cb->vdev[vdev_id].dev != NULL) {
        device_destroy(g_mnt_vdev_cb->vdev_class, devno);
        cdev_del(&g_mnt_vdev_cb->vdev[vdev_id].cdev);
        g_mnt_vdev_cb->vdev[vdev_id].dev = NULL;
    }
}
#endif
...
STATIC int dev_mnt_destory_vdevice_single(u32 phy_id, u32 vdev_id)
{
    ...
    /*
     * @attention: vmng_destory_container_vdev will call
    ...
    if (ret != 0) {
        mutex_unlock(&g_mnt_vdev_cb->vdev[vdev_id].lock);
        devdrv_drv_err("destory_container failed, phy_id:%u,vfid:%u,ret:%d\n", phy_id,
...
#ifndef CFG_FEATURE_VF_USE_DEVID
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
    dev_mnt_vdevice_release_one_device(vdev_id);
#endif
#endif
...
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
STATIC const struct file_operations dev_mnt_vdev_vm_fops_fake = {
    .owner = THIS_MODULE,
    .open = NULL,
    .release = NULL,
    .unlocked_ioctl = NULL,
...
int dev_mnt_create_one_vm_vdevice(unsigned int phy_id, unsigned int fid)
{
    ...
    int ret;
    if (!fid) {
        return 0; // invalid fid
    }

    g_mnt_vdev_cb->vdev[vdev_id].cdev.owner = THIS_MODULE;
    devno = MKDEV(g_mnt_vdev_cb->vdev_major, offset_vdevid);
    cdev_init(&g_mnt_vdev_cb->vdev[vdev_id].cdev, &dev_mnt_vdev_vm_fops_fake);
    ret = cdev_add(&g_mnt_vdev_cb->vdev[vdev_id].cdev, devno, MAX_VDEV_NUM);
    if (ret) {
        devdrv_drv_err("cdev_add failed, ret(%d). dev_id(%u)\n", ret, offset_vdevid);
        return ret;
...
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
STATIC int dev_mnt_vdevice_create_dev(DEV_MNT_VDEV_CB_T *vdev_cb)
{
    dev_t vdev;
    int ret;

    ret = alloc_chrdev_region(&vdev, VDAVINCI_VDEV_OFFSET, MAX_VDEV_NUM, "vdevdrv-cdev");
    if (ret != 0) {
...
STATIC void dev_mnt_vdevice_release_dev(DEV_MNT_VDEV_CB_T *vdev_cb)
{
    class_destroy(vdev_cb->vdev_class);
    unregister_chrdev_region(MKDEV(vdev_cb->vdev_major, VDAVINCI_VDEV_OFFSET), MAX_VDEV_NUM);
    mutex_destroy(&g_mnt_vdev_cb->global_lock);
}
#endif
...
#ifndef CFG_FEATURE_VF_USE_DEVID
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
    ret = dev_mnt_vdevice_create_one_device(vinfo->vdevid);
    if (ret != 0) {
        dev_mnt_vdevice_free(vinfo->vdevid);
    #ifdef CFG_FEATURE_RC_MODE
        vmng_destory_container_vdev(phy_id, vfid);
    #else
        vmngh_destory_container_vdev(phy_id, vfid);
    #endif
        devdrv_drv_err("create vdevice cdev failed, phy_id:%u, core_num:%u, vdevid:%u, ret:%d\n",
                       phy_id, core_num, VDAVINCI_VDEV_OFFSET_PLUS(vinfo->vdevid), ret);
        return ret;
    }
#endif
#endif
...
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
    ret = devdrv_manager_container_logical_id_to_cdev_id(logical_id, &vdev_id);
#else
...
STATIC int get_single_vdev_info(struct vdev_query_info *query_info, struct vmng_soc_resource_enquire *vm_info)
{
    ...
    query_info->computing.device_aicpu =
        bitmap_weight((unsigned long *)&stars_refresh->device_aicpu, VDEVMNG_AICPU_BITMAP_LEN);
    ...
    struct vmng_soc_resource_enquire *vm_info)
{
    ...
    device_aicpu = stars_refresh->device_aicpu;
    query_info->computing.device_aicpu =
        bitmap_weight((unsigned long *)&device_aicpu, VDEVMNG_AICPU_BITMAP_LEN);
    dev_mng_get_media_info(&query_info->media, stars_refresh);
    ...
    struct vmng_soc_resource_enquire *vm_info)
{
    ...
    vdevices = (u32 *)vzalloc(sizeof(u32) * VDAVINCI_MAX_VFID_NUM);
    if (vdevices == NULL) {
    ...
    if (memory_used > memory_total) {
        devdrv_drv_err("Memory used is larger than tatal. (phy_id=%u; used=%lld; total=%lld)\n",
...
STATIC int dev_mnt_get_vdevice_info(u32 phy_id, struct vdev_query_info *vdev_info)
{
    ...
    vm_info = kzalloc(sizeof(struct vmng_soc_resource_enquire), GFP_KERNEL | __GFP_ACCOUNT);
    if (vm_info == NULL) {
...
int dev_mnt_vdevice_init(void)
{
    ...
    g_mnt_vdev_cb = kzalloc(sizeof(DEV_MNT_VDEV_CB_T), GFP_KERNEL | __GFP_ACCOUNT);
...
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
    ret = dev_mnt_vdevice_create_dev(g_mnt_vdev_cb);
    if (ret != 0) {
        devdrv_drv_err("create vdevice failed, ret(%d).\n", ret);
        kfree(g_mnt_vdev_cb);
        g_mnt_vdev_cb = NULL;
        return ret;
    }
...
void dev_mnt_vdevice_uninit(void)
{
    ...
    for (i = 0; i < MAX_VDEV_NUM; i++) {
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
        mutex_lock(&g_mnt_vdev_cb->vdev[i].lock);
        dev_mnt_vdevice_release_one_device(i);
        mutex_unlock(&g_mnt_vdev_cb->vdev[i].lock);
#endif
...
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
    dev_mnt_vdevice_release_dev(g_mnt_vdev_cb);
#endif
    kfree(g_mnt_vdev_cb);
    g_mnt_vdev_cb = NULL;
    return;
...
{
    if (ops == NULL || phy_id >= DEVDRV_MAX_DAVINCI_NUM || vfid > VDAVINCI_MAX_VFID_NUM) {
...
{
    if (phy_id >= DEVDRV_MAX_DAVINCI_NUM || vfid > VDAVINCI_MAX_VFID_NUM) {
        devdrv_drv_err("invalid para phy_id(%u), vfid(%u).\n", phy_id, vfid);
        return -EINVAL;
    }
...
STATIC int dev_mnt_check_create_para_single(u32 phy_id, struct vdev_create_info *vinfo)
{
    ...
    if (vdev_num > VDAVINCI_MAX_VFID_NUM) {
        devdrv_drv_err("devid(%u) invlid,total_num(%u)\n", phy_id, vdev_num);
        return -EINVAL;
    }
...
int dev_mnt_vdevice_add_inform(unsigned int vdev_id, vdev_action action, struct mnt_namespace *ns, u64 container_id)
{
    ...
    new_vdev_action = kzalloc(sizeof(struct dev_mnt_vdev_action), GFP_ATOMIC | __GFP_ACCOUNT);
...
STATIC int devmng_check_host_split(u32 dev_id)
{
    ...
    if (!is_pm_boot) {
        devdrv_drv_err("Sriov is not support for vm boot mode! (dev_id=%u)\n", dev_id);
        return -EOPNOTSUPP;
    }
...
STATIC int dev_check_support_vdev(u32 dev_id)
{
    ...
    if (amp_or_smp == DEVMNG_SMP_MODE) {
        devdrv_drv_err("Not support to create vdevice in smp mode. (dev_id=%u)\n", dev_id);
        return -EOPNOTSUPP;
    }
#endif
...
int devdrv_manager_ioctl_create_vdev(struct file *filep, unsigned int cmd, unsigned long arg)
{
...
#endif
#ifdef CFG_FEATURE_OLD_DEVID_TRANS
    devdrv_manager_container_check_refresh_table();
#endif
    ...
    if (ret) {
        mutex_unlock(&g_mnt_vdev_cb->global_lock);
        devdrv_drv_err("heck_create_para_all fail, devid(%u) ret(%d).\n", vinfo.devid, ret);
        return ret;
    }
    ...
    if (ret) {
        mutex_unlock(&g_mnt_vdev_cb->global_lock);
        devdrv_drv_err("create_vdevice_single fail, devid(%u) ret(%d).\n", vinfo.devid, ret);
        return ret;
    }
#ifdef CFG_FEATURE_VF_SINGLE_AICORE
...
/*
 * vdevid = 0xffff, destory all the vdevice created by devid.
 * vdevid != 0xffff destory the specified vdevid num vdevice.
 */
int devdrv_manager_ioctl_destroy_vdev(struct file *filep, unsigned int cmd, unsigned long arg)
{
    ...
    if (ret) {
        devdrv_drv_err("destory_vdevice fail, devid(%u) vdevid(%d), ret(%d).\n", vinfo.devid, vinfo.vdevid, ret);
        return ret;
    }
...
int devdrv_manager_ioctl_get_vdevinfo(struct file *filep, unsigned int cmd, unsigned long arg)
{
    ...
    vinfo = kzalloc(sizeof(struct vdev_query_info), GFP_KERNEL | __GFP_ACCOUNT);
    if (vinfo == NULL) {
