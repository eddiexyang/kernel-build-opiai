...
void agentdrv_res_sdi_addr(u32 dev_id, u32 func_id, struct agent_res_sdi_addr *sdi_addr)
{
    ...
    sdi_addr->bw_ctrl_base = dev_id * DEVDRV_CHIP_ADDR_SPACE_SIZE + DEVDRV_TS_SRAM_ADDR + DEVDRV_VF_BANDWIDTH_OFFSET;
    sdi_addr->rsv_mem_base = dev_id * DEVDRV_CHIP_ADDR_SPACE_SIZE + DEVDRV_RSV_MEM_BASE;
...
{
    return agent_dev->p_agentdrv_msg_dev[func_idx]->doorbell_base +
...
int agentdrv_platform_get_devid(struct platform_device *pdev)
{
    ...
    ret = device_property_read_u32(&pdev->dev, "devid", (u32 *)&dev_id);
    if (ret) {
        devdrv_err("Got the device ID failed. (ret=%d)\n", ret);
        return -EINVAL;
    }

    if ((dev_id >= MAX_AGENTCHIP_CNT) || (dev_id < 0)) {
        devdrv_err("Device ID error. (dev_id=%d)\n", dev_id);
        return -EINVAL;
    }
...
int agentdrv_get_tx_atu_addr(struct platform_device *pdev, u32 func_id, u32 region_index, u64 *start_addr, u64 *size)
{
    ...
    if (res == NULL) {
        devdrv_err("Got resource failed.\n");
        return -EINVAL;
    }

    *start_addr = (u64)res->start;
