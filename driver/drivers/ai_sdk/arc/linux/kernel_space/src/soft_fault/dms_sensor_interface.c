...
#define SOFT_PRINT_REGISTER_LOG(client, user_node, cfg, handle, user_num, flag) \
    soft_drv_event("soft node register success. (dev_id=%u; name=%s; pid=%d; user_id=%u; node_id=%u; node_type=0x%x;" \
        " sensor_type=0x%x; user_num=%u; node_num=%u; handle=0x%llx; sensor_obj_num=%u; first_register=%d)\n\n", \
        (user_node)->dev_id, (cfg)->name, (client)->pid, (client)->user_id, (user_node)->node_id, (cfg)->node_type, \
        (cfg)->sensor_type, (user_num), (client)->node_num, *(handle), (user_node)->sensor_obj_num, (flag))


STATIC DMS_SENSOR_TYPE_T g_sensor_type_list[] = {DMS_SEN_TYPE_HEARTBEAT, DMS_SEN_TYPE_GENERAL_SOFTWARE_FAULT};
...
STATIC int soft_trans_and_check_id(u32 logic_id, u32 *phy_id, u32 *vfid)
{
    ...
    if (logic_id >= DEVDRV_MAX_DAVINCI_NUM) {
    ...
    unsigned int user_id, unsigned int sub_id)
{
    ...
    unsigned int assert_event_mask =
        (cfg->sensor_type == DMS_SEN_TYPE_GENERAL_SOFTWARE_FAULT) ? 0x3FF : AST_MASK; /* 0x3FF: 0-9 */
    unsigned int deassert_event_mask =
        (cfg->sensor_type == DMS_SEN_TYPE_GENERAL_SOFTWARE_FAULT) ? 0x3FF : DST_MASK; /* 0x3FF: 0-9 */
    ...
    *obj = s_cfg;
    obj->private_data = ((uint64_t)user_node->dev_id << SF_OFFSET_32BIT) | (user_id << SF_OFFSET_16BIT) |
        (user_node->node_id << SF_OFFSET_8BIT) | sub_id;
    ...
    if (ret != 0) {
        soft_drv_err("dms register sensor failed. (dev_id=%u; user_name=%s; node_type=%u; sensor_type=%u; ret=%d)\n",
    ...
    struct dms_sensor_node_cfg *cfg, uint64_t *handle)
{
    ...
    if (ret != 0) {
        soft_drv_err("soft sensor register failed. (sub_id=%u; ret=%d)\n", sensor_idx, ret);
        goto ERROR;
    }
...
STATIC struct soft_dev *soft_dev_node_get(unsigned int dev_id, struct soft_dev_client *client,
    struct dms_sensor_node_cfg *cfg, int *first_register)
{
    ...
    u32 *user_id)
{
    ...
    for (i = SF_SENSOR_USER; i < SF_USER_MAX; i++) {
        if (soft_ctrl->s_dev_t[dev_id][i]->registered == 0) {
            *user_id = i;
            return soft_ctrl->s_dev_t[dev_id][i]; /* a new clinet, get a free element for it */
        }
    }
...
STATIC int dms_soft_node_register(unsigned int dev_id, struct dms_sensor_node_cfg *cfg, uint64_t *handle)
{
    ...
    user_node = soft_dev_node_get(dev_id, client, cfg, &first_register);
    if (user_node == NULL) {
...
STATIC int dms_update_sensor_state(unsigned int dev_id, uint64_t handle, int val)
{
    int i;
    int ret;
    unsigned int node_type, sensor_idx, sensor_type;
    ...
    if (user_node == NULL) {
        mutex_unlock(&soft_ctrl->mutex[dev_id]);
        soft_drv_err("Can not find a maching node. (handle=%llu; node_type=0x%x; sensor_type=0x%x; sensor_idx=%u)\n",
        ...
        if ((p_cfg->sensor_type == sensor_type) && (i == sensor_idx)) {
            ...
            if (ret == 0) {
                soft_drv_event("soft node update state success. (dev_id=%u; pid=%d; sensor_name=%s; user_id=%u;"
                    " node_id=%u; node_type=0x%x; sensor_type=0x%x; val=%d; handle=0x%llx)\n", dev_id, client->pid,
                    p_cfg->sensor_name, client->user_id, user_node->node_id, node_type, sensor_type, val, handle);
            }
            break;
        }
    }
...
int soft_node_register(void *feature, char *in, u32 in_len, char *out, u32 out_len)
{
    ...
    if ((in == NULL) || (in_len != sizeof(struct dms_sensor_user)) || (out_len != sizeof(uint64_t))) {
    ...
    if (ret != 0) {
        soft_drv_err("can't transfor dev_id. (dev_id=%u; ret=%d)\n", dev_id, ret);
        return ret;
    }
...
int soft_node_update_state(void *feature, char *in, u32 in_len, char *out, u32 out_len)
{
    ...
    return dms_update_sensor_state(phy_id, handle, val);
}
...
STATIC void soft_client_free_registered_dev(struct soft_dev_client *client)
{
    int i;
    struct soft_dev *pos = NULL;
    struct soft_dev *n = NULL;
...
void soft_client_release(int owner_pid)
{
    ...
    for (i = 0; i < DEV_ID_MAX; i++) {
        ...
        for (j = SF_SENSOR_USER; j < SF_USER_MAX; j++) {
            ...
            soft_client_free_registered_dev(client);
