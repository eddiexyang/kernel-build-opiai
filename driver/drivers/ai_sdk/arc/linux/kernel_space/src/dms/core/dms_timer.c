...
struct dms_sched_task_timer g_timer = {{0}};
...
STATIC enum hrtimer_restart dms_timer_irq_handler(struct hrtimer *htr)
{
    ...
    hrtimer_forward_now(htr, ms_to_ktime(TIMER_STEP_MS));
...
int dms_timer_task_unregister(u32 node_id)
{
    ...
    mutex_unlock(&g_timer.node_lock);
    dms_err("Timer node is not exist. (node_id=%u)\n", node_id);
    return -EINVAL;
}
...
STATIC void dms_set_timer_workqueue_affinity(struct workqueue_struct *wq)
{
    ...
    if (ret != 0) {
        dms_warn("get ctrl cpu mask failed.(ret=%d)\n", ret);
        return;
    }

    if (set_workqueue_affinity(wq, 0, &wq_cpumask) != 0) {
        dms_warn("bind workqueue dms_timer_common failed\n");
    }
}
#endif
