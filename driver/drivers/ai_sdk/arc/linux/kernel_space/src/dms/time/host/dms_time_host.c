...
int g_heart_lost[DEVICE_NUM_MAX] = {0};
#endif
...
int dms_heartbeat_is_stop(u32 dev_id)
{
    ...
    if (ret != 0) {
        dms_warn("Get device status failed. (dev_id=%u; ret=%d))\n", dev_id, ret);
    }
...
{
    g_dms_time_sync_info[dev_id].pre_timezone = (char *)kzalloc(DMS_LOCALTIME_FILE_SIZE, GFP_KERNEL | __GFP_ACCOUNT);
    ...
    if (g_dms_time_sync_info[dev_id].new_timezone == NULL) {
        ...
        kfree(g_dms_time_sync_info[dev_id].pre_timezone);
...
/* dms init and pcie hot reset will call this functiong */
...
STATIC int dms_send_timezone(u32 dev_id, const char *new_timezone, u16 read_size)
{
    ...
    for (i = 0; i < send_times; i++) {
        ...
        if (ret || (time_msg.header.result != 0)) {
            dms_warn("Send msg unsuccessful. (dev_id=%u; sret=%d; result=%u; times=%u; sendtime=%u)\n",
...
STATIC int dms_wall_time_sync(u32 dev_id)
{
    ...
    send_time_info = kzalloc(sizeof(struct dms_walltime_info), GFP_ATOMIC | __GFP_ACCOUNT);
