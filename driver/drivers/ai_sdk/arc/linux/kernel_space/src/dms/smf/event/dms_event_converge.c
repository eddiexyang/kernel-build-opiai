...
u32 dms_event_get_serial_num(atomic_t *serial_atomic)
{
#ifndef AOS_LLVM_BUILD
    u32 notify_num;
    notify_num = (u32)atomic_inc_return(serial_atomic);
#ifdef CFG_HOST_ENV
        notify_num |= DMS_EVENT_NOTIFY_NUM_MASK_HOST;
#endif
    return notify_num;
#else
    return (int)atomic_inc_return(serial_atomic);
#endif
}
...
static inline unsigned int dms_event_get_owner_node_id(const struct dms_event_obj *event_obj)
{
    if (event_obj->sub_node_type != 0) {
        return event_obj->sub_node_id;
    } else {
        return event_obj->node_id;
    }
}
    ...
    char *event_name, u32 name_len)
{
    ...
    owner_node_id = dms_event_get_owner_node_id(event_obj);
    ...
    ret = snprintf_s(event_name, DMS_MAX_EVENT_NAME_LENGTH, DMS_MAX_EVENT_NAME_LENGTH - 1,
                     "node type=%s, node id=%u, sensor type=%s, sensor name=%s, event state=%s, "
                     "time=%llu, event assertion=%u.", node_type, owner_node_id, sensor_type,
                     event_obj->event.sensor_event.sensor_name, event_state, event_obj->time_stamp,
                     event_obj->event.sensor_event.assertion);
    if (ret <= 0) {
        dms_err("Call snprintf_s failed. (deviceid=%u; node type=\"%s\"; node id=%u; sensor type=\"%s\";"
                " sensor name=\"%s\"; event state=\"%s\"; time=%llu; event_assertion=%d; ret=%d)\n",
                event_obj->deviceid, node_type, owner_node_id, sensor_type,
                event_obj->event.sensor_event.sensor_name, event_state, event_obj->time_stamp,
    ...
    DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    exception_node->event.event_code |= \
        DMS_EVENT_OBJ_ASSERTION_TO_CODE(event_obj->event.sensor_event.assertion);
    exception_node->event.event_code |= \
        DMS_EVENT_OBJ_SEVERITY_TO_CODE((unsigned int)event_obj->severity);
    exception_node->event.event_code |= \
...
{
    int ret;
    unsigned int owner_node_type = dms_event_get_owner_node_type(event_obj);

    exception_node->event.pid = event_obj->pid;
    exception_node->event.event_id = DMS_EVENT_OBJ_NODETYPE_TO_CODE(owner_node_type) |
        DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(exception_node->event.event_code);
    ...
    DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    event_node = kzalloc(sizeof(struct dms_event_sensor_reported), GFP_KERNEL | __GFP_ACCOUNT);
    if (event_node == NULL) {
    ...
    do {
        ...
        if (current_node != NULL) {
            ...
            len = snprintf_s(*str, *avl_len, *avl_len - 1, "  (0x%X)""{0x%X(%c:%d)",
                             DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(parent_bro->event_id),
                             DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(current_node->event_id), opt, attribute_info);
            ...
            *avl_len -= len;
            ...
            while ((protect_j++ <= EVENT_CONVERGE_ID_NUM_MAX) && (current_bro != current_node)) {
                ...
                len = snprintf_s(*str, *avl_len, *avl_len - 1, " - 0x%X(%c:%d)",
                                 DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(current_bro->event_id), opt, attribute_info);
                EVENT_DFX_CHECK_DO_SOMETHING(len < 0, return len);
                *str += len;
                *avl_len -= len;
    ...
    u32 devid, char opt, int *avl_len, char **str)
{
    ...
    len = snprintf_s(*str, *avl_len, *avl_len - 1, "Top node: (event_id=0x%X)\n""  0x%X(%c:%d)\n""    |\n",
                     DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(top_node->event_id),
                     DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(top_node->event_id), opt, attribute_info);
    EVENT_DFX_CHECK_DO_SOMETHING(len < 0, return len);
    *str += len;
    *avl_len -= len;
...
ssize_t dms_event_print_convergent_diagrams(u32 devid, char opt, char *str)
{
    ...
    if ((devid >= DEVICE_NUM_MAX) || !EVENT_CONVERGE_OPT_IS_VALID(opt)) {
    ...
    hash_for_each_safe(g_converge_htable.htable, bkt, local_node, convergent_node, hnode) {
        ...
        if (ret < 0) {
            dms_warn("Print convergent one tree failed. (event_id=0x%X)\n",
                     DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(convergent_node->event_id));
            goto out;
        }
    }
...
ssize_t dms_event_print_event_list(u32 devid, char *str)
{
    ...
    if (list_empty_careful(&dev_cb->dev_event_cb.event_list.head)) {
...
int dms_event_code_validity_check(u32 event_code)
{
    ...
    if (DMS_EVENT_CODE_GET_SERVERITY_ASSERTION(event_code)) {
        dms_err("Invalid server and assertion of event code. (event_code=0x%x)\n", event_code);
        return DRV_ERROR_NO_EVENT;
    }

    if (dms_event_id_to_error_string(0, event_code, event_str, DMS_MAX_EVENT_NAME_LENGTH)) {
...
static int dms_event_convergent_item_check(unsigned int item_converge_id[], unsigned int event_id_num)
{
    ...
    for (i = 0; i < event_id_num; i++) {
        if (dms_event_code_validity_check(item_converge_id[i])) {
            dms_err("Invalid event_id. (event_id[%u]=0x%x)\n", i, item_converge_id[i]);
            return DRV_ERROR_PARA_ERROR;
        }
    }
...
static int dms_event_convergent_item_add(unsigned int item_converge_id[], unsigned int event_id_num)
{
    ...
    child_node = (EVENT_CONVERGE_NODE_T **)kzalloc(sizeof(EVENT_CONVERGE_NODE_T *) * event_id_num,
    ...
    for (i = 0; i < event_id_num; i++) {
        ...
        if (event_node == NULL) {
            event_node = kzalloc(sizeof(EVENT_CONVERGE_NODE_T), GFP_KERNEL | __GFP_ACCOUNT);
            if (event_node == NULL) {
                dms_err("Alloc memory of event_node failed.\n");
...
{
    if (dms_event_convergent_item_check(item_converge_id, event_id_num)) {
        dms_warn("Invalid parameter.\n");
        return DRV_ERROR_PARA_ERROR;
    }

    event_id_num = dms_event_convergent_item_fix(item_converge_id, event_id_num);
    if (dms_event_convergent_item_add(item_converge_id, event_id_num)) {
        dms_warn("Add event convergent item failed.\n");
        return DRV_ERROR_OUT_OF_MEMORY;
    }
...
bool dms_event_convergent_diagrams_layer_check(void)
{
    EVENT_CONVERGE_NODE_T *convergent_node = NULL;
    struct hlist_node *local_node = NULL;
    u32 protect_i = 0;
...
int dms_event_convergent_diagrams_init(void)
{
    ...
    if (ret) {
        ...
        dms_warn("Event convergent config init failed, turn off converge switch. (ret=%d)\n", ret);
        return ret;
    }
    ...
    str_buf = (char *)kzalloc(EVENT_DFX_BUF_SIZE_MAX, GFP_KERNEL | __GFP_ACCOUNT);
    if (str_buf != NULL) {
        ...
        if (ret > 0) {
            ...
            dms_warn("Print convergent diagrams failed.\n");
        }
        kfree(str_buf);
    } else {
        dms_warn("kzalloc str_buf failed.\n");
    }
    ...
    hash_for_each_safe(g_converge_htable.htable, bkt, local_node, event_node, hnode) {
        for (i = 0; i < DEVICE_NUM_MAX; i++) {
            if (event_node->exception_data[i] != NULL) {
                kfree(event_node->exception_data[i]);
...
int dms_event_convergent_diagrams_clear(u32 devid, bool hotreset)
{
    ...
    hash_for_each_safe(g_converge_htable.htable, bkt, local_node, event_node, hnode) {
        ...
        if (event_node->exception_data[devid] != NULL) {
            kfree(event_node->exception_data[devid]);
...
void dms_event_cb_release(int owner_pid)
{
    ...
    for (i = 0; i < DEVICE_NUM_MAX; i++) {
        ...
        if (dev_cb == NULL) {
            dms_warn("Get dev_ctrl block failed. (dev_id=%u)\n", i);
            continue;
        }
...
void dms_event_mask_del_to_event_cb(u32 phyid, struct dms_dev_ctrl_block *dev_cb, u32 event_id)
{
    ...
    mutex_lock(&converge_list->lock);
    converge_list->health_code = 0;
    if (list_empty_careful(&converge_list->head)) {
...
void dms_event_mask_add_to_event_cb(u32 phyid, struct dms_dev_ctrl_block *dev_cb, u32 event_id)
{
    ...
    EVENT_CONVERGE_NODE_T *convergent_node = NULL;
    DMS_EVENT_NODE_STRU exception_data = {{0}};
    ...
    if (memcpy_s(&exception_data, sizeof(DMS_EVENT_NODE_STRU),
                 convergent_node->exception_data[phyid], sizeof(DMS_EVENT_NODE_STRU))) {
        mutex_unlock(&g_converge_htable.lock);
        return;
    }
    mutex_unlock(&g_converge_htable.lock);
...
void dms_event_add_to_mask_list(struct dms_dev_ctrl_block *dev_cb, u32 event_id)
{
    ...
    mask_event = (struct dms_mask_event *)kzalloc(sizeof(struct dms_mask_event), GFP_KERNEL | __GFP_ACCOUNT);
    if (mask_event == NULL) {
...
static int dms_event_obj_converge_one_time(DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    do {
        ...
        exception_node->event.event_id = DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(convergent_node->event_id);
        convergent_node = convergent_node->parent;
...
static int dms_event_obj_converge_occur(DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    do {
        ...
        if ((convergent_node->parent == NULL) && (convergent_node->exception_data[deviceid] == NULL)) {
            convergent_node->exception_data[deviceid] = kzalloc(sizeof(DMS_EVENT_NODE_STRU),
                GFP_KERNEL | __GFP_ACCOUNT);
            ...
            convergent_node->exception_data[deviceid]->event.event_id = \
                DMS_EVENT_CODE_CLEAR_SERVERITY_ASSERTION(convergent_node->event_id);
...
static int dms_event_obj_converge_resume(DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    if (convergent_node->exception_data[deviceid] != NULL) {
        ...
        kfree(convergent_node->exception_data[deviceid]);
    ...
    DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    event_node = kzalloc(sizeof(DMS_EVENT_NODE_STRU), GFP_KERNEL | __GFP_ACCOUNT);
    if (event_node == NULL) {
        dms_err("kzalloc failed. (dev_id=%u)\n", exception_node->event.deviceid);
        return DRV_ERROR_OUT_OF_MEMORY;
    }
...
int dms_event_obj_converge(DMS_EVENT_NODE_STRU *exception_node)
{
    if (g_converge_htable.converge_switch == false) {
        return DRV_ERROR_NONE;
    }
    ...
    struct shm_event_code *event_code, u32 event_len)
{
    ...
    if (i > event_len) {
        dms_warn("Event_list is langer than event_code array. (dev_id=%u; event_num=%u)\n", devid, i);
    }
    return DRV_ERROR_NONE;
}
#ifndef DMS_SHARED_NO_EXPORT
