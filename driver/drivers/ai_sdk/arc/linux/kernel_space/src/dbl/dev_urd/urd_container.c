...
int urd_container_task_struct_check(struct task_struct *tsk)
{
    if (tsk == NULL) {
        dms_err("tsk is NULL.\n");
        return -EINVAL;
...
int urd_get_current_mnt_ns(u64 *current_mnt_ns)
{
    int ret;
...
{
    return urd_container_is_admin_task(current);
}
 ...
 */
bool urd_container_is_host_system(struct mnt_namespace *mnt_ns)
{
...
int urd_container_is_in_container(void)
{
    ...
    if (host_mnt == 0) {
        is_in = false;
    } else if (!urd_container_is_admin() && current_mnt != host_mnt) {
        is_in = true;
    } else {
        is_in = false;
    }
