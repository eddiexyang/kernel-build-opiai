...
#if (defined CFG_SOC_PLATFORM_CLOUD) || (defined CFG_SOC_PLATFORM_MINIV2) || (defined CFG_SOC_PLATFORM_MINIV3)
...
component_area_info *g_component_area_info_chip[] = {
    g_component_area_info_chip0,
#if (DEVICE_NUM_MAX > 1)
    g_component_area_info_chip1,
#endif
#if (DEVICE_NUM_MAX > 2)
    g_component_area_info_chip2,
#endif
#if (DEVICE_NUM_MAX > 3)
    g_component_area_info_chip3,
#endif
};

#ifdef CFG_SOC_PLATFORM_MDC_V11
    static componet_boot_area_info g_boot_info[] = {{DSMI_COMPONENT_TYPE_HBOOT1_A, HBOOT1_A_MASK},\
        {DSMI_COMPONENT_TYPE_HILINK, HILINK_MASK},\
        {DSMI_COMPONENT_TYPE_HBOOT1_B, HBOOT1_B_MASK},\
        {DSMI_COMPONENT_TYPE_HBOOT2, HBOOT2_MASK},\
        {DSMI_COMPONENT_TYPE_DDR, DDR_MASK},\
        {DSMI_COMPONENT_TYPE_HSM, HSM_MASK},\
        {DSMI_COMPONENT_TYPE_SYS_BASE_CONFIG, SYS_BASE_CONFIG_MASK},\
        {DSMI_COMPONENT_TYPE_ATF, ATF_MASK}};
#elif (defined SUPPORT_TEE_SECURE_UPGRADE) || (defined CFG_SOC_PLATFORM_MDC_V51)
    static componet_boot_area_info g_boot_info[] = {{DSMI_COMPONENT_TYPE_HBOOT1_A, HBOOT1_A_MASK},\
        {DSMI_COMPONENT_TYPE_HILINK, HILINK_MASK},\
        {DSMI_COMPONENT_TYPE_HBOOT1_B, HBOOT1_B_MASK},\
        {DSMI_COMPONENT_TYPE_HBOOT2, HBOOT2_MASK},\
        {DSMI_COMPONENT_TYPE_DDR, DDR_MASK},\
        {DSMI_COMPONENT_TYPE_LP, LP_MASK},\
        {DSMI_COMPONENT_TYPE_HSM, HSM_MASK},\
        {DSMI_COMPONENT_TYPE_SAFETY_ISLAND, SAFETY_ISLAND_MASK},\
        {DSMI_COMPONENT_TYPE_SYS_BASE_CONFIG, SYS_BASE_CONFIG_MASK}};
#endif
...
STATIC int dev_flash_read_mtd_blk(const char *part_name, unsigned char *buf, unsigned int len, unsigned int offset)
{
    ...
    if ((part_name == NULL) || (buf == NULL)) {
        dev_upgrade_err("para is invalid, part_name: 0x%d\n", !!part_name);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_NULL_POINT);
    }
    ...
    if (ret != 0) {
        dev_upgrade_err(
            "hisi_sfc_ctl_flash_read fail, part_name: %s, offset: %u, len: %u, ret: %d\n", part_name, offset, len, ret);
        return ret;
    }
...
STATIC int dev_flash_write_mtd_blk(const char *part_name, unsigned char *buf, unsigned int len, unsigned int offset)
{
    ...
    if (ret != 0) {
        dev_upgrade_err(
            "dev_flash_erase_mtd_blk fail, part_name: %s, offset: %u, len: %u, ret: %d\n", part_name, offset, len, ret);
        return ret;
    }
...
{
    int max_chip = sizeof(g_component_area_info_chip) / sizeof(component_area_info *);

    if (dev_id >= max_chip) {
        dev_upgrade_err("Invalid device id. (dev=%d)\n", dev_id);
        return NULL;
    }

    return g_component_area_info_chip[dev_id];
}
...
STATIC int get_nve_desc(const char *part_name, NVE_PARTION_HEADER *desc_nve_partion, unsigned int len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        ret = dev_flash_read_mtd_blk(
            part_name, desc_nve_partion[i].name, NVE_NAME_LEN, i * NVE_AREA_LEN + NVE_NAME_OFFSET);
        if (ret != 0) {
        ...
        ret = dev_flash_read_mtd_blk(part_name,
            (unsigned char *)&desc_nve_partion[i].nve_age,
            sizeof(unsigned int),
        ...
        ret = dev_flash_read_mtd_blk(
            part_name, desc_nve_partion[i].nve_version, NVE_VER_LEN, i * NVE_AREA_LEN + NVE_VER_OFFSET);
        if (ret != 0) {
            dev_upgrade_err("read fash error, partion= %s, offset = 0x%x, len = %u, ret = 0x%x\n",
                part_name,
...
STATIC int check_nve_continer(unsigned char *buff)
{
    ...
    if (valid_item > NVE_BNI_NUM_MAX ||
        (unsigned int)(NVE_AREA_LEN - sizeof(unsigned int)) <
        (unsigned int)(NVE_HEAD_LENGTH + (valid_item - 1) * NVE_BIN_ENTRY_LEN + NVE_BIN_ENTRY_OFFSET) ||
        (unsigned int)(NVE_AREA_LEN - sizeof(unsigned int)) <
        (unsigned int)(NVE_HEAD_LENGTH + (valid_item - 1) * NVE_BIN_ENTRY_LEN + NVE_BIN_SIZE_OFFSET)) {
        dev_upgrade_warn("nvbin num is invalid 0x%x\n", valid_item);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_PARAMS);
    }

    p_nve_last_size = (unsigned int
            *)&buff[(unsigned int)(NVE_HEAD_LENGTH + (valid_item - 1) * NVE_BIN_ENTRY_LEN + NVE_BIN_ENTRY_OFFSET)];
    p_nve_last_bin_size = (unsigned int
            *)&buff[(unsigned int)(NVE_HEAD_LENGTH + (valid_item - 1) * NVE_BIN_ENTRY_LEN + NVE_BIN_SIZE_OFFSET)];
...
STATIC int check_nve_partion_validity(const char *part_name, NVE_PARTION_HEADER *desc_nve_partion)
{
    ...
    unsigned char *continer_temp = NULL;
    int belonged_are;
    ...
    continer_temp = nve_continer;
    ret = check_nve_continer(continer_temp);
    if (ret != 0) {
        DEV_UPGRADE_VFREE(nve_continer);
        dev_upgrade_warn("check nve continer fail, ret: %d\n", ret);
        return ret;
...
STATIC int get_max_age_partion(
    const char *part_name, NVE_PARTION_HEADER *desc_nve_partion, unsigned int len, unsigned int *max_age_partion)
{
    unsigned int i;
    ...
    *max_age_partion = 0;
    ...
    dev_upgrade_info(" the partion 0x%x is the current nve , age =0x%x\n",
        *max_age_partion,
        desc_nve_partion[*max_age_partion].nve_age);
    return 0;
}
...
STATIC int nve_find_current_partion(const char *part_name, unsigned int *current_partion)
{
    ...
    ret = memset_s(
        desc_nve_partion, NVE_AREA_CONT * sizeof(NVE_PARTION_HEADER), 0, NVE_AREA_CONT * sizeof(NVE_PARTION_HEADER));
    if (ret != 0) {
...
int dev_upgrade_component_version_get(
    upg_comm_para *comm_para, unsigned int is_memory, unsigned char *o_buf, unsigned int o_len)
{
    int ret;
    ...
    if (!is_memory) {
        if (comp_area_info->store_in == STORE_IN_FILESYSTEM) {
            ...
            ret = dev_flash_read_mtd_blk(
                comp_area_info->part_name, o_buf, COMM_VERSION_LENGTH, (comp_area_info->offset + ver_offset));
        }
        if (ret != 0) {
...
{
    if ((magic == CMS_MAGIC_FLAG_VALUE) &&
        ((component_type != DSMI_COMPONENT_TYPE_XLOADER) && (component_type != DSMI_COMPONENT_TYPE_UEFI))) {
        return true;
    }
...
STATIC int dev_upgrade_get_size_from_flash(component_area_info *area_info, unsigned int *length)
{
    ...
    ret = dev_flash_read_mtd_blk(
        area_info->part_name, (unsigned char *)&code_size, IMAGE_CODE_LEN, (area_info->offset + IMAGE_CODE_LEN_OFFSET));
    if (ret != 0) {
        dev_upgrade_err("read flash fail.\n");
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }

    ret = dev_flash_read_mtd_blk(area_info->part_name,
        (unsigned char *)&magic,
        CMS_MAGIC_FLAG_SIZE,
        (area_info->offset + CMS_MAGIC_FLAG_OFFSET));
    if (ret != 0) {
    ...
    if (dev_upgrade_is_cms_support(magic, area_info->component_type) == true) {
        ret = dev_flash_read_mtd_blk(area_info->part_name,
            (unsigned char *)&fw_total_size,
            FILE_TOTAL_LEN_SIZE,
            (area_info->offset + FILE_TOTAL_LEN_OFFSET));
        if (ret != 0) {
        ...
        *length = fw_total_size;
        ...
        if (code_size >= area_info->size) {
            dev_upgrade_err("code_size invalid , component(%u), length: %x, max_size: %x\n",
                area_info->component_type,
                *length,
                area_info->size);
            return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_LENGTH);
        }
...
STATIC void dev_upgrade_table_dump(dev_component_list_ctrl *comp_list_ctrl)
{
    ...
    dev_upgrade_debug("\n-----------------------------------------------------------\n"
                      "dev_id: %d, count: %u, bitmap: %x\n",
        comp_list_ctrl->dev_id,
        comp_list_ctrl->count,
        comp_list_ctrl->bitmap);
...
int dev_upgrade_nve_sha256_check(unsigned char *buff_addr, unsigned int valid_items, unsigned int len)
{
    ...
    unsigned int nve_bin_offset, i_count;
    ...
    if ((valid_items < 1) ||
        (len < NVE_HEAD_LENGTH + ((long)valid_items - 1) * NVE_BIN_ENTRY_LEN + NVE_BIN_SIZE_OFFSET)) {
        dev_upgrade_err("buff length is invalid! length = %u, valid_items = %u\n", len, valid_items);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_PARAMS);
    }
    ...
    if (nve_bin_offset > len) {
        dev_upgrade_err("nve_bin_offset is invalid! length = %u\n", len);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_PARAMS);
    }
    ...
    crc_data_buf = (unsigned char *)vzalloc(len);
    ...
    for (i_count = 0; i_count < len; i_count++) {
        if ((i_count >= NVE_SHA256_OFFSET) && (i_count < NVE_RESERVE_HEAD_OFFSET)) {
            continue;
        }
...
int dev_upgrade_component_space_get(int dev_id, unsigned int component_type, unsigned int *space_size)
{
    ...
    if (space_size == NULL) {
        dev_upgrade_err("space_size is NULL\n");
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_NULL_POINT);
    }
    ...
    *space_size = comp_area_info->size;
...
STATIC int dev_upgrade_op_cmd_proc(boot_area_info *bios_info, unsigned int op_flag, unsigned int *op_area)
{
    ...
    if (op_flag == DEV_GET_CURR_BOOT_AREA) {
        /*
         * mini--sysctrl reg(0x1100c3448 byte1)/cloud--isram(0xA0D1028x byte1)
         * 0x1--area0 boot
         * 0x2--area1 boot
         */
    ...
    } else if (op_flag == DEV_GET_BOOT_FAIL_COUNT) {
        value = *(volatile unsigned int *)(map_base + bios_info->boot_area_ofs);
        *op_area = value & bios_info->boot_cnt_mask;
    } else {
        dev_upgrade_warn("Unknow op cmd. (op_flag=%x)\n", op_flag);
    }
...
{
    unsigned int type_to_offset[DSMI_COMPONENT_TYPE_MAX] = {ISRAM_NVE_OFFSET,
        SC_BAK_DATA14,
        0xffffffff,
        ISRAM_UEFI_OFFSET,
        0xffffffff,
...
#ifdef CFG_SOC_PLATFORM_CLOUD
    /* UEFI image only flash in P1 is used */
    if (component_type == DSMI_COMPONENT_TYPE_UEFI) {
        bios_info->base_addr = CPU_ISRAM_BASE;
    } else {
        bios_info->base_addr = CPU_ISRAM_BASE + CHIP_REG_OFFSET * dev_id;
    }
    bios_info->boot_area_ofs = type_to_offset[component_type];
    bios_info->boot_area_shift = BITS_OF_CHAR;
    bios_info->boot_area_mask = 0xff;
    bios_info->boot_cnt_ofs = type_to_offset[component_type];
    bios_info->boot_cnt_mask = 0xff;
#else
...
STATIC int dev_upgrade_flash_copy(const component_area_info *src_area_info, component_area_info *dest_area_info)
{
    ...
    unsigned int block_nums, remain, finish_blks;
    unsigned int block_size = FLASH_BLOCK_SIZE;

    buff = (unsigned char *)vzalloc(block_size);
    if (buff == NULL) {
        dev_upgrade_err("vzalloc fail.\n");
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_MEM_MALLOC);
    }

    block_nums = src_area_info->size / block_size;
    remain = src_area_info->size % block_size;
    finish_blks = 0;
    if (remain) {
        block_nums++;
    }
    ...
    for (index = 0; index < block_nums; index++) {
        ret = dev_flash_read_mtd_blk(src_area_info->part_name, buff, block_size, offset + src_area_info->offset);
        if (ret != 0) {
            DEV_UPGRADE_VFREE(buff);
            dev_upgrade_err("read flash fail, ret: %d\n", ret);
            return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
        }

        ret = dev_flash_write_mtd_blk(dest_area_info->part_name, buff, block_size, offset + dest_area_info->offset);
        if (ret != 0) {
            DEV_UPGRADE_VFREE(buff);
            dev_upgrade_err("write flash fail, ret: %d\n", ret);
            return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_WRITE);
        }
        offset += block_size;
        finish_blks++;

        if (!(finish_blks % FLASH_BLKS_ONCE)) {
            msleep(1);
        }
    }
...
int dev_upgrade_component_image_copy(
    int dev_id, unsigned int component_type, unsigned int dest_area, unsigned int src_area)
{
    int ret;
...
STATIC bool dev_upgrade_mdc_part_name_found(sec_flash_mtd_info_s *mtd_part, int part_num, const char *part_name)
{
    int i;

    for (i = 0; i < part_num; i++) {
        if (strcmp((char *)mtd_part[i].mtd_name, part_name) == 0) {
            return true;
        }
    }
    return false;
...
STATIC int dev_upgrade_mdc_all_flash_match(int dev_id, component_area_info *comp_area_info,
    dev_component_list_ctrl *comp_list_ctrl, sec_flash_mtd_info_s *mtd_part, unsigned int total_count)
{
    ...
    while (comp_area_info->component_type < DSMI_COMPONENT_TYPE_MAX) {
        if (dev_upgrade_mdc_part_name_found(mtd_part, total_count, comp_area_info->part_name) == true) {
...
#endif
STATIC int dev_upgrade_flash_part_match(
    int dev_id, component_area_info *comp_area_info, dev_component_list_ctrl *comp_list_ctrl)
{
    int ret;
    unsigned int total_count = 0;
    sec_flash_mtd_info_s *mtd_part = NULL;

    ret = sec_flash_read_mtd_nums(&total_count);
    if (ret != OK) {
        dev_upgrade_err("dev(%d) sec_flash_read_mtd_nums failed!\n", dev_id);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_SEEK);
    }

    if ((total_count == 0) || (total_count > FLASH_MTD_NUM_MAX)) {
        dev_upgrade_err("dev(%d) total mtd num[%u] is invalid.\n", dev_id, total_count);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_PARAMS);
    }

    mtd_part =
        (sec_flash_mtd_info_s *)kzalloc((total_count * sizeof(sec_flash_mtd_info_s)), GFP_KERNEL | __GFP_ACCOUNT);
    if (mtd_part == NULL) {
        dev_upgrade_err("dev(%d) kzalloc failed!\n", dev_id);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_MEM_MALLOC);
    }

    ret = sec_flash_read_mtd_info(total_count, mtd_part);
    if (ret != OK) {
        dev_upgrade_err("dev(%d) hisi_ufs_read_mtd_info fail, ret: %d\n", dev_id, ret);
        DEV_UPGRADE_KFREE(mtd_part);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }

    ret = dev_upgrade_mdc_all_flash_match(dev_id, comp_area_info, comp_list_ctrl, mtd_part, total_count);
    if (ret != OK) {
        dev_upgrade_err("dev(%d) dev_upgrade_all_flash_match fail, ret: %d\n", dev_id, ret);
        DEV_UPGRADE_KFREE(mtd_part);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }

    dev_upgrade_info("dev(%d) total_count: %u, find_cnt = %u\n", dev_id, total_count, comp_list_ctrl->count);
...
int dev_upgrade_flash_part_match(
    int dev_id, component_area_info *comp_area_info, dev_component_list_ctrl *comp_list_ctrl)
{
    int ret;
    unsigned int find_cnt = 0;
    ...
    mtd_part = (struct hisfc_mtd_info_st *)kzalloc(
        (total_count) * sizeof(struct hisfc_mtd_info_st), GFP_KERNEL | __GFP_ACCOUNT);
    if (mtd_part == NULL) {
        dev_upgrade_err("dev(%d) kzalloc failed!\n", dev_id);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_MEM_MALLOC);
    }
...
STATIC int dev_upgrade_fs_part_match(
    int dev_id, component_area_info *comp_area_info, dev_component_list_ctrl *comp_list_ctrl)
{
...
int dev_upgrade_component_info_match(int dev_id, unsigned int *match_nums)
{
    ...
    ret = memset_s((void *)comp_list_ctrl, sizeof(dev_component_list_ctrl), 0, sizeof(dev_component_list_ctrl));
    if (ret != 0) {
...
{
    if ((component_type >= DSMI_COMPONENT_TYPE_RAWDATA) && (component_type <= DSMI_COMPONENT_TYPE_CUSTOMIZED)) {
        return OK;
    }

    if (component_type == DSMI_COMPONENT_TYPE_RECOVERY) {
        return OK;
    }
...
{
    dev_upgrade_err("sec_flash_read_cmdline error, esl don't support tee");
    return 0;
}
#endif

#ifdef CFG_SOC_PLATFORM_MDC_V51
static inline unsigned char boot_state(unsigned int cmdline, unsigned int area_mask)
{
    return ((cmdline & area_mask) > 0) ? 1 : 0;
}
#endif
...
int dev_upgrade_sync_pre_check(int dev_id)
{
    ...
    unsigned char first_boot_state;
    unsigned char temp_boot_state;

    /* check flash firmware boot area, if not the same, not sync */
    ret = sec_flash_read_cmdline(dev_id, &cmdline);
    if (ret != 0) {
        dev_upgrade_err("dev_id(%d) get sec flash read cmdline failed, ret=%d.\n", dev_id, ret);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }
    /*
     * cmdline: each bit means one component's boot area, 0 means boot from master, 1 means backup
     * bit 8: Hboot1_a.bin,   bit 9: HilinkFirmware.bin,  bit 10: hboot1_b,         bit 11: AS610_HBOOT2_UEFI.fd,
     * bit 12: lpddr_mcu.bin, bit 13: lperika.bin,        bit 14: hisserika.bin,    bit 15: silerika.bin
     * first, get first component's boot area, then compare if other component's boot area is the same,
     * if it's not the same, stop sync
     */
    first_boot_state = boot_state(cmdline, g_boot_info[0].area_mask);
    for (i = 1; i < sizeof(g_boot_info) / sizeof(g_boot_info[0]); i++) {
        temp_boot_state = boot_state(cmdline, g_boot_info[i].area_mask);
        if (temp_boot_state != first_boot_state) {
            dev_upgrade_err("flash boot area is not the same, cmdline=0x%x.\n", cmdline);
            return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_PARAMS);
        }
    }
#endif
...
/* get ufs or flash start up area : master or backup */
STATIC int dev_upgrade_get_boot_area(int dev_id, unsigned int component_type, unsigned int *cur_flag)
{
    int ret;
    int i;
    unsigned int cmdline = 0;
    unsigned int flash_area_mask = 0;

#ifdef CFG_SOC_PLATFORM_MDC_V51
    upgrade_ufs_component_info *component_info = NULL;

    if (dev_upgrade_ufs_valid_chk(component_type) == OK) {
        component_info = dev_upgrade_ufs_get_local_component(component_type);
        if (component_info != NULL) {
            *cur_flag = component_info->cur_flag;
        } else {
            *cur_flag = UPGRADE_MASTER_AREA;
            dev_upgrade_warn("dev(%d) no component type(%u).\n", dev_id, component_type);
        }
        return 0;
    }
#endif
    ...
    ret = sec_flash_read_cmdline(dev_id, &cmdline);
    if (ret != 0) {
        dev_upgrade_err("get sec flash read cmdline failed, dev_id(%d).\n", dev_id);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }

    for (i = 0; i < sizeof(g_boot_info) / sizeof(g_boot_info[0]); i++) {
        if (component_type == g_boot_info[i].component_type) {
            flash_area_mask = g_boot_info[i].area_mask;
            *cur_flag = ((cmdline & flash_area_mask) == flash_area_mask) ? UPGRADE_STANDBY_AREA : UPGRADE_MASTER_AREA;
            return 0;
        }
    }
...
int dev_upgrade_component_boot_area_op(
    int dev_id, unsigned int component_type, unsigned int op_flag, unsigned int *op_area)
{
    int ret;

#ifdef SUPPORT_TEE_SECURE_UPGRADE
    ret = dev_upgrade_get_boot_area(dev_id, component_type, op_area);
    if (ret != 0) {
        dev_upgrade_err("dev(%d) component_type: %u get boot area failed, ret = (%d)\n", dev_id, component_type, ret);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }
#else
...
STATIC int dev_upgrade_xloader_file_type_check(const unsigned char *file_content, unsigned int file_size)
{
    ...
    code_base =
        (unsigned char *)(uintptr_t)(((uintptr_t)file_content) + IMAGE_CODE_OFFSET + XLOADER_FILE_TYPE_CHECK_OFFSET);
...
STATIC int dev_upgrade_uefi_file_type_check(const unsigned char *file_content, unsigned int file_size)
{
    ...
    DRV_CHECK_RETV(
        (file_size > (IMAGE_CODE_OFFSET + UEFI_FILE_TYPE_CHECK_OFFSET_SIGNATURE + UEFI_FILE_TYPE_CHECK_SIGNATURE_LEN)),
        ERRNO_INVALID_LENGTH);
    code_base =
        (unsigned char *)(uintptr_t)(((uintptr_t)file_content) + IMAGE_CODE_OFFSET + UEFI_FILE_TYPE_CHECK_OFFSET_EUID);
    /* check guid */
    ...
    code_base = (unsigned char *)(uintptr_t)(((uintptr_t)file_content) + IMAGE_CODE_OFFSET +
                                             UEFI_FILE_TYPE_CHECK_OFFSET_SIGNATURE);
    /* check Signature */
    if (memcmp((void *)code_base, (void *)UEFI_FILE_TYPE_CHECK_FLAG_SIGNATURE, UEFI_FILE_TYPE_CHECK_SIGNATURE_LEN) !=
        0) {
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FILE_TYPE);
    }
...
#ifdef CFG_SOC_PLATFORM_CLOUD
STATIC int dev_upgrade_nve_file_type_check(const unsigned char *file_content, unsigned int file_size)
{
    unsigned char *code_base = NULL;
    DRV_CHECK_RETV((file_size > (IMAGE_CODE_OFFSET + NVE_FILE_TYPE_CHECK_LEN)), ERRNO_INVALID_LENGTH);
    code_base = (unsigned char *)(uintptr_t)(((uintptr_t)file_content) + IMAGE_CODE_OFFSET);
    if (memcmp((void *)code_base, (void *)NVE_FILE_TYPE_CHECK_FLAG, NVE_FILE_TYPE_CHECK_LEN) == 0) {
        return 0;
    }
...
STATIC int dev_upgrade_imu_file_type_check(const unsigned char *file_content, unsigned int file_size)
{
    unsigned char *code_base = NULL;
    DRV_CHECK_RETV(
        (file_size > (IMAGE_CODE_OFFSET + IMU_FILE_TYPE_CHECK_OFFSET + IMU_FILE_TYPE_CHECK_LEN)), ERRNO_INVALID_LENGTH);
    code_base =
        (unsigned char *)(uintptr_t)(((uintptr_t)file_content) + IMAGE_CODE_OFFSET + IMU_FILE_TYPE_CHECK_OFFSET);
    if (memcmp((void *)code_base, (void *)IMU_FILE_TYPE_CHECK_FLAG, IMU_FILE_TYPE_CHECK_LEN) == 0) {
        return 0;
    }
    return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FILE_TYPE);
}

STATIC int dev_upgrade_imp_file_type_check(const unsigned char *file_content, unsigned int file_size)
{
    unsigned char *code_base = NULL;
    DRV_CHECK_RETV(
        (file_size > (IMAGE_CODE_OFFSET + IMP_FILE_TYPE_CHECK_OFFSET + IMP_FILE_TYPE_CHECK_LEN)), ERRNO_INVALID_LENGTH);
    code_base =
        (unsigned char *)(uintptr_t)(((uintptr_t)file_content) + IMAGE_CODE_OFFSET + IMP_FILE_TYPE_CHECK_OFFSET);
    if (memcmp((void *)code_base, (void *)IMP_FILE_TYPE_CHECK_FLAG, IMP_FILE_TYPE_CHECK_LEN) == 0) {
        return 0;
    }
...
STATIC int dev_upgrade_file_type_check(
    unsigned int component_type, const unsigned char *file_content, unsigned int file_size)
{
    int ret = 0;
    switch (component_type) {
        ...
        case DSMI_COMPONENT_TYPE_UEFI:
            ...
            break;
#ifdef CFG_SOC_PLATFORM_CLOUD
        case DSMI_COMPONENT_TYPE_NVE:
            ret = dev_upgrade_nve_file_type_check(file_content, file_size);
            break;
        case DSMI_COMPONENT_TYPE_IMU:
            ret = dev_upgrade_imu_file_type_check(file_content, file_size);
            break;
        case DSMI_COMPONENT_TYPE_IMP:
            ret = dev_upgrade_imp_file_type_check(file_content, file_size);
            break;
#endif
        default:
            break;
    ...
    if (ret != 0) {
        dev_upgrade_err("file type:%u check failed:%d.\n", component_type, ret);
    }
    return ret;
}

STATIC int dev_upgrade_non_cms_verification(
    int dev_id, unsigned int type, unsigned char *file_content, unsigned int file_size)
{
    int ret;
    int sign;
    ...
    if (ret) {
        dev_upgrade_err("Failed to invoke the devdrv_config_get_pss_cfg. (dev_id=%d; ret=%d)", dev_id, ret);
    ...
    img_len = *(unsigned int *)(file_content + IMAGE_CODE_LEN_OFFSET);
    if (file_size != (img_len + SEC_HEAD_SIZE)) {
        verify_finalize_session(dev_id);
        dev_upgrade_err("dev(%d) file_size(%u) err.\n", dev_id, file_size);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_LENGTH);
    }

    head_offset = 0x00;
    code_base = (unsigned char *)(uintptr_t)(((uintptr_t)file_content) + IMAGE_CODE_OFFSET);
    if (sec_update_verification(dev_id,
...
int dev_upgrade_sec_verification(int dev_id, unsigned int type, unsigned char *file_content, unsigned int file_size)
{
    ...
    if ((type >= DSMI_COMPONENT_TYPE_MAX) || (dev_id >= DEVICE_NUM_MAX)) {
        dev_upgrade_err("dev(%d) type(%u) para invalid\n", dev_id, type);
...
void dev_upgrade_clear_localcfg(int dev_id)
{
    ...
    ret = devdrv_config_clear_user_item((unsigned int)dev_id, UC_ITEM_INDEX_USER_SHADOW);
    if (ret != 0) {
        dev_upgrade_err("devdrv config clear usrshadow info err, Reset user config fail, ret: %d\n", ret);
        return;
    }

#if (defined CFG_SOC_PLATFORM_MINIV2) || (defined CFG_SOC_PLATFORM_MINIV3)
    ret = devdrv_config_clear_user_item((unsigned int)dev_id, UC_ITEM_INDEX_SSH_STATUS);
    if (ret != 0) {
        dev_upgrade_err("devdrv config clear ssh status info err, Reset user config fail, ret: %d\n", ret);
        return;
    }
#endif
    ...
    ret = dev_flash_erase_mtd_blk(
        PART_NAME_USER_CONFIG, DEV_FLASH_DOUBLE_AREAS * PART_NAME_USER_CONFIG_SIZE, PART_NAME_USER_CONFIG_OFFSET);
    if (ret != 0) {
...
STATIC int check_flash_version_crc(int dev_id, struct flash_version_info_stru *version_info,
    unsigned char *component, unsigned int crc_offset)
{
    int ret = 0;
    unsigned int crc;
    ret = sec_flash_read(dev_id, crc_offset, (unsigned char *)&crc, CHECK_LEN);
    if (ret != 0) {
        dev_upgrade_info("cannot read crc.(devid=%d)\n", dev_id);
        return ret;
    }
    version_info->crc = make_crc_checksum(0,
        (const unsigned char *)version_info, sizeof(struct flash_version_info_stru) - sizeof(unsigned int));
    if (version_info->crc != crc) {
        *component = UNKNOWN_COMPONENT;
        dev_upgrade_err("crc check failed (devid=%d; crc1=%x; crc2=%x).\n", dev_id, version_info->crc, crc);
        return -EINVAL;
    }
    return ret;
}

STATIC int get_dev_flash_compatible_version(int dev_id, unsigned int *flash_version, unsigned char *component)
{
    int ret;
    unsigned int magic_num = 0;
    unsigned int crc_offset = 0;
    struct flash_version_info_stru version_info = {0};

    // 判断主区flash 0x1940000
    ret = sec_flash_read(dev_id, FLASH_OFFSET_PRIMARY_MAGIC, (unsigned char *)&magic_num, CHECK_LEN);
    if (ret != 0) {
        dev_upgrade_info("cannot read FLASH_OFFSET_PRIMARY_MAGIC.(devid=%d)\n", dev_id);
        return ret;
    }
    if (magic_num != MAGIC_NUMBER) { // 主区无魔术字,查看备区0x1980000
        dev_upgrade_info("Primary_magic:%x\n", magic_num);
        ret = sec_flash_read(dev_id, FLASH_OFFSET_SECONDARY_MAGIC, (unsigned char *)&magic_num, CHECK_LEN);
        if (ret != 0) {
            dev_upgrade_info("cannot read FLASH_OFFSET_SECONDARY_MAGIC.(devid=%d)\n", dev_id);
            return ret;
        }
        if (magic_num != MAGIC_NUMBER) { // 主备区均无魔术字,可以升级
            return 0;
        } else { // 备区有魔术字,查看支持的最低版本
            *component = (unsigned char)NEW_COMPONENT;
            crc_offset = FLASH_OFFSET_SECONDARY_CRC;
            ret = sec_flash_read(dev_id, FLASH_OFFSET_SECONDARY_VERSION, (unsigned char *)flash_version, VERSION_LEN);
            if (ret != 0) {
                dev_upgrade_info("cannot read FLASH_OFFSET_SECONDARY_VERSION.(devid=%d)\n", dev_id);
                return ret;
            }
        }
    } else { // 主区有魔术字,查看支持的最低版本
        *component = (unsigned char)NEW_COMPONENT;
        crc_offset = FLASH_OFFSET_PRIMARY_CRC;
        ret = sec_flash_read(dev_id, FLASH_OFFSET_PRIMARY_VERSION, (unsigned char *)flash_version, VERSION_LEN);
        if (ret != 0) {
            dev_upgrade_info("cannot read FLASH_OFFSET_PRIMARY_VERSION.(devid=%d)\n", dev_id);
            return ret;
        }
    }

    version_info.magic_num = magic_num;
    version_info.version = *flash_version;
    ret = check_flash_version_crc(dev_id, &version_info, component, crc_offset);
    return ret;
}

STATIC int check_firmware_compatible_version_is_valid(unsigned char magic_num)
{
    if (magic_num != FIRMWARE_MAGIC_NUMBER) {
        dev_upgrade_err("Magic num is invalid.(magic_num=%u)\n", magic_num);
        return -1;
    }
    return 0;
}

STATIC int get_dev_firmware_compatible_version(int dev_id, unsigned char *file_content,
    unsigned int file_size, unsigned short *firmware_version)
{
    int ret;
    ...
    if (file_size < EP_FILE_SIZE) {
        dev_upgrade_err("file_size(%u) invalid\n", file_size);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_NONSUPPORT_ITEM);
    }

    magic = *(unsigned char *)(file_content + FIRMWARE_BASE + FIRMWARE_MAGIC_OFFSET);
    ret = check_firmware_compatible_version_is_valid(magic);
    if (ret != 0) {
        dev_upgrade_err("magic is invalid\n");
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_NONSUPPORT_ITEM);
    }
    *firmware_version = *(unsigned short *)(file_content + FIRMWARE_BASE + FIRMWARE_VERSION_OFFSET);
...
{
    int ret;
    unsigned int flash_version = 0;
    unsigned short firmware_version = 0;
    unsigned char component = OLD_COMPONENT;

    if ((dev_id < 0) || (dev_id >= MAX_DEVICE_NUM)) {
        dev_upgrade_err("device id is invalid.(devid=%d)\n", dev_id);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_NULL_POINT);
    }
    ret = get_dev_flash_compatible_version(dev_id, &flash_version, &component);
    if (ret != 0) {
        if (component == UNKNOWN_COMPONENT) {
            return ret; /*  if the CRC check fails, return error */
        }
        dev_upgrade_err("flash compatible file default.(devid=%d)\n", dev_id);
        return 0; /* if flash file is not exist or invalid, just pass the check process */
    } else if (component == OLD_COMPONENT) {
        return 0; /* If the component is an old component, anti-rollback interception is not required. */
    }

    ret = get_dev_firmware_compatible_version(dev_id, file_content, file_size, &firmware_version);
    if (ret != 0) {
        dev_upgrade_err("get firmware compatible version failed.(devid=%d)\n", dev_id);
        return ret;
    }
    dev_upgrade_info("firmware_version = %x, flash_version = %x\n", firmware_version, flash_version);
    if (flash_version > (unsigned int)firmware_version) {
        dev_upgrade_err("version not matched. flash_version : %x, firmware_version : %x\n",
            flash_version, firmware_version);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_NONSUPPORT_ITEM);
    }
    return 0;
}
...
int dev_upgrade_update_cnt(int dev_id)
{
    ...
    ret = sec_update_cnt(dev_id);
    if (ret != 0) {
        verify_finalize_session(dev_id);
        dev_upgrade_err("dev(%d) sec_update_cnt fail, ret: %d\n", dev_id, ret);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_HASH_CHECK);
    }

    verify_finalize_session(dev_id);
    dev_upgrade_info("dev(%d) sec_update_cnt success.\n", dev_id);
    return ret;
}

STATIC int dev_upgrade_read_firmware_head(int dev_id, unsigned char *buf, unsigned int buf_size)
{
    int ret;
    unsigned int boot_area = 0;
    component_area_info *boot_area_info = NULL;
    unsigned int component_type = DSMI_COMPONENT_TYPE_IMU; /* only read data from imu */
    upg_comm_para comm_para = {0};

    /* get boot area */
    ret = dev_upgrade_component_boot_area_op(dev_id, component_type, DEV_GET_CURR_BOOT_AREA, &boot_area);
    if (ret != 0) {
        dev_upgrade_err("dev(%d) get curr boot area fail, boot_area: %u, ret: %d\n", dev_id, boot_area, ret);
        return ret;
    }
    ...
    /* get boot area info */
    boot_area_info = dev_component_area_info_search(&comm_para);
    if (boot_area_info == NULL) {
        dev_upgrade_err("dev(%d) get comp_area_info struct fail, type(%u) area(%u)\n",
            comm_para.dev_id,
            comm_para.type,
            comm_para.area);
    ...
    /* read imu head content from flash */
    ret = dev_flash_read_mtd_blk(boot_area_info->part_name, buf, buf_size, boot_area_info->offset);
    if (ret != 0) {
        dev_upgrade_err("dev(%d) read flash fail, ret: %d\n", dev_id, ret);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }
...
STATIC int dev_upgrade_adjust_pubk_position(unsigned char *pubk_data, unsigned int pubk_size)
{
    int ret;
    unsigned char *tmp_data = NULL;

    if (pubk_size != ROOT_PUBK_TOTAL_SIZE) {
        dev_upgrade_err("pubk size[%u] is not correct.\n", pubk_size);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_PARAMS);
    }

    tmp_data = (unsigned char *)kzalloc(ROOT_PUBK_LEN_VAL, GFP_KERNEL | __GFP_ACCOUNT);
    if (tmp_data == NULL) {
        dev_upgrade_err("kzalloc fail for tmp_data.\n");
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_NULL_POINT);
    }

    /* reverse pubk data, exchange last 512 bytes and first 512 bytes */
    ret = memcpy_s(tmp_data, ROOT_PUBK_LEN_VAL, pubk_data, ROOT_PUBK_LEN_VAL);
    if (ret != OK) {
        dev_upgrade_err("memcpy from pubk_data(first) to tmp data fail.\n");
        DEV_UPGRADE_KFREE(tmp_data);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_WRITE);
    }

    ret = memcpy_s(pubk_data, pubk_size, (pubk_data + ROOT_PUBK_LEN_VAL), ROOT_PUBK_LEN_VAL);
    if (ret != OK) {
        dev_upgrade_err("memcpy from pubk_data(last) to pubk_data(first) fail.\n");
        DEV_UPGRADE_KFREE(tmp_data);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_WRITE);
    }

    ret = memcpy_s((pubk_data + ROOT_PUBK_LEN_VAL), ROOT_PUBK_LEN_VAL, tmp_data, ROOT_PUBK_LEN_VAL);
    if (ret != OK) {
        dev_upgrade_err("memcpy from tmp data to pubk_data(last) fail.\n");
        DEV_UPGRADE_KFREE(tmp_data);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_WRITE);
    }
...
int dev_upgrade_sec_rim_data(int dev_id, const unsigned char *rim_buf, unsigned int rim_size)
{
    ...
    unsigned int pubk_len;

    head_buf = (unsigned char *)kzalloc(FIRMWARE_HEAD_LEN, GFP_KERNEL | __GFP_ACCOUNT);
    if (head_buf == NULL) {
    ...
    /* read imu head */
    ret = dev_upgrade_read_firmware_head(dev_id, head_buf, FIRMWARE_HEAD_LEN);
    if (ret != 0) {
        dev_upgrade_err("dev(%d) dev_upgrade_read_imu_head fail, ret = %d.\n", dev_id, ret);
        DEV_UPGRADE_KFREE(head_buf);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }

    /* check if flash read content is valid */
    pubk_len = *(unsigned int *)(head_buf + ROOT_BUBK_LEN_OFFSET);
    if (pubk_len != ROOT_PUBK_LEN_VAL) {
        dev_upgrade_err("pubk_len[%u] in flash is err.\n", pubk_len);
        DEV_UPGRADE_KFREE(head_buf);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_FS_READ);
    }

    /* get root pubk from head */
    pubk_buf = head_buf + ROOT_PUBK_OFFSET;
    pubk_size = ROOT_PUBK_TOTAL_SIZE;

    /* exchange pubk data's first 512 bytes and last 512 bytes */
    ret = dev_upgrade_adjust_pubk_position(pubk_buf, pubk_size);
    if (ret != 0) {
        dev_upgrade_err("dev(%d) dev_upgrade_adjust_pubk_position fail, ret: %d\n", dev_id, ret);
        DEV_UPGRADE_KFREE(head_buf);
        return ret;
    }
    ...
    if (ret != 0) {
        dev_upgrade_err("dev(%d) verify_init_session fail, ret: %d\n", dev_id, ret);
        DEV_UPGRADE_KFREE(head_buf);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_VERIFY_OR_REVOCATION);
    }

    ret = sec_rim_data(dev_id, pubk_buf, pubk_size, rim_buf, rim_size);
    if (ret != 0) {
        verify_finalize_session(dev_id);
        dev_upgrade_err("dev(%d) sec_rim_data failed, ret: %d\n", dev_id, ret);
        DEV_UPGRADE_KFREE(head_buf);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_VERIFY_OR_REVOCATION);
    }
...
STATIC int dev_upgrade_get_root_hash(int dev_id, u32 *efuse_hash)
{
    ...
    for (i = 0; i < EFUSE_HASH_WORDS; i++) {
        ...
        if (ret != 0) {
            dev_upgrade_err("Ef hash read fail. (dev_id=%d; ret=%d)\n", dev_id, ret);
            return -1;
        }
    }
...
#ifdef CFG_SOC_PLATFORM_CLOUD
STATIC int dev_upgrade_get_nvcnt(int dev_id, u32 *nvcnt)
{
    int ret;

    ret = verify_init_session(dev_id);
    if (ret != 0) {
        dev_upgrade_err("get nv cnt init session fail. (dev=%d; ret=%d)\n", dev_id, ret);
        return -1;
    }
    ret = soc_get_nvcnt(dev_id, nvcnt, sizeof(u32));
    verify_finalize_session(dev_id);
    if (ret) {
        dev_upgrade_err("get nv cnt fail. (dev=%d; ret=%d)\n", dev_id, ret);
        return -1;
    }
    return 0;
}
...
int init_get_nvcnt()
{
    ...
    if (ret != 0) {
        dev_upgrade_err("Get device number failed. (ret=%d)", ret);
        return ret;
    }
    for (i = 0; i < dev_num; i++) {
        ret = dev_upgrade_get_nvcnt(i, &g_nvcnt_value[i]);
        if (ret) {
            g_nvcnt_value[i] = INVALID_NVCNT;
            continue;
        }
        final_ret = 0;
        dev_upgrade_info("Get nvcnt success. (dev_id=%d; nvcnt=0x%x)", i, g_nvcnt_value[i]);
    }
    return final_ret;
#else
    return 0;
...
{
    int ret;
    uint32_t ns_forbid_data = 0;

    if (g_nvcnt_value[dev_id] == INVALID_NVCNT) {
        dev_upgrade_err("nvcnt invalid. (dev=%d)\n", dev_id);
        return -1;
    }
    info->nv_cnt = g_nvcnt_value[dev_id];

    info->pss_only = 0;
#ifdef CFG_FEATURE_PSS_SIGN
    ret = devdrv_config_get_pss_cfg(0, &info->pss_only);
    if (ret) {
        dev_upgrade_err("get pss config fail. (dev=%d; ret=%d)\n", dev_id, ret);
        return -1;
    }
#endif

    ret = upgrade_read_efuse_reg(dev_id, EFUSE_NS_FORBID_OFFSET, &ns_forbid_data);
    if (ret) {
        dev_upgrade_err("efuse read ns forbid fail. (dev=%d; ret=%d)\n", dev_id, ret);
        return -1;
    }
    if (ns_forbid_data == EFUSE_NS_MAGIC) {
        info->need_check = 0;
        return 0;
    }
    info->need_check = 1;
    ...
    if (ret) {
        dev_upgrade_err("efuse read subkey category fail. (dev=%d; ret=%d)\n", dev_id, ret);
        return -1;
    }
    ret = upgrade_read_efuse_reg(dev_id, SUBKEYID_MASK1_OFFSET, &info->subkey_id);
    if (ret) {
        dev_upgrade_err("efuse read subkey id fail. (dev=%d; ret=%d)\n", dev_id, ret);
        return -1;
    }
    return 0;
}
...
int dev_upgrade_verify_image(int dev_id, const char *file_path, int mode)
{
    ...
    if ((file_size > AICPU_SIZE_MAX) || (file_size < SEC_HEAD_SIZE + NVCNT_INFO_SIZE)) {
        filp_close(filp, NULL);
        filp = NULL;
        dev_upgrade_err("dev_id(%d) file size[%u] is out of range.\n", dev_id, file_size);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_INVALID_PARAMS);
    }
...
#if (defined SUPPORT_TEE_SECURE_UPGRADE)
    ret = soc_verify(dev_id, SOC_VERIFY_IMG_AICPU_KERNELS, file_data, file_size);
#else
    ...
    if (ret != 0) {
        filp_close(filp, NULL);
        filp = NULL;
        dev_upgrade_err("dev(%d) dev_upgrade_sec_verification or soc_verify fail, ret: %d\n", dev_id, ret);
        DEV_UPGRADE_VFREE(file_data);
        return dev_errno_make(DEV_MID_UPGRADE, ERRNO_HASH_CHECK);
    }
    ...
    if (mode & COVER_FILE_WITHOUT_HEAD) {
        ...
        ret = dev_upgrade_write_fs_file(file_path,
            (char *)(file_data + SEC_HEAD_SIZE),
            (file_size - SEC_HEAD_SIZE - NVCNT_INFO_SIZE),
