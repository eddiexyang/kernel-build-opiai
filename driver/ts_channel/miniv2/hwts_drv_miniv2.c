...
void hwts_drv_submit_task(unsigned long data)
{
    ...
    publish_event.event_info.priv = (void*)(uintptr_t)ts_task.mailbox_id;
...
STATIC int  hwts_get_ccpu_index_range(u32 node_id, u32 *ccpu_first, u32 *ccpu_last)
{
    ...
    *ccpu_last = 0;
#ifndef TSDRV_UT
    for (i = 0; (u32)i < node_id; i++) {
        ret = agentdrv_get_cpu_info((u32)i, &cpu_info);
        if (ret != 0) {
            ts_drv_err("Failed to get cpu info. node_id[%d].\n", i);
            return -EINVAL;
        }
        *ccpu_first += cpu_info.aicpu_num * cpu_info.aicpu_os_sched +
            cpu_info.ccpu_num * cpu_info.ccpu_os_sched +
            cpu_info.dcpu_num * cpu_info.dcpu_os_sched +
            cpu_info.tscpu_num * cpu_info.tscpu_os_sched;
    }
    ret = agentdrv_get_cpu_info(node_id, &cpu_info);
    if (ret != 0) {
        ts_drv_err("Failed to get cpu info. node_id[%d].\n", i);
        return -EINVAL;
    }
    *ccpu_last = *ccpu_first + cpu_info.aicpu_num * cpu_info.aicpu_os_sched +
        cpu_info.ccpu_num * cpu_info.ccpu_os_sched +
        cpu_info.dcpu_num * cpu_info.dcpu_os_sched +
        cpu_info.tscpu_num * cpu_info.tscpu_os_sched;
#endif
...
int hwts_drv_get_irq_from_acpi(struct platform_device *pdev, struct drv_hwts_ctrl *hwts)
{
    int ret = 0;
    u32 i = 0;
    ...
    for (i = 0; i < MAX_TS_NUM; i++) {
        ...
        if (ret < 0) {
            ts_drv_info(" Get irq failed (i=%u; ret=%d).\n", i, ret);
            return ret;
        }
        hwts->ts_info[i].irq_info.com_irq_ts_to_aicpu = (u32)ret;
        ts_drv_info("Get irq. (i=%u; irq=%u).\n", i, hwts->ts_info[i].irq_info.com_irq_ts_to_aicpu);
    }
...
#ifndef AOS_LLVM_BUILD
    // read interrupt from ACPI
    if (!acpi_disabled) {
        return hwts_drv_get_irq_from_acpi(pdev, hwts);
    }
#endif
...
void __iomem *hwts_drv_remap_config_system_addr(u32 tsid, int node_id)
{
#ifdef CFG_SOC_PLATFORM_MDC_V51
...
void hwts_drv_unmap_config_system_addr(void __iomem *vaddr)
{
    iounmap(vaddr);
}
...
int hwts_drv_get_config_info(struct drv_hwts_ctrl *hwts, struct platform_device *pdev)
{
    ...
    for (i = 0; i < MAX_TS_NUM; i++) {
        ...
        ts_drv_info("finish ioremap,ts=%u,vaddr=0x%pK !\n", i, vaddr[i]);
    }

    for (i = 0; i < MAX_TS_NUM; i++) {
        ts_drv_info("start build config system i=%u, vaddr=0x%pK!\n", i, vaddr[i]);
        ...
        if (config_system[i]->flag != HWTS_SYSTEM_CONFIG_FLAG) {
            ...
            for (j = 0; j < MAX_TS_NUM; j++) {
                ts_drv_info("iounmap vaddr, j=%u, vaddr=0x%pK!\n", j, vaddr[j]);
                hwts_drv_unmap_config_system_addr(vaddr[j]);
            }
...
int ts_drv_pm_resume(u32 devid)
{
...
int ts_drv_pm_suspend(u32 devid)
{
