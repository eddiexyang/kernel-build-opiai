...
#define HWTS_CHIP_BASEADDR_PA_OFFSET    0x0000000000ULL
 ...
 */
#define STARS_NOTIFY_NUM_OF_SINGLE_TABLE    128
...
/**
 * @ingroup tsch
 * @brief task scheduler only surport 2048 notifies
 */
#define NOTIFY_ID_NUM_MAX   2048

static u64 g_hwts_base_addr[MAX_TS_NUM] = {0x520000000ULL}; /* size 512M */
    ...
    for (i = 0; i < MAX_TS_NUM; i++) {
        if ((node_id >= 0) && (node_id < CHIP_NUM_MAX)) {
            g_hwts_base_va[node_id][i] = ioremap(g_hwts_base_addr[i] + node_id * HWTS_CHIP_BASEADDR_PA_OFFSET,
                g_hwts_reg_size[i]);
            ts_drv_debug("Base aadr remap.(node_id=%d; g_hwts_base_va[%d][%d]=0x%pK)\n", node_id,
                node_id, i, (void *)(u64)(uintptr_t)g_hwts_base_va[node_id][i]);
            if (g_hwts_base_va[node_id][i] == NULL) {
                ts_drv_err("ioremap failed. node_id(%d), tsid(%d)\n", node_id, i);
                goto ERROR;
            }
        }
    }
...
#else
int ut_device_hwts_drv_miniv3(void)
{
    return 0;
}
#endif
