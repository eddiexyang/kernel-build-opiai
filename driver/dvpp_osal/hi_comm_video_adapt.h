...
#ifndef HI_COMM_VIDEO_ADAPT_H__
#define HI_COMM_VIDEO_ADAPT_H__
...
typedef IMG_INFO_S hi_img_info;
typedef BIT_WIDTH_E hi_data_bit_width;
typedef struct {
...
typedef struct {
    ...
    hi_u8       software[DCF_DRSCRIPTION_LENGTH];          /* shows firmware (internal software of digital cameras)
                                                              version number */
    hi_u8       light_source;             /* light source, actually this means white balance setting. '0' means unknown,
                                             '1' daylight, '2' fluorescent, '3' tungsten, '10' flash,
    ...
    hi_u8       gain_control;             /* indicates the degree of overall image gain adjustment. 0 = none,
                                             1 = low gain up, 2 = high gain up, 3 = low gain down,
                                             4 = high gain down. */
    hi_u8       contrast;                 /* indicates the direction of contrast processing applied by the camera when
                                             the image was shot. 0 = normal,1 = soft,2 = hard */
    hi_u8       saturation;               /* indicates the direction of saturation processing applied by the camera
                                             when the image was shot. 0 = normal, 1 = low saturation,
                                             2 = high saturation */
    hi_u8       sharpness;                /* indicates the direction of sharpness processing applied by the camera when
                                             the image was shot. 0 = normal, 1 = soft, 2 = hard. */
    hi_u8       metering_mode;            /* exposure metering method. '0' means unknown, '1' average,
                                            '2' center weighted average, '3' spot, '4' multi-spot, '5' multi-segment,
                                            '6' partial, '255' other */
...
typedef struct {
    ...
    hi_u32          digital_zoom_ratio;                    /* indicates the digital zoom ratio when the image was shot.
                                                              if the numerator of the recorded value is 0, this
...
typedef struct {
    ...
    hi_s64      motion_filter[9];            /* DIS motion filter */
...
typedef struct {
    ...
    hi_dng_rational as_shot_neutral[HI_CFACOLORPLANE]; /* specifies the selected white balance at time of capture,
                                                       encoded as the coordinates of a perfectly neutral color in
                                                       linear reference space values. */
...
typedef struct {
    ...
    hi_u64              private_data;   /* 存储mbuf 头地址 */
...
typedef struct {
    ...
    hi_u8 *ATTRIBUTE addr;   // stream addr
...
typedef struct {
    ...
    hi_ldc_view_type view_type;
    /* RW; range: ABS(center_x_offset)->[0, min(128, width*0.08)],
        horizontal offset of the image distortion center relative to image center. */
    hi_s32 center_x_offset;
    /* RW; range: ABS(center_x_offset)->[0, min(128, height*0.08)],
    ...
    /* RW; range: [-300, 300], to remove the black region around when performing
...
typedef struct {
    ...
    hi_rotation_view_type view_type;
...
typedef enum  {
    HI_BLK_SIZE_4 = 0, /* block size 4 * 4 of MOSAIC */
    HI_BLK_SIZE_8,     /* block size 8 * 8 of MOSAIC */
    HI_BLK_SIZE_16,    /* block size 16 * 16 of MOSAIC */
    HI_BLK_SIZE_32,    /* block size 32 * 32 of MOSAIC */
    HI_BLK_SIZE_64,    /* block size 64 * 64 of MOSAIC */
    HI_BLK_SIZE_128,   /* block size 128 * 128 of MOSAIC */
    HI_BLK_SIZE_BUTT,
} hi_blk_size;
...
typedef struct {
    ...
    hi_point       point[QUAD_POINT_NUM];   /* RW; Four points of the quadrangle. */
} hi_quad_cover;
