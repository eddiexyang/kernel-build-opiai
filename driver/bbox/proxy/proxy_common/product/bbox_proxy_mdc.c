...
#include "bbox_proxy_config_mdc.h"
...
{
    if (proxy != NULL) {
        u32 count = (u32)(sizeof(g_module_proxy) / sizeof(g_module_proxy[0]));
        count = BBOX_MIN(count, BBOX_COREID_MAX);
        for (proxy->count = 0; proxy->count < (s32)count; proxy->count++) {
