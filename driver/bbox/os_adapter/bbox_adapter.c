...
{
    /*
     * log_buf_addr_get()/log_buf_len_get() are Huawei-custom kernel exports
     * not available in upstream 6.18. Skip log cache flush on upstream kernels.
     */
    BB_PRINT_INFO("flush log cache (no-op on upstream kernel).\n");
}
...
STATIC void bbox_adapter_reset(const struct bbox_reset_ops_info *info)
{
    ...
    /*
     * console_flush_on_panic() is not exported to modules in upstream 6.18.
     * The kernel's own panic handler already flushes the console.
     */
    DEAD_WHILE;
}
