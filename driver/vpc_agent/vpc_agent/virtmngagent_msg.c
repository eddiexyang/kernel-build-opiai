...
void vmnga_send_int_to_remote(void *msg_dev_in, u32 vector)
{
...
irqreturn_t vmnga_tx_finish_msix_hander(int irq, void *data)
{
...
int vmnga_tx_irq_init(struct vmng_msg_chan_tx *msg_chan)
{
...
int vmnga_tx_irq_uninit(struct vmng_msg_chan_tx *msg_chan)
{
    struct vmng_msg_dev *msg_dev = (struct vmng_msg_dev *)msg_chan->msg_dev;
    int ret;
...
irqreturn_t vmnga_msg_msix_hander(int irq, void *data)
{
...
STATIC struct vmng_msg_dev *vmnga_msg_dev_alloc(struct vmnga_vpc_unit *unit)
{
    ...
    msg_dev = kzalloc(sizeof(struct vmng_msg_dev), GFP_KERNEL);
    if (msg_dev == NULL) {
    ...
    vmng_msg_chan_init(VMNG_AGENT_SIDE, msg_dev);
