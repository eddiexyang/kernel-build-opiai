...
int vmngh_vpc_para_check(u32 dev_id, u32 fid, enum vmng_vpc_type vpc_type)
{
...
int vmngh_vpc_msg_recv(void *msg_cluster_in, struct vmng_msg_chan_rx_proc_info *proc_info)
{
    struct vmng_msg_cluster *msg_cluster = (struct vmng_msg_cluster *)msg_cluster_in;
    ...
    if (ret != 0) {
        vmng_err("Client message recv error. (dev_id=%u; fid=%u; chan_type=%u; ret=%d)\n",
                 dev_id, fid, msg_cluster->chan_type, ret);
        return ret;
    }
    return 0;
}

int vmngh_vpc_msg_send_para_check(u32 dev_id, u32 fid, enum vmng_vpc_type vpc_type,
...
int vmngh_alloc_vpc_msg_cluster(u32 dev_id, u32 fid, enum vmng_vpc_type vpc_type)
{
    struct vmng_msg_dev *msg_dev = NULL;
    enum vmng_msg_chan_type chan_type;
...
void vmngh_free_vpc_msg_cluster(u32 dev_id, u32 fid, enum vmng_vpc_type vpc_type)
{
    struct vmng_msg_dev *msg_dev = NULL;
    enum vmng_msg_chan_type chan_type;
