    ...
    const struct vmng_msg_chan_res *res, struct vmng_msg_chan_irqs *irq_array)
{
    ...
    irq_array->tx_send_irq = (u32 *)kzalloc(irq_array_len, GFP_KERNEL);
...
int vmngh_tx_irq_init(struct vmng_msg_chan_tx *msg_chan)
{
    return 0;
}

int vmngh_tx_irq_uninit(struct vmng_msg_chan_tx *msg_chan)
{
    return 0;
}
...
STATIC struct vmng_msg_dev *vmngh_alloc_msg_dev(struct vmngh_vpc_unit *unit)
{
    ...
    vmng_msg_chan_init(VMNG_HOST_SIDE, msg_dev);
