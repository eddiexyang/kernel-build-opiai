...
#ifndef HI_COMM_SYS_H
#define HI_COMM_SYS_H
...
#define HI_TRACE_SYS(level, fmt, ...)                                                                         \
    do {                                                                                                      \
        ...
        HI_TRACE(level, HI_ID_SYS, "[Sys]:%s [Line]:%d " fmt, function, line, ##__VA_ARGS__); \
    } while (0)

#define sys_emerg_trace(fmt, ...)                                                                            \
    do {                                                                                                     \
        HI_EMERG_TRACE(HI_ID_SYS, "[Func]:%s [Line]:%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
    } while (0)

#define sys_alert_trace(fmt, ...)                                                                            \
    do {                                                                                                     \
        HI_ALERT_TRACE(HI_ID_SYS, "[Func]:%s [Line]:%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
    } while (0)

#define sys_crit_trace(fmt, ...)                                                                            \
    do {                                                                                                    \
        HI_CRIT_TRACE(HI_ID_SYS, "[Func]:%s [Line]:%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
    } while (0)

#define sys_err_trace(fmt, ...)                                                                            \
    do {                                                                                                   \
        HI_ERR_TRACE(HI_ID_SYS, "[Func]:%s [Line]:%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
    } while (0)

#define sys_warn_trace(fmt, ...)                                                                            \
    do {                                                                                                    \
        HI_WARN_TRACE(HI_ID_SYS, "[Func]:%s [Line]:%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
    } while (0)

#define sys_notice_trace(fmt, ...)                                                                            \
    do {                                                                                                      \
        HI_NOTICE_TRACE(HI_ID_SYS, "[Func]:%s [Line]:%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
    } while (0)

#define sys_info_trace(fmt, ...)                                                                            \
    do {                                                                                                    \
        HI_INFO_TRACE(HI_ID_SYS, "[Func]:%s [Line]:%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
    } while (0)

#define sys_debug_trace(fmt, ...)                                                                            \
    do {                                                                                                     \
        HI_DEBUG_TRACE(HI_ID_GDC, "[Func]:%s [Line]:%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
    } while (0)
...
typedef struct hiVPSS_VENC_WRAP_PARAM_S {
    ...
    VIDEO_SIZE_S stLargeStreamSize; /* VENC Large Stream Size */
    VIDEO_SIZE_S stSmallStreamSize; /* VENC Small Stream Size */
