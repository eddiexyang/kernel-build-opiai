...
#define OSAL_LIST_HEAD(name) struct osal_list_head name = OSAL_LIST_HEAD_INIT(name) //lint !e773
 ...
 */
static inline void osal___list_add(struct osal_list_head *list_new,
 ...
 */
static inline void osal_list_add(struct osal_list_head *list_new, struct osal_list_head *head)
{
    osal___list_add(list_new, head, head->next);
}
 ...
 */
static inline void osal_list_add_tail(struct osal_list_head *list_new, struct osal_list_head *head)
{
    osal___list_add(list_new, head->prev, head);
}
 ...
 */
static inline void osal___list_del_entry(const struct osal_list_head *entry)
{
 ...
 */
static inline void osal_list_replace(const struct osal_list_head *old,
                                     struct osal_list_head *list_new)
{
...
static inline void osal_list_replace_init(struct osal_list_head *old,
                                          struct osal_list_head *list_new)
{
    osal_list_replace(old, list_new);
...
{
    const struct osal_list_head *next = head->next;
 ...
 * @head: the place to add it in the first list.
 */
static inline void osal_list_splice_tail(const struct osal_list_head *list,
                                         struct osal_list_head *head)
{
    if (!osal_list_empty(list)) {
...
#define osal_container_of(ptr, type, member) ({          \
    ...
    (type *)((uintptr_t)mptr - osal_offsetof(type, member)); })
...
{
    return (h->pprev == OSAL_NULL) ? 1 : 0;
}

static inline int osal_hlist_empty(const struct osal_hlist_head *h)
{
    return (h->first == OSAL_NULL) ? 1 : 0;
}

static inline void osal___hlist_del(const struct osal_hlist_node *n)
{
...
static inline void osal_hlist_move_list(struct osal_hlist_head *old,
                                        struct osal_hlist_head *list_new)
{
