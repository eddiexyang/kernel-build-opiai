    ...
    struct devdrv_ts_pdata *ts_pdata)
{
    ...
    ret = tsdrv_get_ts_doorbell_addr(devid, tsid,
        &ts_pdata->doorbell_paddr, &ts_pdata->doorbell_size);
    ret |= tsdrv_get_ts_sram_addr(devid, tsid,
        &ts_pdata->sram_paddr, &ts_pdata->sram_size);
    ret |= tsdrv_get_ts_sysctrl_addr(devid, tsid,
        &ts_pdata->ts_sysctl_paddr, &ts_pdata->ts_sysctl_size);
    ret |= tsdrv_get_tsensor_shm_addr(devid, tsid,
        &ts_pdata->tsensor_shm_paddr, &ts_pdata->tsensor_shm_size);
    ret |= tsdrv_get_ts_stars_rtsq_addr(devid, tsid,
        &ts_pdata->stars_ctrl_paddr, &ts_pdata->stars_ctrl_size);
    if (ret != 0) {
        return ret;
    }
...
static int get_prof_hwts_through_irq(u32 devid, struct devdrv_ts_pdata *ts_pdata)
{
    u32 tsid;
    int ret;
    ...
    ret = tsdrv_get_prof_aicore_irq(devid, tsid, (u32 *)&ts_pdata->irq_prof_aicore_request);
    if (ret != 0) {
        return ret;
    }

    ret = tsdrv_get_prof_hwts_log_irq(devid, tsid, (u32 *)&ts_pdata->irq_prof_hwts_log_request);
    if (ret != 0) {
        return ret;
    }

    TSDRV_PRINT_DEBUG("Get prof irq. (aic irq=%d; hwts log irq=%d)\n",
...
static int get_ts_irq(u32 devid, struct devdrv_ts_pdata *ts_pdata)
{
    ...
    ret |= tsdrv_get_trigger_irq_info(devid, ts_pdata);
#endif
#ifdef CFG_FEATURE_PROF_HWTS_THROUGH
    ret |= get_prof_hwts_through_irq(devid, ts_pdata);
#endif
    if (ret != 0) {
    ...
    struct devdrv_platform_data *pdata)
{
    ...
    for (tsid = 0; tsid < pdata->ts_num; tsid++) {
...
#if defined(CFG_TRS_REFACTOR_FEATURE) || defined(CFG_FEATURE_PROF_HWTS_THROUGH)
        ...
        ret = set_soc_resmng_bitmap_res(devid, tsid);
        if (ret != 0) {
            goto get_irq_fail;
        }
#endif
...
static void tsdrv_set_ts_mem_restrict_valid(u32 dev_id, u32 *valid)
{
    ...
    if (num_online_nodes() == tsdrv_get_numa_num_per_dev() * tsdrv_get_cpu_node_num()) {
    ...
    struct devdrv_platform_data *pdata)
{
    ...
    ret = set_soc_resmng_subsys_ras(pdata->dev_id, pdata);
    if (ret != 0) {
        TSDRV_PRINT_ERR("Set ras resmng failed. (ret=%d)\n", ret);
        goto set_fail;
    }
#endif
    return 0;
