...
#define MDC_RETRY_TIMEOUT 20000
#ifdef CFG_SOC_PLATFORM_MDC_V11
#define MINIV2_RETRY_TIMEOUT 200
#else
#define MINIV2_RETRY_TIMEOUT 1000
#endif
...
const char *tsdrv_get_tsfw_path(enum tsdrv_hwts_spec ts_spec, char *tsfw_path, u32 size)
{
    ...
    if (ret != EOK) {
        devdrv_drv_warn("ignore memset tsfw path failed. ret=%d\n", ret);
    }
...
#define TS_MDC_AIC_STL_BASE   (TS_MDC_DCACHE_BASE + TS_MDC_DCACHE_SIZE)
#define TS_MDC_STL_SIZE       __ULL(4 * 1024 * 1024)
#define TS_MDC_AIV_STL_BASE   (TS_MDC_AIC_STL_BASE + TS_MDC_STL_SIZE)
...
STATIC void tsdrv_config_cpu_info(struct devdrv_info *dev_info, struct aicpu_system_config *config)
{
    ...
    config->board_id = dev_info->pdata->platform_info.board_id;
...
int devdrv_wait_ts_load_done(struct devdrv_info *dev_info, u32 tsid,
                            u32 __iomem *sc_testreg, struct devdrv_platform_data *pdata)
{
    ...
    while (1) {
        ...
        times++;
        /* the maximum time to load TS */
        if (times > retry) {
            tsdrv_set_ts_status(dev_info->dev_id, tsid, TS_DOWN);
            TSDRV_PRINT_ERR("TS firmware load timeout. (devid=%u; tsid=%u; reg_val=0x%x; retry=%u).\n",
                dev_info->dev_id, tsid, reg_val, retry);
...
#ifndef CFG_SOC_PLATFORM_MINIV3
    if (devdrv_load_riscv_fw(tsid, risc_v_bin_path[tsid]) != 0) {
        TSDRV_PRINT_WARN("load riscv binary failed\n");
    }
