...
const u32 g_cycle_time = TSDRV_HEARTBEAT_CYCLE_TIME;
    ...
    u64 main_thread_pa, u64 doorbell_pa, struct tsdrv_heart_beat_tsfw_dfx_reg *tsfw_reg)
{
    ...
    offset = tsdrv_get_addr_chip_die_offset(devid, chip_id, die_id);
    tsfw_reg->heart_beart_dotting = (void *)ioremap(heart_beat_pa + offset, TS_DFX_REG_SIZE);
    if (tsfw_reg->heart_beart_dotting == NULL) {
        TSDRV_PRINT_ERR("ioremap heart_beart_dotting failed\n");
        return -ENOMEM;
    }

    tsfw_reg->main_thread_dotting = (void *)ioremap(main_thread_pa + offset, TS_DFX_REG_SIZE);
    if (tsfw_reg->main_thread_dotting == NULL) {
        TSDRV_PRINT_ERR("ioremap main_thread_dotting failed\n");
        goto err1;
    }

    tsfw_reg->doorbell_dotting = (void *)ioremap(doorbell_pa + offset, TS_DFX_REG_SIZE);
...
{
#ifndef AOS_LLVM_BUILD
    int chip_tyep = uda_get_chip_type(devid);
#else
    int chip_tyep = HISI_MINI_V2;
#endif
    int ret;

    if (chip_tyep == HISI_CLOUD_V2) {
        ret = tsdrv_heart_beat_tsfw_dfx_reg_ioremap(devid, ASCEND910B_TS_HEART_BEAT_DOTTING_REG,
            ASCEND910B_TS_MAIN_THREAD_DOTTING_REG,
            ASCEND910B_TS_DOORBELL_DOTTING_REG, tsfw_reg);
    } else if (chip_tyep == HISI_CLOUD_V1) {
        ret = tsdrv_heart_beat_tsfw_dfx_reg_ioremap(devid, ASCEND910_TS_HEART_BEAT_DOTTING_REG,
            ASCEND910_TS_MAIN_THREAD_DOTTING_REG,
            ASCEND910_TS_DOORBELL_DOTTING_REG, tsfw_reg);
    } else {
        if (tsid == 0) {
...
void tsdrv_hb_cq_callback(u32 devid, u32 tsid, const u8 *cq_slot, u8 *sq_slot)
{
    ...
    heart_beat->resp_msg.ts_status = cq->ts_status;
...
#ifndef TSDRV_UT
    TSDRV_PRINT_ERR("[devid-%u][tsid-%u] heart beat exception is detected, process ts down exception. ts_status=%u\n",
        heart_beat->instance.devid, heart_beat->instance.tsid, heart_beat->resp_msg.ts_status);
#endif
...
#ifdef CFG_TRS_REFACTOR_FEATURE
    (void)trs_set_ts_hb_status(heart_beat->instance.devid, heart_beat->instance.tsid, TRS_INST_STATUS_ABNORMAL);

    for (i = TRS_DEVICE_VIR_DEV_BASE; i < TRS_DEVICE_MAX_DEV; i++) {
        if (!uda_is_udevid_exist(i) || (uda_udevid_to_mia_devid(i, &mia_para) != 0) ||
            (mia_para.phy_devid != heart_beat->instance.devid)) {
            continue;
        }

        (void)trs_set_ts_hb_status(i, heart_beat->instance.tsid, TRS_INST_STATUS_ABNORMAL);
    }
#endif
#ifndef AOS_LLVM_BUILD
...
#else
        hb_sqcq_send(heart_beat->instance.devid, heart_beat->instance.tsid, heart_beat->chan_id, (u8 *)&sq);
#endif

        msleep(g_cycle_time);

        if ((heart_beat->resp_msg.sn == heart_beat->msg_sn) && (heart_beat->resp_msg.ts_status == 0)) {
            heart_beat->lost_count = 0;
        } else {
...
STATIC int tsdrv_hb_create_and_run_thread(struct tsdrv_heart_beat_info *heart_beat)
{
    ...
    tsk = kthread_run(tsdrv_hb_thread_fun, heart_beat, "ts_hb_dev%u_ts%u", heart_beat->instance.devid,
        heart_beat->instance.tsid);
    if (IS_ERR(tsk)) {
...
STATIC int tsdrv_hb_init_per_ts(struct tsdrv_heart_beat_info *heart_beat)
{
    ...
    heart_beat->resp_msg.ts_status = 0;
...
STATIC int tsdrv_hb_init_per_ts(struct tsdrv_heart_beat_info *heart_beat)
{
    ...
    ret = hb_sqcq_alloc(heart_beat->instance.devid, heart_beat->instance.tsid, &heart_beat->chan_id);
    if (ret != 0) {
        return ret;
    }
    ...
    ret = tsdrv_hb_create_and_run_thread(heart_beat);
    if (ret != 0) {
        hb_sqcq_free(heart_beat->instance.devid, heart_beat->instance.tsid, heart_beat->chan_id);
        return ret;
    }
