...
static struct tsdrv_d2h_rx_handler pci_slow_rx_handler[TSDRV_MAX_DAVINCI_NUM];
static struct tsdrv_d2h_rx_handler pci_fast_rx_handler[TSDRV_MAX_DAVINCI_NUM];
...
{
    if (cmd_type >= MAX_D2H_RX_CMD_NUM) {
        TSDRV_PRINT_ERR("invalid cmd_type=%u\n", cmd_type);
        return -ENODEV;
    }
...
STATIC int tsdrv_pci_fast_rx_handler_register(u32 devid, u32 cmd_type,
    int (*rx_handler)(u32 devid, struct tsdrv_msg_info *msg, u32 *ack_len))
{
#ifndef TSDRV_UT
    if (cmd_type >= MAX_D2H_RX_CMD_NUM) {
...
STATIC struct agentdrv_common_msg_client pci_slow_chan_client = {
    .type = AGENTDRV_COMMON_MSG_DEVDRV_TSDRV,
...
{
#if ((!defined AOS_LLVM_BUILD) && (!defined CFG_NOT_SURPPORT_PCIE_HOST_DEVICE_COMM))
    int err;
    ...
    err = agentdrv_register_common_msg_client(&pci_slow_chan_client);
    if (err != 0) {
        (void)set_pci_chan_cleanup(PCI_CHAN_SLOW);
    }
    return err;
#else
    return 0;
...
static int pci_slow_chan_cleanup(void)
{
    ...
    err = agentdrv_unregister_common_msg_client(&pci_slow_chan_client);
    if (err != 0) {
...
{
    int err = set_pci_chan_setup(PCI_CHAN_FAST);
    if (err != 0) {
        return err;
    }
    return err;
...
{
    int err = set_pci_chan_cleanup(PCI_CHAN_FAST);
    if (err != 0) {
        return err;
    }
    return 0;
...
STATIC int tsdrv_pci_slow_msg_send(u32 devid, void *tx, size_t tx_size)
{
    ...
    return agentdrv_common_msg_send(devid, tx, tx_size, tx_size, &out_len, AGENTDRV_COMMON_MSG_DEVDRV_TSDRV);
#else
    return 0;
#endif
}
...
STATIC int tsdrv_pci_fast_msg_send(u32 devid, void *tx, size_t tx_size)
{
    ...
    return agentdrv_sync_msg_send(chan->msg_chan, tx, tx_size, tx_size, &out_len);
#else
    return 0;
#endif
#endif
...
STATIC int tsdrv_pci_chan_setup(void)
{
...
STATIC int tsdrv_pci_chan_cleanup(void)
{
...
static const struct tsdrv_d2h_chan_ops g_pci_chan_ops = {
    .d2h_chan_setup = tsdrv_pci_chan_setup,
    .d2h_chan_cleanup = tsdrv_pci_chan_cleanup,

    .d2h_fast_chan_init = tsdrv_pci_fast_chan_init,
    .d2h_fast_chan_exit = tsdrv_pci_fast_chan_exit,

    .d2h_slow_chan_ready = tsdrv_pci_slow_chan_ready,

    .d2h_slow_chan_init = tsdrv_pci_slow_chan_init,
    .d2h_slow_chan_exit = tsdrv_pci_slow_chan_exit,

    .d2h_slow_msg_send = tsdrv_pci_slow_msg_send,
    .d2h_fast_msg_send = tsdrv_pci_fast_msg_send,
