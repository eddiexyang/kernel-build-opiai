...
#define L2BUF_UNMASK_IRQ_RES_SZ    0x4
...
void fault_unmask_ras_irq(struct work_struct *work)
{
    ...
    if (node_type == DMS_DEV_TYPE_L2BUF) {
        writel(L2BUF_UNMASK_IRQ_RES_VAL, (volatile void *)l2buf_io_base[devid]);
...
void fault_unmap_l2buf(u32 devid)
{
    ...
    TSDRV_PRINT_DEBUG("Unmask l2buf success. (devid=%u)\n", devid);
}
...
struct dms_node *tsdrv_get_dms_node(u32 devid, int node_type, u32 node_id)
{
    switch (node_type) {
        ...
        default:
            TSDRV_PRINT_ERR("Para error. (devid=%u; node_type=%d; node_id=%u)\n", devid, node_type, node_id);
            break;
    }
