...
typedef void (*pidHandle)(char *buf, ssize_t *offset, struct sched_proc_ctx *proc_ctx);

STATIC void pidListForEachHandle(char *buf, ssize_t *offset, pidHandle handle)
{
...
STATIC void sched_sysfs_write_file(struct file *file, const void *buf, size_t count, loff_t *pos)
{
    ...
    if (ret != (ssize_t)count) {
        sched_warn("Failed to invoke the kernel_write. (count=%lu; ret=%ld)\n", count, ret);
    }
#endif
}
...
{
    if (node->node_event_sample == NULL) {
        node->node_event_sample = (struct sched_event_sample *)vzalloc(sizeof(struct sched_event_sample));
        if (node->node_event_sample == NULL) {
    ...
    if (node->proc_event_sample == NULL) {
        node->proc_event_sample = (struct sched_event_sample *)vzalloc(sizeof(struct sched_event_sample));
        if (node->proc_event_sample == NULL) {
            sched_err("Failed to vzalloc memory. (size=0x%lx)\n", sizeof(struct sched_event_sample));
            return -ENOMEM;
        }
...
STATIC ssize_t sched_sysfs_node_proc_list(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    pidListForEachHandle(buf, &offset, sched_sysfs_node_get_proc_list);
...
STATIC ssize_t sched_sysfs_node_event_resource(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1,
        "resouce total:use:que head:que tail,max use,deque empty\n");
    if (ret >= 0) {
        offset += ret;
    }

    res_que = &node->event_res;
    ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1, "%u:%u:%u:%u:%u:%llu\n",
        res_que->depth, res_que->depth - sched_que_element_num(res_que), res_que->head, res_que->tail,
        res_que->stat.max_use, res_que->stat.deque_empty);
    if (ret >= 0) {
        offset += ret;
    }
...
STATIC ssize_t sched_sysfs_event_sample(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (sample == NULL) {
        mutex_unlock(&node->node_guard_work_mutex);
        sched_warn("The node_event_sample is NULL.\n");
        esched_dev_put(node);
        return offset;
    }
...
STATIC void sched_sysfs_get_all_cpu_event_summary(struct sched_cpu_perf_stat *perf_stat)
{
    ...
    for (i = 0; i < node->cpu_num; i++) {
        ...
        for (j = 0 ; j < SCHED_MAX_EVENT_TYPE_NUM; j++) {
            perf_stat->publish_event_num[j] += cpu_ctx->perf_stat.publish_event_num[j];
...
STATIC ssize_t sched_sysfs_node_event_summary(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    perf_stat = (struct sched_cpu_perf_stat *)kzalloc(sizeof(struct sched_cpu_perf_stat), GFP_KERNEL);
    if (perf_stat == NULL) {
    ...
    ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1,
        ...
        perf_stat->total_publish_event_num, perf_stat->total_sched_event_num, perf_stat->total_wakeup_event_num);
    if (ret >= 0) {
        offset += ret;
    }
    ...
    ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1,
        "\nevent id,publish event num,sched event num\n");
    if (ret >= 0) {
        offset += ret;
    }

    for (i = 0; i < SCHED_MAX_EVENT_TYPE_NUM; i++) {
        if ((perf_stat->publish_event_num[i] != 0) || (perf_stat->sched_event_num[i] != 0)) {
            ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1,
                "%d, %llu, %llu\n", i, perf_stat->publish_event_num[i], perf_stat->sched_event_num[i]);
            if (ret >= 0) {
                offset += ret;
            }
        }
...
STATIC ssize_t sched_sysfs_node_cpu_event_resource(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1,
        "cpuid,resouce total:use:que head:que tail,max use,deque empty cnt\n");
    if (ret >= 0) {
        offset += ret;
    }
    ...
    for (i = 0; i < node->cpu_num; i++) {
        ...
        ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1, "%u,%u:%u:%u:%u:%u:%llu\n", i,
            res_que->depth, res_que->depth - sched_que_element_num(res_que), res_que->head, res_que->tail,
            res_que->stat.max_use, res_que->stat.deque_empty);
        if (ret >= 0) {
            offset += ret;
    ...
    struct sched_grp_ctx *grp_ctx, ssize_t *offset, int32_t idx)
{
    ...
    if (grp_ctx->sched_mode == SCHED_MODE_SCHED_CPU) {
        ret = snprintf_s(buf + *offset, PAGE_SIZE - *offset, PAGE_SIZE - *offset - 1,
            ...
            thread_ctx->wakeup_reason, thread_ctx->normal_wakeup_reason,
...
STATIC ssize_t sched_sysfs_node_cpu_thread_info(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1,
        "bind_cpuid,pid,gid,tid,name,kernel_tid,status,wait flag,timeout flag,wakeup reason,normal wakeup reason,"
    ...
    pidListForEachHandle(buf, &offset, sched_sysfs_node_proc_thread_info);
    esched_dev_put(node);
...
STATIC ssize_t sched_sysfs_node_proc_group_event_list(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret = snprintf_s(buf + offset, PAGE_SIZE - offset, PAGE_SIZE - offset - 1, "event id,thread num(thread list)\n");
    if (ret >= 0) {
        offset += ret;
    }
...
void sched_sysfs_show_proc_info(u32 chip_id, int32_t pid)
{
    ...
    buf = (char *)kzalloc(sizeof(char) * PAGE_SIZE, GFP_KERNEL);
    if (buf == NULL) {
        esched_proc_put(proc_ctx);
        return;
    }
...
ssize_t sched_sysfs_node_vf_proc_list(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    pidListForEachHandle(buf, &offset, sched_get_vf_proc_list);
...
#define SCHED_RECORD_DEFLAUT_NUM 1000
#define SCHED_SYSFS_RECORD_MAX_NUM 65536

static uint32_t sched_sysfs_record_num = SCHED_RECORD_DEFLAUT_NUM; // shched record deflaut num is 1000
...
#define drv_procfs_func_write(file, buf, count, store)  do { \
    ...
    if (copy_from_user(store_buf, buf, count) != 0) { \
        sched_err("copy from user failed\n"); \
        vfree(store_buf); \
        return -1; \
    } \
...
#define drv_procfs_func_read(file, buf, count, show)  do { \
    ...
    if (copy_to_user(buf, store_buf, offset) != 0) { \
        sched_err("Failed to invoke the copy_to_user. (ret=%d)\n"); \
        vfree(store_buf); \
        return -1; \
    } \
    vfree(store_buf); \
...
ssize_t drv_procfs_write_##_name(struct file *file, const char __user *buf, size_t count, loff_t *ppos) \
{ \
    drv_procfs_func_write(file, buf, count, _store); \
} \
ssize_t drv_procfs_read_##_name(struct file *file, char __user *buf, size_t count, loff_t *ppos) \
{ \
    drv_procfs_func_read(file, buf, count, _show); \
} \
struct drv_procfs_attr drv_procfs_attr_##_name = { \
    .name = __stringify(_name), \
    .mode = VERIFY_OCTAL_PERMISSIONS(_mode), \
    .ops.write = drv_procfs_write_##_name, \
    .ops.read = drv_procfs_read_##_name, \
}

#define DRV_FS_ATTR DRV_PROC_DEVICE_ATTR
#define DRV_FS_ATTR_NAME_POINTER(_name)  &drv_procfs_attr_##_name
...
#ifdef AOS_LLVM_BUILD
static struct proc_dir_entry *g_esched_procfs_boot_entry;

DECLEAR_ESCHED_FS_ATTR;

DECLEAR_ESCHED_FS_ATTR_NORMAL_LIST(drv_procfs_attr, procfs);

void sched_procfs_boot_dir_create(void)
{
    g_esched_procfs_boot_entry = proc_mkdir("event_sched_node", NULL);
}

void sched_procfs_init(void)
{
    int32_t node_id;
    int32_t i;
    int32_t sum = sizeof(g_sched_procfs_normal_list) / sizeof(g_sched_procfs_normal_list[0]) - 1;
    struct drv_procfs_attr *attr = NULL;

    for (node_id = 0; node_id < SCHED_MAX_CHIP_NUM; node_id++) {
        if (sched_get_numa_node(node_id) != NULL) {
    ...
    sched_procfs_boot_dir_create();
    for (i = 0; i < sum; i++) {
        attr = g_sched_procfs_normal_list[i];
        attr->entry = proc_create(attr->name, attr->mode, g_esched_procfs_boot_entry, &attr->ops);
    }
    sched_info("End of calling sched_procfs_init.\n");
    return;
}

void sched_procfs_uninit(void)
{
    int32_t i;
    int32_t sum = sizeof(g_sched_procfs_normal_list) / sizeof(g_sched_procfs_normal_list[0]) - 1;
    struct drv_procfs_attr *attr = NULL;
    ...
    proc_remove(g_esched_procfs_boot_entry);
}
