...
{
#ifndef AOS_LLVM_BUILD
    return iommu_domain_set_attr((struct iommu_domain*)domain, (enum iommu_attr)attr, data);
#else
    return 0;
#endif
}
...
{
    return iommu_map((struct iommu_domain *)domain, iova, paddr, size, prot);
}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
    *sva = (hi_void *)iommu_sva_bind_device((struct device*)dev, current->mm, NULL);
    *pasid = (hi_s32)current->mm->pasid;
    if (IS_ERR(*sva)) {
        ret = PTR_ERR(*sva);
    }
