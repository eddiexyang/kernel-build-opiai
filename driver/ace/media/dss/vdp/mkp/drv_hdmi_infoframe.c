...
static td_void hdmi_infoframe_checksum(td_void *buffer, td_u32 size)
{
    ...
    if (checksum > 0) {
        ptr[AVI_OFFSET_CHECKSUM] = (td_u8)HDMI_INFOFRAME_CHECCKSUM - checksum;
    } else {
...
static hdmi_video_code_vic avi_header_pack(struct hdmi_avi_infoframe *frame, td_void *buffer, hdmi_3d_mode _3d_mode)
{
    ...
    ptr = buffer;
    ptr[AVI_OFFSET_TYPE] = (td_u8)frame->type;
...
static td_s32 hdmi_avi_infoframe_pack(struct hdmi_avi_infoframe *frame, td_void *buffer, td_u32 size, hdmi_3d_mode mode)
{
    ...
    ptr[AVI_OFFSET_PB1] = (((td_u32)frame->colorspace & AVI_FRAME_COLOSR_SPACE_MASK) << 5) |
             ((td_u32)frame->scan_mode & AVI_FRAME_SCAN_INFOMATION_MASK);
...
static td_s32 hdmi_audio_infoframe_pack(const struct hdmi_audio_infoframe *frame, td_void *buffer, td_u32 size)
{
    ...
    ptr[AUDIO_OFFSET_TYPE]     = (td_u8)frame->type;
    ...
    td_void *buffer, td_u32 size)
{
    ...
    ptr[VENDOR_OFFSET_TYPE]    = (td_u8)frame->type;
    ...
    if (frame->vsif_content.h14_vsif_content.ieee == HDMI14_IEEE_OUI) {
        ...
        /* IEEE OUI */
        ptr[VENDOR_OFFSET_IEEE_LOWER] = ((td_s32)HDMI14_IEEE_OUI) & 0xff;
        /* 8, lower byte of word */
        ptr[VENDOR_OFFSET_IEEE_UPPER] = (((td_s32)HDMI14_IEEE_OUI) >> 8) & 0xff;
        /* 16, len of word */
        ptr[VENDOR_OFFSET_IEEE] = (((td_s32)HDMI14_IEEE_OUI) >> 16) & 0xff;
        ...
        if (vsif_content->format == HDMI_VIDEO_FORMAT_4K) {
            ptr[VENDOR_OFFSET_VIC] = (td_u8)vsif_content->vic;
        ...
        if (vsif_content->_3d_meta_present == 0) {
            ...
            if ((user_content->len != 0) && (length + user_content->len <= size)) {
                hdmi_if_false_return((user_content->len > (td_u8)HDMI_VENDOR_USER_DATA_MAX_LEN), 0);
...
td_void drv_hdmi_infoframe_send(const hdmi_infoframe *info_frame,
    hdmi_infoframe_id infoframe_id, hdmi_infoframe_unit *infoframe)
{
...
td_void drv_hdmi_infoframe_enable_set(const hdmi_infoframe *info_frame, hdmi_infoframe_id infoframe_id,
    td_bool enable)
{
    hdmi_device *hdmi_dev = TD_NULL;
...
td_s32 drv_hdmi_avi_infoframe_send(hdmi_infoframe *info_frame, td_bool enable)
{
    errno_t ret;
    hdmi_infoframe_unit     infoframe      = {0};
    hdmi_device        *hdmi_dev      = TD_NULL;
    ...
    avi_infoframe->pixel_repeat = (td_u8)vo_attr->pixel_repeat;
...
td_s32 drv_hdmi_vendor_infoframe_send(hdmi_infoframe *info_frame, td_bool enable)
{
#ifdef HDMI_VENDOR_SUPPORT
    errno_t ret;
    td_u32 vic, cnt;
    ...
    vic = (td_u32)drv_hdmi_vic_search(vo_attr->video_timing, vo_attr->picture_aspect, TD_FALSE);
    ...
    hdmi_if_false_return((hdmi_dev->vendor_data.len > (td_u8)HDMI_VENDOR_USER_DATA_MAX_LEN), TD_FAILURE);
...
td_s32 drv_hdmi_audio_infoframe_send(hdmi_infoframe *info_frame, td_bool enable)
{
    errno_t ret;
    hdmi_infoframe_unit infoframe = {0};
    ...
    if (ao_attr->sound_intf == HDMI_AUDIO_INTF_SPDIF || ao_attr->sound_intf == HDMI_AUDIO_INTF_HBRA) {
        hdmi_debug("audio channel refer to stream \n");
    } else {
        hdmi_debug("audio channel %u \n", (td_u32)ao_attr->channels);
        audio_infoframe->channels =
            ((td_u32)ao_attr->channels >= 2) ? ((td_u32)ao_attr->channels - 1) : 0; /* 2, channels number */
    }
    ...
    hdmi_debug("aduio code : %u \n", (td_u32)ao_attr->audio_code);
