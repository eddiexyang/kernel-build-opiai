...
static hdmi_event_info g_event_info[(td_s32)HDMI_ID_MAX];
...
td_s32 drv_hdmi_event_init(hdmi_device_id hdmi_id)
{
    ...
    hdmi_check_max_return((td_u32)hdmi_id, (td_u32)HDMI_ID_MAX - 1U, TD_FAILURE);
    ...
    if (evt_info->init != TD_TRUE) {
        ...
        /* init */
        ret = osal_wait_init(&evt_info->wr_queue);
        if (ret != TD_SUCCESS) {
            hdmi_err("wait init wr_queue failed!\n");
            return ret;
        }
        ret = osal_sema_init(&evt_info->event_mutex, 1);
        if (ret != TD_SUCCESS) {
            osal_wait_destroy(&evt_info->wr_queue);
            hdmi_err("sema init event_mutex failed!\n");
            return ret;
        }
        evt_info->total = 0;
...
td_s32 drv_hdmi_event_pool_write(hdmi_device_id hdmi_id, hdmi_event event)
{
    ...
    for (i = 0, tmp_pool = &evt_info->pool[0]; i < HDMI_EVENT_POOL_CNT; i++, tmp_pool++) {
        if (tmp_pool->ctrl.pool_id) {
            ...
            tmp_pool->ctrl.readable_cnt++;
            osal_wakeup(&evt_info->wr_queue);
            hdmi_debug("the event(0x%x) is writed into event pool of proc(%u) success\n",
                       (td_u32)event,
                       tmp_pool->ctrl.pool_id);
        }
    }
    ...
    if (tmp_pool->ctrl.readable_cnt == 0) {
        ...
        *event = tmp_pool->ctrl.event_pool[tmp_pool->ctrl.read_ptr];
        ...
        if ((*event >= HDMI_EVENT_HOTPLUG) && (*event <= HDMI_EVENT_HOTUNPLUG)) {
            ...
            ret = TD_SUCCESS;
            hdmi_debug("the proc(%u) poll event(0x%x) success\n", tmp_pool->ctrl.pool_id, (td_u32)*event);
        } else {
