...
{
    graphic_drv_smart_rect_up_param(layer_id);
}

td_void vou_graphics_close_smart_rect(td_s32 layer_id)
{
    graphic_drv_smart_rect_disable(layer_id);
}
...
td_bool vou_graphics_dev_check(td_void)
{
    ...
    for (i = 0; i < OT_VO_MAX_PHYS_DEV_NUM; i++) {
        vo_dev = i;
        (td_void)graphic_drv_get_dev_enable(vo_dev, &vo_enable);
        if (vo_enable == TD_TRUE) {
            break;
        }
    }
...
td_s32 vou_graphics_init(td_void)
{
    ...
    if (ret != TD_SUCCESS) {
        gfbg_error("fb_graphic_drv_init failed!\n");
        return ret;
    }
    for (i = 0; i < OT_VO_MAX_PHYS_DEV_NUM; i++) {
        vo_dev = i;
...
{
    /*  initialize the interrupt mask reg */
    return fb_int_msk_init(layer_id);
}
...
td_s32 vou_graphics_get_dev_mode(hal_disp_layer layer, fb_vou_scan_mode *scan_mode, td_bool *feild_update)
{
    ...
    if (g_ast_vo_dev[vo_dev].vo_enable != TD_TRUE) {
        gfx_spin_unlock_irqrestore(&vo_gfx_layer_ctx->spin_lock, &lock_flag);
        gfbg_error("vodev %d for graphics layer %u has been disable!\n", vo_dev, layer_index);
        return TD_FAILURE;
    }
    if (fb_graphic_drv_get_scan_mode(vo_dev, (td_bool *)scan_mode) == TD_FALSE) {
        gfx_spin_unlock_irqrestore(&vo_gfx_layer_ctx->spin_lock, &lock_flag);
        gfbg_error("get vodev:%d scan mode failed!\n", vo_dev);
        return TD_FAILURE;
    }
...
td_s32 vou_graphics_resource_init(td_void)
{
    ...
    if (ret != TD_SUCCESS) {
        gfbg_error("graphic drv resource init failed.\n");
    }
...
{
    return graphic_drv_resource_exit();
}

td_s32 vou_graphics_clut_init(td_void)
{
    td_s32 ret;

    ret = graphic_drv_clut_init();
    if (ret != TD_SUCCESS) {
        gfbg_error("graphic drv clut init failed.\n");
        return ret;
    }
...
td_void vou_graphics_clut_deinit(td_void)
{
    graphic_drv_clut_exit();
}
...
td_s32 vou_graphics_get_intf_size(ot_gfx_layer gfx_layer, td_u32 *width, td_u32 *height)
{
    ...
    if (!dev_info->vo_enable) {
        gfbg_error("The vo device (%d) for graphics layer %d has been disable!\n", vo_dev, layer_index);
        return TD_FAILURE;
    }
...
td_s32 vou_graphics_open_layer(ot_gfx_layer gfx_layer)
{
    ...
    if (!g_gfbg_gfx_layer_ctx[layer_index].binded) {
        ...
        gfbg_error("graphics layer %d has not been binded!\n", layer_index);
        return TD_FAILURE;
    }
    // 获得vo_gfxlayer_context类型的数组g_gfbg_gfx_layer_ctx第layer_index个元素的binded_dev值
    vo_dev = g_gfbg_gfx_layer_ctx[layer_index].binded_dev;

    // 判断vo_dev_info类型的数组g_ast_vo_dev第vo_dev个元素的vo_enable值
    if (g_ast_vo_dev[vo_dev].vo_enable != TD_TRUE) {
        gfx_spin_unlock_irqrestore(&g_gfbg_gfx_layer_ctx[layer_index].spin_lock, &lock_flag);
        gfbg_error("vodev %d for graphics layer %d has not been enable!\n", vo_dev, layer_index);
        return TD_FAILURE;
    }
    // 设vo_gfxlayer_context类型的数组g_gfbg_gfx_layer_ctx第layer_index个元素的opened值为true
...
td_void vou_graphics_set_tde_sync(const gfbg_bind_tde_cfg *bind_tde_cfg)
{
    ...
    graphics_drv_set_tde_sync(bind_tde_cfg->is_sync, bind_tde_cfg->is_hw_mute_clr_en, bind_tde_cfg->is_mute_en,
        bind_tde_cfg->safe_dist);
}

td_void vou_graphics_set_axi_type(td_void)
{
    graphic_drv_set_axi_type();
    return;
}

td_s32 vou_graphics_set_smmu_enable(td_u32 sid, td_u32 ssid)
{
    return graphic_set_smmu_enable(sid, ssid);
}