...
{
    (void)pid_spin_lock_destory(&g_pid_lock);
}

static void mipi_clear_pid(void)
{
    g_cur_pid = 0;
    return;
}

static bool mipi_check_and_set_pid(void)
{
    bool check_pid;
    ...
    if (g_cur_pid == 0) {
        ...
        /* 如果pid不一致，说明已有进程打开mipitx设备，返回FALSE */
        check_pid = FALSE;
    } else {
        /* 剩下情况即为pid一致，返回TRUE */
        check_pid = TRUE;
...
static bool mipi_check_is_same_pid(void)
{
    unsigned long flags = 0;
    bool is_same_pid;

    pid_spin_lock_irqsave(&g_pid_lock, flags);
    is_same_pid = mipi_check_and_set_pid();
...
static int mipi_tx_set_combo_dev_cfg(const combo_dev_cfg_t *dev_cfg)
{
    ...
    int copy_result = memcpy_s(&g_mipi_tx_dev_ctx.dev_cfg, sizeof(combo_dev_cfg_t), dev_cfg, sizeof(combo_dev_cfg_t));
    if (copy_result != EOK) {
        mipi_tx_up_sem();
        mipi_tx_err("mipi_tx secure function erro!\n");
        return -1;
    }
...
unsigned int g_timing_index_list[][8] = {
    /* hsa, hfp, hbp, hact, vsa, vfp, vbp, vact */
    {0x14, 0x6B, 0x17, 0x438, 0x8, 0xe, 0x1c, 0x780}, // 1080p@60hz
    {0x02, 0xA, 0x28, 0x320, 0x2, 0x8, 0xA, 0x500},   // 800x1080@60hz
};

mipitx_timing_index mipi_tx_get_timing_index(int32_t timing_index)
{
    mipitx_timing_index timing_mode = { 0 };
    timing_mode.hbp_hsa.u32 = 0;
    timing_mode.hbp_hsa.bits.hbp_in = g_timing_index_list[timing_index][2U];
    timing_mode.hbp_hsa.bits.hsa_in = g_timing_index_list[timing_index][0U];
    timing_mode.hfp_hact.u32 = 0;
    timing_mode.hfp_hact.bits.hact_in = g_timing_index_list[timing_index][3U];
    timing_mode.hfp_hact.bits.hfp_in = g_timing_index_list[timing_index][1U];
    timing_mode.vsa.u32 = 0;
    timing_mode.vsa.bits.vsa_lines = g_timing_index_list[timing_index][4U];
    timing_mode.vbp.u32 = 0;
    timing_mode.vbp.bits.vbp_lines = g_timing_index_list[timing_index][6U];
    timing_mode.vfp.u32 = 0;
    timing_mode.vfp.bits.vfp_lines = g_timing_index_list[timing_index][5U];
    timing_mode.vactive.u32 = 0;
    timing_mode.vactive.bits.v_active_lines = g_timing_index_list[timing_index][7U];
    timing_mode.hsa.u32 = 0;
    timing_mode.hsa.bits.videom_hsa_size = g_timing_index_list[timing_index][0U];
    timing_mode.hbp.u32 = 0;
    timing_mode.hbp.bits.videom_hbp_size = g_timing_index_list[timing_index][2U];
    timing_mode.hline.u32 = 0;
    timing_mode.hline.bits.video_hline_size = g_timing_index_list[timing_index][0U] +
        g_timing_index_list[timing_index][1U] + g_timing_index_list[timing_index][2U] +
        g_timing_index_list[timing_index][3U];

    return timing_mode;
}

static int mipi_tx_colorbar(void)
{
    void *mipi_base = NULL;
    void *operation_mode = NULL;
    void *hbp_hsa_in = NULL;
    void *hfp_hact_in = NULL;
    void *videom_vsa_lines = NULL;
    void *videom_vbp_lines = NULL;
    void *videom_vfp_lines = NULL;
    void *videom_vactive_lines = NULL;
    void *videom_hsa_size = NULL;
    void *videom_hbp_size = NULL;
    void *videom_hline_size = NULL;

    unsigned long base = MIPITX_REG_BASE_ADDRESS;
    mipi_base = osal_ioremap(base, 0x03e0);
    if (mipi_base == NULL) {
        mipi_tx_err("remap mipi_base reg addr fail\n");
        return -1;
    }

    operation_mode = mipi_base + OPERATION_MODE_OFFSET;
    hbp_hsa_in = mipi_base + HBP_HSA_IN_OFFSET;
    hfp_hact_in = mipi_base + HFP_HACT_IN_OFFSET;
    videom_vsa_lines = mipi_base + VIDEOM_VSA_LINES_OFFSET;
    videom_vbp_lines = mipi_base + VIDEOM_VBP_LINES_OFFSET;
    videom_vfp_lines = mipi_base + VIDEOM_VFP_LINES_OFFSET;
    videom_vactive_lines = mipi_base + VIDEOM_VACTIVE_LINES_OFFSET;
    videom_hsa_size = mipi_base + VIDEOM_HSA_SIZE_OFFSET;
    videom_hbp_size = mipi_base + VIDEOM_HBP_SIZE_OFFSET;
    videom_hline_size = mipi_base + VIDEOM_HLINE_SIZE_OFFSET;

    int32_t timing_index = (int32_t)MIPI_1080P_60;

    mipitx_timing_index timing_mode = mipi_tx_get_timing_index(timing_index);

    osal_writel(0x0, operation_mode); // close mipi colorbar
    osal_writel(timing_mode.hbp_hsa.u32, hbp_hsa_in);
    osal_writel(timing_mode.hfp_hact.u32, hfp_hact_in);

    osal_writel(timing_mode.vsa.u32, videom_vsa_lines);
    osal_writel(timing_mode.vbp.u32, videom_vbp_lines);

    osal_writel(timing_mode.vfp.u32, videom_vfp_lines);
    osal_writel(timing_mode.vactive.u32, videom_vactive_lines);

    osal_writel(timing_mode.hsa.u32, videom_hsa_size);
    osal_writel(timing_mode.hbp.u32, videom_hbp_size);
    osal_writel(timing_mode.hline.u32, videom_hline_size);
    osal_writel(0x00110300, operation_mode); // enable mipi colorbar
...
static long mipi_tx_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    ...
    if (mipi_check_is_same_pid() != TRUE) {
        mipi_tx_info("the ioctl cmd if not from the same pid \n");
        return (-1);
    }
    ...
    switch (cmd) {
        ...
        case OT_MIPI_TX_COLORBAR:
            return mipi_tx_colorbar();
...
static int mipi_tx_open(void *private_data)
{
    ...
    if (mipi_check_is_same_pid() != TRUE) {
        mipi_tx_err("MIPITX functions must work in the same pid!!!");
        return -1;
    }
...
int mipi_tx_module_init(int smooth)
{
    ...
    (void)osal_printk("load mipi_tx.ko ....OK!\n");
    return 0;
...
#endif
    mipi_tx_unregister_device();
    (void)osal_printk("unload mipi_tx.ko ....OK!\n");
}
