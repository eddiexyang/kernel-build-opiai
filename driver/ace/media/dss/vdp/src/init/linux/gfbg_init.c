...
char g_tmp_video[128] = "hifb:vram0_size:8100,vram1_size:8100,vram2_size:256,vram3_size:1620,vram4_size:1620";
...
static int pts_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
    ...
    (td_void)memset_s(g_nowdata, VSYNC_LEN, 0, VSYNC_LEN);
...
static int ot_gfbg_probe(struct platform_device *pdev)
{
    ...
    fifb_irq = osal_platform_get_irq_byname(pdev, "GFBG Int");
    if (fifb_irq <= 0) {
        dev_err(&pdev->dev, "cannot find gfbg IRQ\n");
        return TD_FAILURE;
    }
    vou_set_gfbg_irq((unsigned int)fifb_irq);

    if (gfbg_init() != TD_SUCCESS) {
        gfbg_error("gfbg_init FAILURE!\n");
    }
    return 0;
}
