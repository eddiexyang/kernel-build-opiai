...
struct media_mem_file *g_gfbg_mem_file[GFBG_MAX_LAYER_NUM] = { 0 };
...
#define GFBG_DHD_CTRL_ENABLE 0x1
...
#ifdef CONFIG_OT_PROC_SHOW_SUPPORT
static td_s32 gfbg_read_proc(const struct osal_proc_dir_entry *entry);
...
static td_s32 gfbg_bind_smmu(struct fb_info *info);
static td_s32 gfbg_exit_smmu(struct fb_info *info);
...
static drv_gfbg_ioctl_support_item g_drv_gfbg_ioctl_support[DRV_GFBG_IOCTL_FUNC_ITEM_NUM_MAX] = {
    {0, TD_FALSE},
    {FBIOGET_COLORKEY_GFBG, TD_FALSE},
    {FBIOPUT_COLORKEY_GFBG, TD_FALSE},
    {FBIOGET_ALPHA_GFBG, TD_TRUE},
    {FBIOPUT_ALPHA_GFBG, TD_TRUE},
    {FBIOGET_SCREEN_ORIGIN_GFBG, TD_TRUE},
    {FBIOPUT_SCREEN_ORIGIN_GFBG, TD_TRUE},
    {FBIOGET_DEFLICKER_GFBG, TD_FALSE},
    {FBIOPUT_DEFLICKER_GFBG, TD_FALSE},
    {FBIOGET_VER_BLANK_GFBG, TD_FALSE},
    {FBIOPUT_SHOW_GFBG, TD_TRUE},
    {FBIOGET_SHOW_GFBG, TD_TRUE},
    {FBIOGET_CAPABILITY_GFBG, TD_FALSE},
    {FBIOPUT_CURSOR_INFO, TD_FALSE},
    {FBIOGET_CURSOR_INFO, TD_FALSE},
    {FBIOPUT_CURSOR_STATE, TD_FALSE},
    {FBIOGET_CURSOR_STATE, TD_FALSE},
    {FBIOPUT_CURSOR_POS, TD_FALSE},
    {FBIOGET_CURSOR_POS, TD_FALSE},
    {FBIOPUT_CURSOR_COLORKEY, TD_FALSE},
    {FBIOGET_CURSOR_COLORKEY, TD_FALSE},
    {FBIOPUT_CURSOR_ALPHA, TD_FALSE},
    {FBIOGET_CURSOR_ALPHA, TD_FALSE},
    {FBIOPUT_CURSOR_ATTCH_CURSOR, TD_FALSE},
    {FBIOPUT_CURSOR_DETACH_CURSOR, TD_FALSE},
    {FBIOPUT_LAYER_INFO, TD_FALSE},
    {FBIOGET_LAYER_INFO, TD_FALSE},
    {FBIOGET_CANVAS_BUF, TD_FALSE},
    {FBIO_REFRESH, TD_FALSE},
    {FBIO_WAITFOR_FREFRESH_DONE, TD_FALSE},
    {FBIOPUT_MIRROR_MODE, TD_FALSE},
    {FBIOGET_MIRROR_MODE, TD_FALSE},
    {FBIOPUT_ROTATE_MODE, TD_FALSE},
    {FBIOGET_ROTATE_MODE, TD_FALSE},
    {FBIOPUT_SCREEN_SIZE, TD_FALSE},
    {FBIOGET_SCREEN_SIZE, TD_FALSE},
    {FBIOFLIP_SURFACE, TD_FALSE},
    {FBIOPUT_COMPRESSION_GFBG, TD_FALSE},
    {FBIOGET_COMPRESSION_GFBG, TD_FALSE},
    {FBIOPUT_MDDRDETECT_GFBG, TD_FALSE},
    {FBIOGET_MDDRDETECT_GFBG, TD_FALSE},
    {FBIOPUT_DYNAMIC_RANGE_GFBG, TD_FALSE},
    {FBIOGET_DYNAMIC_RANGE_GFBG, TD_FALSE},
    {FBIO_CREATE_LAYER, TD_FALSE},
    {FBIO_DESTROY_LAYER, TD_FALSE},
    {FBIO_DRAW_SMART_RECT, TD_FALSE},
};
...
static td_u64 gfbg_get_smem_start(const struct fb_info* info)
{
    return (td_u64)info->fix.mmio_start;
}

static td_u64 gfbg_get_smem_start_nomap(const struct fb_info* info)
{
    return (td_u64)info->fix.smem_start;
}
...
static struct fb_fix_screeninfo g_default_fix[GFBG_LAYER_TYPE_BUTT] = {
    ...
    },
    {
        ...
        .ypanstep    =                     1,
        .ywrapstep   =                     0,
        .line_length = GFBG_CURSOR_DEF_STRIDE,
        .accel       = FB_ACCEL_NONE,
        .mmio_len    =                     0,
        .mmio_start  =                     0,
    }
...
static struct fb_var_screeninfo g_default_var[GFBG_LAYER_TYPE_BUTT] = {
    /* for 4K layer */
    {
        ...
        .xoffset        = 0,
        .yoffset        = 0,
        .bits_per_pixel = GFBG_HIGH_DEPTH,
        .red            = {16, 8, 0},
        .green          = {8, 8, 0},
        .blue           = {0, 8, 0},
        .transp         = {24, 8, 0},
 ...
 */
static td_s32 gfbg_fill_data(const struct fb_info* info, td_u32 fill_data, size_t n)
{
    td_s32 ret;
    ot_fb_buf dst_img;
    td_s8 *rest = TD_NULL;
     ...
     * address)
     */
    dst_img.canvas.phys_addr = info->fix.smem_start;
    ...
    canvas_addr = dst_img.canvas.phys_addr;
    ...
    if ((n - dst_img.canvas.height * can_pitch) != 0) {
        ...
        ret = memset_s(rest, n - dst_img.canvas.height * can_pitch, fill_data,
            n - dst_img.canvas.height * can_pitch);
        if (ret != TD_SUCCESS) {
            gfbg_error("memset_s failed!\n");
            return ret;
        }
    }
    return TD_SUCCESS;
...
static td_void gfbg_set_dcmp_info(struct fb_info *info)
{
    ...
    dcmp_info.ar_phy_addr = refresh_info->screen_addr;
    dcmp_info.gb_phy_addr = refresh_info->gb_screen_addr;
...
{
#ifdef CONFIG_OT_PROC_SHOW_SUPPORT
    const td_char *entry_name[GFBG_MAX_LAYER_NUM] = {
        "gfbg0",
        "gfbg1",
        "gfbg2",
        "gfbg3",
...
static td_s32 gfbg_init_register(td_void)
{
    ...
    for (i = 0; i < g_drv_ops.layer_count; i++) {
        if (gfbg_register(i) != TD_SUCCESS) {
            ...
            gfbg_error("ERROR: Load ascend_vdp_hifb.ko ....FAILED!\n");
            return TD_FAILURE;
        }
    }
    return TD_SUCCESS;
...
td_s32 gfbg_init(td_void)
{
    ...
    if (gfbg_parse_cfg() < 0) {
        gfbg_error("Usage: insmod ascend_vdp_hifb.ko video=\"hifb: vrami_size:xxx, vramj_size:xxx, ...\"\n");
        gfbg_error("i, j means layer id, xxx means layer size in kbytes!\n");
        gfbg_error("example: insmod ascend_vdp_hifb.ko video=\"hifb: vram0_size: 1620, vram1_size: 1620\"\n\n");
    ...
    */
    ...
    if (gfbg_vsync_init() != HI_SUCCESS) {
        gfbg_error("gfbg_vsync_init failed!\n");
#ifdef CONFIG_OT_VO_FB_SEPARATE
        goto free_irq;
#else
        goto err2;
    ...
    if (gfbg_pts_init() != HI_SUCCESS) {
        gfbg_error("gfbg_pts_init failed!\n");
        goto vsync_exit;
    }
#endif
    gfbg_info("load ascend_vdp_hifb.ko ....OK!\n");
...
td_void gfbg_cleanup(td_void)
{
    ...
    if (g_gfbg_register) {
        ...
        gfbg_deinit_register();
        g_gfbg_register = HI_FALSE;
    }
    osal_printk("unload ascend_vdp_hifb.ko ....OK!\n");
    return;
}
...
static td_s32 gfbg_interrupt(td_void)
{
    ...
    graphic_drv_int_set_dhd0_ctrl(GFBG_DHD_CTRL_ENABLE);
    graphic_drv_int_set_dhd1_ctrl(GFBG_DHD_CTRL_ENABLE);
...
static td_s32 gfbg_interrupt_route(td_s32 irq, td_void *dev_id)
{
    ot_unused(irq);
    ot_unused(dev_id);
...
static td_s32 gfbg_open_start(struct fb_info *info)
{
    ...
    par = (gfbg_par *)info->par;
    layer_id = par->layer_id;
    vo_gfxlayer_context* tmp = vou_get_gfx_layer_ctx();

    if (!atomic_read(&par->ref_count)) {
        /* mem addr map */
        if (gfbg_bind_smmu(info) != TD_SUCCESS) {
            gfbg_error("gfbg memory map failed!\n");
            return TD_FAILURE;
        }

        if (vou_graphics_init() != TD_SUCCESS) {
            goto EXIT_SMMU;
        }
        if (vou_int_msk_init(layer_id) != TD_SUCCESS) {
            gfbg_error("Init iterrupt mask layer %u# failed!\n", layer_id);
            goto EXIT_SMMU;
        }
        ...
        if (gfbg_open_default_param(info, &color_format) != TD_SUCCESS) {
            gfbg_error("gfbg_open_default_param failed!\n");
            goto CLOSE_LAYER;
        }
...
CLOSE_LAYER:
    g_drv_ops.gfbg_close_layer(layer_id);
EXIT_SMMU:
    gfbg_exit_smmu(info);
    return TD_FAILURE;
}

static struct media_mem_file *gfbg_get_mem_file(td_u32 layer_id)
{
    /* get the mem_file from the global variable g_gfbg_mem_file */
    if (layer_id < GFBG_MAX_LAYER_NUM) {
        return g_gfbg_mem_file[layer_id];
    } else {
        gfbg_error("invalid layer_id!\n");
        return TD_NULL;
    }
}

static td_void gfbg_set_mem_file(struct media_mem_file * mem_file, td_u32 layer_id)
{
    /* store the mem file by layer id in the global g_gfbg_mem_file */
    if (layer_id < GFBG_MAX_LAYER_NUM) {
        g_gfbg_mem_file[layer_id] = mem_file;
    } else {
        gfbg_error("invalid layer_id!\n");
    }
}

static td_s32 gfbg_addr_mmap(struct fb_info *info)
{
    td_s32 ret;
    td_u64 smem_start;
    td_u32 layer_id;
    ...
    layer_id = par->layer_id;
    fix = &info->fix;
    /* get the mem_file by layer id */
    struct media_mem_file* mem_file = gfbg_get_mem_file(layer_id);
    if (mem_file == TD_NULL) {
        gfbg_error("NULL mem file!\n");
        return TD_FAILURE;
    }

    /* get the address by dvpp alloc */
    smem_start = gfbg_get_smem_start_nomap(info);

    /* map the address by dvpp mem map */
    ret = media_kernel_mem_map(mem_file, fix->smem_len, (td_void *)smem_start, (td_u64 *)&(fix->mmio_start));
    if (ret != TD_SUCCESS) {
        gfbg_error("map mempool buffer phy addr failed! ret = %d \n", ret);
        return ret;
    }
    fix->mmio_len = fix->smem_len;
    return TD_SUCCESS;
}


static td_s32 gfbg_addr_unmap(struct fb_info *info)
{
    td_s32 ret;
    td_u32 layer_id;
    gfbg_par *par = TD_NULL;
    ...
    struct media_mem_file* mem_file = gfbg_get_mem_file(layer_id);
    if (mem_file == TD_NULL) {
        gfbg_error("mem_file is NULL!\n");
        return TD_FAILURE;
    }

    /* unmap the smem_start_map address by dvpp mem unmap */
    ret = media_kernel_mem_unmap(mem_file, fix->mmio_len, fix->mmio_start);
    if (ret != TD_SUCCESS) {
        gfbg_error("unmap mempool buffer phy addr failed! ret = %d \n", ret);
    }
...
static td_s32 gfbg_bind_smmu(struct fb_info *info)
{
    ...
    par = (gfbg_par *)info->par;
    layer_id = par->layer_id;
    struct media_mem_file* mem_file = TD_NULL;
    /* malloc the memory for the mem_file */
    mem_file = (struct media_mem_file *)osal_kmalloc(sizeof(struct media_mem_file), osal_gfp_kernel);
    if (mem_file == NULL) {
        gfbg_error("mem_file is null\n");
        return TD_FAILURE;
    }
    /* initialize the value of mem file */
    ret = memset_s(mem_file, sizeof(struct media_mem_file), 0, sizeof(struct media_mem_file));
    if (ret != 0) {
        gfbg_error("memset_s failed!\n");
        osal_kfree(mem_file);
        mem_file = TD_NULL;
    ...
    /* init the mem file */
    ret = media_mem_file_init(mem_file, current->tgid, 0, (hi_s32)HI_ID_FB);
    if (ret != 0) {
        gfbg_error("media_mem_file_init failed\n");
        osal_kfree(mem_file);
        mem_file = TD_NULL;
        return TD_FAILURE;
    }
    /* set the global mem_file by layer id */
    gfbg_set_mem_file(mem_file, layer_id);

    /* start to map the address */
    ret = gfbg_addr_mmap(info);
    if (ret != TD_SUCCESS) {
        gfbg_error("gfbg_addr_mmap failed\n");
        /* uninit the mem file */
        media_mem_file_uninit(mem_file);
        /* free the mem file memory */
        osal_kfree(mem_file);
        mem_file = TD_NULL;
        return ret;
    }
...
static td_s32 gfbg_exit_smmu(struct fb_info *info)
{
    ...
    /* start to unmap the address by info */
    ret = gfbg_addr_unmap(info);
    if (ret != TD_SUCCESS) {
        gfbg_error("gfbg free addr FAILED !!! \n");
        /* uninit the mem file */
        media_mem_file_uninit(mem_file);
        osal_kfree(mem_file);
        mem_file = TD_NULL;
...
/*
 * Function        : gfbg_get_open_layer_count
 * Description     : get the count of opened layer
 * Return          : return count of opened layer
 */
static td_s32 gfbg_get_open_layer_count(td_void)
{
    td_s32 layer_id;
    td_s32 cnt = 0;
    vo_gfxlayer_context* layer_ctx = vou_get_gfx_layer_ctx();
    for (layer_id = 0; layer_id < GFX_MAX; ++layer_id) {
        if (layer_ctx[layer_id].opened == TD_TRUE) {
            cnt++;
        }
    }
    return cnt;
...
/*
 * Function        : gfbg_set_smmu
 * Description     : set gfbg ssmu
 * Return          : return TD_SUCCESS or TD_FAILURE
 */
static td_s32 gfbg_set_smmu(td_void)
{
    td_u32 sid = 0x1FU; /* 0x1f: smmu sid; fixed value */
    td_s32 ssid = -1;
    if (g_drv_ops.gfbg_drv_set_smmu_enable == TD_NULL) {
        return TD_FAILURE;
    }
    ...
    if (ssid < 0) {
        gfbg_error("gfbg current pid is %d, get ssid failed! ssdid:%d\n", current->tgid, ssid);
        return TD_FAILURE;
    }
    gfbg_debug("config sid=0x%x, ssid=0x%x\n", sid, ssid);

    /* 配置SMMU */
    if (g_drv_ops.gfbg_drv_set_smmu_enable(sid, ssid) != TD_SUCCESS) {
        return TD_FAILURE;
    }
...
/*
 * Function        : gfbg_set_chn_master
 * Description     : set fb axi
 * Return          : return TD_SUCCESS or TD_FAILURE
 */
td_s32 gfbg_set_chn_master(td_void)
{
    if (g_drv_ops.gfbg_drv_set_axi_type == TD_NULL) {
        return TD_FAILURE;
    }
    g_drv_ops.gfbg_drv_set_axi_type();
    return TD_SUCCESS;
}

/*
 * Function        : gfbg_check_drv_init
 * Description     : 1.init clut and open tde; 2.set smmu
 * Return          : return TD_SUCCESS or TD_FAILURE
 */
static td_s32 gfbg_check_drv_extra_module_init(td_void)
{
    if (gfbg_get_open_layer_count() == 0) {
        if (g_drv_ops.gfbg_drv_extra_module_init() != TD_SUCCESS) {
            return TD_FAILURE;
        }
        if (gfbg_set_chn_master() != TD_SUCCESS) {
            g_drv_ops.gfbg_drv_extra_module_deinit();
            return TD_FAILURE;
        }
        if (gfbg_set_smmu() != TD_SUCCESS) {
            g_drv_ops.gfbg_drv_extra_module_deinit();
            return TD_FAILURE;
        }
    }
...
/*
 * Function        : gfbg_check_drv_extra_module_deinit
 * Description     : release clut and close tde
 * Return          : return TD_SUCCESS or TD_FAILURE
 */
static td_void gfbg_check_drv_extra_module_deinit(td_void)
{
    if (gfbg_get_open_layer_count() == 0) {
        g_drv_ops.gfbg_drv_extra_module_deinit();
    }
...
/*
 * Function        : gfbg_vdp_close
 * Description     : disable gfbg vdp
 * Return          : void
 */
static td_void gfbg_vdp_close(td_void)
{
    if ((gfbg_get_open_layer_count() == 0) && (g_drv_ops.gfbg_exit_vo != TD_NULL)) {
        vou_graphics_deinit();
        /* check dev enable */
        if (!vou_graphics_dev_check()) {
            g_drv_ops.gfbg_exit_vo();
        }
    }
}
...
static td_s32 gfbg_open(struct fb_info *info, td_s32 user)
{
    ...
    if (gfbg_check_drv_extra_module_init() != TD_SUCCESS) {
        return TD_FAILURE;
    }
    ...
    if (ret != TD_SUCCESS) {
        gfbg_check_drv_extra_module_deinit();
        return TD_FAILURE;
    }
...
static td_void gfbg_release_info(gfbg_par *par)
{
    ...
    gfbg_spin_unlock_irqrestore(&par->lock, lock_flag);
    osal_msleep_uninterrupt(60); /* 等待60毫秒,让vo完成当前帧送显 */
...
static td_s32 gfbg_release(struct fb_info *info, td_s32 user)
{
    ...
    /* release layer */
    if (wait_event_interruptible_timeout(par->vbl_event, par->vblflag, (td_s32)msecs_to_jiffies(60U)) < 0) {
        gfbg_error("gfbg_release_info timeout!\n");
        return TD_FAILURE;
    }
    ...
    if (atomic_dec_and_test(&par->ref_count)) {
        ...
        ret = gfbg_exit_smmu(info);
    }

    /* gfbg extra module deint when all layer have been closed */
    gfbg_check_drv_extra_module_deinit();
#ifdef CONFIG_OT_VO_FB_SEPARATE
    gfbg_vdp_close();
#endif
...
static td_s32 drv_gfbg_set_colorkey(struct fb_info *info, unsigned long arg)
{
    ...
    // 获取用户指定的colorkey信息，存储到colorkey中
    ...
    // 什么是预乘:就是将位图和三原色R,G,B分别和alpha值相乘
...
static td_s32 flip_surface_check_param(const struct fb_info *info, const ot_fb_surfaceex *surface_ex)
{
    ...
    addr = (td_ulong)surface_ex->phys_addr;
...
static td_s32 drv_gfbg_set_compression_mode(struct fb_info *info, unsigned long arg)
{
    ...
    if (!g_drv_ops.capability[par->layer_id].is_decompress) {
        gfbg_error("Layer %u doesn't support compression operation!\n", par->layer_id);
        return TD_FAILURE;
    }
...
static td_s32 gfbg_ioctl(struct fb_info *info, td_u32 cmd, unsigned long arg)
{
    ...
    if (cmd != g_drv_gfbg_ioctl_func[g_drv_gfbg_ctl_num[gfbg_cmd]].cmd ||
        g_drv_gfbg_ioctl_support[g_drv_gfbg_ctl_num[gfbg_cmd]].support != TD_TRUE) {
...
static td_s32 gfbg_check_output(const struct fb_var_screeninfo *var, const struct fb_info *info)
{
    ...
    if (((var->xres % 2) != 0) || ((var->yres % 2) != 0)) { /* 2 for calculate */
        gfbg_error("xres(%u) yres(%u) of layer_id %u should be even!\n", var->xres, var->yres, layer_id);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
...
static td_s32 gfbg_check_virtual_resolution(const struct fb_var_screeninfo *var, const struct fb_info *info)
{
    ...
    if (var->xres < gfbg_min_width(layer_id)) {
        gfbg_error("xres(%u) of layer_id %u can't be less than min_width(%u)\n", var->xres, layer_id,
                   gfbg_min_width(layer_id));
        return TD_FAILURE;
    }
    if (var->yres < gfbg_min_height(layer_id)) {
        gfbg_error("yres(%u) of layer_id %u can't be less than min_height(%u)\n", var->yres, layer_id,
                   gfbg_min_height(layer_id));
        return TD_FAILURE;
    }

    if (var->xres > var->xres_virtual) {
        gfbg_error("xres(%u) of layer_id %u should be less than xres_virtual(%u)\n", var->xres, layer_id,
                   var->xres_virtual);
        return TD_FAILURE;
    }
    if (var->yres > var->yres_virtual) {
        gfbg_error("yres(%u) of layer_id %u should be less than yres_virtual(%u)\n", var->yres, layer_id,
                   var->yres_virtual);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
...
static td_s32 gfbg_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    /*
     * the actual width and height of the layer must be an even number
     */
...
static td_s32 gfbg_refresh_2buf_immediate_display(td_u32 layer_id, const ot_fb_buf *canvas_buf)
{
    ...
    if (wait_event_interruptible_timeout(par->do_refresh_job, par->refresh_info.do_refresh_job,
        ...
        gfbg_error("Func:%s, Line:%d, wait interrupt.\n", __FUNCTION__, __LINE__);
    }
    /*
...
static td_void gfbg_get_idledispbuf(const gfbg_par *par, td_phys_addr_t *phy_addr)
{
    ...
    if (index_for_int > 1) {
        gfbg_error("invalid refresh index!\n");
        return;
    }
    /*
...
static td_phys_addr_t gfbg_buf_allocmem(const td_char *buf_name, td_ulong layer_size, const td_char *mmz_name)
{
    ...
    ret = media_kernel_mem_malloc(layer_size, DVPP_MEM_PLACEMENT_MBUF | DVPP_PAGE_FLAG_NORMAL, &vir_addr);
    if (ret != 0) {
        return 0;
    }

    return (td_phys_addr_t)vir_addr;
}
...
static td_void callback_modify_dynamic_range(gfbg_par *par, td_u32 layer_id)
{
    ...
    if (par->param_modify_mask & GFBG_LAYER_PARAMODIFY_DYNAMICRANGE) {
        ...
        if (display_info->dynamic_range == OT_FB_DYNAMIC_RANGE_HDR10) {
            ...
            if (g_drv_ops.gfbg_drv_enable_ghdr != TD_NULL) {
                g_drv_ops.gfbg_drv_enable_ghdr(layer_id, TD_TRUE);
            }
            ...
            if (g_drv_ops.gfbg_drv_enable_ghdr != TD_NULL) {
                g_drv_ops.gfbg_drv_enable_ghdr(layer_id, TD_FALSE);
            }
        }
        /*
...
static td_s32 onrefresh_get_canvas_buf(gfbg_par *par, td_void __user *argp, ot_fb_buf *canvas_buf)
{
    ...
    if ((canvas_buf->canvas.width < canvas_buf->update_rect.x) ||
        (canvas_buf->canvas.height < canvas_buf->update_rect.y)) {
        return TD_FAILURE;
    }
...
static td_void gfbg_overlay_cleanup(td_u32 layer_id, td_bool unregister)
{
    ...
    if (info != TD_NULL) {
        ...
        if (gfbg_get_smem_start_nomap(info)) {
            gfbg_buf_freemem(gfbg_get_smem_start_nomap(info));
        }
                                      ...
                                      struct fb_var_screeninfo *var)
{
    ...
    if (g_layer[layer_id].layer_size != 0) {
        ...
        fix->smem_start = gfbg_buf_allocmem(name,
        ...
        info->screen_base = (char __iomem *)(td_uintptr_t)(fix->smem_start);
...
static td_s32 __init gfbg_overlay_probe(td_u32 layer_id)
{
    ...
    if (ret != TD_SUCCESS) {
        gfbg_error("overlay_probe_alloc_mem failed!\n");
        goto ERR;
    }
 ...
 */
static td_ulong gfbg_get_vram_size(const char* pstr)
{
    td_s32 str_is_valid = TD_TRUE;
    td_ulong vram_size = 0;
    td_ulong vram_size_temp;
    const char* ptr = pstr;
...
static td_ulong parse_cfg_clip_layer_size(td_ulong layer_size, td_u32 layer_id)
{
    td_ulong vram_max_size, vram_min_size;
    td_ulong vram_max_size_clut = 0;
    td_ulong vram_max_size_argb = 0;

    if (layer_size == 0) {
        return 0;
    }
    if (layer_id > GRAPHICS_LAYER_G4) {
        gfbg_error("layer_id illegal!\n");
        return 0;
    }
    if (layer_id == GRAPHICS_LAYER_G3 || layer_id == GRAPHICS_LAYER_G4) {
        /* G3/G4 clut2 and clut4 support 4K */
        vram_max_size_clut = GFBG_MAX_LAYER_WIDTH_CLUT_G3 *
                             GFBG_MAX_LAYER_HEIGTH_CLUT_G3 / 2 / 1024;  /* 1/2 byte/pixel 1024 */
    }
    vram_max_size_argb = gfbg_max_width(layer_id) * gfbg_max_height(layer_id) * 4 / 1024;  /* 4 byte/pixel 1024 */

    vram_max_size = gfbg_max(vram_max_size_clut, vram_max_size_argb);
    vram_min_size = gfbg_min_width(layer_id) * gfbg_min_height(layer_id) * 4 / 1024;      /* 4 byte/pixel 1024 */

    if (layer_size > vram_max_size || layer_size < vram_min_size) {
        layer_size = gfbg_max(gfbg_min(layer_size, vram_min_size), vram_max_size);
        layer_size = ((layer_size * 1024 + PAGE_SIZE - 1) & PAGE_MASK) / 1024;            /* align 1024 */
        gfbg_warning("layer_size exceeds the limit of G%u. It is set to %luKB. \n", layer_id, layer_size);
    } else {
        layer_size = ((layer_size * 1024 + PAGE_SIZE - 1) & PAGE_MASK) / 1024;            /* align 1024 */
    }
    gfbg_info("The layer_size is %lu.\n", layer_size);
...
{
    td_ulong layer_size;
    layer_size = parse_cfg_clip_layer_size(gfbg_get_vram_size(sc_str), layer_id);
...
static td_s32 gfbg_read_proc(const osal_proc_entry_t *entry)
{
    struct fb_info *info = TD_NULL;
    gfbg_par *par = TD_NULL;
    if (entry == TD_NULL) {
    ...
    info = (struct fb_info *)(entry->private_osal);
    if (info == TD_NULL) {
        return TD_FAILURE;
    }
    par = (gfbg_par *)info->par;
