...
#define tranverse_32(X)	((((u32)(X) & 0xff000000) >> 24) | \
			(((u32)(X) & 0x00ff0000) >> 8) | \
			(((u32)(X) & 0x0000ff00) << 8) | \
			(((u32)(X) & 0x000000ff) << 24))
...
/*
 * @base base address of PMIC Arbiter core registers.
 * @rdbase, @wrbase base address of PMIC Arbiter read core registers.
 *     For HW-v1, it's equal to base.
 *     For HW-v2, it's the same in eeraly probing, in order to
 *     read PMIC_ARB_CORE registers, then chnls, and obsrvr are set
 *     to PMIC_ARB_CORE_REGISTERS and PMIC_ARB_CORE_REGISTERS_OBS respectivly.
 * @intr base address of spmi interrupt control registers
 * @ppid_2_chnl_tbl lookup table f(SID, Periph-ID) -> channel_nums
 *      entry is valid only if corresponding bit is set in valid_ppid_bitmap.
 * @valid_ppid_bitmap bit is set only for the valid ppids.
 * @fmt_cmd formats one command to be set into PMIC_ARBq_CHNLn_CMD
 * @chnl_ofst calculates offset of the base of one channel reg space
 * @ee a execution environment id
 * @irq_acc0_init_val initial value of interrupt accumulator at probe time.
 *      Use for a HW workaround. On handling interrupts, the first accumulator
 *      register will be compared against this value, and bits which are
 *      set at boot will be ignored.
 * @reserved_chnl entry of ppid_2_chnl_tbl that the driver should never touch.
 *      value is a positive channel number or negative to mark it unused.
 */
				...
				u8 opc, u8 sid, u16 addr, u8 bc, u8 *buf)
{
	...
	if (bc > SPMI_CONTROLLER_MAX_TRANS_BYTES) {
		dev_err(spmi_controller->dev, "spmi_controller supports 1..%d bytes per trans, but:%d requested",
	...
	i = 0;
	do {
		spmi_readl(spmi_controller->dev, data, spmi_controller->base + chnl_ofst + SPMI_SLAVE_OFFSET*sid +
						SPMI_APB_SPMI_RDATA0_BASE_ADDR + i*SPMI_PER_DATAREG_BYTE);/*lint !e732 */
		data = tranverse_32(data);
		if ((bc - (i * SPMI_PER_DATAREG_BYTE)) >> 2) {/*lint !e702 */\
			if (memcpy_s(buf, bc, &data, sizeof(data)) != 0) {
				goto done;
			}
			buf += sizeof(data);
		} else {
			if (memcpy_s(buf, bc, &data, bc % SPMI_PER_DATAREG_BYTE) != 0) {/*lint !e747 */
				goto done;
			}
				...
				u8 opc, u8 sid, u16 addr, u8 bc, u8 *buf)
{
	...
	do {
		...
		if ((bc - i * SPMI_PER_DATAREG_BYTE) >> 2) {/*lint !e702 */
			if (memcpy_s(&data, sizeof(data), buf, sizeof(data)) != 0) {
				dev_err(spmi_controller->dev, "memcpy_s failed.\n");
				return -EINVAL;
			}
			buf +=sizeof(data);
		} else {
			if (memcpy_s(&data, sizeof(data), buf, bc % SPMI_PER_DATAREG_BYTE) != 0) {/*lint !e747 */
				dev_err(spmi_controller->dev, "memcpy_s failed.\n");
				return -EINVAL;
			}
			buf += (bc % SPMI_PER_DATAREG_BYTE);
...
static int spmi_controller_probe(struct platform_device *pdev)
{
	...
	(void)devm_kfree(&pdev->dev, spmi_controller);
	...
	(void)iounmap(spmi_controller->base);
...
static struct of_device_id g_spmi_controller_match_table[] = {
	{ .compatible = "hisilicon,spmi-controller", }, /*lint !e785*/
	{} /*lint !e785*/
};

static struct platform_driver g_spmi_controller_driver = {
	.probe		= spmi_controller_probe,
	.remove		= spmi_controller_remove,
	.driver		= {
		.name	= SPMI_CONTROLLER_NAME,
		.owner	= THIS_MODULE, /*lint !e64*/
		.of_match_table = g_spmi_controller_match_table,
	}, /*lint !e785*/
};/*lint !e785*/
/*lint -e528 -esym(528,*)*/
static int __init spmi_controller_init(void)
{
	return platform_driver_register(&g_spmi_controller_driver);/*lint !e64*/
}
postcore_initcall(spmi_controller_init);

static void __exit spmi_controller_exit(void)
{
	platform_driver_unregister(&g_spmi_controller_driver);
}
...
/*lint -e528 +esym(528,*)*/
#else
void spmi_controller_exit(void)
{
}
#endif
